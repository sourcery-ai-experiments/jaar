...
def test_polity_set_agent_CreatesAgentFile(env_dir_setup_cleanup):
    # GIVEN
...
def test_polity_get_agents_dir_list_of_obj_CreatesAgentFilesList(
    env_dir_setup_cleanup,
):
    # GIVEN
    polity_name = get_temp_env_name()
...
def test_polity_get_agent_currentlyGetsAgent(env_dir_setup_cleanup):
    # GIVEN
    polity_name = get_temp_env_name()
...
def test_polity_rename_agent_in_agents_dir_ChangesAgentName(env_dir_setup_cleanup):
    # GIVEN
    polity_name = get_temp_env_name()
    e5 = PolityUnit(name=polity_name, politys_dir=get_test_politys_dir())
    e5.create_dirs_if_null(in_memory_bank=True)
...
def test_personunit_refresh_agentlinks_CorrectlyPullsAllPublicAgents(
    ...
    px_ernie = e1.get_person_obj_from_polity(name=ernie_text)
    px_jessi = e1.get_person_obj_from_polity(name=jessi_text)
    # px_steve = e1.get_person_obj_from_polity(name=steve_text)
