...
from lib.world.person import (
    PersonUnit,
    personunit_shop,
    get_from_json as get_person_from_json,
)
from lib.world.agentlink import AgentLink
...
class WorldUnit:
    ...
    def set_agent_attr_defined_by_world(self, agent_name: str):
        ...
        for brandunit_x in agent_obj._brands.values():
            if brandunit_x._allylinks_set_by_world_road != None:
                brandunit_x.clear_allylinks()
                ic = get_idea_catalog_dict(
                    self.get_bank_conn(), brandunit_x._allylinks_set_by_world_road
                )
        ...
        with self.get_bank_conn() as bank_conn:
            ...
            for brandunit_x in agentunit_x._brands.values():
                brandunit_catalog_x = BrandUnitCatalog(
                    ...
                    allylinks_set_by_world_road=brandunit_x._allylinks_set_by_world_road,
                )
    ...
    def set_worldunit_name(self, name: str):
        self.name = name
    ...
    def get_person_obj_from_world(self, name: str) -> PersonUnit:
    ...
    def load_personunit(self, name: str):
        ...
        self.set_personunits_empty_if_null()
        self.set_personunit_to_world(person_x)

    def set_personunit_to_world(self, person: PersonUnit):
    ...
    def save_person_file(self, person_name: str):
        person_x = self.get_person_obj_from_world(name=person_name)
        x_func_save_file(
    ...
    def rename_personunit(self, old_name: str, new_name: str):
        person_x = self.get_person_obj_from_world(name=old_name)
        old_person_dir = person_x._person_dir
        person_x.set_person_name(new_name=new_name)
        self.set_personunit_to_world(person=person_x)
        x_func_delete_dir(old_person_dir)
        self.del_person_from_world(person_name=old_name)

    def del_person_from_world(self, person_name):
