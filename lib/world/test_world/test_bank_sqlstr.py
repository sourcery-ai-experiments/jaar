...
from lib.world.y_func import get_single_result_back
...
def test_world_get_idea_catalog_table_insert_sqlstr_CorrectlyPopulatesTable01(
    env_dir_setup_cleanup,
):
    # GIVEN
    e1 = WorldUnit(name=get_temp_env_name(), worlds_dir=get_test_worlds_dir())
...
def test_world_get_idea_catalog_dict_ReturnsCorrectData(env_dir_setup_cleanup):
    # GIVEN
    e1 = WorldUnit(name=get_temp_env_name(), worlds_dir=get_test_worlds_dir())
    e1.create_dirs_if_null(in_memory_bank=True)
    e1.refresh_bank_metrics()
...
def test_world_get_brandunit_catalog_table_insert_sqlstr_CorrectlyPopulatesTable01(
    env_dir_setup_cleanup,
):
    # GIVEN Create example world with 4 Persons, each with 3 Allyunits = 12 ledger rows
