...
from lib.world.examples.example_persons import (
...
def get_temp_env_dir():
    return f"{get_test_worlds_dir()}/{get_temp_env_name()}"
...
def create_person_file_for_worlds(world_dir: str, person_name: str):
    person_x = personunit_shop(name=person_name, env_dir=world_dir)
    person_dir = f"{world_dir}/persons/{person_x.name}"
...
def _delete_and_set_ex3():
    ...
    # w1_obj = ex.get_person_obj_from_world(name=w1_text)

    bob_text = "bob wurld"
    create_agent_file_for_worlds(
        world_dir=ex.get_object_root_dir(), agent_desc=bob_text
    )
    ...
    land_text = "tim wurld"
    create_agent_file_for_worlds(
        world_dir=ex.get_object_root_dir(), agent_desc=land_text
    )
...
def rename_test_world(world_obj: WorldUnit, new_name):
    ...
    world_obj.set_worldunit_name(name=new_name)


class InvalidWorldCopyException(Exception):
    pass
...
def copy_test_world(src_name: str, dest_name: str):
    ...
    if os_path.exists(new_dir):
        raise InvalidWorldCopyException(
            f"Cannot copy world to '{new_dir}' dir already exists."
        )
    # base_dir = world_obj.get_object_root_dir()
