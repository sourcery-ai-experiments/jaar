...
class EditMainView(qtw.QWidget, Ui_Form):
    ...
    def acptfact_del_action(self):
        self.pact_x.del_acptfact(base=self.acptfact_base_update_combo.currentText())
        self.refresh_all()
    ...
    def get_acptfacts_list(self):
        ...
        if self.display_problem_acptfacts_cb.checkState() == 2:
            x_list.extend(
                acptfact
                for acptfact in self.pact_x._idearoot._acptfactunits.values()
                if (
                    self.pact_x.get_idea_kid(road=acptfact.base)._problem_bool
                    or self.pact_x.get_idea_kid(road=acptfact.pick)._problem_bool
                )
            )
        else:
            x_list = self.pact_x._idearoot._acptfactunits.values()
        return x_list
    ...
    def acptfact_table_select(self):
        ...
        self.acptfact_base_update_combo.addItems(
            self.pact_x.get_idea_tree_ordered_road_list()
        )
    ...
    def acptfact_pick_combo_load(self):
        ...
        self.acptfact_pick_update_combo.addItems(
            self.pact_x.get_heir_road_list(
    ...
    def refresh_party_list(self):
        ...
        partys_list.sort(key=lambda x: x._pact_credit, reverse=True)
        ...
        for row, party in enumerate(partys_list, start=1):
            ...
            self.party_list.setItem(row - 1, 1, qt_pact_credit)
