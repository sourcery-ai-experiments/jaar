...
class EditParty2bd(qtw.QWidget, Ui_Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.gp_party2belief_delete_button.clicked.connect(self.party2belief_delete)
        self.gp_add_button.clicked.connect(self.party2belief_add)
        self.update_weight_button.clicked.connect(self.party2belief_update)
    ...
    def party2belief_update(self):
        ...
        #     weight = int(self.party2belief_weight_edit.text())
        #     Party2OG.party2belief_update(party_id=party_id, gp_id=gp_id, weight=weight)
        #     self.refreshAll()

    def party2belief_delete(self):
        currentRowInt = self.gp_party_yes.currentRow()
        # if self.gp_party_yes.rowCount() > 0:
        #     currentRowInt = max(currentRowInt, 0)
        #     party_id = self.party_id
        #     gp_id = int(self.gp_party_yes.item(currentRowInt, 4).text())
        #     Party2OG.party2belief_delete(party_id=party_id, gp_id=gp_id)
        #     self.refreshAll()

    def party2belief_add(self):
        currentRowInt = self.gp_party_no.currentRow()
        # if self.gp_party_no.rowCount() > 0:
        #     currentRowInt = max(currentRowInt, 0)
        #     party_id = self.party_id
        #     gp_id = int(self.gp_party_no.item(currentRowInt, 2).text())
        #     Party2OG.party2belief_insert(party_id=party_id, gp_id=gp_id, weight=1)
        # self.refreshAll()
    ...
    def gp_party_yes_clicked(self):
        self.party2belief_weight_edit.setText(
    ...
    def gp_party_no_clicked(self):
        ...
        self.gp_delete_button.setText(f'Delete Party Belief " {pid} "')
