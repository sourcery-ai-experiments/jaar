...
class EditMainView(qtw.QWidget, Ui_Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fact_base_update_combo.currentTextChanged.connect(
    ...
    def fact_del_action(self):
        self.agenda_x.del_fact(base=self.fact_base_update_combo.currentText())
        self.refresh_all()

    def get_facts_list(self):
        x_list = []
        if self.display_problem_facts_cb.checkState() == 2:
            x_list.extend(
                fact
                for fact in self.agenda_x._idearoot._factunits.values()
                if (
                    self.agenda_x.get_idea_obj(road=fact.base)._problem_bool
                    or self.agenda_x.get_idea_obj(road=fact.pick)._problem_bool
                )
            )
        else:
            x_list = self.agenda_x._idearoot._factunits.values()
        return x_list
    ...
    def fact_pick_combo_load(self):
        ...
        self.fact_pick_update_combo.addItems(
            self.agenda_x.get_heir_road_list(self.fact_base_update_combo.currentText())
        )
