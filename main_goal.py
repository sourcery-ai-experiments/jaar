# # command to for converting ui form to python file: pyuic5 ui\GoalMainUI.ui -o ui\GoalMainUI.py
from ui.GoalMainUI import Ui_MainWindow
...
from src.goal.examples.goal_env_kit import (
    create_example_goals_list,
    setup_test_example_environment,
    create_example_goal,
    delete_dir_example_goal,
    rename_example_goal,
    get_test_goals_dir,
)
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.goal_insert_button.clicked.connect(self.goal_insert)
        self.goal_load_button.clicked.connect(self.goal_load_from_file)
        self.goal_update_button.clicked.connect(self.goal_update_title)
        self.goal_delete_button.clicked.connect(self.goal_delete)
    ...
    def reload_all_src_contracts(self):
        if self.goal_x != None:
            self.goal_x.reload_all_owners_src_contractunits()
    ...
    def _owner_load(self, owner_title: str):
        self.goal_x.create_ownerunit_from_public(title=owner_title)
        self.owner_x = self.goal_x._ownerunits.get(owner_title)
    ...
    def ignores_table_select(self):
        ...
        # self.ignore_contract_x = self.goal_x.get_public_contract(
        self.ignore_contract_x = self.goal_x.get_contract_from_ignores_dir(
    ...
    def ignore_contract_file_update(self):
        self.goal_x.set_ignore_contract_file(
    ...
    def contract_insert(self):
        self.goal_x.save_public_contract(
    ...
    def owner_insert(self):
        self.goal_x.create_new_ownerunit(owner_title=self.owner_title.text())
        self.refresh_owners()
    ...
    def owner_update_title(self):
        ...
        if currently_selected != typed_in:
            self.goal_x.rename_ownerunit(
                old_label=currently_selected, new_label=typed_in
            )
            self.refresh_owners()

    def owner_delete(self):
        self.goal_x.del_owner_dir(
    ...
    def depotlink_insert(self):
        ...
        if self.owner_x != None:
            ...
            self.goal_x.save_owner_file(owner_title=self.owner_x._admin.title)
        self.refresh_owner()

    def depotlink_update(self):
        owner_title_x = self.owner_x._admin.title
        self.goal_x.update_depotlink(
        ...
        self.goal_x.save_owner_file(owner_title=owner_title_x)
        self.refresh_owner()

    def depotlink_delete(self):
        owner_title_x = self.owner_x._admin.title
        self.goal_x.del_depotlink(
    ...
    def get_owner_title_list(self):
        ...
        if self.goal_x != None:
            owners_owner_list.extend(
                [owner_dir] for owner_dir in self.goal_x.get_owner_dir_paths_list()
            )
