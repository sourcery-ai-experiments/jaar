# # command to for converting ui form to python file: pyuic5 ui\HealMainUI.ui -o ui\HealMainUI.py
from ui.HealMainUI import Ui_MainWindow
...
from src.heal.examples.heal_env_kit import (
    create_example_heals_list,
    setup_test_example_environment,
    create_example_heal,
    delete_dir_example_heal,
    rename_example_heal,
    get_test_heals_dir,
)
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.heal_insert_button.clicked.connect(self.heal_insert)
        self.heal_load_button.clicked.connect(self.heal_load_from_file)
        self.heal_update_button.clicked.connect(self.heal_update_title)
        self.heal_delete_button.clicked.connect(self.heal_delete)
        ...
        setup_test_example_environment()
        first_env = "ex5"
        self.heal_x = healunit_shop(title=first_env, heals_dir=get_test_heals_dir())
        self.refresh_heal()
        self.heal_kind_combo_refresh()
        self.heal_kind_combo.setCurrentText(first_env)
    ...
    def reload_all_src_contracts(self):
        if self.heal_x != None:
            self.heal_x.reload_all_owners_src_contractunits()
    ...
    def heal_load_from_file(self):
        heal_selected = self.heal_kind_combo.currentText()
        self.heal_x = healunit_shop(title=heal_selected, heals_dir=get_test_heals_dir())
        self.heal_x.create_dirs_if_null(in_memory_bank=False)
        self.heal_kind.setText(heal_selected)
        self.refresh_heal()
    ...
    def _owner_load(self, owner_title: str):
        self.heal_x.create_ownerunit_from_public(title=owner_title)
        self.owner_x = self.heal_x._ownerunits.get(owner_title)
    ...
    def ignores_table_select(self):
        ...
        # self.ignore_contract_x = self.heal_x.get_public_contract(
        self.ignore_contract_x = self.heal_x.get_contract_from_ignores_dir(
    ...
    def ignore_contract_file_update(self):
        self.heal_x.set_ignore_contract_file(
    ...
    def heal_insert(self):
        create_example_heal(heal_kind=self.heal_kind.text())
        self.heal_kind_combo_refresh()

    def heal_update_title(self):
        rename_example_heal(heal_obj=self.heal_x, new_title=self.heal_kind.text())
        self.heal_kind_combo_refresh()

    def heal_delete(self):
        delete_dir_example_heal(heal_obj=self.heal_x)
        self.heal_x = None
        self.heal_kind_combo_refresh()
        self.refresh_heal()

    def contract_insert(self):
        self.heal_x.save_public_contract(
    ...
    def owner_insert(self):
        self.heal_x.create_new_ownerunit(owner_title=self.owner_title.text())
        self.refresh_owners()
    ...
    def owner_update_title(self):
        ...
        if currently_selected != typed_in:
            self.heal_x.rename_ownerunit(
                old_label=currently_selected, new_label=typed_in
            )
            self.refresh_owners()

    def owner_delete(self):
        self.heal_x.del_owner_dir(
    ...
    def depotlink_insert(self):
        ...
        if self.owner_x != None:
            ...
            self.heal_x.save_owner_file(owner_title=self.owner_x._admin.title)
        self.refresh_owner()

    def depotlink_update(self):
        owner_title_x = self.owner_x._admin.title
        self.heal_x.update_depotlink(
        ...
        self.heal_x.save_owner_file(owner_title=owner_title_x)
        self.refresh_owner()

    def depotlink_delete(self):
        owner_title_x = self.owner_x._admin.title
        self.heal_x.del_depotlink(
    ...
    def get_owner_title_list(self):
        ...
        if self.heal_x != None:
            owners_owner_list.extend(
                [owner_dir] for owner_dir in self.heal_x.get_owner_dir_paths_list()
            )
    ...
    def heal_kind_combo_refresh(self):
        self.heal_kind_combo.clear()
        self.heal_kind_combo.addItems(create_example_heals_list())
