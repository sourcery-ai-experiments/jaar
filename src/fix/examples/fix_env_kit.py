...
def _delete_and_set_ex3():
    ...
    # x_fix.set_healer(x_healing=healingunit_shop(title="w1", env_dir=x_fix.get_object_root_dir()))
    # x_fix.set_healer(x_healing=healingunit_shop(title="w2", env_dir=x_fix.get_object_root_dir()))
    xia_text = "Xia"
    x_fix.create_new_healingunit(healing_title=xia_text)
    healer_text = "Mydeal"
    x_fix.set_healer_depotlink(
        xia_text, deal_healer=healer_text, depotlink_type="blind_trust"
    )
    # w1_obj = x_fix.get_healingunit(title=w1_text)
    ...
    # x_fix.create_depotlink_to_generated_deal(healing_title=w1_text, deal_healer="test9")
    # x_fix.create_depotlink_to_generated_deal(healing_title=w1_text, deal_healer="Bobs deal")
    x_fix.save_healingunit_file(healing_title=xia_text)
    # print(f"WHAT WHAT {x_fix.get_object_root_dir()}")
    # print(f"WHAT WHAT {x_fix.get_object_root_dir()}/healingunits/w1/w1.json")
    # file_text = x_func_open_file(
    #     dest_dir=f"{x_fix.get_object_root_dir}/healingunits/w1", file_title="w1.json"
    # )
    # print(f"{file_text=}")
    # print(f"{len(x_fix._healingunits.get(w1_text)._depotlinks)=}")
    # print(f"{x_fix._healingunits.get(w1_text)._depotlinks.get(bob_text)=}")
    # print(f"{x_fix._healingunits.get(w1_text).get_json=}")

    w2_text = "w2"
    x_fix.create_new_healingunit(healing_title=w2_text)
    # , env_dir=x_fix.get_object_root_dir())
    x_fix.save_healingunit_file(healing_title=w2_text)
...
def _delete_and_set_ex5():
    ...
    x_fix.create_new_healingunit(healing_title=deal_1._healer)
    x_fix.create_new_healingunit(healing_title=deal_2._healer)
    x_fix.create_new_healingunit(healing_title=deal_3._healer)
    x_fix.create_new_healingunit(healing_title=deal_4._healer)
    x_fix.create_new_healingunit(healing_title=deal_5._healer)
    ...
    x_fix.save_healingunit_file(healing_title=deal_1._healer)
    x_fix.save_healingunit_file(healing_title=deal_2._healer)
    x_fix.save_healingunit_file(healing_title=deal_3._healer)
    x_fix.save_healingunit_file(healing_title=deal_4._healer)
    x_fix.save_healingunit_file(healing_title=deal_5._healer)
