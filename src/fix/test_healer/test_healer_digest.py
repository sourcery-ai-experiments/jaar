from src.deal.deal import DealUnit, originunit_shop
from src.deal.examples.example_deals import (
    get_deal_with_4_levels as example_deals_get_deal_with_4_levels,
)
from src.deal.x_func import (
    open_file as x_func_open_file,
    count_files as x_func_count_files,
)
from src.fix.healing import healingunit_shop
from src.fix.examples.example_healers import (
    get_2node_deal as example_healers_get_2node_deal,
    get_7nodeJRoot_deal as example_healers_get_7nodeJRoot_deal,
)
from src.fix.examples.healer_env_kit import (
    healer_dir_setup_cleanup,
    get_temp_healingunit_dir,
    get_temp_fix_handle,
)
from os import path as os_path
from pytest import raises as pytest_raises


# def test_healer_save_isol_deal_CreateStartingDealFile(
#     healer_dir_setup_cleanup,
# ):
#     # GIVEN
#     lai_title = "Lai"
#     env_dir = get_temp_healingunit_dir()
#     lai_deal = healingunit_shop(title=lai_title, env_dir=env_dir)
#     lai_isol_file_title = lai_deal._admin._isol_file_title
#     with pytest_raises(Exception) as excinfo:
#         x_func_open_file(lai_deal._admin._healingunit_dir, lai_isol_file_title)
#     assert (
#         str(excinfo.value)
#         == f"Could not load file {lai_deal._admin._isol_file_path} (2, 'No such file or directory')"
#     )

#     # WHEN
#     lai_deal._admin.save_isol_deal(
#         deal_x=example_deals_get_deal_with_4_levels()
#     )

#     # THEN
#     assert x_func_open_file(lai_deal._admin._healingunit_dir, lai_isol_file_title) != None


def test_healeropen_isol_deal_WhenStartingDealFileDoesNotExists(
    healer_dir_setup_cleanup,
):
    # GIVEN
    tim_text = "Tim"
    env_dir = get_temp_healingunit_dir()
    fix_handle_text = get_temp_fix_handle()
    ux = healingunit_shop(title=tim_text, env_dir=env_dir, fix_handle=fix_handle_text)

    # WHEN
    isol_deal = ux._admin.open_isol_deal()
    assert isol_deal != None
    assert isol_deal._fix_handle == fix_handle_text

    # THEN
    x_deal = DealUnit(_healer=tim_text)
    x_deal.set_fix_handle(get_temp_fix_handle())
    x_deal.set_deal_metrics()
    # x_idearoot = IdeaRoot(_label=p_title, _pad="")
    # x_idearoot.set_balancelines_empty_if_null()
    # x_idearoot.set_kids_empty_if_null()
    # x_idearoot.set_balancelink_empty_if_null()
    # x_idearoot.set_balanceheir_empty_if_null()
    # x_idearoot.set_requiredunits_empty_if_null()
    # x_idearoot.set_requiredheirs_empty_if_null()
    # x_idearoot._deal_importance = 1
    # x_idearoot._level = 0
    # x_idearoot._ancestor_promise_count = 0
    # x_idearoot._descendant_promise_count = 0
    # x_idearoot._all_party_credit = True
    # x_idearoot._all_party_debt = True

    assert isol_deal._idearoot == x_deal._idearoot
    assert isol_deal._idearoot._acptfactunits == {}
    assert list(isol_deal._partys.keys()) == [tim_text]
    assert list(isol_deal._groups.keys()) == [tim_text]


def test_healer_save_isol_deal_IsolPersonNameMustBeHealer(
    healer_dir_setup_cleanup,
):
    # GIVEN
    p_title = "Game1"
    env_dir = get_temp_healingunit_dir()
    ux = healingunit_shop(p_title, env_dir, get_temp_fix_handle())
    cx1 = example_deals_get_deal_with_4_levels()
    assert cx1._healer != p_title

    # WHEN
    ux._admin.save_isol_deal(deal_x=cx1)

    # THEN
    assert ux._admin.open_isol_deal()._healer == ux._admin._healing_title


def test_healer_open_isol_deal_WhenStartingDealFileExists(
    healer_dir_setup_cleanup,
):
    # GIVEN
    p_title = "Game1"
    env_dir = get_temp_healingunit_dir()
    ux = healingunit_shop(p_title, env_dir, get_temp_fix_handle())
    ux._admin.save_isol_deal(deal_x=example_deals_get_deal_with_4_levels())

    # WHEN
    assert ux._admin.open_isol_deal() != None
    isol_deal = ux._admin.open_isol_deal()

    # THEN
    x_deal = example_deals_get_deal_with_4_levels()
    x_deal.set_healer(new_healer=p_title)
    x_deal.set_deal_metrics()

    assert isol_deal._idearoot._kids == x_deal._idearoot._kids
    assert isol_deal._idearoot == x_deal._idearoot
    assert isol_deal._idearoot._acptfactunits == {}
    assert isol_deal._partys == {}
    assert isol_deal._groups == {}
    assert isol_deal._healer == ux._admin._healing_title


def test_healer_erase_isol_deal_file_DeletesFileCorrectly(
    healer_dir_setup_cleanup,
):
    # GIVEN
    p_title = "Game1"
    env_dir = get_temp_healingunit_dir()
    ux = healingunit_shop(p_title, env_dir, get_temp_fix_handle())
    ux._admin.save_isol_deal(example_deals_get_deal_with_4_levels())
    file_title = ux._admin._isol_file_title
    assert x_func_open_file(ux._admin._healingunit_dir, file_title) != None

    # WHEN
    ux._admin.erase_isol_deal_file()

    # THEN
    with pytest_raises(Exception) as excinfo:
        x_func_open_file(ux._admin._healingunit_dir, file_title)
    assert (
        str(excinfo.value)
        == f"Could not load file {ux._admin._healingunit_dir}/isol_deal.json (2, 'No such file or directory')"
    )


def test_healingunit_save_deal_to_digest_SavesFileCorrectly(
    healer_dir_setup_cleanup,
):
    # GIVEN
    healing_title = "healer1"
    env_dir = get_temp_healingunit_dir()
    ux = healingunit_shop(healing_title, env_dir, get_temp_fix_handle())
    ux.create_core_dir_and_files()
    cx = example_healers_get_2node_deal()
    src_deal_healer = cx._healer
    assert x_func_count_files(ux._admin._deals_digest_dir) == 0

    # WHEN
    ux._admin.save_deal_to_digest(cx, src_deal_healer=src_deal_healer)

    # THEN
    cx_file_title = f"{cx._healer}.json"
    digest_file_path = f"{ux._admin._deals_digest_dir}/{cx_file_title}"
    print(f"Saving to {digest_file_path=}")
    assert os_path.exists(digest_file_path)
    # for path_x in os_scandir(ux._admin._deals_digest_dir):
    #     print(f"{path_x=}")
    assert x_func_count_files(ux._admin._deals_digest_dir) == 1
    digest_x_deal_json = x_func_open_file(
        dest_dir=ux._admin._deals_digest_dir,
        file_title=f"{src_deal_healer}.json",
    )
    assert digest_x_deal_json == cx.get_json()


def test_presonunit__set_depotlink_CorrectlySets_blind_trust_DigestDeal(
    healer_dir_setup_cleanup,
):
    # GIVEN
    sue_text = "Sue"
    env_dir = get_temp_healingunit_dir()
    sue_cx = healingunit_shop(sue_text, env_dir, get_temp_fix_handle())
    sue_cx.create_core_dir_and_files()
    cx = example_healers_get_2node_deal()
    src_deal_healer = cx._healer
    assert x_func_count_files(sue_cx._admin._deals_digest_dir) == 0

    # WHEN
    sue_cx.set_depot_deal(deal_x=cx, depotlink_type="blind_trust")

    # THEN
    cx_file_title = f"{cx._healer}.json"
    digest_file_path = f"{sue_cx._admin._deals_digest_dir}/{cx_file_title}"
    print(f"Saving to {digest_file_path=}")
    assert os_path.exists(digest_file_path)
    # for path_x in os_scandir(sue_cx._admin._deals_digest_dir):
    #     print(f"{path_x=}")
    assert x_func_count_files(sue_cx._admin._deals_digest_dir) == 1
    digest_x_deal_json = x_func_open_file(
        dest_dir=sue_cx._admin._deals_digest_dir,
        file_title=f"{src_deal_healer}.json",
    )
    assert digest_x_deal_json == cx.get_json()


def test_healer_get_remelded_output_deal_withEmptyDigestDict(
    healer_dir_setup_cleanup,
):
    # GIVEN
    healing_title_x = "boots3"
    ux = healingunit_shop(
        healing_title_x, get_temp_healingunit_dir(), get_temp_fix_handle()
    )
    ux.create_core_dir_and_files()
    sx_output_before = ux._admin.get_remelded_output_deal()
    assert str(type(sx_output_before)).find(".deal.DealUnit'>")
    assert sx_output_before._healer == healing_title_x
    assert sx_output_before._idearoot._label == get_temp_fix_handle()
    # ux.set_digested_deal(deal_x=DealUnit(_healer="digested1"))

    # WHEN
    sx_output_after = ux._admin.get_remelded_output_deal()

    # THEN
    healer_deal_x = DealUnit(_healer=healing_title_x, _weight=0.0)
    healer_deal_x.set_fix_handle(get_temp_fix_handle())
    healer_deal_x._idearoot._pad = ""
    healer_deal_x.set_deal_metrics()

    assert str(type(sx_output_after)).find(".deal.DealUnit'>")
    assert sx_output_after._weight == healer_deal_x._weight
    assert sx_output_after._idearoot._pad == healer_deal_x._idearoot._pad
    assert (
        sx_output_after._idearoot._acptfactunits
        == healer_deal_x._idearoot._acptfactunits
    )
    assert sx_output_after._idearoot == healer_deal_x._idearoot


def test_healer_get_remelded_output_deal_with1DigestedDeal(
    healer_dir_setup_cleanup,
):
    # GIVEN
    yao_text = "Yao"
    env_dir = get_temp_healingunit_dir()
    ux = healingunit_shop(yao_text, env_dir, get_temp_fix_handle())
    ux.create_core_dir_and_files()
    sx_output_old = ux._admin.get_remelded_output_deal()
    assert str(type(sx_output_old)).find(".deal.DealUnit'>")
    assert sx_output_old._healer == yao_text
    assert sx_output_old._idearoot._label == get_temp_fix_handle()
    input_deal = example_healers_get_2node_deal()
    input_deal.meld(input_deal)
    ux.set_depot_deal(deal_x=input_deal, depotlink_type="blind_trust")

    # WHEN
    sx_output_new = ux._admin.get_remelded_output_deal()

    # THEN
    assert str(type(sx_output_new)).find(".deal.DealUnit'>")

    assert sx_output_new._weight == 0
    assert sx_output_new._weight != input_deal._weight
    sx_idearoot = sx_output_new._idearoot
    input_idearoot = input_deal._idearoot
    assert sx_idearoot._pad == input_idearoot._pad
    assert sx_idearoot._acptfactunits == input_idearoot._acptfactunits
    input_b_idea = input_idearoot._kids.get("B")
    sx_output_new_b_idea = sx_idearoot._kids.get("B")
    assert sx_output_new_b_idea._pad == input_b_idea._pad
    assert sx_output_new._idearoot._kids == input_deal._idearoot._kids
    assert sx_idearoot._kids_total_weight == input_idearoot._kids_total_weight
    assert sx_idearoot == input_idearoot
    assert sx_output_new._healer != input_deal._healer
    assert sx_output_new != input_deal


# def test_healer_set_digested_deal_with2Groups(healer_dir_setup_cleanup):
#     # GIVEN
#     env_dir = get_temp_healingunit_dir()
#     ux = healingunit_shop(title="test8", env_dir=env_dir)
#     sx_output_old = ux._admin.get_remelded_output_deal()
#     assert str(type(sx_output_old)).find(".deal.DealUnit'>")
#     assert sx_output_old._groups == {}
#     assert sx_output_old._partys == {}
#     assert sx_output_old._acptfacts == {}

#     src1 = "test1"
#     src1_road = Road(f"{src1}")
#     s1 = DealUnit(_healer=src1)

#     ceci_text = "Ceci"
#     s1.set_partyunit(partyunit=PartyUnit(title=ceci_text))
#     swim_text = "swimmers"
#     swim_group = BraUnit(title=swim_text)
#     swim_group.set_partylink(partylink=partylink_shop(title=ceci_text))
#     s1.set_groupunit(groupunit=swim_group)

#     yaya_text = "yaya"
#     yaya_road = Road(f"{src1},{yaya_text}")
#     s1.add_idea(idea_kid=IdeaKid(_label=yaya_text), pad=src1_road)
#     s1.set_acptfact(base=yaya_road, acptfact=yaya_road)

#     assert s1._groups.get(swim_text).title == swim_text
#     assert s1._partys.get(ceci_text).title == ceci_text
#     assert s1._idearoot._label == src1
#     assert s1._acptfacts.get(yaya_road).base == yaya_road

#     # WHEN
#     ux.set_single_digested_deal(_deal_healer="test1", digest_deal_x=s1)
#     sx_output_new = ux._admin.get_remelded_output_deal()

#     # THEN
#     assert str(type(sx_output_new)).find(".deal.DealUnit'>")
#     assert sx_output_new._acptfacts == s1._acptfacts
#     assert sx_output_new._partys == s1._partys
#     assert sx_output_new._groups == s1._groups
#     assert sx_output_new._weight == s1._weight
#     assert sx_output_new._weight == s1._weight
#     assert sx_output_new._idearoot._pad == s1._idearoot._pad
#     assert sx_output_new._idearoot._acptfactunits == s1._idearoot._acptfactunits
#     assert sx_output_new._idearoot._kids == s1._idearoot._kids
#     assert sx_output_new._idearoot._kids_total_weight == s1._idearoot._kids_total_weight
#     assert sx_output_new._idearoot == s1._idearoot
#     assert sx_output_new._label != s1._label
#     assert sx_output_new != s1


def test_healer_isol_deal_CorrectlysHasOriginLinksWithHealerAsSource(
    healer_dir_setup_cleanup,
):
    # GIVEN
    # healingunit with isol_deal and no other depot deals
    yao_text = "Yao"
    isol_origin_weight = 1
    yao_originunit = originunit_shop()
    yao_originunit.set_originlink(title=yao_text, weight=isol_origin_weight)
    isol_deal_x = example_healers_get_7nodeJRoot_deal()
    isol_deal_x.set_healer(yao_text)

    assert isol_deal_x._idearoot._originunit == originunit_shop()
    assert isol_deal_x._idearoot._originunit != yao_originunit

    ux = healingunit_shop(yao_text, get_temp_healingunit_dir(), get_temp_fix_handle())
    ux.create_core_dir_and_files()
    ux._admin.save_isol_deal(deal_x=isol_deal_x)

    # WHEN
    output_deal_x = ux._admin.get_remelded_output_deal()

    # THEN
    assert output_deal_x._idearoot._originunit == originunit_shop()
    d_road = "J,A,C,D"
    d_idea = output_deal_x.get_idea_kid(road=d_road)
    assert d_idea._originunit == yao_originunit

    print(f"{output_deal_x._originunit=}")
    assert output_deal_x._originunit == yao_originunit

    output_originlink = output_deal_x._originunit._links.get(yao_text)
    assert output_originlink.title == yao_text
    assert output_originlink.weight == isol_origin_weight
