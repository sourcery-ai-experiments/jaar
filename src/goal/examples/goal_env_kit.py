...
def get_test_goals_dir():
    return "src/goal/examples/goals"
...
def create_contract_file_for_goals(goal_dir: str, contract_owner: str):
    contract_x = ContractUnit(_owner=contract_owner)
    contract_dir = f"{goal_dir}/contracts"
...
def create_example_goals_list():
    return x_func_dir_files(
        dir_path=get_test_goals_dir(), include_dirs=True, include_files=False
    )
...
def _delete_and_set_ex3():
    ...
    bob_text = "bob wurld"
    create_contract_file_for_goals(sx.get_object_root_dir(), bob_text)
    ...
    land_text = "tim wurld"
    create_contract_file_for_goals(
        goal_dir=sx.get_object_root_dir(), contract_owner=land_text
    )
    sx.create_depotlink_to_generated_contract(
...
def delete_dir_example_goal(goal_obj: GoalUnit):
    x_func_delete_dir(goal_obj.get_object_root_dir())
