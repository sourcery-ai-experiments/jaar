...
def test_goal_exists():
    goal_kind = "test1"
    sx = GoalUnit(kind=goal_kind, goals_dir=get_test_goals_dir())
    assert sx.kind == goal_kind
...
def test_goal_create_dirs_if_null_CreatesDirAndFiles(env_dir_setup_cleanup):
    ...
    goal_dir = f"src/goal/examples/goals/{goal_kind}"
...
def test_rename_example_goal_CorrectlyRenamesDirAndFiles(env_dir_setup_cleanup):
    # GIVEN create goal
    old_goal_kind = get_temp_env_kind()
    old_goal_dir = f"src/goal/examples/goals/{old_goal_kind}"
    ...
    new_goal_kind = "ex_env1"
    new_goal_dir = f"src/goal/examples/goals/{new_goal_kind}"
    ...
    sx = goalunit_shop(kind=old_goal_kind, goals_dir=get_test_goals_dir())
    # x_func_delete_dir(sx.get_object_root_dir())
    ...
    assert sx.kind != new_goal_kind

    # WHEN
    rename_example_goal(goal_obj=sx, new_title=new_goal_kind)
...
def test_copy_evaluation_goal_CorrectlyCopiesDirAndFiles(env_dir_setup_cleanup):
    ...
    # WHEN
    copy_evaluation_goal(src_kind=sx.kind, dest_kind=new_goal_kind)
...
def test_copy_evaluation_goal_CorrectlyRaisesError(env_dir_setup_cleanup):
    ...
    with pytest_raises(Exception) as excinfo:
        copy_evaluation_goal(src_kind=sx.kind, dest_kind=old_goal_kind)
    assert (
        str(excinfo.value)
...
def test_goalunit_shop_CorrectlyReturnsObj(env_dir_setup_cleanup):
    ...
    # WHEN
    sx = goalunit_shop(kind=park_text, goals_dir=get_test_goals_dir())
