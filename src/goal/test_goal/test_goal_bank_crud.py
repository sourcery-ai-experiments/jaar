...
from src.goal.y_func import get_single_result_back
...
def test_goal_refresh_bank_metrics_CorrectlyDeletesOldBankInMemory(
    env_dir_setup_cleanup,
):
    # GIVEN
    sx = goalunit_shop(kind=get_temp_env_kind(), goals_dir=get_test_goals_dir())
...
def test_goal_refresh_bank_metrics_CorrectlyDeletesOldBankFile(
    env_dir_setup_cleanup,
):
    # GIVEN
    sx = goalunit_shop(kind=get_temp_env_kind(), goals_dir=get_test_goals_dir())
...
def test_goal_refresh_bank_metrics_CorrectlyPopulatesLedgerTable01(
    env_dir_setup_cleanup,
):
...
def test_goal_refresh_bank_metrics_CorrectlyPopulatesContractTable01(
    env_dir_setup_cleanup,
):
    # GIVEN Create example goal with 4 Owners, each with 3 Partyunits = 12 ledger rows
    sx = goalunit_shop(kind=get_temp_env_kind(), goals_dir=get_test_goals_dir())
...
def test_goal_refresh_bank_metrics_CorrectlyPopulates_groupunit_catalog(
    env_dir_setup_cleanup,
):
    # GIVEN
    sx = goalunit_shop(kind=get_temp_env_kind(), goals_dir=get_test_goals_dir())
...
def test_goal_set_contract_bank_attrs_CorrectlyPopulatesContract_Groupunit_Partylinks(
    env_dir_setup_cleanup,
):
    # GIVEN
    sx = goalunit_shop(kind=get_temp_env_kind(), goals_dir=get_test_goals_dir())
    ...
    # change groupunit "swimming expert" _partylinks_set_by_goal_road ==  f"{root_label()}sports,swimmer"
    # run set_bank_metrics
    ...
    swim_group_unit.set_attr(_partylinks_set_by_goal_road=sal_swim_road)
    sal_contract.set_groupunit(groupunit=swim_group_unit)
    sx.save_public_contract(contract_x=sal_contract)
    sx.set_contract_bank_attrs(contract_owner=sal_text)
