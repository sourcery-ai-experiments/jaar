...
from src.pact.origin import OriginUnit
...
class IdeaCore:
    ...
    _pact_importance: float = None
    _pact_coin_onset: float = None
    _pact_coin_cease: float = None
    _task: bool = None
    ...
    def set_pact_importance(
        self,
        coin_onset_x: float,
        parent_pact_importance: float = None,
        parent_coin_cease: float = None,
    ):
        parent_pact_importance = x_func_return1ifnone(parent_pact_importance)
        self.set_kids_total_weight()
        self._pact_importance = None
        self._pact_coin_onset = None
        self._pact_coin_cease = None
        sibling_ratio = self._weight / self._sibling_total_weight
        self._pact_importance = parent_pact_importance * sibling_ratio
        self._pact_coin_onset = coin_onset_x
        self._pact_coin_cease = self._pact_coin_onset + self._pact_importance
        self._pact_coin_cease = min(self._pact_coin_cease, parent_coin_cease)
        self.set_balanceheirs_pact_credit_debit()
    ...
    def set_kidless_balancelines(self):
        ...
        for bh in self._balanceheirs.values():
            balanceline_x = Balanceline(
                brand=bh.brand,
                _pact_credit=bh._pact_credit,
                _pact_debt=bh._pact_debt,
            )
    ...
    def set_balancelines(self, child_balancelines: dict[GroupBrand:Balanceline] = None):
        ...
        for bl in child_balancelines.values():
            ...
            self._balancelines[bl.brand].add_pact_credit_debt(
                pact_credit=bl._pact_credit, pact_debt=bl._pact_debt
            )
    ...
    def set_balanceheirs_pact_credit_debit(self):
        ...
        for balanceheir_x in self._balanceheirs.values():
            balanceheir_x.set_pact_credit_debt(
                idea_pact_importance=self._pact_importance,
    ...
    def set_active_status(
        self,
        tree_traverse_count: int,
        pact_groups: dict[GroupBrand:GroupUnit] = None,
        pact_healer: str = None,
    ):
        self.set_acptfactheirs_empty_if_null()
        ...
        if (
            self._active_status
            and pact_groups != None
            and pact_healer != None
            and self._assignedheir._suffgroups != {}
        ):
            self._assignedheir.set_group_party(pact_groups, pact_healer)
    ...
    def set_requiredheirs(
        ...
        for required in coalesced_requireds.values():
            ...
            if pact_idea_dict != None:
                base_idea = pact_idea_dict.get(required.base)
                if base_idea != None:
    ...
    def set_assignedheir(
        ...
        pact_groups: dict[GroupBrand:GroupUnit],
    ):
...
class IdeaRoot(IdeaCore):
    ...
    def set_idea_label(self, _label: str, pact_cure_handle: str = None):
        if _label != root_label() and pact_cure_handle is None:
            raise IdeaRootLabelNotEmptyException(
                f"Cannot set idearoot to string other than '{root_label()}'"
            )
        elif _label != pact_cure_handle != None:
            raise IdeaRootLabelNotEmptyException(
                f"Cannot set idearoot to string other than '{pact_cure_handle}'"
            )
        elif _label != root_label() and pact_cure_handle == _label:
            self._label = _label
        else:
