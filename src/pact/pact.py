...
from src.pact.x_func import x_get_json
from src.pact.idea import IdeaCore, IdeaKid, IdeaRoot, IdeaAttrHolder
...
from src.pact.lemma import Lemmas
from src.pact.road import (
    get_pad_from_road,
    is_sub_road,
    road_validate,
    change_road,
...
from src.pact.origin import originunit_get_from_dict, originunit_shop, OriginUnit
...
class PactUnit:
    ...
    def export_all_bonds(self, dir: str):
        ...
        for yx in self.get_idea_list():
            if yx.promise:
                cx = self.get_pact_sprung_from_single_idea(yx.get_road())
    ...
    def get_pact_sprung_from_single_idea(self, road: Road):
        self.set_pact_metrics()
    ...
    def _are_all_partys_groups_are_in_idea_kid(self, road: Road) -> bool:
        ...
        # check all pact._partys are in balanceheir_partyunits
    ...
    def add_to_group_pact_credit_debt(
        self,
        groupbrand: GroupBrand,
        balanceheir_pact_credit: float,
        balanceheir_pact_debt: float,
    ):
        for group in self._groups.values():
            if group.brand == groupbrand:
                group.set_empty_pact_credit_debt_to_zero()
                group._pact_credit += balanceheir_pact_credit
                group._pact_debt += balanceheir_pact_debt

    def add_to_group_pact_agenda_credit_debt(
        self,
        groupbrand: GroupBrand,
        balanceline_pact_credit: float,
        balanceline_pact_debt: float,
    ):
        for group in self._groups.values():
            if (
                group.brand == groupbrand
                and balanceline_pact_credit != None
                and balanceline_pact_debt != None
            ):
                group.set_empty_pact_credit_debt_to_zero()
                group._pact_agenda_credit += balanceline_pact_credit
                group._pact_agenda_debt += balanceline_pact_debt

    def add_to_partyunit_pact_credit_debt(
        self,
        partyunit_title: PartyTitle,
        pact_credit,
        pact_debt: float,
        pact_agenda_credit: float,
        pact_agenda_debt: float,
    ):
        for partyunit in self._partys.values():
            if partyunit.title == partyunit_title:
                partyunit.add_pact_credit_debt(
                    pact_credit=pact_credit,
                    pact_debt=pact_debt,
                    pact_agenda_credit=pact_agenda_credit,
                    pact_agenda_debt=pact_agenda_debt,
                )
        ...
        if old_road != new_road:
            ...
            if pad == "":
                self._idearoot.set_idea_label(
                    new_label, pact_cure_handle=self._cure_handle
                )
    ...
    def _add_to_partyunits_pact_credit_debt(self, idea_pact_importance: float):
        ...
        for partyunit_x in self._partys.values():
            au_pact_credit = (
                idea_pact_importance * partyunit_x.get_creditor_weight()
            ) / sum_partyunit_creditor_weight

            au_pact_debt = (
                idea_pact_importance * partyunit_x.get_debtor_weight()
            ) / sum_partyunit_debtor_weight

            partyunit_x.add_pact_credit_debt(
                pact_credit=au_pact_credit,
                pact_debt=au_pact_debt,
                pact_agenda_credit=0,
                pact_agenda_debt=0,
            )

    def _add_to_partyunits_pact_agenda_credit_debt(self, idea_pact_importance: float):
        sum_partyunit_creditor_weight = self.get_partyunit_total_creditor_weight()
        sum_partyunit_debtor_weight = self.get_partyunit_total_debtor_weight()

        for partyunit_x in self._partys.values():
            au_pact_agenda_credit = (
                idea_pact_importance * partyunit_x.get_creditor_weight()
            ) / sum_partyunit_creditor_weight
            ...
            partyunit_x.add_pact_credit_debt(
                pact_credit=0,
                pact_debt=0,
                pact_agenda_credit=au_pact_agenda_credit,
                pact_agenda_debt=au_pact_agenda_debt,
            )

    def _set_partyunits_pact_agenda_importance(self, pact_agenda_importance: float):
        sum_partyunit_creditor_weight = self.get_partyunit_total_creditor_weight()
        sum_partyunit_debtor_weight = self.get_partyunit_total_debtor_weight()

        for partyunit_x in self._partys.values():
            au_pact_agenda_credit = (
                pact_agenda_importance * partyunit_x.get_creditor_weight()
            ) / sum_partyunit_creditor_weight

            au_pact_agenda_debt = (
                pact_agenda_importance * partyunit_x.get_debtor_weight()
            ) / sum_partyunit_debtor_weight

            partyunit_x.add_pact_agenda_credit_debt(
                pact_agenda_credit=au_pact_agenda_credit,
                pact_agenda_debt=au_pact_agenda_debt,
            )

    def _reset_groupunits_pact_credit_debt(self):
        self.set_groupunits_empty_if_null()
        for balancelink_obj in self._groups.values():
            balancelink_obj.reset_pact_credit_debt()

    def _set_groupunits_pact_importance(
        self, balanceheirs: dict[GroupBrand:Balancelink]
    ):
        self.set_groupunits_empty_if_null()
        for balancelink_obj in balanceheirs.values():
            self.add_to_group_pact_credit_debt(
                groupbrand=balancelink_obj.brand,
                balanceheir_pact_credit=balancelink_obj._pact_credit,
                balanceheir_pact_debt=balancelink_obj._pact_debt,
            )

    def _distribute_pact_agenda_importance(self):
        for idea in self._idea_dict.values():
            # If there are no balancelines associated with idea
            # distribute pact_importance via general partyunit
            # credit ratio and debt ratio
            # if idea.is_agenda_item() and idea._balancelines == {}:
            if idea.is_agenda_item():
                if idea._balancelines == {}:
                    self._add_to_partyunits_pact_agenda_credit_debt(
                        idea._pact_importance
                    )
                else:
                    for balanceline_x in idea._balancelines.values():
                        self.add_to_group_pact_agenda_credit_debt(
                            groupbrand=balanceline_x.brand,
                            balanceline_pact_credit=balanceline_x._pact_credit,
                            balanceline_pact_debt=balanceline_x._pact_debt,
                        )

    def _distribute_groups_pact_importance(self):
        for group_obj in self._groups.values():
            group_obj._set_partylink_pact_credit_debt()
            for partylink in group_obj._partys.values():
                self.add_to_partyunit_pact_credit_debt(
                    partyunit_title=partylink.title,
                    pact_credit=partylink._pact_credit,
                    pact_debt=partylink._pact_debt,
                    pact_agenda_credit=partylink._pact_agenda_credit,
                    pact_agenda_debt=partylink._pact_agenda_debt,
                )

    def _set_pact_agenda_ratio_credit_debt(self):
        pact_agenda_ratio_credit_sum = 0
        pact_agenda_ratio_debt_sum = 0

        for partyunit_x in self._partys.values():
            pact_agenda_ratio_credit_sum += partyunit_x._pact_agenda_credit
            pact_agenda_ratio_debt_sum += partyunit_x._pact_agenda_debt

        for partyunit_x in self._partys.values():
            partyunit_x.set_pact_agenda_ratio_credit_debt(
                pact_agenda_ratio_credit_sum=pact_agenda_ratio_credit_sum,
                pact_agenda_ratio_debt_sum=pact_agenda_ratio_debt_sum,
                pact_partyunit_total_creditor_weight=self.get_partyunit_total_creditor_weight(),
                pact_partyunit_total_debtor_weight=self.get_partyunit_total_debtor_weight(),
            )
    ...
    def _reset_partyunit_pact_credit_debt(self):
        self.set_partys_empty_if_null()
        for partyunit in self._partys.values():
            partyunit.reset_pact_credit_debt()
    ...
    def _set_root_attributes(self):
        ...
        self._idearoot.set_requiredheirs(pact_idea_dict=self._idea_dict)
        self._idearoot.set_assignedheir(
            parent_assignheir=None, pact_groups=self._groups
        )
        ...
        self._idearoot.set_pact_importance(coin_onset_x=0, parent_coin_cease=1)
        self._idearoot.set_balanceheirs_pact_credit_debit()
        ...
        if self._idearoot.is_kidless():
            ...
            self._distribute_pact_importance(idea=self._idearoot)
    ...
    def _set_kids_attributes(
        ...
        idea_kid.set_active_status(
            ...
            pact_groups=self._groups,
            pact_healer=self._healer,
        )
        idea_kid.set_sibling_total_weight(parent_idea._kids_total_weight)
        idea_kid.set_pact_importance(
            coin_onset_x=coin_onset,
            parent_pact_importance=parent_idea._pact_importance,
        ...
        if idea_kid.is_kidless():
            # set idea's ancestor metrics using pact root as common reference
            self._set_ancestor_metrics(road=idea_kid.get_road())
            self._distribute_pact_importance(idea=idea_kid)

    def _distribute_pact_importance(self, idea: IdeaCore):
        ...
        if idea.is_balanceheirless() == False:
            self._set_groupunits_pact_importance(idea._balanceheirs)
        elif idea.is_balanceheirless():
            self._add_to_partyunits_pact_credit_debt(idea._pact_importance)

    def get_pact_importance(
        self, parent_pact_importance: float, weight: int, sibling_total_weight: int
    ):
        sibling_ratio = weight / sibling_total_weight
        return parent_pact_importance * sibling_ratio

    def get_idea_list(self):
        self.set_pact_metrics()
        return list(self._idea_dict.values())

    def set_pact_metrics(self):
        self._set_acptfacts_empty_if_null()
    ...
    def _execute_tree_traverse(self):
        ...
        coin_onset = self._idearoot._pact_coin_onset
        parent_coin_cease = self._idearoot._pact_coin_cease
        ...
        for idea_kid in self._idearoot._kids.values():
            ...
            coin_onset += idea_kid._pact_importance
        ...
        while cache_idea_list != []:
            ...
            if parent_idea._kids != None:
                coin_onset = parent_idea._pact_coin_onset
                parent_coin_cease = parent_idea._pact_coin_cease
    ...
    def _run_after_idea_all_tree_traverses(self):
        self._distribute_pact_agenda_importance()
        self._distribute_groups_pact_importance()
        self._set_pact_agenda_ratio_credit_debt()

    def _run_before_idea_tree_traverse(self):
        self._reset_groupunits_pact_credit_debt()
        self._reset_groupunits_pact_credit_debt()
        self._reset_partyunit_pact_credit_debt()
    ...
    def get_pact4party(
        ...
        # set pact4party by traversing the idea tree and selecting associated groups
        # set root
        not_included_pact_importance = 0
        pact4party._idearoot._kids = {}
        for ykx in self._idearoot._kids.values():
            ...
            if y4a_included:
                y4a_new = IdeaKid(
                    _label=ykx._label,
                    _pact_importance=ykx._pact_importance,
                ...
                pact4party._idearoot._kids[ykx._label] = y4a_new
            else:
                not_included_pact_importance += ykx._pact_importance

        if not_included_pact_importance > 0:
            y4a_other = IdeaKid(
                _label="__other__",
                _pact_importance=not_included_pact_importance,
            )
            pact4party._idearoot._kids[y4a_other._label] = y4a_other
    ...
    def meld(self, other_pact, party_weight: float = None):
        self._meld_groups(other_pact)
        self._meld_partys(other_pact)
        self._meld_ideas(other_pact, party_weight)
        self._meld_acptfacts(other_pact)
        self._weight = get_meld_weight(
            src_weight=self._weight,
            src_on_meld_weight_action="default",
            other_weight=other_pact._weight,
            other_on_meld_weight_action="default",
        )
        self._meld_originlinks(other_pact._healer, party_weight)

    def _meld_ideas(self, other_pact, party_weight: float):
        # meld idearoot
        self._idearoot.meld(other_idea=other_pact._idearoot, _idearoot=True)

        # meld all other ideas
        party_title = other_pact._healer
        o_idea_list = other_pact.get_idea_list_without_idearoot()
    ...
    def _meld_partys(self, other_pact):
        self.set_partys_empty_if_null()
        other_pact.set_partys_empty_if_null()
        for partyunit in other_pact._partys.values():
            if self._partys.get(partyunit.title) is None:
                self.set_partyunit(partyunit=partyunit)
            else:
                self._partys.get(partyunit.title).meld(partyunit)

    def _meld_groups(self, other_pact):
        self.set_groupunits_empty_if_null()
        other_pact.set_groupunits_empty_if_null()
        for brx in other_pact._groups.values():
            if self._groups.get(brx.brand) is None:
                self.set_groupunit(groupunit=brx)
            else:
                self._groups.get(brx.brand).meld(brx)

    def _meld_acptfacts(self, other_pact):
        self._set_acptfacts_empty_if_null()
        other_pact._set_acptfacts_empty_if_null()
        for hx in other_pact._idearoot._acptfactunits.values():
    ...
    def get_assignment(
        ...
        assignor_promises = self._get_assignor_promise_ideas(pact_x, assignor_title)
        relevant_roads = self._get_relevant_roads(assignor_promises)
        self._set_assignment_ideas(pact_x, relevant_roads)
        return pact_x

    def _set_assignment_ideas(self, pact_x, relevant_roads: dict[Road:str]):
        ...
        for relevant_road in sorted_relevants:
            ...
            #     relevant_idea._kids = {}
            #     pact_x.add_idea(idea_kid=relevant_idea, pad=relevant_idea._pad)
    ...
    def _set_assignment_partys(
        ...
        pact_x.set_partys_empty_if_null()
        if self._partys.get(assignor_title) != None:
            ...
            for partytitle_x in partys_set:
                pact_x.set_partyunit(partyunit=self._partys.get(partytitle_x))
        return pact_x

    def _set_assignment_groups(self, pact_x):
        revelant_groups = get_partys_relevant_groups(self._groups, pact_x._partys)
        for group_title, group_partys in revelant_groups.items():
            if pact_x._groups.get(group_title) is None:
                ...
                pact_x.set_groupunit(group_x)

    def _get_assignor_promise_ideas(
        self, pact_x, assignor_title: GroupBrand
    ) -> dict[Road:int]:
        pact_x.set_groupunits_empty_if_null()
        assignor_groups = get_party_relevant_groups(pact_x._groups, assignor_title)
        return {
            idea_road: -1
...
def get_from_dict(cx_dict: dict) -> PactUnit:
    ...
    c_x.set_pact_metrics()  # clean up tree traverse defined fields
    return c_x
...
def get_dict_of_pact_from_dict(x_dict: dict[str:dict]) -> dict[str:PactUnit]:
    ...
    for pactunit_dict in x_dict.values():
        ...
        pactunits[x_pact._healer] = x_pact
    return pactunits
