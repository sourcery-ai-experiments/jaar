import dataclasses
from src.pact.group import groupunit_shop, GroupUnit, GroupBrand
...
class AssignedHeir:
    ...
    def _get_all_partys(
        ...
        for groupbrand_x in groupbrand_dict:
            dict_x |= pact_groups.get(groupbrand_x)._partys
        return dict_x

    def _get_all_suff_partys(self, pact_groups: dict[GroupBrand:GroupUnit]):
        return self._get_all_partys(pact_groups, self._suffgroups)

    def set_group_party(
        self, pact_groups: dict[GroupBrand:GroupUnit], pact_healer: PartyTitle
    ):
        self._group_party = False
        if self._suffgroups == {}:
            self._group_party = True
        else:
            all_suff_partys_x = self._get_all_suff_partys(pact_groups)
            if all_suff_partys_x.get(pact_healer) != None:
                self._group_party = True
