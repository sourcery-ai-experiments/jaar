...
class GroupUnit(GroupCore):
    ...
    def reset_pact_credit_debt(self):
        ...
        for partylink in self._partys.values():
            partylink.reset_pact_credit_debt()

    def _set_partylink_pact_credit_debt(self):
        ...
        for partylink in self._partys.values():
            partylink.set_pact_credit_debt(
                partylinks_creditor_weight_sum=partylinks_creditor_weight_sum,
                partylinks_debtor_weight_sum=partylinks_debtor_weight_sum,
                group_pact_credit=self._pact_credit,
                group_pact_debt=self._pact_debt,
                group_pact_agenda_credit=self._pact_agenda_credit,
                group_pact_agenda_debt=self._pact_agenda_debt,
            )
...
class Balanceline(GroupCore):
    ...
    def add_pact_credit_debt(self, pact_credit: float, pact_debt: float):
        self.set_pact_credit_debt_zero_if_null()
        self._pact_credit += pact_credit
        self._pact_debt += pact_debt

    def set_pact_credit_debt_zero_if_null(self):
        if self._pact_credit is None:
            self._pact_credit = 0
        if self._pact_debt is None:
            self._pact_debt = 0
