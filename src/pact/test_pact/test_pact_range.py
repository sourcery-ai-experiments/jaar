...
def test_pactAddingIdeaWithAddinCorrectlyTransformsRangeScenario1():
    ...
    x_pact.add_idea(pad=x_pact._cure_handle, idea_kid=idea_kid_l1)
    l1_road = f"{x_pact._cure_handle},{l1}"

    rx1 = "range_root_example"
    idea_kid_rx1 = IdeaKid(_weight=30, _label=rx1)
    x_pact.add_idea(pad=l1_road, idea_kid=idea_kid_rx1)
    rx1_road = f"{l1_road},{rx1}"
    x_pact.edit_idea_attr(road=rx1_road, begin=10, close=25)

    y_idea = x_pact.get_idea_kid(road=rx1_road)
    ...
    x_pact.add_idea(pad=rx1_road, idea_kid=idea_kid_rcA)

    rcA_road = f"{rx1_road},{rcA}"
    x_idea = x_pact.get_idea_kid(road=rcA_road)

    assert x_idea._begin == 10
    assert x_idea._close == 25

    # When
    x_pact.edit_idea_attr(road=rcA_road, addin=7)
...
def test_pactAddingIdeaWithAddinCorrectlyTransformsRangeScenario2():
    ...
    # When
    x_pact.edit_idea_attr(road=rcA_road, addin=15, denom=5)
...
def test_get_idea_ranged_kids_CorrectlyReturnsAllChildren():
    ...
    # WHEN
    weekunit_road = f"{x_pact._cure_handle},time,tech,week"
    ranged_ideas = x_pact.get_idea_ranged_kids(idea_road=weekunit_road)
...
def test_get_idea_ranged_kids_CorrectlyReturnsSomeChildrenScen1():
    ...
    begin_x = 1440
    close_x = 4 * 1440
    ranged_ideas = x_pact.get_idea_ranged_kids(
...
def test_get_idea_ranged_kids_CorrectlyReturnsSomeChildrenScen2():
    ...
    # WHEN THEN
    week_road = f"{x_pact._cure_handle},time,tech,week"
    assert (
        len(x_pact.get_idea_ranged_kids(idea_road=week_road, begin=0, close=1440)) == 1
    )
    assert (
        len(x_pact.get_idea_ranged_kids(idea_road=week_road, begin=0, close=2000)) == 2
    )
    assert (
        len(x_pact.get_idea_ranged_kids(idea_road=week_road, begin=0, close=3000)) == 3
    )
...
def test_get_idea_ranged_kids_CorrectlyReturnsSomeChildrenScen3():
    ...
    # WHEN THEN
    week_road = f"{x_pact._cure_handle},time,tech,week"
    assert len(x_pact.get_idea_ranged_kids(idea_road=week_road, begin=0)) == 1
    assert len(x_pact.get_idea_ranged_kids(idea_road=week_road, begin=1440)) == 1
