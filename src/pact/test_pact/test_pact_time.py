...
def test_time_get_time_min_from_dt_WorksCorrectly():
    ...
    x_pact.set_time_hreg_ideas(c400_count=6)
    assert x_pact.get_time_min_from_dt(dt=datetime(2000, 1, 1, 0, 0))
    assert x_pact.get_time_min_from_dt(dt=datetime(1, 1, 1, 0, 0)) == 527040
    assert x_pact.get_time_min_from_dt(dt=datetime(1, 1, 2, 0, 0)) == 527040 + 1440
    assert x_pact.get_time_min_from_dt(dt=datetime(400, 1, 1, 0, 0)) == 210379680
    assert x_pact.get_time_min_from_dt(dt=datetime(800, 1, 1, 0, 0)) == 420759360
    assert x_pact.get_time_min_from_dt(dt=datetime(1200, 1, 1, 0, 0)) == 631139040
...
def test_get_time_400YearCycle_from_min_WorksCorrectly():
    ...
    x_pact = PactUnit(_healer=healer_text)
    x_pact.set_time_hreg_ideas(c400_count=6)
    assert x_pact.get_time_c400_from_min(min=0)[0] == 0
    assert x_pact.get_time_c400_from_min(min=210379680)[0] == 1
    assert x_pact.get_time_c400_from_min(min=210379681)[0] == 1
    assert x_pact.get_time_c400_from_min(min=841518720)[0] == 4
...
def test_get_time_c400year_from_min_WorksCorrectly():
    ...
    x_pact = PactUnit(_healer=healer_text)
    x_pact.set_time_hreg_ideas(c400_count=6)
    assert x_pact.get_time_c400yr_from_min(min=0)[0] == 0
    assert x_pact.get_time_c400yr_from_min(min=1)[0] == 0
    assert x_pact.get_time_c400yr_from_min(min=1)[2] == 1
    assert x_pact.get_time_c400yr_from_min(min=210379680)[0] == 0
    assert x_pact.get_time_c400yr_from_min(min=210379680)[0] == 0
    assert x_pact.get_time_c400yr_from_min(min=210379681)[0] == 0
    assert x_pact.get_time_c400yr_from_min(min=841518720)[0] == 0
    assert x_pact.get_time_c400yr_from_min(min=576000)[0] == 1
    assert x_pact.get_time_c400yr_from_min(min=4608000)[0] == 8
    assert x_pact.get_time_c400yr_from_min(min=157785120)[0] == 300


def _check_time_conversion_works_with_random_inputs(x_pact: PactUnit):
    py_dt = datetime(
        year=randint(1, 2800),
        month=randint(1, 12),
        day=randint(1, 28),
    ...
    assert py_dt == x_pact.get_time_dt_from_min(
        min=x_pact.get_time_min_from_dt(dt=py_dt)
    )
...
def test_get_time_dt_from_min_WorksCorrectly():
    ...
    x_pact = PactUnit(_healer=healer_text)
    x_pact.set_time_hreg_ideas(c400_count=6)
    assert x_pact.get_time_dt_from_min(min=5000000)
    # assert x_pact.get_time_dt_from_min(
    #     min=x_pact.get_time_min_from_dt(dt=datetime(2000, 1, 1, 0, 0))
    # ) == datetime(2000, 1, 1, 0, 0)
    assert x_pact.get_time_dt_from_min(min=420759360) == datetime(800, 1, 1, 0, 0)
    assert x_pact.get_time_dt_from_min(min=631139040) == datetime(1200, 1, 1, 0, 0)
    assert x_pact.get_time_dt_from_min(min=631751040) == datetime(1201, 3, 1, 0, 0)
    assert x_pact.get_time_dt_from_min(min=631751060) == datetime(1201, 3, 1, 0, 20)

    x_minutes = 1063903680
    assert x_pact.get_time_dt_from_min(min=x_minutes) == datetime(2022, 10, 29, 0, 0)
    x_next_day = x_minutes + 1440
    assert x_pact.get_time_dt_from_min(min=x_next_day) == datetime(2022, 10, 30, 0, 0)

    _check_time_conversion_works_with_random_inputs(x_pact)
    _check_time_conversion_works_with_random_inputs(x_pact)
    _check_time_conversion_works_with_random_inputs(x_pact)
    ...
    #     jaja_min = x_pact.get_time_min_from_dt(dt=py_dt)
    #     # print(f"assert for {year=} {month=} {day=} {jaja_min}")

    #     jaja_dt = x_pact.get_time_dt_from_min(min=jaja_min)
    #     print(
    ...
    #             assert x_pact.get_time_dt_from_min(
    #                 min=x_pact.get_time_min_from_dt(dt=datetime(year, month, day, 0, 0))
...
def test_get_time_():
    ...
    assert x_pact._idearoot._acptfactunits[jaja_road]
    assert x_pact._idearoot._acptfactunits[jaja_road].open == 1051898400  # - 1440
    assert x_pact._idearoot._acptfactunits[jaja_road].nigh == 1053934800  # - 1440


# def test_time_hreg_set_exists():
#     x_pact = PactUnit(_healer=healer_text)
#     x_pact.set_time_hreg_ideas(c400_count=6)
#     idea_x = x_pact.get_idea_kid(road=f"{root_label()},hreg")
#     assert idea_x != None
#     assert x_pact._kids["hreg"]
#     for kid in x_pact._kids["hreg"]._kids.values():
#         print(f"hreg kid= {kid._label=}")

#     assert len(x_pact._kids["hreg"]._kids) > 0


# def test_time_hreg_set_creates_idea():
#     x_pact = examples.get_pact_base_time_example()

#     hreg_title = "hreg"
#     with pytest.raises(KeyError) as excinfo:
#         x_pact._kids[hreg_title]
#     assert str(excinfo.value) == f"'{hreg_title}'"
#     print(f"added {hreg_title}")
#     x_pact.set_time_hreg_ideas(c400_count=6)
#     hreg_idea = x_pact._kids[hreg_title]
...
#     weekday = x_pact.get_idea_kid(road=f"{root_label()},hreg,{weekday_title}")
#     assert weekday != None
...
#     h_x_pact = examples.get_pact_base_time_example()
#     c400_count = 6
#     h_x_pact.set_time_hreg_ideas(c400_count=c400_count)

#     hy400_title = "cycle400year_years"
#     hy400_road = f"{root_label()},hreg,{hy400_title}"
#     print(f"{hy400_road=}")
#     hy400_idea = h_x_pact.get_idea_kid(road=hy400_road)
...
#     x_pact = examples.get_pact_base_time_example()
#     x_pact.set_time_hreg_ideas(c400_count=6)
#     timetech = x_pact.get_idea_kid(road=f"{root_label()},hreg,day_range")
#     assert timetech != None
#     assert timetech._begin == 0
#     assert timetech._close == 876582

# x_x_pact = PactUnit()
# x_pact.get_idea_kid(road={f"{root_label()},hreg,weekday"})
...
# pact_x.edit_idea_attr(road=work_road, required=work_wk_required)
# work_idea = pact_x._kids["work"]
...
# x_pact = examples.get_pact_gregorian_years()
...
def test_get_jajatime_repeating_legible_text_correctlyText():
    ...
    assert (
        x_pact.get_jajatime_repeating_legible_text(open=480, nigh=480, divisor=1440)
        == "every day at 8am"
    )

    print("ReturnsEvery2DaysText")
    assert (
        x_pact.get_jajatime_repeating_legible_text(open=490, nigh=490, divisor=2880)
        == "every 2nd day at 8:10am"
    )

    print("ReturnsEvery6DaysText")
    assert (
        x_pact.get_jajatime_repeating_legible_text(open=480, nigh=480, divisor=8640)
        == "every 6th day at 8am"
    )

    print("ReturnsWeeklyText")
    assert (
        x_pact.get_jajatime_repeating_legible_text(open=480, nigh=480, divisor=10080)
        == "every Saturday at 8am"
    )

    print("ReturnsEvery2WeeksText")
    assert (
        x_pact.get_jajatime_repeating_legible_text(open=480, nigh=480, divisor=20160)
        == "every 2nd Saturday at 8am"
    )

    print("ReturnsEvery6WeeksText")
    assert (
        x_pact.get_jajatime_repeating_legible_text(open=480, nigh=480, divisor=60480)
        == "every 6th Saturday at 8am"
    )

    print("ReturnsOneTimeEventCorrectlyMorning")
    assert (
        x_pact.get_jajatime_repeating_legible_text(open=1064041020.0, nigh=1064041020.0)
        == "Wed Feb 1st, 2023 at 9am"
    )

    print("ReturnsOneTimeEventCorrectlyEvening")
    assert (
        x_pact.get_jajatime_repeating_legible_text(open=1064041620.0, nigh=1064041620.0)
        == "Wed Feb 1st, 2023 at 7pm"
    )

    print("ReturnsOneTimeEventCorrectlyMidnight")
    assert (
        x_pact.get_jajatime_repeating_legible_text(open=1064041920.0, nigh=1064041920.0)
