...
from src.pact.examples.example_pacts import (
    ...
    get_pact_with_4_levels_and_2requireds as example_pacts_get_pact_with_4_levels_and_2requireds,
    get_pact_with7amCleanTableRequired as example_pacts_get_pact_with7amCleanTableRequired,
    get_pact_with_4_levels_and_2requireds_2acptfacts as example_pacts_get_pact_with_4_levels_and_2requireds_2acptfacts,
...
def test_pactunit_get_pact_sprung_from_single_idea_ReturnsCorrectContractScenario1():
    ...
    # WHEN
    bond_pact = cx.get_pact_sprung_from_single_idea(road=clean_cookery_road)

    # THEN
    # assert bond_pact._label == clean_cookery_text
    print(f"{len(bond_pact._idea_dict)=}")
    assert len(bond_pact._idea_dict) == 3
    b_src_idea = bond_pact.get_idea_kid(road=cx._cure_handle)
    ...
    b_casa_idea = bond_pact.get_idea_kid(road=casa_road)
    ...
    b_clean_cookery_idea = bond_pact.get_idea_kid(road=clean_cookery_road)
    ...
    assert bond_pact._idearoot._kids.get(water_text) is None

    # for byx in bond_pact._idea_dict.values():
    ...
    #             print(f"checking src pact idea kid_label='{cx3._label}'")
...
def test_pactunit_export_all_bonds_ExportsFileOfBonds_2files(env_dir_setup_cleanup):
    # GIVEN
    cx = example_pacts_get_pact_with_4_levels_and_2requireds_2acptfacts()
...
# def test_pactunit_export_all_bonds_ExportsFileOfBonds_69files(env_dir_setup_cleanup):
#     # GIVEN
#     cx = example_pacts_pact_v001()
#     cx_idea_list = cx.get_idea_list()
#     action_count = 0
...
# def test_pactunit_export_all_bonds_ReturnsDictOfBonds(env_dir_setup_cleanup):
#     # GIVEN
#     cx = example_pacts_get_pact_with_4_levels_and_2requireds_2acptfacts()
#     cx_idea_list = cx.get_idea_list()
#     action_count = sum(bool(yx.promise) for yx in cx_idea_list)
