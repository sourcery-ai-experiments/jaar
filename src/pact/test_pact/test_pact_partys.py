...
from src.pact.pact import ContractUnit, get_intersection_of_partys
...
def test_pact_get_idea_list_CorrectlySetsPartyLinkContractCreditAndDebt():
    # GIVEN
    prom_text = "prom"
    ...
    #     print(
    #         f"{cx._pact_importance=} {balancelink.brand=} {balancelink._pact_credit=} {balancelink._pact_debt=}"
    #     )
...
def test_pact_get_idea_list_CorrectlySetsPartyUnitContractImportance():
    # GIVEN
    prom_text = "prom"
    cx = ContractUnit(_healer=prom_text)
    swim_text = "swim"
...
def test_pact_get_idea_list_CorrectlySetsPartGroupedLWPartyUnitContractImportance():
    # GIVEN
    prom_text = "prom"
    cx = ContractUnit(_healer=prom_text)
    swim_text = "swim"
...
def clear_all_partyunits_groupunits_pact_agenda_credit_debt(cx: ContractUnit):
...
def test_pact_agenda_credit_debt_IsCorrectlySet():
    # GIVEN
    cx = examples_pact_v001_with_large_agenda()
    clear_all_partyunits_groupunits_pact_agenda_credit_debt(cx=cx)
...
def test_pact_agenda_ratio_credit_debt_IsCorrectlySetWhenAgendaIsEmpty():
    ...
    assert cx_rico_party._pact_agenda_credit in [0, None]
    assert cx_rico_party._pact_agenda_debt in [0, None]
    assert cx_carm_party._pact_agenda_credit in [0, None]
    assert cx_carm_party._pact_agenda_debt in [0, None]
    assert cx_patr_party._pact_agenda_credit in [0, None]
    assert cx_patr_party._pact_agenda_debt in [0, None]
    assert cx_rico_party._pact_agenda_ratio_credit != 0.05
    assert cx_rico_party._pact_agenda_ratio_debt != 0.2
    assert cx_carm_party._pact_agenda_ratio_credit != 0.15
    assert cx_carm_party._pact_agenda_ratio_debt != 0.3
    assert cx_patr_party._pact_agenda_ratio_credit != 0.8
    assert cx_patr_party._pact_agenda_ratio_debt != 0.5

    # WHEN
    cx.set_pact_metrics()

    # THEN
    assert cx_rico_party._pact_agenda_credit == 0
    assert cx_rico_party._pact_agenda_debt == 0
    assert cx_carm_party._pact_agenda_credit == 0
    assert cx_carm_party._pact_agenda_debt == 0
    assert cx_patr_party._pact_agenda_credit == 0
    assert cx_patr_party._pact_agenda_debt == 0
    assert cx_rico_party._pact_agenda_ratio_credit == 0.05
    assert cx_rico_party._pact_agenda_ratio_debt == 0.2
    assert cx_carm_party._pact_agenda_ratio_credit == 0.15
    assert cx_carm_party._pact_agenda_ratio_debt == 0.3
