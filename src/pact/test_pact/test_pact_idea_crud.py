...
def test_root_has_kids():
    ...
    # WHEN
    cx.add_idea(idea_kid=idea1, pad=cx._healer)
    cx.add_idea(idea_kid=idea2, pad=cx._healer)
...
def test_pact_add_idea_CanAddKidToRootIdea():
    ...
    # WHEN
    cx.add_idea(idea_kid=new_idea, pad=new_idea_parent_road)
    cx.set_pact_metrics()
...
def test_pact_add_idea_CanAddKidToKidIdea():
    ...
    new_idea_parent_road = f"{cx._cure_handle},work"
    ...
    assert (
        cx._idearoot._kids["work"]._kids["new_york"]._pad == f"{cx._cure_handle},work"
    )
    cx._idearoot._kids["work"]._kids["new_york"].set_pad(parent_road="testing")
    assert cx._idearoot._kids["work"]._kids["new_york"]._pad == "testing"
...
def test_pact_add_idea_CanAddKidToGrandkidIdea():
    ...
    assert cx.get_node_count() == 17
    assert cx.get_level_count(level=3) == 2
    new_idea_parent_road = f"{cx._cure_handle},weekdays,Wednesday"
    new_idea = IdeaKid(_weight=40, _label="new_idea")
...
def test_pact_add_idea_CanCreateRoadToGrandkidIdea():
    ...
    assert cx.get_node_count() == 17
    assert cx.get_level_count(level=3) == 2
    new_idea_parent_road = f"{cx._cure_handle},ww2,battles,coralsea"
...
def test_pact_add_idea_creates_requireds_ideas():
    ...
    assert cx.get_node_count() == 17
    assert cx.get_level_count(level=3) == 2
    new_idea_parent_road = f"{cx._cure_handle},work,cleaning"
    ...
    buildings_road = Road(f"{cx._cure_handle},{buildings_text}")
    cookery_room_text = "cookery"
    cookery_room_road = Road(f"{cx._cure_handle},{buildings_text},{cookery_room_text}")
...
def test_pact_del_idea_kid_IdeaLevel1CanBeDeleted_ChildrenDeleted():
    ...
    new_sunday_road = f"{cx._cure_handle},Sunday"
    with pytest_raises(Exception) as excinfo:
...
def test_pact_del_idea_kid_IdeaLevel2CanBeDeleted_ChildrenDeleted():
    # GIVEN
    cx = get_pact_with_4_levels()
    monday_road = f"{cx._cure_handle},weekdays,Monday"
...
def test_pact_edit_idea_attr_IsAbleToEditAnyAncestor_Idea():
    ...
    acptfact_road = f"{cx._cure_handle},weekdays,Sunday"
...
def test_pact_edit_idea_attr_pactIsAbleToEditDenomAnyIdeaIfInvaildDenomThrowsError():
    ...
    cx.add_idea(work_idea, pad=cx._cure_handle)
    clean = "clean"
    clean_idea = IdeaKid(_label=clean)
    c_road = f"{w_road},{clean}"
    cx.add_idea(clean_idea, pad=w_road)
    ...
    assert (
        str(excinfo.value)
        == f"Idea cannot edit numor=1/denom/reest of '{cx._cure_handle},work,clean' if parent '{cx._cure_handle},work' or ideacore._numeric_road does not have begin/close range"
    )
...
def test_pact_edit_idea_attr_pactIsAbleToEditDenomAnyIdeaInvaildDenomThrowsError():
    ...
    day_road = f"{cx._cure_handle},{day}"
...
def test_pact_edit_idea_attr_pactWhenParentAndNumeric_roadBothHaveRangeThrowError():
    ...
    assert (
        str(excinfo.value)
        == f"Idea cannot edit numor=1/denom/reest of '{cx._cure_handle},work' if parent '{cx._cure_handle}' or ideacore._numeric_road does not have begin/close range"
    )
...
def test_pact_add_idea_adoptee_RaisesErrorIfAdopteeIdeaDoesNotHaveCorrectParent():
    ...
    swim_text = "swim"
    cx.add_idea(IdeaKid(_label=swim_text), pad=sports_road)
...
def test_pact_add_idea_adoptee_CorrectlyAddsAdoptee():
    ...
    cx.add_idea(IdeaKid(_label=swim_text), pad=sports_road)
    hike_text = "hike"
    cx.add_idea(IdeaKid(_label=hike_text), pad=sports_road)
...
def test_pact_add_idea_bundling_SetsNewParentWithWeightEqualToSumOfAdoptedIdeas():
    ...
    swim_text = "swim"
    swim_weight = 3
    cx.add_idea(IdeaKid(_label=swim_text, _weight=swim_weight), pad=sports_road)
    hike_text = "hike"
    hike_weight = 5
    cx.add_idea(IdeaKid(_label=hike_text, _weight=hike_weight), pad=sports_road)
    bball_text = "bball"
    bball_weight = 7
    cx.add_idea(IdeaKid(_label=bball_text, _weight=bball_weight), pad=sports_road)
