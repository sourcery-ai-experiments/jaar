...
from src.pact.examples.example_pacts import (
    get_pact_with_4_levels_and_2requireds,
    get_pact_with7amCleanTableRequired,
    pact_v001,
)
...
def test_pact_yoke_dict_isDictionaryAndIsFullyPopulated():
    # GIVEN
    x_pact = get_pact_with_4_levels_and_2requireds()

    # WHEN
    x_pact.set_pact_metrics()

    # THEN
    assert len(x_pact._idea_dict) == 17


def test_pact_get_idea_list_SetsSatiateStatusCorrectlyWhenAcptFactSaysNo():
    # GIVEN
    x_pact = get_pact_with_4_levels_and_2requireds()
    week_text = "weekdays"
    ...
    # for idea in x_pact._idea_dict.values():
    #     print(f"{work_road=} {idea.get_road()=}")
    work_text = "work"
    work_road = f"{x_pact._cure_handle},{work_text}"
    assert x_pact._idea_dict.get(work_road)._active_status == False


def test_pact_get_idea_list_SetsSatiateStatusCorrectlyWhenAcptFactChanges():
    # GIVEN
    x_pact = get_pact_with_4_levels_and_2requireds()
    week_text = "weekdays"
    week_road = f"{x_pact._cure_handle},{week_text}"
    ...
    x_pact.set_acptfact(base=states_road, pick=usa_road)

    # THEN
    idea_list = x_pact.get_idea_list()
    assert idea_list
    assert len(idea_list) == 17
    assert x_pact._idea_dict.get(work_road)._active_status

    # WHEN
    france_text = "France"
    france_road = f"{states_road},{france_text}"
    x_pact.set_acptfact(base=states_road, pick=france_road)
...
def test_pact_get_idea_list_returns_correct_list():
    # GIVEN
    x_pact = get_pact_with_4_levels_and_2requireds()
    week_text = "weekdays"
    week_road = f"{x_pact._cure_handle},{week_text}"
    ...
    state_road = f"{x_pact._cure_handle},{state_text}"
    france_text = "France"
    france_road = f"{state_road},{france_text}"
    x_pact.set_acptfact(base=week_road, pick=wed_road)
    x_pact.set_acptfact(base=state_road, pick=france_road)

    work_text = "work"
    work_road = f"{x_pact._cure_handle},{work_text}"
    work_idea = x_pact.get_idea_kid(work_road)
    print(f"{x_pact._healer=} {len(work_idea._requiredunits)=}")
    # print(f"{work_idea._requiredunits=}")
    print(f"{x_pact._healer=} {len(x_pact._idearoot._acptfactunits)=}")
    # print(f"{x_pact._idearoot._acptfactunits=}")
    ...
    # WHEN
    x_pact.set_acptfact(base=state_road, pick=oregon_road)

    # THEN
    work_idea = x_pact._idea_dict.get(work_road)
    print(f"\nlook at {work_idea.get_road()=}")
    assert work_idea._pad == f"{x_pact._cure_handle}"
    ...
    #             x_dict=required.sufffacts, type_str="src.pact.required.SuffFactUnit"
    #         )


def test_pact_set_pact_metrics_CorrectlyClears_pact_coin():
    # GIVEN
    x_pact = get_pact_with7amCleanTableRequired()
    work_text = "work"
    catt_text = "feed cat"
    week_text = "weekdays"
    x_pact._idearoot._pact_coin_onset = 13
    x_pact._idearoot._pact_coin_cease = 13
    x_pact._idearoot._kids.get(work_text)._pact_coin_onset = 13
    x_pact._idearoot._kids.get(work_text)._pact_coin_cease = 13
    x_pact._idearoot._kids.get(catt_text)._pact_coin_onset = 13
    x_pact._idearoot._kids.get(catt_text)._pact_coin_cease = 13
    x_pact._idearoot._kids.get(week_text)._pact_coin_onset = 13
    x_pact._idearoot._kids.get(week_text)._pact_coin_cease = 13

    assert x_pact._idearoot._pact_coin_onset == 13
    assert x_pact._idearoot._pact_coin_cease == 13
    assert x_pact._idearoot._kids.get(work_text)._pact_coin_onset == 13
    assert x_pact._idearoot._kids.get(work_text)._pact_coin_cease == 13
    assert x_pact._idearoot._kids.get(catt_text)._pact_coin_onset == 13
    assert x_pact._idearoot._kids.get(catt_text)._pact_coin_cease == 13
    assert x_pact._idearoot._kids.get(week_text)._pact_coin_onset == 13
    assert x_pact._idearoot._kids.get(week_text)._pact_coin_cease == 13
...
def test_pact_get_idea_list_CorrectlyCalculatesIdeaAttr_pact_coin():
    ...
    x_pact.add_idea(idea_kid=auto_idea, pad=x_pact._cure_handle)

    barn_text = "barn"
    barn_road = f"{x_pact._cure_handle},{barn_text}"
    barn_idea = IdeaKid(_label=barn_text, _weight=60)
    x_pact.add_idea(idea_kid=barn_idea, pad=x_pact._cure_handle)
    lamb_text = "lambs"
    lamb_road = f"{barn_road},{lamb_text}"
    lamb_idea = IdeaKid(_label=lamb_text, _weight=1)
    x_pact.add_idea(idea_kid=lamb_idea, pad=barn_road)
    duck_text = "ducks"
    duck_road = f"{barn_road},{duck_text}"
    duck_idea = IdeaKid(_label=duck_text, _weight=2)
    x_pact.add_idea(idea_kid=duck_idea, pad=barn_road)

    coal_text = "coal"
    coal_idea = IdeaKid(_label=coal_text, _weight=30)
    x_pact.add_idea(idea_kid=coal_idea, pad=x_pact._cure_handle)

    assert x_pact._idearoot._pact_coin_onset is None
    assert x_pact._idearoot._pact_coin_cease is None
    assert x_pact._idearoot._kids.get(auto_text)._pact_coin_onset is None
    assert x_pact._idearoot._kids.get(auto_text)._pact_coin_cease is None
    assert x_pact._idearoot._kids.get(barn_text)._pact_coin_onset is None
    assert x_pact._idearoot._kids.get(barn_text)._pact_coin_cease is None
    assert x_pact._idearoot._kids.get(coal_text)._pact_coin_onset is None
    assert x_pact._idearoot._kids.get(coal_text)._pact_coin_cease is None
    lamb_before = x_pact.get_idea_kid(road=lamb_road)
    assert lamb_before._pact_coin_onset is None
    assert lamb_before._pact_coin_cease is None
    duck_before = x_pact.get_idea_kid(road=duck_road)
    assert duck_before._pact_coin_onset is None
    assert duck_before._pact_coin_cease is None

    # WHEN
    x_pact.set_pact_metrics()

    # THEN
    assert x_pact._idearoot._pact_coin_onset == 0.0
    assert x_pact._idearoot._pact_coin_cease == 1.0
    assert x_pact._idearoot._kids.get(auto_text)._pact_coin_onset == 0.0
    assert x_pact._idearoot._kids.get(auto_text)._pact_coin_cease == 0.1
    assert x_pact._idearoot._kids.get(barn_text)._pact_coin_onset == 0.1
    assert x_pact._idearoot._kids.get(barn_text)._pact_coin_cease == 0.7
    assert x_pact._idearoot._kids.get(coal_text)._pact_coin_onset == 0.7
    assert x_pact._idearoot._kids.get(coal_text)._pact_coin_cease == 1.0

    duck_after = x_pact.get_idea_kid(road=duck_road)
    assert duck_after._pact_coin_onset == 0.1
    assert duck_after._pact_coin_cease == 0.5
    lamb_after = x_pact.get_idea_kid(road=lamb_road)
    assert lamb_after._pact_coin_onset == 0.5
    assert lamb_after._pact_coin_cease == 0.7


def test_pact_get_idea_list_without_root_CorrectlyCalculatesIdeaAttributes():
    # GIVEN
    x_pact = get_pact_with7amCleanTableRequired()

    # WHEN
    idea_list_without_idearoot = x_pact.get_idea_list_without_idearoot()
    idea_list_with_idearoot = x_pact.get_idea_list()

    # THEN
    assert len(idea_list_without_idearoot) == 28
    assert len(idea_list_without_idearoot) + 1 == len(idea_list_with_idearoot)

    # for idea in x_pact.get_idea_list_without_idearoot():
...
def test_pact_get_idea_list_CorrectlyCalculatesRangeAttributes():
    # GIVEN
    x_pact = get_pact_with7amCleanTableRequired()
    idea_list = x_pact.get_idea_list()
    house_text = "housework"
    house_road = f"{x_pact._cure_handle},{house_text}"
    clean_text = "clean table"
    clean_road = f"{house_road},{clean_text}"
    assert x_pact._idea_dict.get(clean_road)._active_status == False
    ...
    # THEN
    x_pact.set_pact_metrics()
    assert x_pact._idea_dict.get(clean_road)._active_status
    ...
    print(x_pact._idearoot._acptfactunits[day24hr_road])
    print(x_pact._idearoot._kids[house_text]._kids[clean_text]._requiredunits)
    # x_pact._idearoot._kids["housework"]._kids[clean_text]._active_status = None
...
def test_get_agenda_items():
    ...
    # WHEN
    promise_items = x_pact.get_agenda_items()
...
def test_exammple_idea_list_HasCorrectData():
    ...
    # day_hour = f"{x_pact._cure_handle},day_hour"
    # x_pact.set_acptfact(base=day_hour, pick=day_hour, open=0, nigh=23)
    day_min_text = "day_minute"
    day_min_road = f"{x_pact._cure_handle},{day_min_text}"
    x_pact.set_acptfact(base=day_min_road, pick=day_min_road, open=0, nigh=1439)
    ...
    yr_mon_road = f"{x_pact._cure_handle},{yr_mon_text}"
    x_pact.set_acptfact(base=yr_mon_road, pick=yr_mon_road)
    inter_text = "Internet"
    inter_road = f"{x_pact._cure_handle},{inter_text}"
    x_pact.set_acptfact(base=inter_road, pick=inter_road)
    assert x_pact != None
    # print(f"{x_pact._healer=}")
    # print(f"{len(x_pact._idearoot._kids)=}")
    ulty_text = "Ultimate Frisbee"
    ulty_road = f"{x_pact._cure_handle},{ulty_text}"

    # if x_pact._idearoot._kids["Ultimate Frisbee"]._label == "Ultimate Frisbee":
    assert x_pact._idearoot._kids[ulty_text]._requiredunits != None
    ...
    laundry_road = f"{x_pact._cure_handle},casa,cleaning,{laundry_text}"
    ...
    assert x_pact._idea_dict.get(laundry_road)._active_status == False
    ...
    x_pact.set_acptfact(base=week_road, pick=mon_road)
...
def test_exammple_idea_list_OptionWeekdaysCorrectlyWork():
    ...
    day_hr_road = f"{x_pact._cure_handle},{day_hr_text}"
    x_pact.set_acptfact(base=day_hr_road, pick=day_hr_road, open=0, nigh=23)
    day_min_text = "day_minute"
    day_min_road = f"{x_pact._cure_handle},{day_min_text}"
    x_pact.set_acptfact(base=day_min_road, pick=day_min_road, open=0, nigh=59)
    mon_wk_text = "month_week"
    mon_wk_road = f"{x_pact._cure_handle},{mon_wk_text}"
    x_pact.set_acptfact(base=mon_wk_road, pick=mon_wk_road)
    nation_text = "Nation-States"
    nation_road = f"{x_pact._cure_handle},{nation_text}"
    x_pact.set_acptfact(base=nation_road, pick=nation_road)
    mood_text = "Moods"
    mood_road = f"{x_pact._cure_handle},{mood_text}"
    x_pact.set_acptfact(base=mood_road, pick=mood_road)
    ...
    yr_mon_text = "year_month"
    yr_mon_road = f"{x_pact._cure_handle},{yr_mon_text}"
    x_pact.set_acptfact(base=yr_mon_road, pick=yr_mon_road, open=0, nigh=1000)

    idea_list = x_pact.get_idea_list()
    missing_acptfacts = x_pact.get_missing_acptfact_bases()
    ...
    x_pact._idearoot.set_required_unit(required=mt_required)
    # print(f"{x_pact._requiredunits[week_road].base=}")
    # print(f"{x_pact._requiredunits[week_road].sufffacts[mon_road].need=}")
    # print(f"{x_pact._requiredunits[week_road].sufffacts[tue_road].need=}")
    print(f"{x_pact._idearoot._requiredunits[week_road].sufffacts=}")
    sufffact_mon = x_pact._idearoot._requiredunits[week_road].sufffacts.get(mon_road)
    sufffact_tue = x_pact._idearoot._requiredunits[week_road].sufffacts.get(tue_road)
    assert sufffact_mon
    assert sufffact_mon == mt_required.sufffacts[sufffact_mon.need]
    assert sufffact_tue
    assert sufffact_tue == mt_required.sufffacts[sufffact_tue.need]
    assert x_pact._idearoot._requiredunits[week_road] == mt_required
    ...
    assert x_pact._idearoot._requiredheirs[week_road] == mt_required_x
    ...
    x_pact.set_acptfact(base=week_road, pick=mon_road)
    idea_list = x_pact.get_idea_list()
    casa_idea = x_pact._idearoot._kids[casa_text]
    twee_idea = casa_idea._kids[bird_text]
    print(f"{len(x_pact._idearoot._requiredheirs)=}")
    ...
    # x_pact.set_acptfact(base=f"{x_pact._cure_handle},weekdays", pick=f"{x_pact._cure_handle},weekdays,Tuesday")
    # idea_list = x_pact.get_idea_list()
    # assert YR.get_active_status(road=bird_idea, idea_list=idea_list) == True

    # x_pact.set_acptfact(base=f"{x_pact._cure_handle},weekdays", pick=f"{x_pact._cure_handle},weekdays,Wednesday")
    # idea_list = x_pact.get_idea_list()
...
def test_exammple_idea_list_Every6WeeksRequired():
    ...
    min_text = "day_minute"
    min_road = f"{x_pact._cure_handle},{day_text}"

    # WHEN
    x_pact.set_acptfact(base=day_road, pick=day_road, open=0, nigh=23)
    x_pact.set_acptfact(base=min_road, pick=min_road, open=0, nigh=59)
    idea_list = x_pact.get_idea_list()

    # THEN
    ced_week_base = f"{x_pact._cure_handle},ced_week"
    ...
    clean_sheet_road = (
        f"{x_pact._cure_handle},casa,cleaning,clean sheets couch blankets"
    )
    clean_sheet_idea = x_pact.get_idea_kid(road=clean_sheet_road)
    ...
    clean_couch_road = f"{x_pact._cure_handle},casa,cleaning,{clean_couch_text}"
    clean_couch_idea = x_pact.get_idea_kid(road=clean_couch_road)
...
def test_exammple_idea_list_EveryOtherMonthWorks():
    ...
    minute_road = f"{x_pact._cure_handle},{minute_text}"
    x_pact.set_acptfact(base=minute_road, pick=minute_road, open=0, nigh=1399)
    month_text = "month_week"
    month_road = f"{x_pact._cure_handle},{month_text}"
    x_pact.set_acptfact(base=month_road, pick=month_road)
    ...
    x_pact.set_acptfact(base=weekdays_road, pick=weekdays_road)
    idea_list = x_pact.get_idea_list()
    print(f"{len(idea_list)=}")
    ...
    year_month_base = f"{x_pact._cure_handle},year_month"
    print(f"{year_month_base=}, {year_month_base=}")

    # WHEN
    x_pact.set_acptfact(base=year_month_base, pick=year_month_base, open=0, nigh=8)
    ced_week = f"{x_pact._cure_handle},ced_week"
    x_pact.set_acptfact(base=ced_week, pick=ced_week, open=0, nigh=4)

    # THEN
    idea_list = x_pact.get_idea_list()
    print(f"{len(idea_list)=}")
    print(f"{len(x_pact._idearoot._acptfactunits)=}")
