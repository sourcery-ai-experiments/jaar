from src.pact.party import PartyTitle, partylink_shop
...
def test_groupunit_exists():
    ...
    assert swimmers_group._pact_credit != None
    assert swimmers_group._pact_debt != None
    assert swimmers_group._pact_agenda_credit != None
    assert swimmers_group._pact_agenda_debt != None
...
def test_Groupunit_reset_pact_importance_WorkCorrectly():
    # GIVEN
    maria_brand = "maria"
    maria_group = groupunit_shop(
        ...
        _pact_credit=0.33,
        _pact_debt=0.44,
        _pact_agenda_credit=0.13,
        _pact_agenda_debt=0.23,
    )
    print(f"{maria_group}")
    assert maria_group._pact_credit == 0.33
    assert maria_group._pact_debt == 0.44
    assert maria_group._pact_agenda_credit == 0.13
    assert maria_group._pact_agenda_debt == 0.23

    # WHEN
    maria_group.reset_pact_credit_debt()

    # THEN
    assert maria_group._pact_credit == 0
    assert maria_group._pact_debt == 0
    assert maria_group._pact_agenda_credit == 0
    assert maria_group._pact_agenda_debt == 0


def test_Groupunit_reset_pact_importance_reset_partylinks():
    # GIVEN
    todd_text = "Todd"
    mery_text = "Merry"
    ...
    mery_party = partylink_shop(
        ...
        _pact_credit=0.23,
        _pact_debt=0.5,
        _pact_agenda_credit=0.54,
        _pact_agenda_debt=0.57,
    )
    ...
    bikers_group = groupunit_shop(
        ...
        _pact_credit=0.33,
        _pact_debt=0.44,
        _pact_agenda_credit=0.1,
        _pact_agenda_debt=0.2,
    )
    ...
    assert biker_partylink_todd._pact_credit == 0.13
    assert biker_partylink_todd._pact_debt == 0.7
    assert biker_partylink_todd._pact_agenda_credit == 0.53
    assert biker_partylink_todd._pact_agenda_debt == 0.77

    biker_partylink_mery = bikers_group._partys.get(mery_text)
    assert biker_partylink_mery._pact_credit == 0.23
    assert biker_partylink_mery._pact_debt == 0.5
    assert biker_partylink_mery._pact_agenda_credit == 0.54
    assert biker_partylink_mery._pact_agenda_debt == 0.57

    # WHEN
    bikers_group.reset_pact_credit_debt()

    # THEN
    assert biker_partylink_todd._pact_credit == 0
    assert biker_partylink_todd._pact_debt == 0
    assert biker_partylink_todd._pact_agenda_credit == 0
    assert biker_partylink_todd._pact_agenda_debt == 0
    assert biker_partylink_mery._pact_credit == 0
    assert biker_partylink_mery._pact_debt == 0
    assert biker_partylink_mery._pact_agenda_credit == 0
    assert biker_partylink_mery._pact_agenda_debt == 0
...
def test_brandLink_set_pact_importanceCorrectly():
    # GIVEN
    bikers_brand = GroupBrand("bikers")
    bikers_creditor_weight = 3.0
    ...
    # WHEN
    group_heir_x.set_pact_credit_debt(
        idea_pact_importance=idea_pact_importance,
    ...
    # THEN
    assert group_heir_x._pact_credit == 0.05
    assert group_heir_x._pact_debt == 0.1
...
def test_Balanceline_exists():
    # GIVEN
    bikers_brand = GroupBrand("bikers")
    balanceline_x = Balanceline(brand=bikers_brand, _pact_credit=0.33, _pact_debt=0.55)
    assert balanceline_x.brand == bikers_brand
    assert balanceline_x._pact_credit == 0.33
    assert balanceline_x._pact_debt == 0.55

    # WHEN
    balanceline_x.add_pact_credit_debt(pact_credit=0.11, pact_debt=0.2)

    # THEN
    assert balanceline_x._pact_credit == 0.44
    assert balanceline_x._pact_debt == 0.75
