...
from src.pact.x_func import x_get_dict, return1ifnone as x_func_return1ifnone
...
class PartyUnit(PartyCore):
    ...
    _pact_credit: float = None
    _pact_debt: float = None
    _pact_agenda_credit: float = None
    _pact_agenda_debt: float = None
    _pact_agenda_ratio_credit: float = None
    _pact_agenda_ratio_debt: float = None
    ...
    def set_banking_data(self, tax_paid: float, tax_diff: float):
        ...
        # if tax_diff is None or self._pact_agenda_ratio_credit is None:
        #     self._bank_tax_diff = tax_diff
        # elif (
        #     round(self._bank_tax_paid - self._pact_agenda_ratio_credit, 15) == tax_diff
        # ):
        #     self._bank_tax_diff = tax_diff
        # else:
        #     raise Exception(
        #         f"PartyUnit.set_banking_data fail: tax_paid={tax_paid} + tax_diff={tax_diff} not equal to _pact_agenda_ratio_credit={self._pact_agenda_ratio_credit}"
        #     )
    ...
    def set_empty_pact_credit_debt_to_zero(self):
        if self._pact_credit is None:
            self._pact_credit = 0
        if self._pact_debt is None:
            self._pact_debt = 0
        if self._pact_agenda_credit is None:
            self._pact_agenda_credit = 0
        if self._pact_agenda_debt is None:
            self._pact_agenda_debt = 0
        if self._pact_agenda_ratio_credit is None:
            self._pact_agenda_ratio_credit = 0
        if self._pact_agenda_ratio_debt is None:
            self._pact_agenda_ratio_debt = 0

    def reset_pact_credit_debt(self):
        self._pact_credit = 0
        self._pact_debt = 0
        self._pact_agenda_credit = 0
        self._pact_agenda_debt = 0
    ...
    def add_pact_credit_debt(
        ...
        self.set_empty_pact_credit_debt_to_zero()
        self._pact_credit += pact_credit
        self._pact_debt += pact_debt
        self._pact_agenda_credit += pact_agenda_credit
        self._pact_agenda_debt += pact_agenda_debt

    def set_pact_agenda_ratio_credit_debt(
        self,
        pact_agenda_ratio_credit_sum: float,
        pact_agenda_ratio_debt_sum: float,
        pact_partyunit_total_creditor_weight: float,
        pact_partyunit_total_debtor_weight: float,
    ):
        if pact_agenda_ratio_credit_sum == 0:
            self._pact_agenda_ratio_credit = (
                self.get_creditor_weight() / pact_partyunit_total_creditor_weight
            )
        else:
            self._pact_agenda_ratio_credit = (
                self._pact_agenda_credit / pact_agenda_ratio_credit_sum
            )

        if pact_agenda_ratio_debt_sum == 0:
            self._pact_agenda_ratio_debt = (
                self.get_debtor_weight() / pact_partyunit_total_debtor_weight
            )
        else:
            self._pact_agenda_ratio_debt = (
                self._pact_agenda_debt / pact_agenda_ratio_debt_sum
            )
...
def partyunit_shop(
    ...
    partyunit_x = PartyUnit(
        ...
        _pact_credit=_pact_credit,
        _pact_debt=_pact_debt,
        _pact_agenda_credit=_pact_agenda_credit,
        _pact_agenda_debt=_pact_agenda_debt,
        _pact_agenda_ratio_credit=_pact_agenda_ratio_credit,
        _pact_agenda_ratio_debt=_pact_agenda_ratio_debt,
...
class PartyLink(PartyCore):
    ...
    def set_pact_credit_debt(
        self,
        partylinks_creditor_weight_sum: float,
        partylinks_debtor_weight_sum: float,
        group_pact_credit: float,
        group_pact_debt: float,
        group_pact_agenda_credit: float,
        group_pact_agenda_debt: float,
    ):
        group_pact_credit = x_func_return1ifnone(group_pact_credit)
        group_pact_debt = x_func_return1ifnone(group_pact_debt)
        ...
        self._pact_credit = group_pact_credit * creditor_ratio
        self._pact_debt = group_pact_debt * debtor_ratio
        self._pact_agenda_credit = group_pact_agenda_credit * creditor_ratio
        self._pact_agenda_debt = group_pact_agenda_debt * debtor_ratio
