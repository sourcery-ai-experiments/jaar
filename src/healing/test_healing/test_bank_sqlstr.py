...
from src.healing.y_func import get_single_result_back


def test_healing_get_ledger_table_insert_sqlstr_CorrectlyPopulatesTable01(
    env_dir_setup_cleanup,
):
    ...
    assert ledger_x.contract_healer == bob_text
...
def test_RiverFlowUnit_exists():
    ...
    river_flow_x = RiverFlowUnit(
        currency_contract_healer=bob_text,
        src_title=None,
        dst_title=tom_text,
    ...
    # THEN
    assert river_flow_x.currency_contract_healer == bob_text
...
def test_RiverFlowUnit_flow_returned_WorksCorrectly():
    ...
    assert river_flow_x.currency_contract_healer != river_flow_x.dst_title
...
def test_get_river_ledger_unit_CorrectlyReturnsRiverLedgerUnit(env_dir_setup_cleanup):
    ...
    bob_text = "bob"
    sal_text = "sal"
    contract_bob = ContractUnit(_healer=bob_text)
    ...
    # THEN
    assert river_ledger_x.contract_healer == bob_text
...
def test_river_flow_insert_CorrectlyPopulatesTable01(
    ...
    with sx.get_bank_conn() as bank_conn:
        bank_conn.execute(insert_sqlstr)
        river_flows = get_river_flow_dict(bank_conn, currency_contract_healer=bob_text)
        print(f"{river_flows=}")
    ...
    assert flow_0.currency_contract_healer == bob_text
...
def test_RiverLedgerUnit_Exists():
    ...
    # THEN
    assert river_ledger_unit.contract_healer == bob_text
...
def test_get_river_tparty_table_insert_sqlstr_CorrectlyPopulatesTable01(
    ...
    # WHEN
    mstr_sqlstr = get_river_tparty_table_insert_sqlstr(
        currency_contract_healer=bob_text
    )
    with sx.get_bank_conn() as bank_conn:
        print(mstr_sqlstr)
    ...
    with sx.get_bank_conn() as bank_conn:
        river_tpartys = get_river_tparty_dict(
            bank_conn, currency_contract_healer=bob_text
        )
...
def test_get_river_bucket_table_insert_sqlstr_CorrectlyPopulatesTable01(
    ...
    with sx.get_bank_conn() as bank_conn:
        ...
        assert (
            len(get_river_bucket_dict(bank_conn, currency_contract_healer=sal_text))
            == 0
        )

    # WHEN / THEN
    mstr_sqlstr = get_river_bucket_table_insert_sqlstr(
        currency_contract_healer=sal_text
    )
    with sx.get_bank_conn() as bank_conn:
        print(mstr_sqlstr)
        bank_conn.execute(mstr_sqlstr)
    ...
    with sx.get_bank_conn() as bank_conn:
        river_buckets = get_river_bucket_dict(
            bank_conn, currency_contract_healer=sal_text
        )
...
def test_healing_get_idea_catalog_table_insert_sqlstr_CorrectlyPopulatesTable01(
    env_dir_setup_cleanup,
):
    # GIVEN
    sx = healingunit_shop(
...
def test_refresh_bank_metrics_Populates_idea_catalog_table(env_dir_setup_cleanup):
    ...
    bob_contract.set_healer(new_healer=bob_text)
    tim_contract.set_healer(new_healer=tim_text)
    sal_contract.set_healer(new_healer=sal_text)
    sx.save_public_contract(contract_x=bob_contract)
...
def test_healing_get_idea_catalog_dict_ReturnsCorrectData(env_dir_setup_cleanup):
    ...
    tim_contract.set_healer(new_healer=tim_text)
    sal_contract.set_healer(new_healer=sal_text)
    elu_contract.set_healer(new_healer=elu_text)
    sx.save_public_contract(contract_x=bob_contract)
    sx.save_public_contract(contract_x=tim_contract)
    sx.save_public_contract(contract_x=sal_contract)
    sx.save_public_contract(contract_x=elu_contract)
...
def test_healing_get_acptfact_catalog_table_insert_sqlstr_CorrectlyPopulatesTable01(
    env_dir_setup_cleanup,
):
    # GIVEN Create example healing with 4 Healers, each with 3 Partyunits = 12 ledger rows
...
def test_refresh_bank_metrics_Populates_acptfact_catalog_table(
    ...
    sal_contract.set_healer(new_healer=sal_text)
    c_text = "C"
    c_road = f"{tim_contract._healer},{c_text}"
    f_text = "F"
    f_road = f"{c_road},{f_text}"
    b_text = "B"
    b_road = f"{tim_contract._healer},{b_text}"
    ...
    casa_text = "casa"
    casa_road = f"{sal_contract._healer},{casa_text}"
...
def test_healing_get_groupunit_catalog_table_insert_sqlstr_CorrectlyPopulatesTable01(
    env_dir_setup_cleanup,
):
    # GIVEN Create example healing with 4 Healers, each with 3 Partyunits = 12 ledger rows
