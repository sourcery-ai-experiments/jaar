...
def test_healing_refresh_bank_metrics_CorrectlyDeletesOldBankInMemory(
    env_dir_setup_cleanup,
):
    # GIVEN
    sx = healingunit_shop(
...
def test_healing_refresh_bank_metrics_CorrectlyDeletesOldBankFile(
    env_dir_setup_cleanup,
):
    # GIVEN
    sx = healingunit_shop(
...
def test_healing_refresh_bank_metrics_CorrectlyPopulatesLedgerTable01(
    env_dir_setup_cleanup,
):
...
def test_healing_refresh_bank_metrics_CorrectlyPopulatesContractTable01(
    env_dir_setup_cleanup,
):
    # GIVEN Create example healing with 4 Healers, each with 3 Partyunits = 12 ledger rows
    sx = healingunit_shop(
    ...
    sx.save_public_contract(contract_x=ContractUnit(_healer=bob_text))
    sx.save_public_contract(contract_x=ContractUnit(_healer=tom_text))
    sx.save_public_contract(contract_x=ContractUnit(_healer=sal_text))
    sx.save_public_contract(contract_x=ContractUnit(_healer=elu_text))
...
def test_healing_refresh_bank_metrics_CorrectlyPopulates_groupunit_catalog(
    env_dir_setup_cleanup,
):
    # GIVEN
    sx = healingunit_shop(
...
def test_healing_set_contract_bank_attrs_CorrectlyPopulatesContract_Groupunit_Partylinks(
    env_dir_setup_cleanup,
):
    # GIVEN
    sx = healingunit_shop(
    ...
    # assert
    # _partylinks_set_by_healing_road
    # assert group "swimming expert" has 1 party
    # change groupunit "swimming expert" _partylinks_set_by_healing_road ==  f"{root_label()}sports,swimmer"
    # run set_bank_metrics
    ...
    sx.set_contract_bank_attrs(contract_healer=sal_text)
    e1_sal_contract = sx.get_public_contract(healer=sal_text)
    assert len(e1_sal_contract._groups.get(swim_group_text)._partys) == 1

    # WHEN
    # change groupunit "swimming expert" _partylinks_set_by_healing_road ==  f"{root_label()},sports,swimmer"
    sal_swim_road = f"{sal_sports_road},{swim_text}"
    swim_group_unit.set_attr(_partylinks_set_by_healing_road=sal_swim_road)
    sal_contract.set_groupunit(groupunit=swim_group_unit)
    sx.save_public_contract(contract_x=sal_contract)
    sx.set_contract_bank_attrs(contract_healer=sal_text)
