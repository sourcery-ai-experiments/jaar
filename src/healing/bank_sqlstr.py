...
from src.healing.y_func import sqlite_bool, sqlite_null
...
def get_groupunit_catalog_table_create_sqlstr() -> str:
    return """
        ...
        , partylinks_set_by_healing_road VARCHAR(1000) NULL
        )
        ;
    """
...
class GroupUnitCatalog:
    contract_healer: str
    groupunit_brand: str
    partylinks_set_by_healing_road: str
...
def get_groupunit_catalog_table_insert_sqlstr(
    ...
    return f"""
        INSERT INTO groupunit_catalog (
          contract_healer
        , groupunit_brand
        , partylinks_set_by_healing_road
        )
        VALUES (
          '{groupunit_catalog.contract_healer}'
        , '{groupunit_catalog.groupunit_brand}'
        , '{groupunit_catalog.partylinks_set_by_healing_road}'
        )
        ;
    """
...
def get_groupunit_catalog_dict(db_conn: Connection) -> dict[str:GroupUnitCatalog]:
    ...
    for row in results.fetchall():
        groupunit_catalog_x = GroupUnitCatalog(
            ...
            partylinks_set_by_healing_road=row[2],
        )
