...
def test_save_changeunit_file_SaveCorrectObj(reals_dir_setup_cleanup):
    ...
    sue_change2_path = f"{sue_userunit._changes_dir}/{two_filename}"
    sue_change0_path = f"{sue_userunit._changes_dir}/{six_filename}"
    ...
    sue_changeunit = changeunit_shop(
        ...
        _atoms_dir=sue_userunit._atoms_dir,
        _changes_dir=sue_userunit._changes_dir,
    )
    assert os_path_exists(sue_change2_path) == False
    assert os_path_exists(sue_change0_path) == False

    # WHEN
    save_changeunit_file(sue_userunit, sue_changeunit, correct_invalid_attrs=False)

    # THEN
    assert os_path_exists(sue_change2_path)
    assert os_path_exists(sue_change0_path) == False
    two_file_json = open_file(sue_userunit._changes_dir, two_filename)
...
def test_save_changeunit_file_RaisesErrorIfchangeUnit_atoms_dir_IsWrong(
    ...
    assert (
        str(excinfo.value)
        == f"changeUnit file cannot be saved because changeunit._atoms_dir is incorrect: {sue_changeunit._atoms_dir}. It must be {sue_userunit._atoms_dir}."
    )
...
def test_save_changeunit_file_RaisesErrorIfchangeUnit_changes_dir_IsWrong(
    ...
    assert (
        str(excinfo.value)
        == f"changeUnit file cannot be saved because changeunit._changes_dir is incorrect: {sue_changeunit._changes_dir}. It must be {sue_userunit._changes_dir}."
    )
...
def test_changeunit_file_exists_ReturnsCorrectObj(reals_dir_setup_cleanup):
    ...
    assert os_path_exists(sue_change2_path) == False
    assert os_path_exists(sue_change0_path) == False
    assert changeunit_file_exists(sue_userunit, x_change_id) == False
    assert changeunit_file_exists(sue_userunit, six_int) == False
    ...
    assert os_path_exists(sue_change2_path)
    assert os_path_exists(sue_change0_path) == False
    assert changeunit_file_exists(sue_userunit, x_change_id)
    assert changeunit_file_exists(sue_userunit, six_int) == False
    two_file_json = open_file(sue_userunit._changes_dir, two_filename)
    assert two_file_json == sue_changeunit.get_bookmetric_json()
...
def test_save_changeunit_file_RaisesErrorIf_replace_IsFalse(
    ...
    saved_changeunit = save_changeunit_file(sue_userunit, sue_changeunit)
    ...
    with pytest_raises(Exception) as excinfo:
        save_changeunit_file(
            sue_userunit, saved_changeunit, replace=False, correct_invalid_attrs=False
        )
    assert (
        str(excinfo.value)
...
def test_validate_changeunit_ReturnsObjWithAttributesFixed(
    ...
    invalid_sue_changeunit = changeunit_shop(
        _giver="Bob",
        _change_id=_get_next_change_file_number(sue_userunit) - 5,
        _atoms_dir=f"{sue_userunit._econs_dir}/swimming",
        _changes_dir=f"{sue_userunit._econs_dir}/swimming",
    )
    valid_changeunit = validate_changeunit(sue_userunit, invalid_sue_changeunit)

    # THEN
    assert valid_changeunit._atoms_dir == sue_userunit._atoms_dir
    assert valid_changeunit._changes_dir == sue_userunit._changes_dir
    assert valid_changeunit._change_id == _get_next_change_file_number(sue_userunit)
    correct_sue_changeunit = changeunit_shop(
        _giver=sue_text,
        _change_id=_get_next_change_file_number(sue_userunit),
        _atoms_dir=sue_userunit._atoms_dir,
        _changes_dir=sue_userunit._changes_dir,
    )
...
def test_save_changeunit_file_SaveCorrectObj_correct_invalid_attrs_IsTrue(
    ...
    sue_text = "Sue"
    sue_userunit = userunit_shop(None, None, sue_text)
    next_int = _get_next_change_file_number(sue_userunit)
    next_filename = get_json_filename(next_int)
    sue_change2_path = f"{sue_userunit._changes_dir}/{next_filename}"
    print(f"{sue_change2_path=}")
    assert os_path_exists(sue_change2_path) == False
    ...
    save_changeunit_file(sue_userunit, invalid_sue_changeunit)

    # THEN
    assert os_path_exists(sue_change2_path)
    two_file_json = open_file(sue_userunit._changes_dir, next_filename)
...
def test_get_max_change_file_number_ReturnsCorrectObj(
    ...
    assert get_max_change_file_number(sue_userunit) is None
    assert _get_next_change_file_number(sue_userunit) == init_change_id()
    assert _get_next_change_file_number(sue_userunit) == 0
    ...
    save_changeunit_file(sue_userunit, sue6_changeunit, correct_invalid_attrs=False)
    save_changeunit_file(sue_userunit, sue2_changeunit, correct_invalid_attrs=False)

    # WHEN / THEN
    assert get_max_change_file_number(sue_userunit) == six_int
    assert _get_next_change_file_number(sue_userunit) == 7
...
def test__create_new_changeunit_ReturnsObjWithCorrect_change_id_WhenNochangeFilesExist(
    ...
    # WHEN
    delete_dir(sue_userunit._changes_dir)
    sue_changeunit = _create_new_changeunit(sue_userunit)

    # THEN
    assert sue_changeunit._giver == sue_text
    assert sue_changeunit._change_id == init_change_id()
    assert sue_changeunit._change_id == 0
    assert sue_changeunit._change_id == _get_next_change_file_number(sue_userunit)
    assert sue_changeunit._faces == set()
    assert sue_changeunit._atoms_dir == sue_userunit._atoms_dir
    assert sue_changeunit._changes_dir == sue_userunit._changes_dir
...
def test__create_new_changeunit_ReturnsObjWithCorrect_change_id_WhenchangeFilesExist(
    ...
    zero_changeunit._change_id = _get_next_change_file_number(sue_userunit)
    zero_changeunit._atoms_dir = sue_userunit._atoms_dir
    zero_changeunit._changes_dir = sue_userunit._changes_dir
    save_changeunit_file(sue_userunit, zero_changeunit)
...
def test_get_changeunit_ReturnsCorrectObjWhenFilesDoesExist(
    ...
    sue_userunit = userunit_shop(None, None, sue_text)
    yao_text = "yao"
    x0_changeunit = _create_new_changeunit(sue_userunit)
    x0_changeunit.set_face(yao_text)
    save_changeunit_file(sue_userunit, x0_changeunit)
    bob_text = "Bob"
    x1_changeunit = _create_new_changeunit(sue_userunit)
    x1_changeunit.set_face(bob_text)
    save_changeunit_file(sue_userunit, x1_changeunit)

    # WHEN
    y0_changeunit = get_changeunit(sue_userunit, x0_changeunit._change_id)
    y1_changeunit = get_changeunit(sue_userunit, x1_changeunit._change_id)
...
def test_get_changeunit_RaisesExceptionWhenFileDoesNotExist(
    ...
    six_file_number = 6
    with pytest_raises(Exception) as excinfo:
        get_changeunit(sue_userunit, six_file_number)
    assert (
        str(excinfo.value)
...
def test_del_changeunit_DeleteschangejsonAndNotAgendaAtomjsons(
    ...
    sue_change0_path = f"{sue_userunit._changes_dir}/{six_filename}"
    sue_atom0_path = f"{sue_userunit._atoms_dir}/{atom0_filename}"
    assert os_path_exists(sue_change0_path) == False
    ...
    print(f"{dir_files(sue_userunit._atoms_dir)}")
    assert os_path_exists(sue_change0_path)
    assert os_path_exists(sue_atom0_path)

    # WHEN
    del_changeunit_file(sue_userunit, sue_changeunit._change_id)
...
def test_save_changeunit_file_CanCreateAndModify3changeunits(
    ...
    sue_userunit = userunit_shop(None, None, sue_text)
    delete_dir(sue_userunit._changes_dir)
    delete_dir(sue_userunit._atoms_dir)
    set_dir(sue_userunit._changes_dir)
    set_dir(sue_userunit._atoms_dir)
    assert len(dir_files(sue_userunit._changes_dir)) == 0
    assert len(dir_files(sue_userunit._atoms_dir)) == 0

    # WHEN
    save_changeunit_file(sue_userunit, sue_2atomunits_changeunit())
    save_changeunit_file(sue_userunit, sue_3atomunits_changeunit())
    save_changeunit_file(sue_userunit, sue_4atomunits_changeunit())

    # THEN
    assert len(dir_files(sue_userunit._changes_dir)) == 3
    assert len(dir_files(sue_userunit._atoms_dir)) == 9
...
def test_save_changeunit_file_ReturnsValidObj(reals_dir_setup_cleanup):
    ...
    sue2_changeunit._atoms_dir = f"{sue_userunit._econs_dir}/swimming"
    sue2_changeunit._changes_dir = f"{sue_userunit._econs_dir}/swimming"
    sue2_changeunit._giver = "Bob"
    sue2_changeunit._change_id = _get_next_change_file_number(sue_userunit) - 5
    prev_sue2_changeunit = copy_deepcopy(sue2_changeunit)

    # WHEN
    valid_changeunit = save_changeunit_file(sue_userunit, sue2_changeunit)
...
def test_create_save_changeunit_SaveCorrectObj(reals_dir_setup_cleanup):
    ...
    sue_change2_path = f"{sue_userunit._changes_dir}/{two_filename}"
    sue_change3_path = f"{sue_userunit._changes_dir}/{three_filename}"
    print(f"{sue_change2_path=}")
    ...
    # WHEN
    before_agenda = get_duty_file_agenda(sue_userunit)
    bob_text = "Bob"
    after_agenda = copy_deepcopy(before_agenda)
    after_agenda.add_partyunit(bob_text)
    create_save_changeunit(sue_userunit, before_agenda, after_agenda)
...
def test_merge_changes_into_agenda_ReturnsSameObj(reals_dir_setup_cleanup):
    ...
    # WHEN
    new_agenda = _merge_changes_into_agenda(sue_userunit, duty_agenda)
...
def test_merge_changes_into_agenda_ReturnsObj_WithSinglechangeModifys_1atom(
    ...
    sue_userunit = userunit_shop(None, None, sue_text)
    userunit_create_core_dir_and_files(sue_userunit)
    save_changeunit_file(sue_userunit, sue_1atomunits_changeunit())
...
def test_append_changes_to_duty_file_AddschangesToDutyFile(
    ...
    # WHEN
    new_agenda = append_changes_to_duty_file(sue_userunit)
...
def test_add_pledge_change_Addspledgechange(reals_dir_setup_cleanup):
    ...
    sue_userunit = userunit_shop(None, None, sue_text)
    userunit_create_core_dir_and_files(sue_userunit)
    old_sue_duty = get_duty_file_agenda(sue_userunit)
    clean_text = "clean"
    clean_road = old_sue_duty.make_l1_road(clean_text)
    change_filename = get_json_filename(1)
    sue_change_path = f"{sue_userunit._changes_dir}/{change_filename}"
    ...
    # WHEN
    add_pledge_change(sue_userunit, clean_road)

    # THEN
    assert os_path_exists(sue_change_path)
    new_sue_duty = get_duty_file_agenda(sue_userunit)
...
def test_add_pledge_change_SetsDutyAgendapledgeIdea_suffgroup(reals_dir_setup_cleanup):
    ...
    # WHEN
    bob_text = "Bob"
    add_pledge_change(sue_userunit, clean_road, x_suffgroup=bob_text)
