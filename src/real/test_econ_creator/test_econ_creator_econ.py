...
def test_get_person_econ_dir_ReturnsCorrectObj(reals_dir_setup_cleanup):
    ...
    texas_path = get_econ_path(sue_userunit, texas_road)
    dallas_path = get_econ_path(sue_userunit, dallas_road)
    elpaso_path = get_econ_path(sue_userunit, elpaso_road)
    kern_path = get_econ_path(sue_userunit, kern_road)

    # THEN
    idearoot_dir = f"{sue_userunit._econs_dir}/{get_rootpart_of_econ_dir()}"
...
def test_create_econ_dir_CreatesDir(reals_dir_setup_cleanup):
    ...
    dallas_dir = get_econ_path(sue_userunit, dallas_road)
    print(f"{dallas_dir=}")
    assert os_path_exists(dallas_dir) == False

    # WHEN
    create_econ_dir(sue_userunit, dallas_text)
...
def test_init_econunit_CreatesAndReturnsObj(reals_dir_setup_cleanup):
    ...
    pound_text = "#"
    sue_text = "Sue"
    sue_userunit = userunit_shop(None, None, sue_text, pound_text)
    ...
    dallas_dir = create_econ_dir(sue_userunit, dallas_road)
    ...
    # WHEN
    sue_dallas_econunit = init_econunit(sue_userunit, dallas_road)

    # THEN
    assert os_path_exists(dallas_db_path)
    assert sue_dallas_econunit.real_id == sue_userunit.real_id
    assert sue_dallas_econunit.econ_dir == dallas_dir
    assert sue_dallas_econunit._manager_person_id == sue_text
    assert sue_dallas_econunit._road_delimiter == sue_userunit._road_delimiter
...
def test_create_person_econunits_RaisesErrorWhen__econs_justified_IsFalse(
    ...
    with pytest_raises(Exception) as excinfo:
        create_person_econunits(sue_userunit)
    assert (
        str(excinfo.value)
...
def test_create_person_econunits_CreatesEconUnits(reals_dir_setup_cleanup):
    ...
    dallas_dir = create_econ_dir(sue_userunit, dallas_road)
    elpaso_dir = create_econ_dir(sue_userunit, elpaso_road)
    dallas_db_path = f"{dallas_dir}/{treasury_db_filename()}"
...
def test_EngineUnit_get_econ_ReturnsCorrectObj(reals_dir_setup_cleanup):
    ...
    # WHEN
    create_person_econunits(sue_userunit)
    dallas_econ = get_econunit(sue_userunit, dallas_road)

    # THEN
    assert dallas_econ != None
    assert dallas_econ.real_id == sue_userunit.real_id
...
def test_EngineUnit_set_econunit_role_CorrectlySets_role(reals_dir_setup_cleanup):
    ...
    save_duty_file(sue_userunit, sue_duty_agenda)
    create_person_econunits(sue_userunit)
    print(f"{_get_econs_roads(sue_userunit).keys()=}")
    dallas_econ = get_econunit(sue_userunit, dallas_road)
    ...
    # WHEN
    set_econunit_role(sue_userunit, dallas_road, sue_duty_agenda)
...
def test_EngineUnit_set_econunits_role_CorrectlySets_roles(reals_dir_setup_cleanup):
    ...
    elpaso_econ = get_econunit(sue_userunit, elpaso_road)
    ...
    # WHEN
    set_econunits_role(sue_userunit, sue_duty_agenda)
...
def test_EngineUnit_set_person_econunits_role_CorrectlySetsroles(
    ...
    # WHEN
    set_person_econunits_role(sue_userunit)
