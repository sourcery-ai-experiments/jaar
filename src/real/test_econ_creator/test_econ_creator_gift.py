...
def test_save_giftunit_file_SaveCorrectObj(reals_dir_setup_cleanup):
    ...
    sue_gift2_path = f"{sue_nookunit._gifts_dir}/{two_filename}"
    sue_gift0_path = f"{sue_nookunit._gifts_dir}/{six_filename}"
    ...
    assert os_path_exists(sue_gift0_path) == False
    two_file_json = open_file(sue_nookunit._gifts_dir, two_filename)
...
def test_giftunit_file_exists_ReturnsCorrectObj(reals_dir_setup_cleanup):
    ...
    assert os_path_exists(sue_gift2_path) == False
    assert os_path_exists(sue_gift0_path) == False
    assert giftunit_file_exists(sue_nookunit, x_gift_id) == False
    ...
    assert os_path_exists(sue_gift2_path)
    assert os_path_exists(sue_gift0_path) == False
    assert giftunit_file_exists(sue_nookunit, x_gift_id)
    assert giftunit_file_exists(sue_nookunit, six_int) == False
    two_file_json = open_file(sue_nookunit._gifts_dir, two_filename)
    assert two_file_json == sue_giftunit.get_bookmetric_json()
...
def test_save_giftunit_file_RaisesErrorIf_replace_IsFalse(
    ...
    saved_giftunit = save_giftunit_file(sue_nookunit, sue_giftunit)
...
def test_validate_giftunit_ReturnsObjWithAttributesFixed(
    ...
    invalid_sue_giftunit = giftunit_shop(
        _giver="Bob",
        _gift_id=_get_next_gift_file_number(sue_nookunit) - 5,
        _atoms_dir=f"{sue_nookunit._econs_dir}/swimming",
        _gifts_dir=f"{sue_nookunit._econs_dir}/swimming",
    )
    valid_giftunit = validate_giftunit(sue_nookunit, invalid_sue_giftunit)

    # THEN
    assert valid_giftunit._atoms_dir == sue_nookunit._atoms_dir
    assert valid_giftunit._gifts_dir == sue_nookunit._gifts_dir
    assert valid_giftunit._gift_id == _get_next_gift_file_number(sue_nookunit)
    correct_sue_giftunit = giftunit_shop(
        _giver=sue_text,
        _gift_id=_get_next_gift_file_number(sue_nookunit),
        _atoms_dir=sue_nookunit._atoms_dir,
        _gifts_dir=sue_nookunit._gifts_dir,
    )
...
def test_save_giftunit_file_SaveCorrectObj_correct_invalid_attrs_IsTrue(
    ...
    sue_text = "Sue"
    sue_nookunit = nookunit_shop(None, None, sue_text)
    next_int = _get_next_gift_file_number(sue_nookunit)
    next_filename = get_json_filename(next_int)
    sue_gift2_path = f"{sue_nookunit._gifts_dir}/{next_filename}"
    print(f"{sue_gift2_path=}")
    assert os_path_exists(sue_gift2_path) == False
    ...
    save_giftunit_file(sue_nookunit, invalid_sue_giftunit)

    # THEN
    assert os_path_exists(sue_gift2_path)
    two_file_json = open_file(sue_nookunit._gifts_dir, next_filename)
...
def test_get_max_gift_file_number_ReturnsCorrectObj(
    ...
    assert get_max_gift_file_number(sue_nookunit) is None
    assert _get_next_gift_file_number(sue_nookunit) == init_gift_id()
    assert _get_next_gift_file_number(sue_nookunit) == 0
    ...
    assert _get_next_gift_file_number(sue_nookunit) == 7
...
def test__create_new_giftunit_ReturnsObjWithCorrect_gift_id_WhenNoGiftFilesExist(
    ...
    # WHEN
    delete_dir(sue_nookunit._gifts_dir)
    sue_giftunit = _create_new_giftunit(sue_nookunit)

    # THEN
    assert sue_giftunit._giver == sue_text
    assert sue_giftunit._gift_id == init_gift_id()
    assert sue_giftunit._gift_id == 0
    assert sue_giftunit._gift_id == _get_next_gift_file_number(sue_nookunit)
    assert sue_giftunit._takers == set()
    assert sue_giftunit._atoms_dir == sue_nookunit._atoms_dir
    assert sue_giftunit._gifts_dir == sue_nookunit._gifts_dir
...
def test__create_new_giftunit_ReturnsObjWithCorrect_gift_id_WhenGiftFilesExist(
    ...
    zero_giftunit._gift_id = _get_next_gift_file_number(sue_nookunit)
    zero_giftunit._atoms_dir = sue_nookunit._atoms_dir
    zero_giftunit._gifts_dir = sue_nookunit._gifts_dir
    save_giftunit_file(sue_nookunit, zero_giftunit)
...
def test_get_giftunit_ReturnsCorrectObjWhenFilesDoesExist(
    ...
    sue_nookunit = nookunit_shop(None, None, sue_text)
    yao_text = "yao"
    x0_giftunit = _create_new_giftunit(sue_nookunit)
    x0_giftunit.set_taker(yao_text)
    save_giftunit_file(sue_nookunit, x0_giftunit)
    bob_text = "Bob"
    x1_giftunit = _create_new_giftunit(sue_nookunit)
    x1_giftunit.set_taker(bob_text)
    save_giftunit_file(sue_nookunit, x1_giftunit)

    # WHEN
    y0_giftunit = get_giftunit(sue_nookunit, x0_giftunit._gift_id)
    y1_giftunit = get_giftunit(sue_nookunit, x1_giftunit._gift_id)
...
def test_get_giftunit_RaisesExceptionWhenFileDoesNotExist(
    ...
    six_file_number = 6
    with pytest_raises(Exception) as excinfo:
        get_giftunit(sue_nookunit, six_file_number)
    assert (
...
def test_del_giftunit_DeletesGiftjsonAndNotAgendaAtomjsons(
    ...
    # WHEN
    del_giftunit_file(sue_nookunit, sue_giftunit._gift_id)
...
def test_save_giftunit_file_CanCreateAndChange3Giftunits(
    ...
    set_dir(sue_nookunit._gifts_dir)
    set_dir(sue_nookunit._atoms_dir)
    assert len(dir_files(sue_nookunit._gifts_dir)) == 0
    assert len(dir_files(sue_nookunit._atoms_dir)) == 0

    # WHEN
    save_giftunit_file(sue_nookunit, sue_2atomunits_giftunit())
    save_giftunit_file(sue_nookunit, sue_3atomunits_giftunit())
    save_giftunit_file(sue_nookunit, sue_4atomunits_giftunit())

    # THEN
    assert len(dir_files(sue_nookunit._gifts_dir)) == 3
...
def test_save_giftunit_file_ReturnsValidObj(reals_dir_setup_cleanup):
    ...
    sue2_giftunit._atoms_dir = f"{sue_nookunit._econs_dir}/swimming"
    sue2_giftunit._gifts_dir = f"{sue_nookunit._econs_dir}/swimming"
    sue2_giftunit._giver = "Bob"
    sue2_giftunit._gift_id = _get_next_gift_file_number(sue_nookunit) - 5
    prev_sue2_giftunit = copy_deepcopy(sue2_giftunit)

    # WHEN
    valid_giftunit = save_giftunit_file(sue_nookunit, sue2_giftunit)
...
def test_create_save_giftunit_SaveCorrectObj(reals_dir_setup_cleanup):
    ...
    sue_gift2_path = f"{sue_nookunit._gifts_dir}/{two_filename}"
    sue_gift3_path = f"{sue_nookunit._gifts_dir}/{three_filename}"
    print(f"{sue_gift2_path=}")
    ...
    create_save_giftunit(sue_nookunit, before_agenda, after_agenda)
...
def test_merge_gifts_into_agenda_ReturnsObj_NoChange(
    ...
    # WHEN
    new_agenda = _merge_gifts_into_agenda(sue_nookunit, duty_agenda)
...
def test_merge_gifts_into_agenda_ReturnsObj_WithSingleGiftChanges_1atom(
    ...
    sue_nookunit = nookunit_shop(None, None, sue_text)
    nookunit_create_core_dir_and_files(sue_nookunit)
    save_giftunit_file(sue_nookunit, sue_1atomunits_giftunit())
...
def test_append_gifts_to_duty_file_AddsGiftsToDutyFile(
    ...
    # WHEN
    new_agenda = append_gifts_to_duty_file(sue_nookunit)
...
def test_add_pledge_gift_AddspledgeGift(reals_dir_setup_cleanup):
    ...
    sue_gift_path = f"{sue_nookunit._gifts_dir}/{gift_filename}"
    ...
    # WHEN
    add_pledge_gift(sue_nookunit, clean_road)
...
def test_add_pledge_gift_SetsDutyAgendapledgeIdea_suffgroup(reals_dir_setup_cleanup):
    ...
    # WHEN
    bob_text = "Bob"
    add_pledge_gift(sue_nookunit, clean_road, x_suffgroup=bob_text)
