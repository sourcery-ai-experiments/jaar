...
def create_example_real1() -> RealUnit:
    ...
    yao_usernox = usernox_shop(None, music_text, yao_text)
    yao_duty_agenda = get_duty_file_agenda(yao_usernox)
    ...
    save_duty_file(yao_usernox, yao_duty_agenda)
    create_person_econunits(yao_usernox)
...
def create_example_real2() -> RealUnit:
    ...
    music_real.init_person_econs(yao_text)
    yao_usernox = usernox_shop(None, music_text, yao_text)
    wei_usernox = usernox_shop(None, music_text, wei_text)
    zia_usernox = usernox_shop(None, music_text, zia_text)
    music_real.init_person_econs(wei_text)
    music_real.init_person_econs(zia_text)
    yao_duty_agenda = get_duty_file_agenda(yao_usernox)
    wei_duty_agenda = get_duty_file_agenda(wei_usernox)
    zia_duty_agenda = get_duty_file_agenda(zia_usernox)
    ...
    # display_agenda(yao_duty_agenda.calc_agenda_metrics(), mode="Econ").show()
    save_duty_file(yao_usernox, yao_duty_agenda)
    save_duty_file(wei_usernox, wei_duty_agenda)
    save_duty_file(zia_usernox, zia_duty_agenda)
    create_person_econunits(yao_usernox)
    create_person_econunits(wei_usernox)
    create_person_econunits(zia_usernox)
...
def create_example_real4() -> RealUnit:
    ...
    # yao_dallas_econ.agendahub.save_file_role(yao_duty_agenda)
    # yao_dallas_econ.agendahub.save_file_role(wei_duty_agenda)
    # yao_dallas_econ.agendahub.save_file_role(zia_duty_agenda)
    # zia_dallas_econ.agendahub.save_file_role(yao_duty_agenda)
    # zia_dallas_econ.agendahub.save_file_role(wei_duty_agenda)
    # zia_dallas_econ.agendahub.save_file_role(zia_duty_agenda)
    # yao_elpaso_econ.agendahub.save_file_role(yao_duty_agenda)
    # yao_elpaso_econ.agendahub.save_file_role(wei_duty_agenda)
    # yao_elpaso_econ.agendahub.save_file_role(zia_duty_agenda)
