...
def test_RealUnit_get_person_same_from_file_ReturnsCorrectObj(env_dir_setup_cleanup):
    ...
    bob_text = "Bob"
    luca_same = luca_userhub.get_same_truth()
    luca_same.add_otherunit(bob_text)
    luca_userhub.save_same_truth(luca_same)
...
def test_RealUnit__set_all_healer_roles_CorrectlySetsroles(
    ...
    luca_same_truth = luca_userhub.get_same_truth()
    todd_same_truth = todd_userhub.get_same_truth()

    luca_same_truth.add_otherunit(luca_text)
    luca_same_truth.add_otherunit(todd_text)
    todd_same_truth.add_otherunit(luca_text)
    todd_same_truth.add_otherunit(todd_text)
    texas_text = "Texas"
    texas_road = luca_same_truth.make_l1_road(texas_text)
    luca_same_truth.add_l1_idea(ideaunit_shop(texas_text, _problem_bool=True))
    todd_same_truth.add_l1_idea(ideaunit_shop(texas_text, _problem_bool=True))
    dallas_text = "dallas"
    dallas_road = luca_same_truth.make_road(texas_road, dallas_text)
    dallas_healerhold = healerhold_shop({luca_text, todd_text})
    dallas_idea = ideaunit_shop(dallas_text, _healerhold=dallas_healerhold)
    elpaso_text = "el paso"
    elpaso_road = luca_same_truth.make_road(texas_road, elpaso_text)
    elpaso_healerhold = healerhold_shop({luca_text})
    elpaso_idea = ideaunit_shop(elpaso_text, _healerhold=elpaso_healerhold)

    luca_same_truth.add_idea(dallas_idea, texas_road)
    luca_same_truth.add_idea(elpaso_idea, texas_road)
    todd_same_truth.add_idea(dallas_idea, texas_road)
    todd_same_truth.add_idea(elpaso_idea, texas_road)
    # display_ideatree(luca_same_truth.calc_truth_metrics(), mode="Econ").show()
    luca_userhub.save_same_truth(luca_same_truth)
    todd_userhub.save_same_truth(todd_same_truth)
