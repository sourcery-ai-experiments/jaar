...
from src.oath.x_func import open_file as x_func_open_file
from src.oath.examples.oath_env import oath_env


def oath_v001() -> OathUnit:
    return oath_get_from_json(
        x_func_open_file(dest_dir=oath_env(), file_title="example_oath1.json")
    )


def oath_v001_with_large_agenda() -> OathUnit:
    a1 = oath_v001()
    ...
    weekdays_road = f"{a1._cure_handle},{weekdays_text}"
...
def oath_v002() -> OathUnit:
    return oath_get_from_json(
        x_func_open_file(dest_dir=oath_env(), file_title="example_oath2.json")
    )


def get_oath_with_4_levels() -> OathUnit:
    healer_text = "Noa"
    a1 = OathUnit(_healer=healer_text, _weight=10)
...
def get_oath_with_4_levels_and_2requireds() -> OathUnit:
    a1 = get_oath_with_4_levels()
    week_text = "weekdays"
    week_road = f"{a1._cure_handle},{week_text}"
    wed_text = "Wednesday"
    ...
    nation_road = f"{a1._cure_handle},{nation_text}"
    usa_text = "USA"
    ...
    work_text = "work"
    work_road = f"{a1._cure_handle},{work_text}"
    a1.edit_idea_attr(road=work_road, required=week_required)
    a1.edit_idea_attr(road=work_road, required=nation_required)
    return a1


def get_oath_with_4_levels_and_2requireds_2acptfacts() -> OathUnit:
    a1 = get_oath_with_4_levels_and_2requireds()
    week_text = "weekdays"
    week_road = f"{a1._cure_handle},{week_text}"
    wed_text = "Wednesday"
    wed_road = f"{week_road},{wed_text}"
...
def get_oath_with7amCleanTableRequired() -> OathUnit:
    a1 = get_oath_with_4_levels_and_2requireds_2acptfacts()
    ...
    a1.add_idea(time_idea, a1._cure_handle)
    ...
    house_road = f"{a1._cure_handle},{house_text}"
...
def get_oath_1Task_1CE0MinutesRequired_1AcptFact() -> OathUnit:
    healer_text = "Bob"
    a1 = OathUnit(_healer=healer_text, _weight=10)
    ...
    mail_road = f"{a1._cure_handle},{mail_label}"
...
def get_oath_x1_3levels_1required_1acptfacts() -> OathUnit:
    healer_text = "Kol"
    a1 = OathUnit(_healer=healer_text, _weight=10)
    shave_text = "shave"
...
def get_oath_base_time_example() -> OathUnit:
    healer_text = "Sue"
    a1 = OathUnit(_healer=healer_text)
    plant = "plant"
...
def get_oath_irrational_example() -> OathUnit:
    # this oath has no conclusive agenda because 2 promise ideas are in contradiction
    ...
    # after a1.set_oath_metrics these should be true:
...
def get_assignment_oath_example1():
    healer_text = "Neo"
    a1 = OathUnit(_healer=healer_text)
    casa_text = "casa"
