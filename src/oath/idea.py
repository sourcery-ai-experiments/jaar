...
from src.oath.origin import OriginUnit
...
class IdeaCore:
    ...
    _oath_importance: float = None
    _oath_coin_onset: float = None
    _oath_coin_cease: float = None
    _task: bool = None
    ...
    def set_oath_importance(
        self,
        coin_onset_x: float,
        parent_oath_importance: float = None,
        parent_coin_cease: float = None,
    ):
        parent_oath_importance = x_func_return1ifnone(parent_oath_importance)
        self.set_kids_total_weight()
        self._oath_importance = None
        self._oath_coin_onset = None
        self._oath_coin_cease = None
        sibling_ratio = self._weight / self._sibling_total_weight
        self._oath_importance = parent_oath_importance * sibling_ratio
        self._oath_coin_onset = coin_onset_x
        self._oath_coin_cease = self._oath_coin_onset + self._oath_importance
        self._oath_coin_cease = min(self._oath_coin_cease, parent_coin_cease)
        self.set_balanceheirs_oath_credit_debit()
    ...
    def set_kidless_balancelines(self):
        ...
        for bh in self._balanceheirs.values():
            balanceline_x = Balanceline(
                brand=bh.brand,
                _oath_credit=bh._oath_credit,
                _oath_debt=bh._oath_debt,
            )
    ...
    def set_balancelines(self, child_balancelines: dict[GroupBrand:Balanceline] = None):
        ...
        for bl in child_balancelines.values():
            ...
            self._balancelines[bl.brand].add_oath_credit_debt(
                oath_credit=bl._oath_credit, oath_debt=bl._oath_debt
            )
    ...
    def set_balanceheirs_oath_credit_debit(self):
        ...
        for balanceheir_x in self._balanceheirs.values():
            balanceheir_x.set_oath_credit_debt(
                idea_oath_importance=self._oath_importance,
    ...
    def set_active_status(
        self,
        tree_traverse_count: int,
        oath_groups: dict[GroupBrand:GroupUnit] = None,
        oath_healer: str = None,
    ):
        self.set_acptfactheirs_empty_if_null()
        ...
        if (
            self._active_status
            and oath_groups != None
            and oath_healer != None
            and self._assignedheir._suffgroups != {}
        ):
            self._assignedheir.set_group_party(oath_groups, oath_healer)
    ...
    def set_requiredheirs(
        ...
        for required in coalesced_requireds.values():
            ...
            if oath_idea_dict != None:
                base_idea = oath_idea_dict.get(required.base)
                if base_idea != None:
    ...
    def set_assignedheir(
        ...
        oath_groups: dict[GroupBrand:GroupUnit],
    ):
...
class IdeaRoot(IdeaCore):
    ...
    def set_idea_label(self, _label: str, oath_cure_handle: str = None):
        if _label != root_label() and oath_cure_handle is None:
            raise IdeaRootLabelNotEmptyException(
                f"Cannot set idearoot to string other than '{root_label()}'"
            )
        elif _label != oath_cure_handle != None:
            raise IdeaRootLabelNotEmptyException(
                f"Cannot set idearoot to string other than '{oath_cure_handle}'"
            )
        elif _label != root_label() and oath_cure_handle == _label:
            self._label = _label
        else:
