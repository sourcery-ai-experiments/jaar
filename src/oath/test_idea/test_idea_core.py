from src.oath.idea import IdeaCore
from src.oath.group import GroupBrand, balancelink_shop, balanceheir_shop
...
from src.oath.required_assign import assigned_unit_shop, assigned_heir_shop
...
def test_idea_core_exists():
    ...
    assert new_obj._oath_importance is None
    assert new_obj._oath_coin_onset is None
    assert new_obj._oath_coin_cease is None
...
def test_idea_core_get_inherited_balanceheirs_weight_sum_WorksCorrectlyWithValues():
    ...
    assert swimmer_balanceheir._oath_credit is None
    assert swimmer_balanceheir._oath_debt is None
    biker_balanceheir = idea_core._balanceheirs.get(biker_text)
    assert biker_balanceheir._oath_credit is None
    assert biker_balanceheir._oath_debt is None

    # WHEN
    idea_core._oath_importance = 0.25
    idea_core.set_balanceheirs_oath_credit_debit()

    # THEN
    print(f"{len(idea_core._balanceheirs)=}")
    swimmer_balanceheir = idea_core._balanceheirs.get(swimmer_text)
    assert swimmer_balanceheir._oath_credit != None
    assert swimmer_balanceheir._oath_debt != None
    biker_balanceheir = idea_core._balanceheirs.get(biker_text)
    assert biker_balanceheir._oath_credit != None
    assert biker_balanceheir._oath_debt != None
...
def test_idea_core_set_requiredheirsCorrectlyTakesFromOutside():
    ...
    idea.set_requiredheirs(requiredheirs=requiredheirs, oath_idea_dict=None)
...
def test_idea_core_set_requiredheirsCorrectlyTakesFromSelf():
    ...
    assert idea._requiredheirs is None
    idea.set_requiredheirs(requiredheirs=None, oath_idea_dict=None)
    assert idea._requiredheirs == requiredheirs
...
def test_idea_get_requiredheir_correctlyReturnsRequiredHeir():
    ...
    idea_x.set_requiredheirs(requiredheirs=required_heirs_x, oath_idea_dict=None)
...
def test_idea_set_assignedunit_empty_if_null():
    ...
    # WHEN
    idea_x.set_assignedheir(parent_assignheir=None, oath_groups=None)
    ...
    assigned_heir_x.set_suffgroups(
        assignunit=assigned_unit_x, parent_assignheir=None, oath_groups=None
    )
