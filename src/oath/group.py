...
class GroupUnit(GroupCore):
    ...
    def reset_oath_credit_debt(self):
        ...
        for partylink in self._partys.values():
            partylink.reset_oath_credit_debt()

    def _set_partylink_oath_credit_debt(self):
        ...
        for partylink in self._partys.values():
            partylink.set_oath_credit_debt(
                partylinks_creditor_weight_sum=partylinks_creditor_weight_sum,
                partylinks_debtor_weight_sum=partylinks_debtor_weight_sum,
                group_oath_credit=self._oath_credit,
                group_oath_debt=self._oath_debt,
                group_oath_agenda_credit=self._oath_agenda_credit,
                group_oath_agenda_debt=self._oath_agenda_debt,
            )
...
class Balanceline(GroupCore):
    ...
    def add_oath_credit_debt(self, oath_credit: float, oath_debt: float):
        self.set_oath_credit_debt_zero_if_null()
        self._oath_credit += oath_credit
        self._oath_debt += oath_debt

    def set_oath_credit_debt_zero_if_null(self):
        if self._oath_credit is None:
            self._oath_credit = 0
        if self._oath_debt is None:
            self._oath_debt = 0
