from src.oath.party import PartyTitle, partylink_shop
...
def test_groupunit_exists():
    ...
    swimmers_group = groupunit_shop(
        ...
        _oath_agenda_debt=0.77,
        _partylinks_set_by_cure_road=usa_road,
    )
    ...
    assert swimmers_group._oath_credit != None
    assert swimmers_group._oath_debt != None
    assert swimmers_group._oath_agenda_credit != None
    assert swimmers_group._oath_agenda_debt != None
    assert swimmers_group._partylinks_set_by_cure_road == usa_road
...
def test_groupunit_set_attr_WorksCorrectly():
    ...
    swim_text = "swimmers"
    swim_group = groupunit_shop(brand=swim_text)
    assert swim_group._partylinks_set_by_cure_road is None

    # WHEN
    water_road = f"{root_label()},sports,water"
    swim_group.set_attr(_partylinks_set_by_cure_road=water_road)

    # THEN
    assert swim_group._partylinks_set_by_cure_road == water_road


def test_groupunit_shop_WhenSinglePartyCorrectlyRemoves_partylinks_set_by_cure_road():
    # GIVEN
    swimmers = "swimmers"
    usa_road = f"{root_label()},nation-states,USA"
    ...
    assert (
        str(excinfo.value)
        == f"_partylinks_set_by_cure_road cannot be '{usa_road}' for a single_party GroupUnit. It must have no value."
    )
...
def test_Groupunit_reset_oath_importance_WorkCorrectly():
    # GIVEN
    maria_brand = "maria"
    maria_group = groupunit_shop(
        ...
        _oath_credit=0.33,
        _oath_debt=0.44,
        _oath_agenda_credit=0.13,
        _oath_agenda_debt=0.23,
    )
    print(f"{maria_group}")
    assert maria_group._oath_credit == 0.33
    assert maria_group._oath_debt == 0.44
    assert maria_group._oath_agenda_credit == 0.13
    assert maria_group._oath_agenda_debt == 0.23

    # WHEN
    maria_group.reset_oath_credit_debt()

    # THEN
    assert maria_group._oath_credit == 0
    assert maria_group._oath_debt == 0
    assert maria_group._oath_agenda_credit == 0
    assert maria_group._oath_agenda_debt == 0


def test_Groupunit_reset_oath_importance_reset_partylinks():
    # GIVEN
    todd_text = "Todd"
    mery_text = "Merry"
    ...
    mery_party = partylink_shop(
        ...
        _oath_credit=0.23,
        _oath_debt=0.5,
        _oath_agenda_credit=0.54,
        _oath_agenda_debt=0.57,
    )
    ...
    bikers_group = groupunit_shop(
        ...
        _oath_credit=0.33,
        _oath_debt=0.44,
        _oath_agenda_credit=0.1,
        _oath_agenda_debt=0.2,
    )
    ...
    assert biker_partylink_todd._oath_credit == 0.13
    assert biker_partylink_todd._oath_debt == 0.7
    assert biker_partylink_todd._oath_agenda_credit == 0.53
    assert biker_partylink_todd._oath_agenda_debt == 0.77

    biker_partylink_mery = bikers_group._partys.get(mery_text)
    assert biker_partylink_mery._oath_credit == 0.23
    assert biker_partylink_mery._oath_debt == 0.5
    assert biker_partylink_mery._oath_agenda_credit == 0.54
    assert biker_partylink_mery._oath_agenda_debt == 0.57

    # WHEN
    bikers_group.reset_oath_credit_debt()

    # THEN
    assert biker_partylink_todd._oath_credit == 0
    assert biker_partylink_todd._oath_debt == 0
    assert biker_partylink_todd._oath_agenda_credit == 0
    assert biker_partylink_todd._oath_agenda_debt == 0
    assert biker_partylink_mery._oath_credit == 0
    assert biker_partylink_mery._oath_debt == 0
    assert biker_partylink_mery._oath_agenda_credit == 0
    assert biker_partylink_mery._oath_agenda_debt == 0
...
def test_groupUnit_get_dict_ReturnsDictWithNecessaryDataForJSON():
    ...
    assert ee_dict == {
        ...
        "_single_party": False,
        "_partys": {},
        "_partylinks_set_by_cure_road": None,
    }
    ...
    teachers_group = groupunit_shop(
        ...
        _partylinks_set_by_cure_road=swim_road,
    )
...
def test_groupunit_get_from_JSON_SimpleExampleWorks():
    ...
    _partylinks_set_by_cure_road_text = "_partylinks_set_by_cure_road"
    print(f"{teacher_dict.get(_partylinks_set_by_cure_road_text)=}")
    groups_dict = {"teachers": teacher_dict}

    teachers_json = x_get_json(dict_x=groups_dict)
    print(f"{teachers_json.find(_partylinks_set_by_cure_road_text)=}")
    ...
    partylinks_set_by_cure_road_text = "_partylinks_set_by_cure_road"
    print(f"{teachers_obj_check_dict.get(partylinks_set_by_cure_road_text)=}")
...
def test_brandLink_set_oath_importanceCorrectly():
    # GIVEN
    bikers_brand = GroupBrand("bikers")
    bikers_creditor_weight = 3.0
    ...
    # WHEN
    group_heir_x.set_oath_credit_debt(
        idea_oath_importance=idea_oath_importance,
    ...
    # THEN
    assert group_heir_x._oath_credit == 0.05
    assert group_heir_x._oath_debt == 0.1
...
def test_Balanceline_exists():
    # GIVEN
    bikers_brand = GroupBrand("bikers")
    balanceline_x = Balanceline(brand=bikers_brand, _oath_credit=0.33, _oath_debt=0.55)
    assert balanceline_x.brand == bikers_brand
    assert balanceline_x._oath_credit == 0.33
    assert balanceline_x._oath_debt == 0.55

    # WHEN
    balanceline_x.add_oath_credit_debt(oath_credit=0.11, oath_debt=0.2)

    # THEN
    assert balanceline_x._oath_credit == 0.44
    assert balanceline_x._oath_debt == 0.75
