...
from src.oath.examples.example_oaths import (
    ...
    get_oath_irrational_example as example_oaths_get_oath_irrational_example,
)
...
def test_oath_requiredunits_create():
    ...
    x_oath.edit_idea_attr(road=work_road, required=work_wk_required)
    work_idea = x_oath._idearoot._kids[work_text]
...
def test_oath_set_requiredunits_status():
    x_oath = example_oaths_get_oath_with_4_levels()
    work_text = "work"
    work_road = f"{x_oath._cure_handle},{work_text}"
    weekday_text = "weekdays"
...
def test_agenda_returned_WhenNoRequiredsExist():
    lw_x = example_oaths_get_oath_with_4_levels()
    lw_x.set_oath_metrics()
    work_text = "work"
...
def test_oath_requiredheirs_AreCorrectlyInherited_v1():
    # GIVEN
    x_oath = example_oaths_get_oath_with_4_levels()
    work_text = "work"
    work_road = f"{x_oath._cure_handle},{work_text}"
    week_label = "weekdays"
    week_road = f"{x_oath._cure_handle},{week_label}"
    wed_text = "Wednesday"
    ...
    x_oath.edit_idea_attr(road=work_road, required=work_wk_build_requiredunit)
    work_idea = x_oath._idearoot._kids[work_text]
    assert work_idea._requiredunits != None
    # print(work_idea._requiredunits)
...
def test_oath_requiredheirs_AreCorrectlyInheritedTo4LevelsFromRoot():
    # GIVEN
    a4_oath = example_oaths_get_oath_with_4_levels()
    work_text = "work"
    work_road = f"{a4_oath._cure_handle},{work_text}"
    week_text = "weekdays"
    week_road = f"{a4_oath._cure_handle},{week_text}"
    wed_text = "Wednesday"
    wed_road = f"{week_road},{wed_text}"
    ...
    a4_oath.edit_idea_attr(road=work_road, required=work_wk_build_requiredunit)

    # WHEN
    rla_text = "hp"
    rla_road = f"{work_road},{rla_text}"
    a4_oath.add_idea(idea_kid=IdeaKid(_label=rla_text), pad=rla_road)
    cost_text = "cost_tracking"
    cost_road = f"{rla_road},{cost_text}"
    a4_oath.add_idea(idea_kid=IdeaKid(_label=cost_text), pad=cost_road)
    a4_oath.get_idea_list()

    # THEN
    work_idea = a4_oath._idearoot._kids[work_text]
...
def test_oath_requiredheirs_AreCorrectlyInheritedTo4LevelsFromLevel2():
    a4_oath = example_oaths_get_oath_with_4_levels()
    work_text = "work"
    work_road = f"{a4_oath._cure_handle},{work_text}"
    week_label = "weekdays"
    week_road = f"{a4_oath._cure_handle},{week_label}"
    wed_text = "Wednesday"
    wed_road = f"{week_road},{wed_text}"
    ...
    assert a4_oath._idearoot._requiredheirs is None
    assert work_idea._requiredheirs is None
    assert rla_idea._requiredheirs is None
    assert cost_idea._requiredheirs is None

    # WHEN
    idea_list = a4_oath.get_idea_list()

    # THEN
    assert a4_oath._idearoot._requiredheirs == {}  # work_wk_built_requiredheir
...
def test_oath_requiredunits_set_UnCoupledMethod():
    # Given
    x_oath = example_oaths_get_oath_with_4_levels()
    work_text = "work"
    work_road = f"{x_oath._cure_handle},{work_text}"
    ...
    # Then
    work_idea1 = x_oath.get_idea_kid(road=work_road)
...
def test_oath_requiredunits_set_sufffactIdeaWithDenomSetsSuffFactDivision():
    # Given
    x_oath = example_oaths_get_oath_with_4_levels()
    work_text = "work"
    work_road = f"{x_oath._cure_handle},{work_text}"
    ...
    week_text = "week"
    week_road = f"{x_oath._cure_handle},{time_text},{week_text}"
    x_oath.add_idea(
        idea_kid=IdeaKid(_label=time_text, _begin=100, _close=2000),
        pad=x_oath._cure_handle,
    )
    x_oath.add_idea(idea_kid=IdeaKid(_label=week_text, _denom=7), pad=time_road)
...
def test_oath_requiredunits_set_sufffactIdeaWithBeginCloseSetsSuffFactOpenNigh():
    # Given
    x_oath = example_oaths_get_oath_with_4_levels()
    work = "work"
    work_road = f"{x_oath._cure_handle},{work}"
    time = "time"
    time_road = f"{x_oath._cure_handle},{time}"
    rus_war = "rus_war"
    rus_war_road = f"{x_oath._cure_handle},{time},{rus_war}"
    x_oath.add_idea(
        idea_kid=IdeaKid(_label=time, _begin=100, _close=2000), pad=x_oath._cure_handle
    )
    x_oath.add_idea(
...
def test_oath_requiredunits_del_required_sufffact_UncoupledMethod1():
    # Given
    x_oath = example_oaths_get_oath_with_4_levels()
    work_road = f"{x_oath._cure_handle},work"
    weekday_road = f"{x_oath._cure_handle},weekdays"
    wed_road = f"{x_oath._cure_handle},weekdays,Wednesday"

    x_oath.edit_idea_attr(
        road=work_road, required_base=weekday_road, required_sufffact=wed_road
    )
    thu_road = f"{x_oath._cure_handle},weekdays,Thursday"
    x_oath.edit_idea_attr(
        road=work_road,
    ...
    # When
    x_oath.del_idea_required_sufffact(
        road=work_road,
        required_base=weekday_road,
        required_sufffact=thu_road,
    )
...
def test_oath_requiredunits_del_required_sufffact_UncoupledMethod2():
    # Given
    x_oath = example_oaths_get_oath_with_4_levels()
    work_road = f"{x_oath._cure_handle},work"
    weekdays_road = f"{x_oath._cure_handle},weekdays"
    work_idea1 = x_oath.get_idea_kid(road=work_road)
...
def test_oath_edit_idea_attr_oathIsAbleToEdit_suff_idea_active_status_AnyIdeaIfInvaildThrowsError():
    ...
    commute_road = f"{x_oath._cure_handle},{commute_text}"
    x_oath.add_idea(idea_kid=IdeaKid(_label=commute_text), pad=x_oath._cure_handle)
    x_oath.get_idea_list()  # set tree metrics
    commute_idea = x_oath.get_idea_kid(road=commute_road)
...
def test_oath_requiredunits_IdeaUnitActiveStatusInfluencesRequiredUnitStatus():
    # GIVEN an Oath with 5 ideas, 1 AcptFact:
    ...
    x_oath.set_acptfact(base=weekdays_road, pick=thu_road)
...
def test_oath_set_oath_metrics_InitipartySetsRationalAttrToFalse():
    # GIVEN
    x_oath = example_oaths_get_oath_with_4_levels()
    assert x_oath._rational == False
    # x_oath.set_oath_metrics()
    x_oath._rational = True
    assert x_oath._rational

    # WHEN
    # hack oath to set _max_tree_traverse = 1 (not allowed, should always be 2 or more)
    x_oath._max_tree_traverse = 1
    x_oath.set_oath_metrics()

    # THEN
    assert not x_oath._rational


def test_oath_tree_traverses_StopWhenNoChangeInStatusIsDetected():
    # GIVEN
    x_oath = example_oaths_get_oath_with_4_levels()
    assert x_oath._max_tree_traverse != 2

    # WHEN
    x_oath.set_oath_metrics()
    # for idea_key in x_oath._idea_dict.keys():
    #     print(f"{idea_key=}")

    # THEN
    assert x_oath._tree_traverse_count == 2


def test_oath_tree_traverse_count_CorrectlyCountsTreeTraversesForIrrationalOaths():
    # GIVEN irrational oath
    x_oath = example_oaths_get_oath_irrational_example()
    x_oath.set_oath_metrics()
    assert x_oath._tree_traverse_count == 3

    # WHEN
    x_oath.set_max_tree_traverse(int_x=21)
    x_oath.set_oath_metrics()

    # THEN
    assert x_oath._tree_traverse_count == 21
