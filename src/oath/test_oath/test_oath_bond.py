...
def test_oathunit_get_bond_status_ReturnsCorrectBool():
    # GIVEN
    jessi_text = "jessi"
    cx = OathUnit(_healer=jessi_text)
    casa_text = "case"
...
def test_oathunit_get_bond_status_ReturnsCorrectBoolWhenOnlyActionIdeaBalanceHeirsMatchOathGroups():
    # GIVEN
    jessi_text = "jessi"
    cx = OathUnit(_healer=jessi_text)
    casa_text = "case"
...
def test_oathunit_get_bond_status_ChecksActionIdeaGroupsheirsEqualOathGroupunits():
    # GIVEN
    jessi_text = "jessi"
    cx = OathUnit(_healer=jessi_text)
    casa_text = "case"
...
def test_oathunit_get_bond_status_ChecksActionIdeaGroupsheirsEqualOathGroupunits2():
    # GIVEN
    jessi_text = "jessi"
    cx = OathUnit(_healer=jessi_text)
    casa_text = "case"
...
# def test_oathunit_get_bond_status_ChecksOnlyNecessaryIdeasExist_MultipleScenario():
#     # GIVEN
#     jessi_text = "jessi"
#     cx = OathUnit(_healer=jessi_text)
#     casa_text = "case"
...
def test_oathunit_get_oath_sprung_from_single_idea_ReturnsCorrectOathScenario1():
    # GIVEN
    jessi_text = "jessi"
    cx = OathUnit(_healer=jessi_text)
    casa_text = "case"
    ...
    # WHEN
    bond_oath = cx.get_oath_sprung_from_single_idea(road=clean_cookery_road)

    # THEN
    # assert bond_oath._label == clean_cookery_text
    print(f"{len(bond_oath._idea_dict)=}")
    assert len(bond_oath._idea_dict) == 3
    b_src_idea = bond_oath.get_idea_kid(road=cx._cure_handle)
    source_x_idea = cx.get_idea_kid(road=cx._cure_handle)
    ...
    b_casa_idea = bond_oath.get_idea_kid(road=casa_road)
    ...
    b_clean_cookery_idea = bond_oath.get_idea_kid(road=clean_cookery_road)
    ...
    assert bond_oath._idearoot._kids.get(water_text) is None

    # for byx in bond_oath._idea_dict.values():
    ...
    #             print(f"checking src oath idea kid_label='{cx3._label}'")
...
def test_oathunit_export_all_bonds_ExportsFileOfBonds_2files(env_dir_setup_cleanup):
    # GIVEN
    cx = example_oaths_get_oath_with_4_levels_and_2requireds_2acptfacts()
...
# def test_oathunit_export_all_bonds_ExportsFileOfBonds_69files(env_dir_setup_cleanup):
#     # GIVEN
#     cx = example_oaths_oath_v001()
#     cx_idea_list = cx.get_idea_list()
#     action_count = 0
...
# def test_oathunit_export_all_bonds_ReturnsDictOfBonds(env_dir_setup_cleanup):
#     # GIVEN
#     cx = example_oaths_get_oath_with_4_levels_and_2requireds_2acptfacts()
#     cx_idea_list = cx.get_idea_list()
#     action_count = sum(bool(yx.promise) for yx in cx_idea_list)
...
#     bond_17 = oath_get_from_json(x_oath_json=json_17)
...
#     bond_2 = oath_get_from_json(x_oath_json=json_2)
#     assert bond_2.get_bond_status()


def test_oathunit_get_meld_of_oath_files_MeldsIntoSourceOath_Scenario1(
    env_dir_setup_cleanup,
):
    # GIVEN
    healer_text = "Nia"
    primary_oath = OathUnit(_healer=healer_text, _weight=10)

    work = "work"
    idea_kid_work = IdeaKid(_weight=30, _label=work, promise=True)
    primary_oath.add_idea(idea_kid=idea_kid_work, pad=f"{primary_oath._cure_handle}")

    cat = "feed cat"
    idea_kid_feedcat = IdeaKid(_weight=20, _label=cat, promise=True)
    primary_oath.add_idea(idea_kid=idea_kid_feedcat, pad=f"{primary_oath._cure_handle}")

    primary_oath.export_all_bonds(dir=get_temp_env_dir())
    cat_t = "feed cat"
    src_cat_idea = primary_oath._idearoot._kids.get(cat_t)
    src_cat_idea.set_originunit_empty_if_null()

    # WHEN
    new_oath = get_meld_of_oath_files(
        primary_oath=primary_oath,
        meldees_dir=get_temp_env_dir(),
    )

    # THEN
    assert primary_oath._weight == new_oath._weight
    assert primary_oath._idearoot._weight == new_oath._idearoot._weight
    src_cat_idea = primary_oath._idearoot._kids.get(cat_t)
    new_cat_idea = new_oath._idearoot._kids.get(cat_t)
    assert src_cat_idea._oath_coin_onset == new_cat_idea._oath_coin_onset
    assert src_cat_idea._originunit == new_cat_idea._originunit
    assert src_cat_idea == new_cat_idea
    assert primary_oath._idearoot._kids == new_oath._idearoot._kids
    assert primary_oath._idearoot == new_oath._idearoot
    assert primary_oath == new_oath


# def test_oathunit_get_meld_of_oath_files_MeldsIntoSourceOath_Scenario2(
#     env_dir_setup_cleanup,
# ):
#     # GIVEN
#     sourrcecx = OathUnit(_healer=healer_text, _weight=10)
...
#     sourrce_oath.add_idea(idea_kid=cat_idea, pad=work_road)

#     plant_text = "water plant"
#     plant_idea = IdeaKid(_weight=30, _label=plant_text, promise=True)
#     sourrce_oath.add_idea(idea_kid=plant_idea, pad=work_road)
#     sourrce_oath.export_all_bonds(dir=get_temp_env_dir())

#     # WHEN
#     new_oath = get_meld_of_oath_files(
#         oathunit=OathUnit(_healer=sourrce_oath._healer, _weight=0), dir=get_temp_env_dir()
#     )

#     # THEN
#     assert sourrce_oath._weight == new_oath._weight
#     assert sourrce_oath._idearoot._weight == new_oath._idearoot._weight
#     assert len(sourrce_oath._idearoot._kids) == 1
#     assert len(sourrce_oath._idearoot._kids) == len(new_oath._idearoot._kids)
#     sourrce_work_idea = sourrce_oath._idearoot._kids.get(work_text)
#     new_work_idea = new_oath._idearoot._kids.get(work_text)
#     sourrce_cat_idea = sourrce_work_idea._kids.get(cat_text)
#     new_cat_idea = new_work_idea._kids.get(cat_text)
#     print(f"{sourrce_cat_idea._oath_importance=} {new_cat_idea._oath_importance=}")
...
#     assert sourrce_oath._idearoot._kids.get(cat_text) == new_oath._idearoot._kids.get(cat_text)
#     assert sourrce_oath._idearoot._kids == new_oath._idearoot._kids
#     assert sourrce_oath._idearoot == new_oath._idearoot
#     assert sourrce_oath == new_oath


# - [ ] create test_oathunit_get_bond_status_ReturnsFalseWhenNotOnlyActionIdeaAcptFactsExist
