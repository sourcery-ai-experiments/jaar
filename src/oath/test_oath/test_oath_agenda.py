...
from src.oath.idea import IdeaCore, IdeaKid
from src.oath.road import Road
from src.oath.required_idea import RequiredUnit, SuffFactStatusFinder
from src.oath.group import groupunit_shop, balancelink_shop
from src.oath.party import partylink_shop
from src.oath.required_assign import assigned_unit_shop
from src.oath.examples.example_oaths import (
    get_oath_with_4_levels as example_oaths_get_oath_with_4_levels,
    get_oath_with_4_levels_and_2requireds as example_oaths_get_oath_with_4_levels_and_2requireds,
    get_oath_with7amCleanTableRequired as example_oaths_get_oath_with7amCleanTableRequired,
    get_oath_with_4_levels_and_2requireds_2acptfacts as example_oaths_get_oath_with_4_levels_and_2requireds_2acptfacts,
    oath_v001 as example_oaths_oath_v001,
    oath_v001_with_large_agenda as example_oaths_oath_v001_with_large_agenda,
    oath_v002 as example_oaths_oath_v002,
)
from src.oath.x_func import yr_explanation, open_file as x_func_open_file
...
def test_get_agenda_returns_agenda_with_only_required_allowed():
    # GIVEN
    x_oath = example_oaths_get_oath_with_4_levels_and_2requireds()
    week_text = "weekdays"
    week_road = f"{x_oath._cure_handle},{week_text}"
    sun_text = "Sunday"
    sun_road = f"{week_road},{sun_text}"
    x_oath.set_acptfact(base=week_road, pick=sun_road)
...
def test_get_agenda_returns_agenda_with_oath_importance():
    # GIVEN
    x_oath = example_oaths_get_oath_with_4_levels_and_2requireds_2acptfacts()
    ...
    assert agenda_list[0]._oath_importance

    print(f"{agenda_list[1]._label=}")
    assert agenda_list[1]._oath_importance


def test_get_agenda_with_No7amItem():
    # GIVEN
    x_oath = example_oaths_get_oath_with7amCleanTableRequired()
...
def test_get_agenda_with_7amItem():
    ...
    x_oath = example_oaths_get_oath_with7amCleanTableRequired()
    print(f"{len(x_oath.get_agenda_items())=}")
    assert len(x_oath.get_agenda_items()) == 1

    # WHEN
    day24hr_road = f"{x_oath._cure_handle},timetech,24hr day"
    day24hr_open = 0.0
    day24hr_nigh = 8.0
    ...
    print(x_oath._idearoot._acptfactunits[day24hr_road])
    print(x_oath._idearoot._kids[housework_text]._kids[clean_text]._requiredunits)
    print(x_oath._idearoot._kids[housework_text]._kids[clean_text]._active_status)
...
def test_get_agenda_does_not_return_promise_items_outside_range():
    ...
    x_oath.set_time_hreg_ideas(c400_count=7)
    c_label = "clean"
    c_idea = IdeaKid(_label=c_label, promise=True)
    x_oath.add_idea(idea_kid=c_idea, pad=x_oath._cure_handle)
    c_road = f"{x_oath._cure_handle},{c_label}"
    jajatime = f"{x_oath._cure_handle},time,jajatime"
    jajaday = f"{x_oath._cure_handle},time,jajatime,day"
    ...
    open_x = 1063971180
    nigh_x1 = 2063971523
    x_oath.set_acptfact(base=jajatime, pick=jajaday, open=open_x, nigh=nigh_x1)
    ...
    nigh_x2 = 1063971923
    x_oath.set_acptfact(base=jajatime, pick=jajaday, open=open_x, nigh=nigh_x2)
...
def test_exammple_agenda_exists():
    ...
    min_text = "day_minute"
    min_road = f"{x_oath._cure_handle},{min_text}"
    x_oath.set_acptfact(base=min_road, pick=min_road, open=0, nigh=1399)
    assert x_oath
...
def test_exammple_AgendaHasCorrectAttributes():
    ...
    day_min_road = f"{x_oath._cure_handle},{day_min_text}"
    x_oath.set_acptfact(base=day_min_road, pick=day_min_road, open=0, nigh=1399)
    month_week_text = "month_week"
    month_week_road = f"{x_oath._cure_handle},{month_week_text}"
    nations_text = "Nation-States"
    nations_road = f"{x_oath._cure_handle},{nations_text}"
    mood_text = "Moods"
    mood_road = f"{x_oath._cure_handle},{mood_text}"
    aaron_text = "Aaron Donald sphere"
    aaron_road = f"{x_oath._cure_handle},{aaron_text}"
    # internet_text = "Internet"
    # internet_road = f"{x_oath._cure_handle},{internet_text}"
    year_month_text = "year_month"
    year_month_road = f"{x_oath._cure_handle},{year_month_text}"
    x_oath.set_acptfact(base=month_week_road, pick=month_week_road)
    x_oath.set_acptfact(base=nations_road, pick=nations_road)
    x_oath.set_acptfact(base=mood_road, pick=mood_road)
    x_oath.set_acptfact(base=aaron_road, pick=aaron_road)
    # x_oath.set_acptfact(base=internet_road, pick=internet_road)
    x_oath.set_acptfact(base=year_month_road, pick=year_month_road)
    # season_text = "Seasons"
    # season_road = f"{x_oath._cure_handle},{season_text}"
    # x_oath.set_acptfact(base=season_road, pick=season_road)
    ced_week_text = "ced_week"
    ced_week_road = f"{x_oath._cure_handle},{ced_week_text}"
    x_oath.set_acptfact(base=ced_week_road, pick=ced_week_road)
    # water_text = "WaterBeing"
    # water_road = f"{x_oath._cure_handle},{water_text}"
    # x_oath.set_acptfact(base=water_road, pick=water_road)
    # movie_text = "No Movie playing"
    # movie_road = f"{x_oath._cure_handle},{movie_text}"
    # x_oath.set_acptfact(base=movie_road, pick=movie_text)

    # WHEN
    idea_action_list = x_oath.get_agenda_items()

    # THEN
    print("Test might be deprecated if it's not worthy it to fix this test source.")
    assert len(idea_action_list) == 27

    x_oath.set_acptfact(base=month_week_road, pick=f"{month_week_road},1st week")
    idea_action_list = x_oath.get_agenda_items()
    assert len(idea_action_list) == 27

    weekday_text = "weekdays"
    weekday_road = f"{x_oath._cure_handle},{weekday_text}"
    monday_text = "Monday"
    monday_road = f"{weekday_road},{monday_text}"

    x_oath.set_acptfact(base=weekday_road, pick=monday_road)
    idea_action_list = x_oath.get_agenda_items()
    assert len(idea_action_list) == 39

    x_oath.set_acptfact(base=weekday_road, pick=weekday_road)
    idea_action_list = x_oath.get_agenda_items()
    ...
    # for base in x_oath.get_missing_acptfact_bases():
    #     print(f"{base=}")
...
def test_exammple_AgendaCanFiltersOnBase():
    # GIVEN
    x_oath = example_oaths_oath_v001_with_large_agenda()
    week_text = "weekdays"
    week_road = f"{x_oath._cure_handle},{week_text}"
    print(f"{type(x_oath)=}")
    # for base in x_oath.get_missing_acptfact_bases():
    ...
    # for agenda_item in x_oath.get_agenda_items():
    #     print(
    ...
    # x_oath.edit_idea_attr(
    #     road="{x_oath._cure_handle},sufffacts,cleaning,laundry wednesday",
    #     required_del_sufffact_base=weekdays,
    #     required_del_sufffact_need=weekdays,
    # )
    assert len(x_oath.get_agenda_items()) == 68

    # When
    action_list = x_oath.get_agenda_items(base=week_road)
...
def test_set_agenda_task_as_complete_RangeWorksCorrectly():
    ...
    run_text = "run"
    run_road = f"{x_oath._cure_handle},{run_text}"
    time_text = "time"
    time_road = f"{x_oath._cure_handle},{time_text}"
    day_text = "day"
    day_road = f"{time_road},{day_text}"

    x_oath.add_idea(
        idea_kid=IdeaKid(_label=run_text, promise=True), pad=x_oath._cure_handle
    )
    x_oath.add_idea(
    ...
    x_oath.set_acptfact(base=day_road, pick=day_road, open=30, nigh=87)
    x_oath.get_agenda_items()
    run_requiredunits = x_oath._idearoot._kids[run_text]._requiredunits[day_road]
    ...
    print(f"{x_oath.get_required_bases()=}")
    assert len(x_oath.get_idea_list()) == 4
    assert len(x_oath.get_agenda_items()) == 1
    assert x_oath.get_agenda_items()[0]._task == True

    # When
    x_oath.set_agenda_task_complete(task_road=run_road, base=day_road)
...
def test_set_agenda_task_as_complete_DivisionWorksCorrectly():
    ...
    run_idea = x_oath.get_idea_kid(road=run_road)
    # print(f"{run_idea._acptfactheirs=}")
    x_oath.set_acptfact(base=day_road, pick=day_road, open=1, nigh=2)
    assert len(x_oath.get_agenda_items()) == 1
    x_oath.set_acptfact(base=day_road, pick=day_road, open=2, nigh=2)
    assert len(x_oath.get_agenda_items()) == 0
    x_oath.set_acptfact(base=day_road, pick=day_road, open=400, nigh=400)
    assert len(x_oath.get_agenda_items()) == 0
    x_oath.set_acptfact(base=day_road, pick=day_road, open=401, nigh=402)
    assert len(x_oath.get_agenda_items()) == 1
    # print(f"{run_idea._acptfactheirs=}")
...
def test_oath_get_from_json_LoadsActionFromJSONCorrectly():
    # GIVEN
    file_dir = oath_env()
    file_title = "example_oath1.json"
    x_oath_json = x_func_open_file(dest_dir=file_dir, file_title=file_title)

    # WHEN
    x_oath = get_from_json(x_oath_json=x_oath_json)

    # THEN
    assert len(x_oath.get_idea_list()) == 253
    print(f"{len(x_oath.get_idea_list())=}")
    casa_text = "casa"
    casa_road = f"{x_oath._cure_handle},{casa_text}"
    body_text = "workout"
    body_road = f"{casa_road},{body_text}"
    veg_text = "make veggies every morning"
    veg_road = f"{body_road},{veg_text}"
    veg_idea = x_oath.get_idea_kid(road=veg_road)
...
def test_weekdayAgendaItemsCorrectlyReturned():
    ...
    x_oath._set_acptfacts_empty_if_null()
    x_oath.set_time_hreg_ideas(c400_count=7)

    things_text = "things to do"
    x_oath.add_idea(IdeaKid(_label=things_text), pad=x_oath._cure_handle)
    t_road = f"{x_oath._cure_handle},{things_text}"
    clean = "clean"
    run = "run"
    swim = "swim"
    jog = "jog"
    veg = "veg"
    lift = "life"
    x_oath.add_idea(IdeaKid(_label=clean, promise=True), pad=t_road)
    x_oath.add_idea(IdeaKid(_label=run, promise=True), pad=t_road)
    x_oath.add_idea(IdeaKid(_label=swim, promise=True), pad=t_road)
    x_oath.add_idea(IdeaKid(_label=jog, promise=True), pad=t_road)
    x_oath.add_idea(IdeaKid(_label=veg, promise=True), pad=t_road)
    x_oath.add_idea(IdeaKid(_label=lift, promise=True), pad=t_road)
    time_text = "time"
    time_road = f"{x_oath._cure_handle},{time_text}"
    jaja_text = "jajatime"
    ...
    c_idea = x_oath.get_idea_kid(road=c_road)
    ...
    # for base, count_x in x_oath.get_required_bases().items():
    ...
    mon_min = x_oath.get_time_min_from_dt(dt=mon_dt)
    tue_min = x_oath.get_time_min_from_dt(dt=tue_dt)
    wed_min = x_oath.get_time_min_from_dt(dt=wed_dt)
    thu_min = x_oath.get_time_min_from_dt(dt=thu_dt)
    fri_min = x_oath.get_time_min_from_dt(dt=fri_dt)
    sat_min = x_oath.get_time_min_from_dt(dt=sat_dt)
    sun_min = x_oath.get_time_min_from_dt(dt=sun_dt)
    assert x_oath._idearoot._acptfactunits.get(jaja_road) is None

    # When
    print("\nset acptfact for Sunday")
    x_oath.set_acptfact(base=jaja_road, pick=jaja_road, open=sun_min, nigh=sun_min)
    # for acptfact in x_oath._idearoot._acptfactunits.values():
    #     print(f"{acptfact.base=} (H: {acptfact.acptfact}) {acptfact.active=} {acptfact.open=} {acptfact.nigh=}")

    # Then
    assert len(x_oath._idearoot._acptfactunits) == 7
    print(x_oath._idearoot._acptfactunits[jaja_road])
    print(x_oath._idearoot._acptfactunits[sat_road])
    print(x_oath._idearoot._acptfactunits[sun_road])
    print(x_oath._idearoot._acptfactunits[tue_road])
    print(x_oath._idearoot._acptfactunits[wed_road])
    print(x_oath._idearoot._acptfactunits[thu_road])
    print(x_oath._idearoot._acptfactunits[fri_road])
    assert x_oath._idearoot._acptfactunits[sun_road]
    assert x_oath._idearoot._acptfactunits[sun_road].open == 1440
    assert x_oath._idearoot._acptfactunits[sun_road].nigh == 1440
    # assert x_oath._idearoot._acptfactunits[sun_road].active == True

    # assert x_oath._idearoot._acptfactunits[tue_road].active == False
    # assert x_oath._idearoot._acptfactunits[wed_road].active == False
    # assert x_oath._idearoot._acptfactunits[thu_road].active == False
    # assert x_oath._idearoot._acptfactunits[fri_road].active == False
    # assert x_oath._idearoot._acptfactunits[sat_road].active == False

    # When
    print("\nset acptfact for Sat through Monday")
    x_oath.set_acptfact(base=jaja_road, pick=jaja_road, open=sat_min, nigh=mon_min)
    # for acptfact in x_oath._idearoot._acptfactunits.values():
    #     print(f"{acptfact.base=} (H: {acptfact.acptfact}) {acptfact.active=} {acptfact.open=} {acptfact.nigh=}")

    # Then
    assert x_oath._idearoot._acptfactunits[sat_road]
    # assert x_oath._idearoot._acptfactunits[sat_road].active == True
    # assert x_oath._idearoot._acptfactunits[sun_road].active == True
    # assert x_oath._idearoot._acptfactunits[tue_road].active == False
    # assert x_oath._idearoot._acptfactunits[wed_road].active == False
    # assert x_oath._idearoot._acptfactunits[thu_road].active == False
    ...
    assert x_oath._idearoot._acptfactunits[sat_road].open == 0
    assert x_oath._idearoot._acptfactunits[sat_road].nigh == 1440
    assert x_oath._idearoot._acptfactunits[sun_road].open == 1440
    assert x_oath._idearoot._acptfactunits[sun_road].nigh == 2880

    # When
    print("\nset acptfacts for Sunday through Friday")
    x_oath.set_acptfact(base=jaja_road, pick=jaja_road, open=sun_min, nigh=fri_min)
    # for acptfact in x_oath._idearoot._acptfactunits.values():
    #     print(f"{acptfact.base=} (H: {acptfact.acptfact}) {acptfact.active=} {acptfact.open=} {acptfact.nigh=}")
    ...
    # assert x_oath._idearoot._acptfactunits[sat_road].active == False
    # assert x_oath._idearoot._acptfactunits[sun_road].active == True
    # assert x_oath._idearoot._acptfactunits[tue_road].active == True
    # assert x_oath._idearoot._acptfactunits[wed_road].active == True
    # assert x_oath._idearoot._acptfactunits[thu_road].active == True
    # assert x_oath._idearoot._acptfactunits[fri_road].active == False
    ...
    dayzero_min = x_oath.get_time_min_from_dt(dt=dayzero_dt)
    dayten_min = x_oath.get_time_min_from_dt(dt=dayten_dt)
    x_oath.set_acptfact(
    ...
    # assert x_oath._idearoot._acptfactunits[wed_road].active == True
    # assert x_oath._idearoot._acptfactunits[thu_road].active == True
    # assert x_oath._idearoot._acptfactunits[fri_road].active == True


def test_oath_create_agenda_item_CorrectlyCreatesAllOathAttributes():
    ...
    assert len(x_oath._idearoot._kids) == 0

    clean_things_text = "cleaning things"
    clean_things_road = Road(f"{x_oath._cure_handle},{clean_things_text}")
    ...
    house_text = "house"
    house_road = Road(f"{x_oath._cure_handle},{house_text}")
    ...
    x_oath.set_time_hreg_ideas(c400_count=7)
    daytime_road = Road(f"{x_oath._cure_handle},time,jajatime,day")
    open_8am = 480
    nigh_8am = 480
    ...
    assert len(x_oath._partys) == 0
    assert len(x_oath._groups) == 0
    assert len(x_oath._idearoot._kids) == 1
    assert x_oath.get_idea_kid(road=daytime_road)._begin == 0
    assert x_oath.get_idea_kid(road=daytime_road)._close == 1440
    print(f"{clean_cookery_idea.get_road()=}")

    # GIVEN
    x_oath.set_dominate_promise_idea(idea_kid=clean_cookery_idea)

    # THEN
    # for idea_kid in x_oath._idearoot._kids.keys():
    #     print(f"  {idea_kid=}")

    print(f"{clean_cookery_idea.get_road()=}")
    assert x_oath.get_idea_kid(road=clean_cookery_road) != None
    assert x_oath.get_idea_kid(road=clean_cookery_road)._label == clean_cookery_text
    assert x_oath.get_idea_kid(road=clean_cookery_road).promise
    assert len(x_oath.get_idea_kid(road=clean_cookery_road)._requiredunits) == 2
    assert x_oath.get_idea_kid(road=clean_things_road) != None
    assert x_oath.get_idea_kid(road=cookery_room_road) != None
    ...
    assert len(x_oath._idearoot._kids) == 3
...
def test_Issue116Resolved_correctlySetsTaskAsTrue():
    # GIVEN
    x_oath = example_oaths_oath_v002()

    assert len(x_oath.get_agenda_items()) == 44
    jajatime_road = f"{x_oath._cure_handle},time,jajatime"

    # WHEN
    x_oath.set_acptfact(
        base=jajatime_road, pick=jajatime_road, open=1063998720, nigh=1064130373
    )
    action_idea_list = x_oath.get_agenda_items()

    # THEN
    assert len(action_idea_list) == 66
    jajatime_road = f"{x_oath._cure_handle},time,jajatime"
    night_text = "late_night_go_to_sleep"
    night_road = f"{x_oath._cure_handle},D&B,{night_text}"
    night_idea = x_oath._idea_dict.get(night_road)
    # for idea_x in x_oath.get_agenda_items():
    ...
    print(f"      {x_oath.get_jajatime_repeating_legible_text(open=1063998720)}")
    print(f"      {x_oath.get_jajatime_repeating_legible_text(open=1064130373)}")
