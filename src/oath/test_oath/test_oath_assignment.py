...
from src.oath.required_idea import RequiredUnit
from src.oath.party import partyunit_shop, partylink_shop
...
from src.oath.examples.example_oaths import (
    ...
    get_assignment_oath_example1 as example_oath_get_assignment_oath_example1,
)
...
def test_oathunit_get_assignment_ReturnsOath():
    # GIVEN
    jes_text = "jessi"
    jes1_oath = OathUnit(_healer=jes_text)
    jes1_oath.set_groupunits_empty_if_null()

    # WHEN
    bob_text = "bob"
    oath_x = OathUnit(_healer=jes_text)
    oath_x.set_groupunits_empty_if_null()
    assignor_known_partys_x = {}
    cx_assignment = jes1_oath.get_assignment(
        oath_x=oath_x,
    ...
    # THEN
    assert str(type(cx_assignment)) == "<class 'src.oath.oath.OathUnit'>"
    assert cx_assignment == oath_x


def test_oathunit_get_assignment_ReturnsEmptyBecauseAssignorIsNotInPartys():
    # GIVEN
    noa_text = "Noa"
    noa_oath = example_oaths_get_oath_with_4_levels()
    noa_oath.set_partyunit(partyunit_shop(title=noa_text))
    zia_text = "Zia"
    yao_text = "Yao"
    noa_oath.set_partyunit(partyunit_shop(title=zia_text))
    noa_oath.set_partyunit(partyunit_shop(title=yao_text))

    # WHEN
    bob_text = "bob"
    cx = OathUnit(_healer=noa_text)
    tx = OathUnit()
    ...
    cx_assignment = noa_oath.get_assignment(cx, tx._partys, bob_text)

    # THEN
    assert len(noa_oath._partys) == 3
    assert len(cx_assignment._partys) == 0


def test_oathunit_get_assignment_ReturnsCorrectPartys():
    # GIVEN
    jes_text = "Jessi"
    ...
    jes_oath.set_partyunit(partyunit_shop(title=zia_text))
    jes_oath.set_partyunit(partyunit_shop(title=noa_text))
    jes_oath.set_partyunit(partyunit_shop(title=yao_text))
    ...
    empty_oath = OathUnit(_healer=jes_text)
    cx_assignment = jes_oath.get_assignment(empty_oath, tx._partys, bob_text)
...
def test_oathunit_get_assignment_ReturnsCorrectGroups_Scenario1():
    # GIVEN
    jes_text = "Jessi"
    jes_oath = OathUnit(_healer=jes_text)
    jes_oath.set_partyunit(partyunit_shop(title=jes_text))
...
def test_oath__get_assignor_promise_ideas_ReturnsCorrectIdeaRoads():
    # GIVEN
    cx = example_oaths_get_oath_with7amCleanTableRequired()
    cx.set_oath_metrics()
...
def test_oath__get_relevant_roads_EmptyRoadReturnsEmpty():
    # GIVEN
    cx = example_oaths_get_oath_with_4_levels()
    cx.set_oath_metrics()
...
def test_oath__get_relevant_roads_RootRoadReturnsOnlyItself():
    # GIVEN
    cx = example_oaths_get_oath_with_4_levels()
    cx.set_oath_metrics()
...
def test_oath__get_relevant_roads_SimpleReturnsOnlyAncestors():
    # GIVEN
    cx = example_oaths_get_oath_with_4_levels()
    cx.set_oath_metrics()
...
def test_oath__get_relevant_roads_ReturnsSimpleRequiredUnitBase():
    # GIVEN
    healer_text = "Neo"
    cx = OathUnit(_healer=healer_text)
    casa_text = "casa"
...
def test_oath__get_relevant_roads_ReturnsRequiredUnitBaseAndDescendents():
    # GIVEN
    cx = example_oath_get_assignment_oath_example1()
    casa_text = "casa"
    casa_road = f"{cx._cure_handle},{casa_text}"
    floor_text = "mop floor"
    floor_road = f"{casa_road},{floor_text}"
...
# def test_oath__get_relevant_roads_ReturnsRequiredUnitBaseRecursively():
#     pass


def test_oath__get_relevant_roads_numeric_road_ReturnSimple():
    # GIVEN
    healer_text = "Yao"
    cx = OathUnit(_healer=healer_text)
    work_text = "work"
...
def test_oath__get_relevant_roads_range_source_road_ReturnSimple():
    # GIVEN
    healer_text = "Yao"
    cx = OathUnit(_healer=healer_text)
...
# def test_oath__get_relevant_roads_numeric_road_range_source_road_ReturnEntireRangeTree():
#
def test_oath__set_assignment_ideas_ReturnsCorrectIdeas():
    # GIVEN
    yao_text = "Yao"
    yao_oath = OathUnit(_healer=yao_text)
    casa_text = "casa"
    casa_road = f"{yao_oath._cure_handle},{casa_text}"
    yao_oath.add_idea(IdeaKid(_label=casa_text), pad=yao_oath._cure_handle)
    yao_oath.set_oath_metrics()

    # WHEN
    bob_text = "Bob"
    bob_oath = OathUnit(_healer=bob_text)
    relevant_roads = {
        yao_oath._cure_handle: "descendant",
        casa_road: "requirementunit_base",
    }
    yao_oath._set_assignment_ideas(oath_x=bob_oath, relevant_roads=relevant_roads)

    # THEN
    bob_oath.set_oath_metrics()
    print(f"{bob_oath._idea_dict.keys()=}")
    assert len(bob_oath._idea_dict) == 2
    assert bob_oath.get_idea_kid(casa_road) != None


def test_oath__set_assignment_ideas_ReturnsCorrectIdeaRoot_acptfacts():
    # GIVEN
    yao_text = "Yao"
    yao_oath = OathUnit(_healer=yao_text)
    ...
    yao_oath.add_idea(IdeaKid(basket_text), pad=casa_road)
    yao_oath.set_acptfact(base=basket_road, pick=basket_road)
    # print(f"{list(yao_oath._idearoot._acptfactunits.keys())=}")

    room_text = "room status"
    room_road = f"{casa_road},{room_text}"
    yao_oath.add_idea(IdeaKid(room_text), pad=casa_road)
    yao_oath.set_acptfact(base=room_road, pick=room_road)
    print(f"{list(yao_oath._idearoot._acptfactunits.keys())=}")
    ...
    yao_oath.set_oath_metrics()
    bob_oath.set_oath_metrics()
    assert list(yao_oath._idearoot._acptfactunits.keys()) == [basket_road, room_road]
    assert not list(bob_oath._idearoot._acptfactunits.keys())
    ...
    # THEN
    bob_oath.set_oath_metrics()
    assert bob_oath._idearoot._acptfactunits.get(room_road) is None
    assert list(bob_oath._idearoot._acptfactunits.keys()) == [basket_road]


def test_oath_get_assignment_getsCorrectIdeas_scenario1():
    # GIVEN
    cx = example_oath_get_assignment_oath_example1()
    casa_text = "casa"
    casa_road = f"{cx._cure_handle},{casa_text}"
...
def test_oath_get_assignment_CorrectlyCreatesAssignmentFile_v1():
    # GIVEN
    america_oath = get_oath_assignment_laundry_example1()
    cure_handle_text = "tiger_econ"
    print(f"{america_oath._cure_handle=} {america_oath._idea_dict.keys()=}")
    america_oath.set_cure_handle(cure_handle_text)
    print(f"{america_oath._cure_handle=} {america_oath._idea_dict.keys()=}")
    do_laundery_idea = america_oath.get_idea_kid("tiger_econ,casa,do_laundry")
    print(f"{do_laundery_idea._requiredunits.keys()=}")

    # WHEN
    joachim_text = "Joachim"
    joachim_oath = OathUnit(_healer=joachim_text)
    joachim_oath.set_cure_handle(cure_handle_text)
    print(f"{joachim_oath._cure_handle=} {joachim_oath._idea_dict.keys()=}")
    joachim_assignment = america_oath.get_assignment(
        oath_x=joachim_oath,
        assignor_partys={joachim_text: -1, america_oath._healer: -1},
        assignor_title=joachim_text,
    )

    # THEN
    assert joachim_assignment != None
    joachim_assignment.set_oath_metrics()
    ...
    casa_text = "casa"
    casa_road = f"{america_oath._cure_handle},{casa_text}"
    basket_text = "laundry basket status"
    basket_road = f"{casa_road},{basket_text}"
    b_full_text = "full"
