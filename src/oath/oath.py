...
from src.oath.x_func import x_get_json
from src.oath.idea import IdeaCore, IdeaKid, IdeaRoot, IdeaAttrHolder
...
from src.oath.lemma import Lemmas
from src.oath.road import (
    get_pad_from_road,
    is_sub_road,
    road_validate,
    change_road,
...
from src.oath.origin import originunit_get_from_dict, originunit_shop, OriginUnit
...
class OathUnit:
    ...
    def set_banking_attr_partyunits(self, river_tpartys: dict):
        for partyunit_x in self._partys.values():
            ...
            river_tparty = river_tpartys.get(partyunit_x.title)
    ...
    def get_oath_sprung_from_single_idea(self, road: Road):
        ...
        new_weight = self._weight * idea_x._oath_importance
        cx = OathUnit(_healer=self._idearoot._label, _weight=new_weight)
    ...
    def _are_all_partys_groups_are_in_idea_kid(self, road: Road) -> bool:
        ...
        # check all oath._partys are in balanceheir_partyunits
    ...
    def add_to_group_oath_credit_debt(
        self,
        groupbrand: GroupBrand,
        balanceheir_oath_credit: float,
        balanceheir_oath_debt: float,
    ):
        for group in self._groups.values():
            if group.brand == groupbrand:
                group.set_empty_oath_credit_debt_to_zero()
                group._oath_credit += balanceheir_oath_credit
                group._oath_debt += balanceheir_oath_debt

    def add_to_group_oath_agenda_credit_debt(
        self,
        groupbrand: GroupBrand,
        balanceline_oath_credit: float,
        balanceline_oath_debt: float,
    ):
        for group in self._groups.values():
            if (
                group.brand == groupbrand
                and balanceline_oath_credit != None
                and balanceline_oath_debt != None
            ):
                group.set_empty_oath_credit_debt_to_zero()
                group._oath_agenda_credit += balanceline_oath_credit
                group._oath_agenda_debt += balanceline_oath_debt

    def add_to_partyunit_oath_credit_debt(
        self,
        partyunit_title: PartyTitle,
        oath_credit,
        oath_debt: float,
        oath_agenda_credit: float,
        oath_agenda_debt: float,
    ):
        for partyunit in self._partys.values():
            if partyunit.title == partyunit_title:
                partyunit.add_oath_credit_debt(
                    oath_credit=oath_credit,
                    oath_debt=oath_debt,
                    oath_agenda_credit=oath_agenda_credit,
                    oath_agenda_debt=oath_agenda_debt,
                )
    ...
    def _get_lemma_acptfactunits(self) -> dict:
        ...
        while lemmas_x.is_lemmas_evaluated() == False or count_x > 10000:
            count_x += 1
            if count_x == 9998:
                raise InvalidOathException("lemma loop failed")
    ...
    def del_idea_kid(self, road: Road, del_children: bool = True):
        ...
        if x_road == []:
            raise InvalidOathException("Object cannot delete itself")
        temp_label = x_road.pop(0)
    ...
    def edit_idea_label(
        ...
        if self.get_idea_kid(road=old_road) is None:
            raise InvalidOathException(f"Idea {old_road=} does not exist")
        ...
        if old_road != new_road:
            ...
            if pad == "":
                self._idearoot.set_idea_label(
                    new_label, oath_cure_handle=self._cure_handle
                )
    ...
    def get_begin_close_if_denom_or_numeric_road(
        ...
        if (addin != None or numor != None or denom != None or reest != None) and len(
            ...
            raise InvalidOathException("Root Idea cannot have numor denom reest.")
        parent_road = self._cure_handle if len(anc_roads) == 1 else anc_roads[1]
    ...
    def _add_to_partyunits_oath_credit_debt(self, idea_oath_importance: float):
        ...
        for partyunit_x in self._partys.values():
            au_oath_credit = (
                idea_oath_importance * partyunit_x.get_creditor_weight()
            ) / sum_partyunit_creditor_weight

            au_oath_debt = (
                idea_oath_importance * partyunit_x.get_debtor_weight()
            ) / sum_partyunit_debtor_weight

            partyunit_x.add_oath_credit_debt(
                oath_credit=au_oath_credit,
                oath_debt=au_oath_debt,
                oath_agenda_credit=0,
                oath_agenda_debt=0,
            )

    def _add_to_partyunits_oath_agenda_credit_debt(self, idea_oath_importance: float):
        sum_partyunit_creditor_weight = self.get_partyunit_total_creditor_weight()
        sum_partyunit_debtor_weight = self.get_partyunit_total_debtor_weight()

        for partyunit_x in self._partys.values():
            au_oath_agenda_credit = (
                idea_oath_importance * partyunit_x.get_creditor_weight()
            ) / sum_partyunit_creditor_weight
            ...
            partyunit_x.add_oath_credit_debt(
                oath_credit=0,
                oath_debt=0,
                oath_agenda_credit=au_oath_agenda_credit,
                oath_agenda_debt=au_oath_agenda_debt,
            )

    def _set_partyunits_oath_agenda_importance(self, oath_agenda_importance: float):
        sum_partyunit_creditor_weight = self.get_partyunit_total_creditor_weight()
        sum_partyunit_debtor_weight = self.get_partyunit_total_debtor_weight()

        for partyunit_x in self._partys.values():
            au_oath_agenda_credit = (
                oath_agenda_importance * partyunit_x.get_creditor_weight()
            ) / sum_partyunit_creditor_weight

            au_oath_agenda_debt = (
                oath_agenda_importance * partyunit_x.get_debtor_weight()
            ) / sum_partyunit_debtor_weight

            partyunit_x.add_oath_agenda_credit_debt(
                oath_agenda_credit=au_oath_agenda_credit,
                oath_agenda_debt=au_oath_agenda_debt,
            )

    def _reset_groupunits_oath_credit_debt(self):
        self.set_groupunits_empty_if_null()
        for balancelink_obj in self._groups.values():
            balancelink_obj.reset_oath_credit_debt()

    def _set_groupunits_oath_importance(
        self, balanceheirs: dict[GroupBrand:Balancelink]
    ):
        self.set_groupunits_empty_if_null()
        for balancelink_obj in balanceheirs.values():
            self.add_to_group_oath_credit_debt(
                groupbrand=balancelink_obj.brand,
                balanceheir_oath_credit=balancelink_obj._oath_credit,
                balanceheir_oath_debt=balancelink_obj._oath_debt,
            )

    def _distribute_oath_agenda_importance(self):
        for idea in self._idea_dict.values():
            # If there are no balancelines associated with idea
            # distribute oath_importance via general partyunit
            # credit ratio and debt ratio
            # if idea.is_agenda_item() and idea._balancelines == {}:
            if idea.is_agenda_item():
                if idea._balancelines == {}:
                    self._add_to_partyunits_oath_agenda_credit_debt(
                        idea._oath_importance
                    )
                else:
                    for balanceline_x in idea._balancelines.values():
                        self.add_to_group_oath_agenda_credit_debt(
                            groupbrand=balanceline_x.brand,
                            balanceline_oath_credit=balanceline_x._oath_credit,
                            balanceline_oath_debt=balanceline_x._oath_debt,
                        )

    def _distribute_groups_oath_importance(self):
        for group_obj in self._groups.values():
            group_obj._set_partylink_oath_credit_debt()
            for partylink in group_obj._partys.values():
                self.add_to_partyunit_oath_credit_debt(
                    partyunit_title=partylink.title,
                    oath_credit=partylink._oath_credit,
                    oath_debt=partylink._oath_debt,
                    oath_agenda_credit=partylink._oath_agenda_credit,
                    oath_agenda_debt=partylink._oath_agenda_debt,
                )

    def _set_oath_agenda_ratio_credit_debt(self):
        oath_agenda_ratio_credit_sum = 0
        oath_agenda_ratio_debt_sum = 0

        for partyunit_x in self._partys.values():
            oath_agenda_ratio_credit_sum += partyunit_x._oath_agenda_credit
            oath_agenda_ratio_debt_sum += partyunit_x._oath_agenda_debt

        for partyunit_x in self._partys.values():
            partyunit_x.set_oath_agenda_ratio_credit_debt(
                oath_agenda_ratio_credit_sum=oath_agenda_ratio_credit_sum,
                oath_agenda_ratio_debt_sum=oath_agenda_ratio_debt_sum,
                oath_partyunit_total_creditor_weight=self.get_partyunit_total_creditor_weight(),
                oath_partyunit_total_debtor_weight=self.get_partyunit_total_debtor_weight(),
            )
    ...
    def _reset_partyunit_oath_credit_debt(self):
        self.set_partys_empty_if_null()
        for partyunit in self._partys.values():
            partyunit.reset_oath_credit_debt()
    ...
    def get_idea_kid(self, road: Road) -> IdeaKid:
        if road is None:
            raise InvalidOathException("get_idea_kid received road=None")
        ...
        if nodes == [] and src == self._idearoot._label:
            temp_idea = self._idearoot
            # raise InvalidOathException(f"Cannot return root '{self._economic_kind()}'")
        else:
    ...
    def _set_root_attributes(self):
        ...
        self._idearoot.set_requiredheirs(oath_idea_dict=self._idea_dict)
        self._idearoot.set_assignedheir(
            parent_assignheir=None, oath_groups=self._groups
        )
        ...
        self._idearoot.set_oath_importance(coin_onset_x=0, parent_coin_cease=1)
        self._idearoot.set_balanceheirs_oath_credit_debit()
        ...
        if self._idearoot.is_kidless():
            ...
            self._distribute_oath_importance(idea=self._idearoot)
    ...
    def _set_kids_attributes(
        ...
        idea_kid.set_active_status(
            ...
            oath_groups=self._groups,
            oath_healer=self._healer,
        )
        idea_kid.set_sibling_total_weight(parent_idea._kids_total_weight)
        idea_kid.set_oath_importance(
            coin_onset_x=coin_onset,
            parent_oath_importance=parent_idea._oath_importance,
        ...
        if idea_kid.is_kidless():
            # set idea's ancestor metrics using oath root as common reference
            self._set_ancestor_metrics(road=idea_kid.get_road())
            self._distribute_oath_importance(idea=idea_kid)

    def _distribute_oath_importance(self, idea: IdeaCore):
        ...
        if idea.is_balanceheirless() == False:
            self._set_groupunits_oath_importance(idea._balanceheirs)
        elif idea.is_balanceheirless():
            self._add_to_partyunits_oath_credit_debt(idea._oath_importance)

    def get_oath_importance(
        self, parent_oath_importance: float, weight: int, sibling_total_weight: int
    ):
        sibling_ratio = weight / sibling_total_weight
        return parent_oath_importance * sibling_ratio

    def get_idea_list(self):
        self.set_oath_metrics()
        return list(self._idea_dict.values())

    def set_oath_metrics(self):
        self._set_acptfacts_empty_if_null()
    ...
    def _execute_tree_traverse(self):
        ...
        coin_onset = self._idearoot._oath_coin_onset
        parent_coin_cease = self._idearoot._oath_coin_cease
        ...
        for idea_kid in self._idearoot._kids.values():
            ...
            coin_onset += idea_kid._oath_importance
        ...
        while cache_idea_list != []:
            ...
            if parent_idea._kids != None:
                coin_onset = parent_idea._oath_coin_onset
                parent_coin_cease = parent_idea._oath_coin_cease
    ...
    def _run_after_idea_all_tree_traverses(self):
        self._distribute_oath_agenda_importance()
        self._distribute_groups_oath_importance()
        self._set_oath_agenda_ratio_credit_debt()

    def _run_before_idea_tree_traverse(self):
        self._reset_groupunits_oath_credit_debt()
        self._reset_groupunits_oath_credit_debt()
        self._reset_partyunit_oath_credit_debt()
    ...
    def get_oath4party(
        ...
        self.set_oath_metrics()
        oath4party = OathUnit(_healer=party_title)
        oath4party._idearoot._oath_importance = self._idearoot._oath_importance
        # get party's partys: partyzone

        # get partyzone groups
        party_groups = self.get_party_groups(party_title=party_title)

        # set oath4party by traversing the idea tree and selecting associated groups
        # set root
        not_included_oath_importance = 0
        oath4party._idearoot._kids = {}
        for ykx in self._idearoot._kids.values():
            ...
            if y4a_included:
                y4a_new = IdeaKid(
                    _label=ykx._label,
                    _oath_importance=ykx._oath_importance,
                ...
                oath4party._idearoot._kids[ykx._label] = y4a_new
            else:
                not_included_oath_importance += ykx._oath_importance

        if not_included_oath_importance > 0:
            y4a_other = IdeaKid(
                _label="__other__",
                _oath_importance=not_included_oath_importance,
            )
            oath4party._idearoot._kids[y4a_other._label] = y4a_other
    ...
    def meld(self, other_oath, party_weight: float = None):
        self._meld_groups(other_oath)
        self._meld_partys(other_oath)
        self._meld_ideas(other_oath, party_weight)
        self._meld_acptfacts(other_oath)
        ...
        self._meld_originlinks(other_oath._healer, party_weight)

    def _meld_ideas(self, other_oath, party_weight: float):
        # meld idearoot
        self._idearoot.meld(other_idea=other_oath._idearoot, _idearoot=True)

        # meld all other ideas
        party_title = other_oath._healer
        o_idea_list = other_oath.get_idea_list_without_idearoot()
    ...
    def _meld_partys(self, other_oath):
        self.set_partys_empty_if_null()
        other_oath.set_partys_empty_if_null()
        for partyunit in other_oath._partys.values():
            if self._partys.get(partyunit.title) is None:
                self.set_partyunit(partyunit=partyunit)
            else:
                self._partys.get(partyunit.title).meld(partyunit)

    def _meld_groups(self, other_oath):
        self.set_groupunits_empty_if_null()
        other_oath.set_groupunits_empty_if_null()
        for brx in other_oath._groups.values():
            if self._groups.get(brx.brand) is None:
                self.set_groupunit(groupunit=brx)
            else:
                self._groups.get(brx.brand).meld(brx)

    def _meld_acptfacts(self, other_oath):
        self._set_acptfacts_empty_if_null()
        other_oath._set_acptfacts_empty_if_null()
        for hx in other_oath._idearoot._acptfactunits.values():
    ...
    def get_assignment(
        ...
        self.set_oath_metrics()
        self._set_assignment_partys(oath_x, assignor_partys, assignor_title)
        self._set_assignment_groups(oath_x)
        assignor_promises = self._get_assignor_promise_ideas(oath_x, assignor_title)
        relevant_roads = self._get_relevant_roads(assignor_promises)
        self._set_assignment_ideas(oath_x, relevant_roads)
        return oath_x

    def _set_assignment_ideas(self, oath_x, relevant_roads: dict[Road:str]):
        ...
        for relevant_road in sorted_relevants:
            ...
            #     relevant_idea._kids = {}
            #     oath_x.add_idea(idea_kid=relevant_idea, pad=relevant_idea._pad)
    ...
    def _set_assignment_partys(
        ...
        oath_x.set_partys_empty_if_null()
        if self._partys.get(assignor_title) != None:
            ...
            for partytitle_x in partys_set:
                oath_x.set_partyunit(partyunit=self._partys.get(partytitle_x))
        return oath_x

    def _set_assignment_groups(self, oath_x):
        revelant_groups = get_partys_relevant_groups(self._groups, oath_x._partys)
        for group_title, group_partys in revelant_groups.items():
            if oath_x._groups.get(group_title) is None:
                ...
                oath_x.set_groupunit(group_x)

    def _get_assignor_promise_ideas(
        self, oath_x, assignor_title: GroupBrand
    ) -> dict[Road:int]:
        oath_x.set_groupunits_empty_if_null()
        assignor_groups = get_party_relevant_groups(oath_x._groups, assignor_title)
        return {
            idea_road: -1
...
def get_from_json(x_oath_json: str) -> OathUnit:
    return get_from_dict(cx_dict=json.loads(x_oath_json))


def get_from_dict(cx_dict: dict) -> OathUnit:
    c_x = OathUnit()
    ...
    c_x.set_oath_metrics()  # clean up tree traverse defined fields
    return c_x


def get_dict_of_oath_from_dict(x_dict: dict[str:dict]) -> dict[str:OathUnit]:
    oathunits = {}
    for oathunit_dict in x_dict.values():
        x_oath = get_from_dict(cx_dict=oathunit_dict)
        oathunits[x_oath._healer] = x_oath
    return oathunits


def get_meld_of_oath_files(primary_oath: OathUnit, meldees_dir: str) -> OathUnit:
    primary_oath.set_oath_metrics()
    for meldee_file_x in x_func_dir_files(dir_path=meldees_dir):
        meldee_x = get_from_json(
            x_oath_json=x_func_open_file(meldees_dir, meldee_file_x)
        )
        primary_oath.meld(other_oath=meldee_x)
