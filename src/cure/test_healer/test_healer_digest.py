...
from src.cure.examples.example_healers import (
    get_2node_contract as example_healers_get_2node_contract,
    get_7nodeJRoot_contract as example_healers_get_7nodeJRoot_contract,
)
...
# def test_healer_save_isol_contract_CreateStartingContractFile(
#     healer_dir_setup_cleanup,
# ):
#     # GIVEN
#     lai_title = "Lai"
#     env_dir = get_temp_healer_dir()
#     lai_contract = healerunit_shop(title=lai_title, env_dir=env_dir)
#     lai_isol_file_title = lai_contract._admin._isol_file_title
#     with pytest_raises(Exception) as excinfo:
#         x_func_open_file(lai_contract._admin._healer_dir, lai_isol_file_title)
#     assert (
#         str(excinfo.value)
...
#     # THEN
#     assert x_func_open_file(lai_contract._admin._healer_dir, lai_isol_file_title) != None


def test_healeropen_isol_contract_WhenStartingContractFileDoesNotExists(
    healer_dir_setup_cleanup,
):
    # GIVEN
    tim_text = "Tim"
    ...
    assert isol_contract._cure_handle == cure_handle_text
...
def test_healer_save_isol_contract_IsolContractHealerMustBeHealer(
    healer_dir_setup_cleanup,
):
    # GIVEN
    p_title = "Game1"
    ...
    # THEN
    assert ux._admin.open_isol_contract()._healer == ux._admin._healer_title


def test_healer_open_isol_contract_WhenStartingContractFileExists(
    healer_dir_setup_cleanup,
):
    # GIVEN
    p_title = "Game1"
    ...
    x_contract.set_healer(new_healer=p_title)
    x_contract.set_contract_metrics()
    ...
    assert isol_contract._healer == ux._admin._healer_title


def test_healer_erase_isol_contract_file_DeletesFileCorrectly(
    healer_dir_setup_cleanup,
):
    # GIVEN
    p_title = "Game1"
    ...
    assert (
        str(excinfo.value)
        == f"Could not load file {ux._admin._healer_dir}/isol_contract.json (2, 'No such file or directory')"
    )


def test_healerunit_save_contract_to_digest_SavesFileCorrectly(
    healer_dir_setup_cleanup,
):
    # GIVEN
    ...
    # WHEN
    ux._admin.save_contract_to_digest(cx, src_contract_healer=src_contract_healer)

    # THEN
    cx_file_title = f"{cx._healer}.json"
    digest_file_path = f"{ux._admin._contracts_digest_dir}/{cx_file_title}"
    ...
    digest_cx_json = x_func_open_file(
        ...
        file_title=f"{src_contract_healer}.json",
    )
...
def test_presonunit__set_depotlink_CorrectlySets_blind_trust_DigestContract(
    ...
    # THEN
    cx_file_title = f"{cx._healer}.json"
    digest_file_path = f"{sue_cx._admin._contracts_digest_dir}/{cx_file_title}"
    print(f"Saving to {digest_file_path=}")
    assert os_path.exists(digest_file_path)
...
def test_healer_get_remelded_output_contract_withEmptyDigestDict(
    healer_dir_setup_cleanup,
):
    # GIVEN
    ...
    assert sx_output_after._idearoot == healer_contract_x._idearoot


def test_healer_get_remelded_output_contract_with1DigestedContract(
    healer_dir_setup_cleanup,
):
    # GIVEN
    yao_text = "Yao"
    ...
    assert sx_output_new._healer != input_contract._healer
    assert sx_output_new != input_contract


# def test_healer_set_digested_contract_with2Groups(healer_dir_setup_cleanup):
#     # GIVEN
#     env_dir = get_temp_healer_dir()
...
#     # WHEN
#     ux.set_single_digested_contract(_contract_healer="test1", digest_contract_x=s1)
#     sx_output_new = ux._admin.get_remelded_output_contract()
...
def test_healer_isol_contract_CorrectlysHasOriginLinksWithHealerAsSource(
    healer_dir_setup_cleanup,
):
    # GIVEN
    # healerunit with isol_contract and no other depot contracts
    yao_text = "Yao"
    isol_origin_weight = 1
    yao_originunit = originunit_shop()
    yao_originunit.set_originlink(title=yao_text, weight=isol_origin_weight)
    isol_contract_x = example_healers_get_7nodeJRoot_contract()
    isol_contract_x.set_healer(yao_text)
