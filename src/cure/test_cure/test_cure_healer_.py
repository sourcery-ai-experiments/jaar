...
def test_cure_create_healerunit_from_public_RaisesErrorWhenHealerDoesNotExist(
    env_dir_setup_cleanup,
):
    # GIVEN
    cure_handle = get_temp_env_handle()
...
def test_cure_rename_healerunit_WorksCorrectly(env_dir_setup_cleanup):
    # GIVEN
    cure_handle = get_temp_env_handle()
...
def test_cure_del_healer_dir_WorksCorrectly(env_dir_setup_cleanup):
    # GIVEN
    cure_handle = get_temp_env_handle()
    sx = cureunit_shop(handle=cure_handle, cures_dir=get_test_cures_dir())
    xia_text = "Xia"
    xia_dir = f"{sx.get_healers_dir()}/{xia_text}"
    xia_file_path = f"{xia_dir}/isol_pact.json"
