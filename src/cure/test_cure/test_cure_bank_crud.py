from src.oath.oath import OathUnit, IdeaKid, groupunit_shop, partylink_shop
...
def test_cure_refresh_bank_metrics_CorrectlyPopulatesOathTable01(
    env_dir_setup_cleanup,
):
    # GIVEN Create example cure with 4 Healers, each with 3 Partyunits = 12 ledger rows
    sx = cureunit_shop(handle=get_temp_env_handle(), cures_dir=get_test_cures_dir())
    ...
    sx.save_public_oath(oath_x=OathUnit(_healer=bob_text))
    sx.save_public_oath(oath_x=OathUnit(_healer=tom_text))
    sx.save_public_oath(oath_x=OathUnit(_healer=sal_text))
    sx.save_public_oath(oath_x=OathUnit(_healer=elu_text))

    sqlstr_count_oaths = get_table_count_sqlstr("oathunits")
    assert get_single_result_back(sx.get_bank_conn(), sqlstr_count_oaths) == 0

    # WHEN
    sx.refresh_bank_metrics()

    # THEN
    assert get_single_result_back(sx.get_bank_conn(), sqlstr_count_oaths) == 4
...
def test_cure_set_oath_bank_attrs_CorrectlyPopulatesOath_Groupunit_Partylinks(
    env_dir_setup_cleanup,
):
    # GIVEN
    sx = cureunit_shop(handle=get_temp_env_handle(), cures_dir=get_test_cures_dir())
    ...
    # create 4 oaths, 1 with group "swimming expert" linked to 1 party
    ...
    sal_oath.add_idea(idea_kid=IdeaKid(_label=swim_text), pad=sal_sports_road)
    bob_oath.add_idea(idea_kid=IdeaKid(_label=swim_text), pad=bob_sports_road)
    tom_oath.add_idea(idea_kid=IdeaKid(_label=swim_text), pad=tom_sports_road)

    sal_oath.add_partyunit(title=bob_text, creditor_weight=2, debtor_weight=2)
    ...
    sal_oath.set_groupunit(groupunit=swim_group_unit)
    ...
    sx.set_oath_bank_attrs(oath_healer=sal_text)
    e1_sal_oath = sx.get_public_oath(healer=sal_text)
    assert len(e1_sal_oath._groups.get(swim_group_text)._partys) == 1
    ...
    # THEN
    e1_sal_oath = sx.get_public_oath(healer=sal_text)
    assert len(e1_sal_oath._groups.get(swim_group_text)._partys) == 2
