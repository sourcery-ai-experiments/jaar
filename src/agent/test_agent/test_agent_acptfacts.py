...
from src.agent.idea import IdeaKid, Road
...
def test_agent_acptfact_exists():
    ...
    agent_x._idearoot._acptfactunits = {sunday_lw_acptfact.base: sunday_lw_acptfact}
    assert agent_x._idearoot._acptfactunits != None
    agent_x._idearoot._acptfactunits = None
    assert agent_x._idearoot._acptfactunits is None
    agent_x.set_acptfact(base=weekday_road, pick=sunday_road)
    assert agent_x._idearoot._acptfactunits == {
    ...
    agent_x._idearoot._acptfactunits = {usa_week_unit.base: usa_week_unit}

    agent_x._idearoot._acptfactunits = None
    assert agent_x._idearoot._acptfactunits is None
    agent_x.set_acptfact(base=usa_week_road, pick=usa_week_road, open=608, nigh=610)
    assert agent_x._idearoot._acptfactunits != None
    assert agent_x._idearoot._acptfactunits == {usa_week_unit.base: usa_week_unit}
...
def test_agent_get_idea_list_AcptFactHeirsCorrectlyInherited():
    # GIVEN
    src_text = "src"
    agent_x = AgentUnit(_desc=src_text)
    swim_text = "swim"
    swim_road = Road(f"{src_text},{swim_text}")
    agent_x.add_idea(idea_kid=IdeaKid(_desc=swim_text), walk=src_text)
    fast_text = "fast"
    slow_text = "slow"
    fast_road = Road(f"{swim_road},{fast_text}")
    slow_road = Road(f"{swim_road},{slow_text}")
    agent_x.add_idea(idea_kid=IdeaKid(_desc=fast_text), walk=swim_road)
    agent_x.add_idea(idea_kid=IdeaKid(_desc=slow_text), walk=swim_road)

    earth_text = "earth"
    earth_road = Road(f"{src_text},{earth_text}")
    agent_x.add_idea(idea_kid=IdeaKid(_desc=earth_text), walk=src_text)

    swim_idea = agent_x.get_idea_kid(road=swim_road)
    fast_idea = agent_x.get_idea_kid(road=fast_road)
    slow_idea = agent_x.get_idea_kid(road=slow_road)
...
def test_agent_get_idea_list_AcptFactHeirCorrectlyDeletesAcptFactUnit():
    # GIVEN
    src_text = "src"
    agent_x = AgentUnit(_desc=src_text)
    swim_text = "swim"
...
def test_get_ranged_acptfacts():
    ...
    place = "place_x"
    lw_x.add_idea(idea_kid=IdeaKid(_desc=place, _begin=600, _close=800), walk=src_text)
    ...
    mood = "mood_x"
    lw_x.add_idea(idea_kid=IdeaKid(_desc=mood), walk=src_text)
...
def test_get_roots_ranged_acptfacts():
    ...
    t_x_road = f"{src_text},{time_x}"
    mood_x = "mood_x"
    lw_x.add_idea(idea_kid=IdeaKid(_desc=mood_x), walk=src_text)
    ...
    mirrow_x = "mirrow_x"
    lw_x.add_idea(idea_kid=IdeaKid(_desc=mirrow_x, _numeric_road=time_x), walk=src_text)
...
def test_create_lemma_acptfacts_CorrectlyCreates1stLevelLemmaAcptFact_Scenario4():
    ...
    lw_x.add_idea(idea_kid=IdeaKid(_desc=arsub1, _begin=0, _close=140), walk=src_text)
    ...
    lw_x.add_idea(idea_kid=IdeaKid(_desc=arsub2, _begin=0, _close=20), walk=src_text)
...
def test_agent_set_acptfact_create_missing_ideas_CreatesBaseAndAcptFact():
    # GIVEN
    src_text = "src"
