...
from src.agent.tribe import TribeName, tribeunit_shop, tribelink_shop
...
def test_agent_set_ally_correctly_sets_allys_1():
    ...
    assert a_x._tribes["rico"]._single_ally == True

    # WHEN
    a_x._idearoot.set_tribelink(
        tribelink=tribelink_shop(name=TribeName("rico"), creditor_weight=10)
    )
    a_x._idearoot.set_tribelink(
        tribelink=tribelink_shop(name=TribeName("carmen"), creditor_weight=10)
    )
    a_x._idearoot.set_tribelink(
        tribelink=tribelink_shop(name=TribeName("patrick"), creditor_weight=10)
    )
...
def test_agent_set_ally_correctly_sets_allys_2():
    ...
    assert len(a_x._allys) == 3
    assert len(a_x._tribes) == 3
    assert a_x._tribes[rico_text]._single_ally == True
...
def test_agent_get_idea_list_CorrectlySetsAllyLinkAgentCreditAndDebt():
    ...
    bl_rico = tribelink_shop(name=rico_text, creditor_weight=20, debtor_weight=40)
    bl_carm = tribelink_shop(name=carm_text, creditor_weight=10, debtor_weight=5)
    bl_patr = tribelink_shop(name=patr_text, creditor_weight=10, debtor_weight=5)
    a_x._idearoot.set_tribelink(tribelink=bl_rico)
    a_x._idearoot.set_tribelink(tribelink=bl_carm)
    a_x._idearoot.set_tribelink(tribelink=bl_patr)

    rico_tribeunit = a_x._tribes.get(rico_text)
    carm_tribeunit = a_x._tribes.get(carm_text)
    patr_tribeunit = a_x._tribes.get(patr_text)
    rico_allylink = rico_tribeunit._allys.get(rico_text)
    carm_allylink = carm_tribeunit._allys.get(carm_text)
    patr_allylink = patr_tribeunit._allys.get(patr_text)
    ...
    # for tribe in a_x._tribes.values():
    ...
    # for tribelink in a_x._tribeheirs.values():
    #     print(
    #         f"{a_x._agent_importance=} {tribelink.name=} {tribelink._agent_credit=} {tribelink._agent_debt=}"
    #     )
    ...
    # for tribe in a_x._tribes.values():
    #     # print(f"{tribe.name=} {tribe._allys=}")
    ...
    #         # print(
    #         #     f"{tribe.name=} {allylink._agent_importance=} {tribe._agent_importance=}"
    #         # )
    ...
    a_x._idearoot.set_tribelink(
        tribelink=tribelink_shop(
            name=TribeName(selena_text), creditor_weight=20, debtor_weight=13
        )
    )
    a_x.set_agent_metrics()

    # THEN
    selena_tribeunit = a_x._tribes.get(selena_text)
    selena_allylink = selena_tribeunit._allys.get(selena_text)
...
def test_agent_get_idea_list_CorrectlySetsAllyUnitAgentImportance():
    ...
    bl_carm = tribelink_shop(name=carm_text, creditor_weight=10, debtor_weight=5)
    bl_patr = tribelink_shop(name=patr_text, creditor_weight=10, debtor_weight=5)
    a_x._idearoot._kids[swim_text].set_tribelink(tribelink=bl_rico)
    a_x._idearoot._kids[swim_text].set_tribelink(tribelink=bl_carm)
    a_x._idearoot._kids[swim_text].set_tribelink(tribelink=bl_patr)
    ...
    #     # print(
    #     #     f"{tribe.name=} {allyunit._agent_creditor=} {tribe._agent_creditor=}"
    #     # )
    ...
    a_x.set_allyunit(allyunit=allyunit_shop(name=AllyName(selena_text)))
    a_x._idearoot.set_tribelink(
        tribelink=tribelink_shop(name=selena_text, creditor_weight=20, debtor_weight=10)
    )
    a_x.set_agent_metrics()
...
def test_agent_get_idea_list_CorrectlySetsPartTribeedLWAllyUnitAgentImportance():
    # GIVEN
    prom_text = "prom"
    a_x = AgentUnit(_desc=prom_text)
    swim_text = "swim"
    ...
    carm_tribeunit = a_x._tribes.get(carm_text)
    patr_tribeunit = a_x._tribes.get(patr_text)
    assert rico_tribeunit._agent_credit != 0.5
    assert rico_tribeunit._agent_debt != 0.8
    assert carm_tribeunit._agent_credit != 0.25
    assert carm_tribeunit._agent_debt != 0.1
    assert patr_tribeunit._agent_credit != 0.25
    assert patr_tribeunit._agent_debt != 0.1
    assert (
        rico_tribeunit._agent_credit
        + carm_tribeunit._agent_credit
        + patr_tribeunit._agent_credit
        == 0.25
    )
    assert (
        rico_tribeunit._agent_debt
        + carm_tribeunit._agent_debt
        + patr_tribeunit._agent_debt
        == 0.25
    )

    # tribeunit_agent_credit_sum = 0.0
    # tribeunit_agent_debt_sum = 0.0
    # for tribeunit in a_x._tribes.values():
    #     assert tribeunit._agent_credit != None
    #     assert tribeunit._agent_credit not in [0.25, 0.5]
    #     assert tribeunit._agent_debt != None
    #     assert tribeunit._agent_debt not in [0.8, 0.1]
    #     # print(
    #     #     f"{tribe.name=} {tribeunit._agent_creditor=} {tribe._agent_creditor=}"
    #     # )
    #     print(f"{tribeunit.name=} {tribeunit._agent_credit=} {tribeunit._agent_debt=} ")
    #     # print(f"{tribeunit_agent_credit_sum=}")
    #     # print(f"{tribeunit_agent_debt_sum=}")
    #     tribeunit_agent_credit_sum += tribeunit._agent_credit
    #     tribeunit_agent_debt_sum += tribeunit._agent_debt
    # assert tribeunit_agent_credit_sum == 0.25
    # assert tribeunit_agent_debt_sum == 0.25
...
def clear_all_allyunits_tribeunits_agent_agenda_credit_debt(a_x: AgentUnit):
    # DELETE agent_agenda_debt and agent_agenda_credit
    for tribeunit_x in a_x._tribes.values():
        tribeunit_x.reset_agent_credit_debt()
        # for allylink_x in tribeunit_x._allys.values():
        #     print(f"{tribeunit_x.name=} {allylink_x.creditor_weight=}  {allylink_x._agent_credit:.6f} {allylink_x.debtor_weight=} {allylink_x._agent_debt:.6f} {allylink_x.name=} ")
...
def test_agent_agenda_credit_debt_IsCorrectlySet():
    ...
    clear_all_allyunits_tribeunits_agent_agenda_credit_debt(a_x=a_x)

    # TEST agent_agenda_debt and agent_agenda_credit are empty
    sum_tribeunit_agent_agenda_credit = 0
    sum_tribeunit_agent_agenda_debt = 0
    sum_allylink_agent_agenda_credit = 0
    sum_allylink_agent_agenda_debt = 0
    for tribeunit_x in a_x._tribes.values():
        # print(f"{allyunit.name=}")
        sum_tribeunit_agent_agenda_credit += tribeunit_x._agent_agenda_credit
        sum_tribeunit_agent_agenda_debt += tribeunit_x._agent_agenda_debt
        for allylink_x in tribeunit_x._allys.values():
    ...
    assert sum_tribeunit_agent_agenda_credit == 0
    assert sum_tribeunit_agent_agenda_debt == 0
    ...
    agenda_no_tribelines_count = 0
    agenda_yes_tribelines_count = 0
    agenda_no_tribelines_agent_i_sum = 0
    agenda_yes_tribelines_agent_i_sum = 0
    for agenda_item in agenda_list:
        sum_agent_agenda_importance += agenda_item._agent_importance
        if agenda_item._tribelines == {}:
            agenda_no_tribelines_count += 1
            agenda_no_tribelines_agent_i_sum += agenda_item._agent_importance
        else:
            agenda_yes_tribelines_count += 1
            agenda_yes_tribelines_agent_i_sum += agenda_item._agent_importance
    ...
    assert agenda_no_tribelines_count == 20
    assert agenda_yes_tribelines_count == 48
    assert agenda_no_tribelines_agent_i_sum == 0.00447826215370075
    assert agenda_yes_tribelines_agent_i_sum == 0.0027152834170378025
    x2 = agenda_no_tribelines_agent_i_sum + agenda_yes_tribelines_agent_i_sum
    e10 = 0.0000000001
    ...
    assert sum_tribeunit_agent_agenda_credit == x_sum
    assert sum_tribeunit_agent_agenda_debt == x_sum
    assert sum_allylink_agent_agenda_credit == x_sum
    assert sum_allylink_agent_agenda_debt == x_sum
    assert (
        abs(agenda_yes_tribelines_agent_i_sum - sum_tribeunit_agent_agenda_credit) < e10
    )
...
def test_agent_get_ally_tribes_returnsCorrectData():
    a_x = AgentUnit(_desc="prom")
    a_x.add_idea(idea_kid=IdeaKid(_desc="swim"), walk="prom")
    rico = "rico"
    carmen = "carmen"
    ...
    carmen_tribe_list = a_x.get_ally_tribes(ally_name=carmen)
    assert carmen_tribe_list == [carmen]

    swimmers = "swimmers"
    carmen_ally_dict = {AllyName(carmen): allylink_shop(name=carmen)}
    swim_tribe = tribeunit_shop(name=swimmers, _allys=carmen_ally_dict)
    a_x._tribes[swim_tribe.name] = swim_tribe
    carmen_tribe_list = a_x.get_ally_tribes(ally_name=carmen)
    assert carmen_tribe_list == [carmen, swimmers]
...
def test_agent_AllyUnit_CorrectlyCreatesNewName():
    ...
    assert len(a_x._tribes) == 3
    assert a_x._tribes.get(rico_text) != None
    assert a_x._tribes.get(rico_text)._single_ally == True
    ...
    a_x.edit_allyunit_name(
        ...
        allow_nonsingle_tribe_overwrite=False,
    )
    ...
    assert len(a_x._allys) == 3
    assert len(a_x._tribes) == 3
    assert a_x._tribes.get(rico_text) is None
    assert a_x._tribes.get(beto_text) != None
    assert a_x._tribes.get(beto_text)._single_ally == True
...
def test_agent_AllyUnit_raiseErrorNewNameAlreadyExists():
    ...
    assert sx._tribes.get(rico_text) != None
    assert sx._tribes.get(rico_text)._single_ally == True
...
def test_agent_AllyUnit_CorrectlyChangesTribeUnitAllyLinks():
    # GIVEN
    prom_text = "prom"
    a_x = AgentUnit(_desc=prom_text)
    rico_text = "rico"
    ...
    swim_tribe = tribeunit_shop(name=swim_text, _allys=carmen_ally_dict)
    ...
    a_x.set_tribeunit(tribeunit=swim_tribe)

    swim_tribe = a_x._tribes.get(swim_text)
    assert len(swim_tribe._allys) == 2
    assert swim_tribe._allys.get(rico_text) != None
    assert swim_tribe._allys.get(rico_text).creditor_weight == 7
    assert swim_tribe._allys.get(rico_text).debtor_weight == 30
    ...
    # THEN
    assert swim_tribe._allys.get(beto_text) != None
    assert swim_tribe._allys.get(beto_text).creditor_weight == 7
    assert swim_tribe._allys.get(beto_text).debtor_weight == 30
    assert swim_tribe._allys.get(rico_text) is None
    assert len(swim_tribe._allys) == 2
...
def test_agent_AllyUnit_CorrectlyMergesTribeUnitAllyLinks():
    # GIVEN
    # GIVEN
    prom_text = "prom"
    a_x = AgentUnit(_desc=prom_text)
    ...
    assert swim_tribe._allys.get(rico_text).creditor_weight == 7
    assert swim_tribe._allys.get(rico_text).debtor_weight == 30
    assert swim_tribe._allys.get(carm_text) != None
    assert swim_tribe._allys.get(carm_text).creditor_weight == 5
    assert swim_tribe._allys.get(carm_text).debtor_weight == 18
    ...
    # THEN
    assert swim_tribe._allys.get(carm_text) != None
    assert swim_tribe._allys.get(carm_text).creditor_weight == 12
    assert swim_tribe._allys.get(carm_text).debtor_weight == 48
    assert swim_tribe._allys.get(rico_text) is None
    assert len(swim_tribe._allys) == 1


def test_agent_AllyUnit_raiseErrorNewNameTribeUnitAlreadyExists():
    # GIVEN
    sx = AgentUnit(_desc="prom")
    rico_text = "rico"
    sx.add_allyunit(name=rico_text, uid=61, creditor_weight=13)
    ...
    carmen_text = "carmen"
    carmen_tribe = tribeunit_shop(name=carmen_text)
    carmen_tribe.set_allylink(allylink=allylink_shop(name=rico_text))
    carmen_tribe.set_allylink(allylink=allylink_shop(name=anna_text))
    sx.set_tribeunit(tribeunit=carmen_tribe)
    assert len(sx._tribes) == 3
    assert sx._allys.get(carmen_text) is None
    assert sx._tribes.get(carmen_text)._single_ally == False
    assert len(sx._tribes.get(carmen_text)._allys) == 2
    ...
    assert (
        str(excinfo.value)
        == f"Ally '{rico_text}' change to '{carmen_text}' failed since non-single tribe '{carmen_text}' already exists."
    )


def test_agent_AllyUnit_CorrectlyOverwriteNewNameTribeUnit():
    # GIVEN
    sx = AgentUnit(_desc="prom")
    rico_text = "rico"
    sx.add_allyunit(name=rico_text, uid=61, creditor_weight=13)
    ...
    carmen_text = "carmen"
    carmen_tribe = tribeunit_shop(name=carmen_text)
    carmen_tribe.set_allylink(allylink=allylink_shop(name=rico_text, creditor_weight=3))
    carmen_tribe.set_allylink(allylink=allylink_shop(name=anna_text, creditor_weight=5))
    sx.set_tribeunit(tribeunit=carmen_tribe)
    assert len(sx._tribes) == 3
    assert sx._allys.get(rico_text) != None
    assert sx._allys.get(carmen_text) is None
    assert sx._tribes.get(carmen_text)._single_ally == False
    assert len(sx._tribes.get(carmen_text)._allys) == 2
    assert sx._tribes.get(carmen_text)._allys.get(anna_text).creditor_weight == 5
    assert sx._tribes.get(carmen_text)._allys.get(rico_text).creditor_weight == 3
    ...
    assert sx._tribes.get(carmen_text)._single_ally == True
    assert len(sx._tribes.get(carmen_text)._allys) == 1
    assert sx._tribes.get(carmen_text)._allys.get(rico_text) is None
    assert sx._tribes.get(carmen_text)._allys.get(carmen_text).creditor_weight == 1


def test_agent_set_all_allyunits_uids_unique_CorrectlySetsEmptyTribeUIDs():
    # GIVEN
    src_text = "src"
...
def test_agent_set_all_allyunits_uids_unique_CorrectlySetsChangesSameTribeUIDs():
    # GIVEN
    src_text = "src"
    sx = AgentUnit(_desc=src_text)
    sx.set_allys_empty_if_null()
...
def test_agent_get_allyunits_name_list_CorrectlyReturnsListOfAllyUnits():
    ...
    fun_text = "fun people"
    fun_tribe = tribeunit_shop(name=fun_text)
    fun_tribe.set_allylink(allylink=allylink_shop(name=will_text))
    sx.set_tribeunit(tribeunit=fun_tribe)
    assert len(sx._tribes) == 4
    assert len(sx._allys) == 3
