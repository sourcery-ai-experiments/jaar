...
from src.heal.owner import OwnerUnit, ownerunit_shop
...
class HealUnit:
    ...
    def set_contract_bank_attrs(self, contract_owner: str):
        ...
        for groupunit_x in contract_obj._groups.values():
            if groupunit_x._partylinks_set_by_heal_road != None:
                groupunit_x.clear_partylinks()
                ic = get_idea_catalog_dict(
                    self.get_bank_conn(), groupunit_x._partylinks_set_by_heal_road
                )
        ...
        with self.get_bank_conn() as bank_conn:
            ...
            for groupunit_x in contractunit_x._groups.values():
                groupunit_catalog_x = GroupUnitCatalog(
                    ...
                    partylinks_set_by_heal_road=groupunit_x._partylinks_set_by_heal_road,
                )
    ...
    def set_healunit_kind(self, kind: str):
        self.kind = kind
    ...
    def create_ownerunit_from_public(self, title: str):
        ...
        self.set_ownerunits_empty_if_null()
        self.set_ownerunit_to_heal(owner_x)

    def set_ownerunit_to_heal(self, owner: OwnerUnit):
    ...
    def rename_ownerunit(self, old_title: str, new_title: str):
        ...
        self.set_ownerunit_to_heal(owner=owner_x)
        x_func_delete_dir(old_owner_dir)
        self.del_owner_from_heal(owner_title=old_title)

    def del_owner_from_heal(self, owner_title):
    ...
    def save_public_contract(self, contract_x: ContractUnit):
        contract_x.set_heal_kind(heal_kind=self.kind)
        x_func_save_file(
...
def healunit_shop(
    ...
    if in_memory_bank is None:
        in_memory_bank = True
    heal_x = HealUnit(kind=kind, heals_dir=heals_dir, _ownerunits=_ownerunits)
    heal_x.create_dirs_if_null(in_memory_bank=in_memory_bank)
    return heal_x
