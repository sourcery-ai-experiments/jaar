...
def get_temp_env_dir():
    return f"{get_test_heals_dir()}/{get_temp_env_kind()}"


def get_test_heals_dir():
    return "src/heal/examples/heals"
...
def create_contract_file_for_heals(heal_dir: str, contract_owner: str):
    contract_x = ContractUnit(_owner=contract_owner)
    contract_dir = f"{heal_dir}/contracts"
...
def create_example_heals_list():
    return x_func_dir_files(
        dir_path=get_test_heals_dir(), include_dirs=True, include_files=False
    )
...
def _delete_and_set_ex3():
    ...
    bob_text = "bob wurld"
    create_contract_file_for_heals(sx.get_object_root_dir(), bob_text)
    ...
    land_text = "tim wurld"
    create_contract_file_for_heals(
        heal_dir=sx.get_object_root_dir(), contract_owner=land_text
    )
    sx.create_depotlink_to_generated_contract(
...
def create_example_heal(heal_kind: str):
    sx = healunit_shop(kind=heal_kind, heals_dir=get_test_heals_dir())
    sx.create_dirs_if_null(in_memory_bank=True)


def delete_dir_example_heal(heal_obj: HealUnit):
    x_func_delete_dir(heal_obj.get_object_root_dir())


def rename_example_heal(heal_obj: HealUnit, new_title):
    # base_dir = heal_obj.get_object_root_dir()
    base_dir = "src/heal/examples/heals"
    src_dir = f"{base_dir}/{heal_obj.kind}"
    dst_dir = f"{base_dir}/{new_title}"
    os_rename(src=src_dir, dst=dst_dir)
    heal_obj.set_healunit_kind(kind=new_title)
...
def copy_evaluation_heal(src_kind: str, dest_kind: str):
    base_dir = "src/heal/examples/heals"
    new_dir = f"{base_dir}/{dest_kind}"
    if os_path.exists(new_dir):
        raise InvalidHealCopyException(
            f"Cannot copy heal to '{new_dir}' directory because '{new_dir}' exists."
        )
    # base_dir = heal_obj.get_object_root_dir()
