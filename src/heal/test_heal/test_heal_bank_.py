...
from src.heal.heal import healunit_shop, HealUnit
...
from src.heal.y_func import check_connection


def test_heal_create_bank_db_CreatesBankDBIfItDoesNotExist(
    env_dir_setup_cleanup,
):
    # GIVEN create heal
...
def test_heal_create_bank_db_CanCreateBankInMemory(
    env_dir_setup_cleanup,
):
    # GIVEN create heal
    sx = healunit_shop(kind=get_temp_env_kind(), heals_dir=get_test_heals_dir())
...
def test_heal_refresh_bank_metrics_CanConnectToBankInMemory(
    env_dir_setup_cleanup,
):
    # GIVEN create heal
    sx = healunit_shop(kind=get_temp_env_kind(), heals_dir=get_test_heals_dir())
...
def test_heal_get_bank_db_conn_CreatesBankDBIfItDoesNotExist(env_dir_setup_cleanup):
    # GIVEN create heal
    sx = HealUnit(kind=get_temp_env_kind(), heals_dir=get_test_heals_dir())
    # WHEN/THEN
