...
def test_heal_get_ledger_table_insert_sqlstr_CorrectlyPopulatesTable01(
    env_dir_setup_cleanup,
):
...
def test_heal_get_idea_catalog_table_insert_sqlstr_CorrectlyPopulatesTable01(
    env_dir_setup_cleanup,
):
    # GIVEN
    sx = healunit_shop(kind=get_temp_env_kind(), heals_dir=get_test_heals_dir())
...
def test_heal_get_idea_catalog_dict_ReturnsCorrectData(env_dir_setup_cleanup):
    # GIVEN
    sx = healunit_shop(kind=get_temp_env_kind(), heals_dir=get_test_heals_dir())
    sx.create_dirs_if_null(in_memory_bank=True)
    sx.refresh_bank_metrics()
...
def test_heal_get_acptfact_catalog_table_insert_sqlstr_CorrectlyPopulatesTable01(
    env_dir_setup_cleanup,
):
    # GIVEN Create example heal with 4 Owners, each with 3 Partyunits = 12 ledger rows
...
def test_heal_get_groupunit_catalog_table_insert_sqlstr_CorrectlyPopulatesTable01(
    env_dir_setup_cleanup,
):
    # GIVEN Create example heal with 4 Owners, each with 3 Partyunits = 12 ledger rows
    ...
    bob_group_x = GroupUnitCatalog(
        ...
        partylinks_set_by_heal_road=f"{get_temp_env_kind()},USA",
    )
