...
class OwnerAdmin:
    ...
    def _get_empty_isol_contract(self):
        ...
        cx.set_heal_kind(self._heal_kind)
        return cx
...
def owneradmin_shop(_owner_title: str, _env_dir: str, _heal_kind: str) -> OwnerAdmin:
    uax = OwnerAdmin(
        _owner_title=_owner_title, _env_dir=_env_dir, _heal_kind=_heal_kind
    )
    uax.set_dirs()
    return uax
...
class OwnerUnit:
    ...
    def _set_depotlink(
        ...
        if link_type == "assignment":
            ...
            new_cx_obj.set_heal_kind(self._admin._heal_kind)
    ...
    def _set_assignment_depotlink(self, outer_owner):
        ...
        empty_cx.set_heal_kind(self._admin._heal_kind)
    ...
    # housekeeping
    def set_env_dir(self, env_dir: str, owner_title: str, heal_kind: str):
        self._admin = owneradmin_shop(
            _owner_title=owner_title, _env_dir=env_dir, _heal_kind=heal_kind
        )
...
def ownerunit_shop(
    title: str, env_dir: str, heal_kind: str, _auto_output_to_public: bool = None
) -> OwnerUnit:
    owner_x = OwnerUnit()
    owner_x.set_env_dir(env_dir, title, heal_kind=heal_kind)
    owner_x.get_isol()
