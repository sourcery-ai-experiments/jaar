...
from src.heal.owner import ownerunit_shop
...
#     lai_contract = ownerunit_shop(title=lai_title, env_dir=env_dir)
#     lai_isol_file_title = lai_contract._admin._isol_file_title
#     with pytest_raises(Exception) as excinfo:
#         x_func_open_file(lai_contract._admin._owner_dir, lai_isol_file_title)
#     assert (
#         str(excinfo.value)
...
#     # THEN
#     assert x_func_open_file(lai_contract._admin._owner_dir, lai_isol_file_title) != None
...
def test_owneropen_isol_contract_WhenStartingContractFileDoesNotExists(
    ...
    tim_text = "Tim"
    env_dir = get_temp_owner_dir()
    heal_kind_text = get_temp_heal_kind()
    ux = ownerunit_shop(title=tim_text, env_dir=env_dir, heal_kind=heal_kind_text)

    # WHEN
    isol_contract = ux._admin.open_isol_contract()
    assert isol_contract != None
    assert isol_contract._heal_kind == heal_kind_text

    # THEN
    x_contract = ContractUnit(_owner=tim_text)
    x_contract.set_heal_kind(get_temp_heal_kind())
...
def test_owner_save_isol_contract_IsolContractOwnerMustBeOwner(
    ...
    p_title = "Game1"
    env_dir = get_temp_owner_dir()
    ux = ownerunit_shop(p_title, env_dir, get_temp_heal_kind())
    ...
    # THEN
    assert ux._admin.open_isol_contract()._owner == ux._admin._owner_title
...
def test_owner_open_isol_contract_WhenStartingContractFileExists(
    ...
    x_contract.set_owner(new_owner=p_title)
    x_contract.set_contract_metrics()
    ...
    assert isol_contract._owner == ux._admin._owner_title
...
def test_owner_erase_isol_contract_file_DeletesFileCorrectly(
    ...
    # THEN
    with pytest_raises(Exception) as excinfo:
        x_func_open_file(ux._admin._owner_dir, file_title)
    assert (
        str(excinfo.value)
...
def test_ownerunit_save_contract_to_digest_SavesFileCorrectly(
    ...
    owner_title = "owner1"
    env_dir = get_temp_owner_dir()
    ux = ownerunit_shop(owner_title, env_dir, get_temp_heal_kind())
    ...
    # THEN
    cx_file_title = f"{cx._owner}.json"
...
def test_presonunit__set_depotlink_CorrectlySets_blind_trust_DigestContract(
    ...
    sue_text = "Sue"
    env_dir = get_temp_owner_dir()
    sue_cx = ownerunit_shop(sue_text, env_dir, get_temp_heal_kind())
...
def test_owner_get_remelded_output_contract_withEmptyDigestDict(
    ...
    ux = ownerunit_shop(owner_title_x, get_temp_owner_dir(), get_temp_heal_kind())
    ux.create_core_dir_and_files()
    ...
    assert sx_output_before._idearoot._label == get_temp_heal_kind()
    ...
    owner_contract_x.set_heal_kind(get_temp_heal_kind())
...
def test_owner_get_remelded_output_contract_with1DigestedContract(
    ...
    yao_text = "Yao"
    env_dir = get_temp_owner_dir()
    ux = ownerunit_shop(yao_text, env_dir, get_temp_heal_kind())
    ux.create_core_dir_and_files()
    ...
    assert sx_output_old._idearoot._label == get_temp_heal_kind()
...
#     # GIVEN
#     env_dir = get_temp_owner_dir()
#     ux = ownerunit_shop(title="test8", env_dir=env_dir)
#     sx_output_old = ux._admin.get_remelded_output_contract()
#     assert str(type(sx_output_old)).find(".contract.ContractUnit'>")
...
def test_owner_isol_contract_CorrectlysHasOriginLinksWithOwnerAsSource(
    ...
    ux = ownerunit_shop(yao_text, get_temp_owner_dir(), get_temp_heal_kind())
    ux.create_core_dir_and_files()
