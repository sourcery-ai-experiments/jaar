...
def test_ownerunit_exists(owner_dir_setup_cleanup):
    ...
    ux = ownerunit_shop(
        ...
        heal_kind=get_temp_heal_kind(),
    )

    # GIVEN
    assert ux._admin._owner_title != None
    assert ux._admin._heal_kind != None
    assert ux._admin._heal_kind == get_temp_heal_kind()
...
def test_ownerunit_auto_output_to_public_SavesContractToPublicDirWhenTrue(
    ...
    # public_file_path = f"src/heal/examples/ex_env/contracts/{public_file_title}"
    ux = ownerunit_shop(tim_text, env_dir, heal_kind, _auto_output_to_public=True)
    ux.create_core_dir_and_files()
    assert os_path.exists(public_file_path) is False

    # WHEN
    tim_contract = ContractUnit(_owner=tim_text)
    tim_contract.set_heal_kind(heal_kind)
...
def test_ownerunit_auto_output_to_public_DoesNotSaveContractToPublicDirWhenFalse(
    ...
    print(f"{public_file_path=}")
    # public_file_path = f"src/heal/examples/ex_env/contracts/{public_file_title}"
    ux = ownerunit_shop(tim_text, env_dir, heal_kind, _auto_output_to_public=False)
    ux.create_core_dir_and_files()
    assert os_path.exists(public_file_path) is False
...
def test_ownerunit_get_isol_createsEmptyContractWhenFileDoesNotExist(
    ...
    # GIVEN
    tim_text = "Tim"
    tim_ux = ownerunit_shop(tim_text, get_temp_owner_dir(), get_temp_heal_kind())
...
def test_ownerunit_set_isol_savesGivenContractSet_isol_None(
    ...
    # GIVEN
    tim_text = "Tim"
    ux = ownerunit_shop(tim_text, get_temp_owner_dir(), get_temp_heal_kind())
    ux.create_core_dir_and_files()
