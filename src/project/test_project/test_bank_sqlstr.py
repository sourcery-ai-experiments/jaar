...
def test_project_get_ledger_table_insert_sqlstr_CorrectlyPopulatesTable01(
    ...
    insert_sqlstr = get_ledger_table_insert_sqlstr(
        x_deal=bob_deal, partyunit_x=partyunit_x
    )
    print(insert_sqlstr)
    ...
    ledger_dict = get_ledger_dict(
        db_conn=x_project.get_bank_conn(), payer_healer=bob_text
    )
    # ledger_x = None
...
def test_get_river_ledger_unit_CorrectlyReturnsRiverLedgerUnit(env_dir_setup_cleanup):
    ...
    insert_sqlstr_sal = get_ledger_table_insert_sqlstr(bob_deal, partyunit_sal)
    ...
    insert_sqlstr_tim = get_ledger_table_insert_sqlstr(bob_deal, partyunit_tim)
...
def test_get_river_tally_table_insert_sqlstr_CorrectlyPopulatesTable01(
    ...
    insert_sqlstr_tom = get_ledger_table_insert_sqlstr(
        x_deal=bob_deal, partyunit_x=partyunit_tom
    )
    partyunit_sal = partyunit_shop(
        title=sal_text,
        _deal_credit=0.9,
    ...
    insert_sqlstr_sal = get_ledger_table_insert_sqlstr(
        x_deal=bob_deal, partyunit_x=partyunit_sal
    )
...
def test_refresh_bank_metrics_Populates_idea_catalog_table(env_dir_setup_cleanup):
    ...
    x_project.save_public_deal(x_deal=bob_deal)
    x_project.save_public_deal(x_deal=tim_deal)
    x_project.save_public_deal(x_deal=sal_deal)
...
def test_project_get_idea_catalog_dict_ReturnsCorrectData(env_dir_setup_cleanup):
    ...
    with x_project.get_bank_conn() as bank_conn:
        ...
        assert get_single_result_back(x_project.get_bank_conn(), i_count_sqlstr) == 20

    # WHEN / THEN
    assert len(get_idea_catalog_dict(x_project.get_bank_conn())) == 20
    b_road = f"{get_temp_env_handle()},B"
    assert len(get_idea_catalog_dict(x_project.get_bank_conn(), b_road)) == 3
    ce_road = f"{get_temp_env_handle()},C,E"
    assert len(get_idea_catalog_dict(x_project.get_bank_conn(), ce_road)) == 2
    ex_road = f"{get_temp_env_handle()}"
    assert len(get_idea_catalog_dict(x_project.get_bank_conn(), ex_road)) == 4
...
def test_get_groupunit_catalog_dict_CorrectlyReturnsGroupUnitData(
    ...
    tom_bob_deal_group = f"{tom_text} {bob_text}"
    tom_deal_elu_group = f"{tom_text} {elu_text}"
    assert groupunit_catalog_dict.get(bob_deal_tom_group) != None
    assert groupunit_catalog_dict.get(tom_bob_deal_group) != None
