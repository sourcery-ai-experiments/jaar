...
def test_MemberUnit_exists():
    ...
    assert bob_member._contract_credit is None
    assert bob_member._contract_debt is None
    assert bob_member._contract_agenda_credit is None
    assert bob_member._contract_agenda_debt is None
...
def test_MemberUnit_set_empty_contract_credit_debt_to_zero_CorrectlySetsZero():
    # GIVEN
    bob_name = "bob"
    bob_member = memberunit_shop(name=bob_name)
    assert bob_member._contract_credit is None
    ...
    assert bob_member._contract_agenda_credit is None
    assert bob_member._contract_agenda_debt is None
    assert bob_member._contract_agenda_ratio_credit is None
    assert bob_member._contract_agenda_ratio_debt is None

    # WHEN
    bob_member.set_empty_contract_credit_debt_to_zero()

    # THEN
    assert bob_member._contract_credit == 0
    assert bob_member._contract_debt == 0
    assert bob_member._contract_agenda_credit == 0
    assert bob_member._contract_agenda_debt == 0
    assert bob_member._contract_agenda_ratio_credit == 0
    assert bob_member._contract_agenda_ratio_debt == 0

    # GIVEN
    bob_member._contract_credit = 0.27
    bob_member._contract_debt = 0.37
    bob_member._contract_agenda_credit = 0.41
    bob_member._contract_agenda_debt = 0.51
    bob_member._contract_agenda_ratio_credit = 0.23
    bob_member._contract_agenda_ratio_debt = 0.87
    assert bob_member._contract_credit == 0.27
    assert bob_member._contract_debt == 0.37
    assert bob_member._contract_agenda_credit == 0.41
    assert bob_member._contract_agenda_debt == 0.51
    assert bob_member._contract_agenda_ratio_credit == 0.23
    assert bob_member._contract_agenda_ratio_debt == 0.87
...
def test_MemberUnit_reset_contract_credit_debt_MethodWorkCorrectly():
    # GIVEN
    bob_name = "bob"
    bob_member = memberunit_shop(name=bob_name)
    bob_member._contract_credit = 0.27
    ...
    bob_member._contract_agenda_credit = 0.41
    bob_member._contract_agenda_debt = 0.51
    bob_member._contract_agenda_ratio_credit = 0.433
    bob_member._contract_agenda_ratio_debt = 0.533
    assert bob_member._contract_credit == 0.27
    assert bob_member._contract_debt == 0.37
    assert bob_member._contract_agenda_credit == 0.41
    assert bob_member._contract_agenda_debt == 0.51
    assert bob_member._contract_agenda_ratio_credit == 0.433
    assert bob_member._contract_agenda_ratio_debt == 0.533

    # WHEN
    bob_member.reset_contract_credit_debt()
...
def test_MemberUnit_add_contract_credit_debt_MethodWorkCorrectly():
    # GIVEN
    bob_name = "bob"
    bob_member = memberunit_shop(name=bob_name)
    bob_member._contract_credit = 0.4106
    bob_member._contract_debt = 0.1106
    bob_member._contract_agenda_credit = 0.41
    bob_member._contract_agenda_debt = 0.51
    assert bob_member._contract_agenda_credit == 0.41
    assert bob_member._contract_agenda_debt == 0.51
    ...
    # WHEN
    bob_member.add_contract_credit_debt(
    ...
    # THEN
    assert bob_member._contract_credit == 0.7406
    assert bob_member._contract_debt == 0.1656
    assert bob_member._contract_agenda_credit == 0.71
    assert bob_member._contract_agenda_debt == 0.56


def test_MemberUnit_set_contract_agenda_ratio_credit_debt_MethodWorkCorrectly():
    # GIVEN
    bob_name = "bob"
    ...
    # WHEN
    bob_member.set_contract_agenda_ratio_credit_debt(
        contract_agenda_ratio_credit_sum=0.2,
        contract_agenda_ratio_debt_sum=0.5,
        contract_memberunit_total_creditor_weight=20,
        contract_memberunit_total_debtor_weight=14,
    )

    # THEN
    assert bob_member._contract_agenda_ratio_credit == 0.205
    assert bob_member._contract_agenda_ratio_debt == 0.102
    ...
    # THEN
    assert bob_member._contract_agenda_ratio_credit == 0.75
    assert bob_member._contract_agenda_ratio_debt == 0.5
...
def test_MemberUnit_set_banking_data_MethodWorkCorrectly():
    ...
    assert bob_member._contract_agenda_ratio_credit == 0.077
    assert bob_member._contract_agenda_ratio_debt == 0.066
    assert bob_member._bank_tax_paid is None
    assert bob_member._bank_tax_diff is None
    ...
    # assert (
    #     str(excinfo.value)
    #     == f"MemberUnit.set_banking_data fail: tax_paid={tax_paid_v2} + tax_diff={tax_diff} not equal to _contract_agenda_ratio_credit={bob_member._contract_agenda_ratio_credit}"
    # )
...
def test_MemberLink_exists():
    ...
    assert member_link_x._contract_credit != None
    assert member_link_x._contract_credit == 0.7
    assert member_link_x._contract_debt == 0.51
    assert member_link_x._contract_agenda_credit == 0.66
    assert member_link_x._contract_agenda_debt == 0.55


def test_memberlink_shop_set_contract_credit_debt_CorrectlyWorks():
    # GIVEN
    bikers_name = MemberName("Marie")
    bikers_creditor_weight = 3.0
    ...
    assert member_link_x._contract_credit is None
    assert member_link_x._contract_debt is None
    assert member_link_x._contract_agenda_credit is None
    assert member_link_x._contract_agenda_debt is None

    # WHEN
    member_link_x.set_contract_credit_debt(
    ...
    # THEN
    assert member_link_x._contract_credit == 0.025
    assert member_link_x._contract_debt == 0.45
    assert member_link_x._contract_agenda_credit == 0.049
    assert member_link_x._contract_agenda_debt == 0.25755


def test_memberlink_shop_reset_contract_credit_debt():
    # GIVEN
    biker_name = "maria"
    ...
    assert biker_member._contract_credit != None
    assert biker_member._contract_debt != None

    # WHEN
    biker_member.reset_contract_credit_debt()

    # THEN
    assert biker_member._contract_credit == 0
    assert biker_member._contract_debt == 0
