...
def test_contract_contract_get_tree_metrics_sets_uids_correctly():
    ...
    swim_text = "swim"
    walk_text = "walk"
    cx.add_idea(idea_kid=IdeaKid(_label=swim_text, _uid=None), walk=cx._goal_tag)
    cx.add_idea(idea_kid=IdeaKid(_label=walk_text, _uid=2), walk=cx._goal_tag)
    assert cx.get_idea_kid(road=f"{cx._goal_tag},{swim_text}")._uid is None

    cx.set_all_idea_uids_unique()

    # THEN
    assert cx.get_idea_kid(road=f"{cx._goal_tag},{swim_text}")._uid != None
...
def test_contract_get_tree_metrics_ReturnsANoneActionIdeaRoad():
    ...
    cx.add_idea(idea_kid=idea_kid_weekdays, walk=f"{cx._goal_tag}")
    tree_metrics_before = cx.get_tree_metrics()
    # WHEN/THEN
...
def test_contract_get_tree_metrics_ReturnsAnActionIdeaRoad():
    ...
    assert (
        ...
        == f"{cx._goal_tag},ACME,ACME Employee Responsiblities,Know Abuse Prevention and Reporting guildlines,Take Fall 2021 training"
    )
