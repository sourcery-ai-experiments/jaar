...
def test_set_contract_metrics_RootOnlyCorrectlySetsDescendantAttributes():
    owner_text = "Tim"
    lw_x = ContractUnit(_owner=owner_text)
...
def test_TreeTraverseSetsClearsGrouplineestorsCorrectly():
    ...
    assert contract_x._idearoot._grouplines == {}
    contract_x._idearoot._grouplines = {1: "testtest"}
    assert contract_x._idearoot._grouplines != {}
    contract_x.set_contract_metrics()
    assert contract_x._idearoot._grouplines == {}

    # test for level 1 and level n
    contract_x._idearoot._kids["work"]._grouplines = {1: "testtest"}
    assert contract_x._idearoot._kids["work"]._grouplines != {}
    contract_x.set_contract_metrics()
    assert contract_x._idearoot._kids["work"]._grouplines == {}
...
def test_TreeTraverseSetsGrouplineestorFromRootCorrectly():
    # GIVEN
    a_x = example_contracts_get_contract_with_4_levels()
...
def test_contract4member_Exists():
    contract_x = example_contracts_get_contract_with_4_levels()
    x1_idea = IdeaKid(_label="email", promise=True)
    contract_x.add_idea(idea_kid=x1_idea, walk=f"{root_label()},work")
    x2_idea = IdeaKid(_label="sweep", promise=True)
    contract_x.add_idea(idea_kid=x2_idea, walk=f"{root_label()},work")

    sandy_name = MemberName("sandy")
    contract_x.add_memberunit(name=sandy_name)
    x_grouplink = GroupLink(name=sandy_name)
    yrx = contract_x._idearoot
    yrx._kids["work"]._kids["email"].set_grouplink(grouplink=x_grouplink)
    sandy_contract4member = contract_x.get_contract4member(
...
def test_contract4member_hasCorrectLevel1StructureNoGrouplessBranches():
    ...
    contract_x.add_memberunit(name=billy_name)
...
def test_contract_get_orderd_node_list_CorrectlyFiltersRangedIdeaRoads():
    ...
    time = "timeline"
    contract_x.add_idea(IdeaKid(_label=time, _begin=0, _close=700), walk=root_label())
    t_road = f"{root_label()},{time}"
    week = "weeks"
    contract_x.add_idea(IdeaKid(_label=week, _denom=7), walk=t_road)

    assert len(contract_x.get_idea_tree_ordered_road_list()) == 3
    assert (
        len(contract_x.get_idea_tree_ordered_road_list(no_range_descendants=True)) == 2
    )
...
#     exp_sandy.add_idea(idea_kid=IdeaKid(_label="C", _contract_importance=0.07), walk=f"{root_label()},A")
#     exp_sandy.add_idea(idea_kid=IdeaKid(_label="E", _contract_importance=0.5), walk=f"{root_label()},A,C")
#     exp_sandy.add_idea(idea_kid=IdeaKid(_label="B", _contract_importance=0.13), walk="blahblah")

#     # generated sandy
#     gen_sandy = lw_x.get_contract4member(acptfacts=None, member_name=sandy_name)

#     # check generated sandy is correct
#     assert gen_sandy.get_idea_kid(road=f"{root_label()},A")._contract_importance == 0.07
#     assert gen_sandy.get_idea_kid(road=f"{root_label()},A,C")._contract_importance == 0.07
#     assert gen_sandy.get_idea_kid(road=f"{root_label()},A,C,E")._contract_importance == 0.5
#     assert gen_sandy.get_idea_kid(road=f"{root_label()},B")._contract_importance == 0.13
#     assert (
#         gen_sandy.get_idea_kid(road=f"{root_label()},A")._contract_importance
#         == exp_sandy.get_idea_kid(road=f"{root_label()},A")._contract_importance
#     )
#     assert (
#         gen_sandy.get_idea_kid(road=f"{root_label()},A,C")._contract_importance
#         == exp_sandy.get_idea_kid(road=f"{root_label()},A,C")._contract_importance
#     )
#     assert (
#         gen_sandy.get_idea_kid(road=f"{root_label()},A,C,E")._contract_importance
#         == exp_sandy.get_idea_kid(road=f"{root_label()},A,C,E")._contract_importance
#     )
#     assert (
#         gen_sandy.get_idea_kid(road=f"{root_label()},B")._contract_importance
#         == exp_sandy.get_idea_kid(road=f"{root_label()},B")._contract_importance
#     )
