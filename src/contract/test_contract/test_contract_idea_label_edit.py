...
def test_idea_label_fails_when_idea_does_not_exist():
    ...
    work_text = "work"
    work_road = f"{sx._healing_handle},{work_text}"
    swim_text = "swim"
    sx.add_idea(walk=sx._healing_handle, idea_kid=IdeaKid(_label=work_text))
    sx.add_idea(walk=work_road, idea_kid=IdeaKid(_label=swim_text))

    # When/Then
    no_idea_road = Road(f"{sx._healing_handle},bees")
...
def test_Contract_level0_idea_edit_idea_label_RaisesError_healing_handle_IsNone():
    # GIVEN
    healer_text = "Tim"
    sx = ContractUnit(_healer=healer_text)
    ...
    work_road = f"{sx._healing_handle},{work_text}"
    swim_text = "swim"
    swim_road = f"{sx._healing_handle},{work_text},{swim_text}"
    sx.add_idea(walk=sx._healing_handle, idea_kid=IdeaKid(_label=work_text))
    sx.add_idea(walk=work_road, idea_kid=IdeaKid(_label=swim_text))
    assert sx._healer == healer_text
    assert sx._healing_handle == sx._healing_handle
    assert sx._idearoot._label == sx._healing_handle
    work_idea = sx.get_idea_kid(road=work_road)
    assert work_idea._walk == sx._healing_handle
    ...
    with pytest_raises(Exception) as excinfo:
        moon_text = "moon"
        sx.edit_idea_label(old_road=sx._healing_handle, new_label=moon_text)
    assert (
        str(excinfo.value)
        == f"Cannot set idearoot to string other than '{sx._healing_handle}'"
    )
...
def test_Contract_level0_idea_edit_idea_label_RaisesError_healing_handle_IsDifferent():
    # GIVEN
    healer_text = "Tim"
    sx = ContractUnit(_healer=healer_text)
    work_text = "work"
    ...
    sx.add_idea(walk=work_road, idea_kid=IdeaKid(_label=swim_text))
    sun_text = "sun"
    sx._healing_handle = sun_text
    assert sx._healer == healer_text
    assert sx._healing_handle == sun_text
...
def test_contract_set_healing_handle_CorrectlySetsAttr():
    ...
    sx = ContractUnit(_healer=healer_text)
    work_text = "work"
    old_work_road = f"{sx._healing_handle},{work_text}"
    swim_text = "swim"
    ...
    sx.set_healing_handle(healing_handle=healing_handle_text)

    # THEN
    new_work_road = f"{healing_handle_text},{work_text}"
    swim_text = "swim"
    new_swim_road = f"{new_work_road},{swim_text}"
    assert sx._healing_handle == healing_handle_text
    assert sx._idearoot._label == healing_handle_text
    work_idea = sx.get_idea_kid(road=new_work_road)
    assert work_idea._walk == healing_handle_text
...
def test_idea_find_replace_road_Changes_kids_scenario1():
    ...
    old_healer_road = Road(f"{sx._healing_handle},{old_healer_text}")
    bloomers_text = "bloomers"
    old_bloomers_road = Road(f"{sx._healing_handle},{old_healer_text},{bloomers_text}")
    roses_text = "roses"
    old_roses_road = Road(
        f"{sx._healing_handle},{old_healer_text},{bloomers_text},{roses_text}"
    )
    red_text = "red"
    old_red_road = Road(
        f"{sx._healing_handle},{old_healer_text},{bloomers_text},{roses_text},{red_text}"
    )

    sx.add_idea(walk=sx._healing_handle, idea_kid=IdeaKid(_label=old_healer_text))
    sx.add_idea(walk=old_healer_road, idea_kid=IdeaKid(_label=bloomers_text))
    ...
    new_healer_road = Road(f"{sx._healing_handle},{new_healer_text}")
    ...
    assert r_idea_bloomers._walk == new_healer_road
    assert r_idea_bloomers._kids.get(roses_text) != None

    r_idea_roses = r_idea_bloomers._kids.get(roses_text)
    new_bloomers_road = Road(f"{sx._healing_handle},{new_healer_text},{bloomers_text}")
    assert r_idea_roses._walk == new_bloomers_road
    assert r_idea_roses._kids.get(red_text) != None
    r_idea_red = r_idea_roses._kids.get(red_text)
    new_roses_road = Road(
        f"{sx._healing_handle},{new_healer_text},{bloomers_text},{roses_text}"
    )
...
def test_contract_edit_idea_label_Changes_acptfactunits():
    ...
    healer = "healer"
    bloomers_text = "bloomers"
    bloomers_road = f"{sx._healing_handle},{healer},{bloomers_text}"
    roses_text = "roses"
    roses_road = f"{sx._healing_handle},{healer},{bloomers_text},{roses_text}"
    old_water_text = "water"
    old_water_road = f"{sx._healing_handle},{old_water_text}"
    rain_text = "rain"
    old_rain_road = f"{sx._healing_handle},{old_water_text},{rain_text}"

    sx.add_idea(walk=sx._healing_handle, idea_kid=IdeaKid(_label=healer))
    ...
    # WHEN
    new_water_text = "h2o"
    new_water_road = f"{sx._healing_handle},{new_water_text}"
    sx.add_idea(walk=sx._healing_handle, idea_kid=IdeaKid(_label=new_water_text))
    ...
    new_rain_road = f"{sx._healing_handle},{new_water_text},{rain_text}"
...
def test_contract_edit_idea_label_ChangesIdeaRoot_range_source_road():
    ...
    old_healer_road = Road(f"{sx._healing_handle},{old_healer_text}")
    sx.add_idea(walk=sx._healing_handle, idea_kid=IdeaKid(_label=old_healer_text))
    sx.edit_idea_attr(road=sx._healing_handle, range_source_road=old_healer_road)
...
def test_contract_edit_idea_label_ChangesIdeaKidN_range_source_road():
    ...
    healer_text = "healer"
    healer_road = Road(f"{sx._healing_handle},{healer_text}")
    old_water_text = "water"
    old_water_road = f"{sx._healing_handle},{healer_text},{old_water_text}"
    rain_text = "rain"
    old_rain_road = f"{sx._healing_handle},{healer_text},{old_water_text},{rain_text}"
    mood_text = "mood"
    mood_road = Road(f"{sx._healing_handle},{mood_text}")
    sx.add_idea(walk=sx._healing_handle, idea_kid=IdeaKid(_label=healer_text))
    sx.add_idea(walk=healer_road, idea_kid=IdeaKid(_label=old_water_text))
    sx.add_idea(walk=old_water_road, idea_kid=IdeaKid(_label=rain_text))
    sx.add_idea(walk=sx._healing_handle, idea_kid=IdeaKid(_label=mood_text))
    ...
    # WHEN
    new_water_text = "h2o"
    new_rain_road = f"{sx._healing_handle},{healer_text},{new_water_text},{rain_text}"
    sx.edit_idea_label(old_road=old_water_road, new_label=new_water_text)
...
def test_contract_edit_idea_label_ChangesIdeaRequiredUnitsScenario1():
    ...
    old_weekday_road = f"{sx._healing_handle},{old_weekday_text}"
    wednesday_text = "Wednesday"
    old_wednesday_road = f"{sx._healing_handle},{old_weekday_text},{wednesday_text}"
    work_idea = sx.get_idea_kid(f"{sx._healing_handle},work")
    usa = f"{sx._healing_handle},nation-state,USA"
    nationstate = f"{sx._healing_handle},nation-state"
    ...
    new_weekday_road = f"{sx._healing_handle},{new_weekday_text}"
    new_wednesday_road = f"{sx._healing_handle},{new_weekday_text},{wednesday_text}"
