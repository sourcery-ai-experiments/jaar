...
def test_idea_label_fails_when_idea_does_not_exist():
    ...
    work_text = "work"
    work_road = f"{sx._goal_kind},{work_text}"
    swim_text = "swim"
    sx.add_idea(walk=sx._goal_kind, idea_kid=IdeaKid(_label=work_text))
    sx.add_idea(walk=work_road, idea_kid=IdeaKid(_label=swim_text))

    # When/Then
    no_idea_road = Road(f"{sx._goal_kind},bees")
...
def test_Contract_level0_idea_edit_idea_label_RaisesError_goal_kind_IsNone():
    # GIVEN
    owner_text = "Tim"
    sx = ContractUnit(_owner=owner_text)
    ...
    work_road = f"{sx._goal_kind},{work_text}"
    swim_text = "swim"
    swim_road = f"{sx._goal_kind},{work_text},{swim_text}"
    sx.add_idea(walk=sx._goal_kind, idea_kid=IdeaKid(_label=work_text))
    sx.add_idea(walk=work_road, idea_kid=IdeaKid(_label=swim_text))
    assert sx._owner == owner_text
    assert sx._goal_kind == sx._goal_kind
    assert sx._idearoot._label == sx._goal_kind
    work_idea = sx.get_idea_kid(road=work_road)
    assert work_idea._walk == sx._goal_kind
    ...
    with pytest_raises(Exception) as excinfo:
        moon_text = "moon"
        sx.edit_idea_label(old_road=sx._goal_kind, new_label=moon_text)
    assert (
        str(excinfo.value)
        == f"Cannot set idearoot to string other than '{sx._goal_kind}'"
    )
...
def test_Contract_level0_idea_edit_idea_label_RaisesError_goal_kind_IsDifferent():
    # GIVEN
    owner_text = "Tim"
    sx = ContractUnit(_owner=owner_text)
    work_text = "work"
    ...
    sx.add_idea(walk=work_road, idea_kid=IdeaKid(_label=swim_text))
    sun_text = "sun"
    sx._goal_kind = sun_text
    assert sx._owner == owner_text
    assert sx._goal_kind == sun_text
...
def test_contract_set_goal_kind_CorrectlySetsAttr():
    ...
    sx = ContractUnit(_owner=owner_text)
    work_text = "work"
    old_work_road = f"{sx._goal_kind},{work_text}"
    swim_text = "swim"
    ...
    # WHEN
    goal_kind_text = "Sun"
    sx.set_goal_kind(goal_kind=goal_kind_text)

    # THEN
    new_work_road = f"{goal_kind_text},{work_text}"
    swim_text = "swim"
    new_swim_road = f"{new_work_road},{swim_text}"
    assert sx._goal_kind == goal_kind_text
    assert sx._idearoot._label == goal_kind_text
    work_idea = sx.get_idea_kid(road=new_work_road)
    assert work_idea._walk == goal_kind_text
...
def test_idea_find_replace_road_Changes_kids_scenario1():
    ...
    old_owner_road = Road(f"{sx._goal_kind},{old_owner_text}")
    bloomers_text = "bloomers"
    old_bloomers_road = Road(f"{sx._goal_kind},{old_owner_text},{bloomers_text}")
    roses_text = "roses"
    old_roses_road = Road(
        f"{sx._goal_kind},{old_owner_text},{bloomers_text},{roses_text}"
    )
    red_text = "red"
    old_red_road = Road(
        f"{sx._goal_kind},{old_owner_text},{bloomers_text},{roses_text},{red_text}"
    )

    sx.add_idea(walk=sx._goal_kind, idea_kid=IdeaKid(_label=old_owner_text))
    ...
    new_owner_road = Road(f"{sx._goal_kind},{new_owner_text}")
    ...
    new_bloomers_road = Road(f"{sx._goal_kind},{new_owner_text},{bloomers_text}")
    assert r_idea_roses._walk == new_bloomers_road
    assert r_idea_roses._kids.get(red_text) != None
    r_idea_red = r_idea_roses._kids.get(red_text)
    new_roses_road = Road(
        f"{sx._goal_kind},{new_owner_text},{bloomers_text},{roses_text}"
    )
...
def test_contract_edit_idea_label_Changes_acptfactunits():
    ...
    owner = "owner"
    bloomers_text = "bloomers"
    bloomers_road = f"{sx._goal_kind},{owner},{bloomers_text}"
    roses_text = "roses"
    roses_road = f"{sx._goal_kind},{owner},{bloomers_text},{roses_text}"
    old_water_text = "water"
    old_water_road = f"{sx._goal_kind},{old_water_text}"
    rain_text = "rain"
    old_rain_road = f"{sx._goal_kind},{old_water_text},{rain_text}"

    sx.add_idea(walk=sx._goal_kind, idea_kid=IdeaKid(_label=owner))
    ...
    # WHEN
    new_water_text = "h2o"
    new_water_road = f"{sx._goal_kind},{new_water_text}"
    sx.add_idea(walk=sx._goal_kind, idea_kid=IdeaKid(_label=new_water_text))
    ...
    new_rain_road = f"{sx._goal_kind},{new_water_text},{rain_text}"
...
def test_contract_edit_idea_label_ChangesIdeaRoot_range_source_road():
    ...
    old_owner_road = Road(f"{sx._goal_kind},{old_owner_text}")
    sx.add_idea(walk=sx._goal_kind, idea_kid=IdeaKid(_label=old_owner_text))
    sx.edit_idea_attr(road=sx._goal_kind, range_source_road=old_owner_road)
...
def test_contract_edit_idea_label_ChangesIdeaKidN_range_source_road():
    ...
    owner_text = "owner"
    owner_road = Road(f"{sx._goal_kind},{owner_text}")
    old_water_text = "water"
    old_water_road = f"{sx._goal_kind},{owner_text},{old_water_text}"
    rain_text = "rain"
    old_rain_road = f"{sx._goal_kind},{owner_text},{old_water_text},{rain_text}"
    mood_text = "mood"
    mood_road = Road(f"{sx._goal_kind},{mood_text}")
    sx.add_idea(walk=sx._goal_kind, idea_kid=IdeaKid(_label=owner_text))
    sx.add_idea(walk=owner_road, idea_kid=IdeaKid(_label=old_water_text))
    sx.add_idea(walk=old_water_road, idea_kid=IdeaKid(_label=rain_text))
    sx.add_idea(walk=sx._goal_kind, idea_kid=IdeaKid(_label=mood_text))
    ...
    # WHEN
    new_water_text = "h2o"
    new_rain_road = f"{sx._goal_kind},{owner_text},{new_water_text},{rain_text}"
    sx.edit_idea_label(old_road=old_water_road, new_label=new_water_text)
...
def test_contract_edit_idea_label_ChangesIdeaRequiredUnitsScenario1():
    ...
    old_weekday_road = f"{sx._goal_kind},{old_weekday_text}"
    wednesday_text = "Wednesday"
    old_wednesday_road = f"{sx._goal_kind},{old_weekday_text},{wednesday_text}"
    work_idea = sx.get_idea_kid(f"{sx._goal_kind},work")
    usa = f"{sx._goal_kind},nation-state,USA"
    nationstate = f"{sx._goal_kind},nation-state"
    ...
    new_weekday_road = f"{sx._goal_kind},{new_weekday_text}"
    new_wednesday_road = f"{sx._goal_kind},{new_weekday_text},{wednesday_text}"
