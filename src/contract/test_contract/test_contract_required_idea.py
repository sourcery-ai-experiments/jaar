...
def test_contract_requiredunits_create():
    ...
    weekday_road = f"{cx._heal_kind},{weekday_text}"
    wed_text = "Wednesday"
...
def test_contract_requiredheirs_AreCorrectlyInherited_v1():
    ...
    work_road = f"{cx._heal_kind},{work_text}"
    week_label = "weekdays"
    week_road = f"{cx._heal_kind},{week_label}"
    wed_text = "Wednesday"
...
def test_contract_requiredheirs_AreCorrectlyInheritedTo4LevelsFromRoot():
    ...
    work_text = "work"
    work_road = f"{a4._heal_kind},{work_text}"
    week_text = "weekdays"
    week_road = f"{a4._heal_kind},{week_text}"
    wed_text = "Wednesday"
    wed_road = f"{week_road},{wed_text}"
...
def test_contract_requiredheirs_AreCorrectlyInheritedTo4LevelsFromLevel2():
    ...
    work_road = f"{a4._heal_kind},{work_text}"
    week_label = "weekdays"
    week_road = f"{a4._heal_kind},{week_label}"
    wed_text = "Wednesday"
    wed_road = f"{week_road},{wed_text}"
...
def test_contract_requiredunits_set_sufffactIdeaWithDenomSetsSuffFactDivision():
    ...
    week_text = "week"
    week_road = f"{cx._heal_kind},{time_text},{week_text}"
    cx.add_idea(
...
def test_contract_requiredunits_set_sufffactIdeaWithBeginCloseSetsSuffFactOpenNigh():
    ...
    time = "time"
    time_road = f"{cx._heal_kind},{time}"
    rus_war = "rus_war"
    rus_war_road = f"{cx._heal_kind},{time},{rus_war}"
    cx.add_idea(
        idea_kid=IdeaKid(_label=time, _begin=100, _close=2000), walk=cx._heal_kind
    )
...
def test_contract_requiredunits_del_required_sufffact_UncoupledMethod1():
    ...
    cx = example_contracts_get_contract_with_4_levels()
    work_road = f"{cx._heal_kind},work"
    weekday_road = f"{cx._heal_kind},weekdays"
    wed_road = f"{cx._heal_kind},weekdays,Wednesday"

    cx.edit_idea_attr(
        road=work_road, required_base=weekday_road, required_sufffact=wed_road
    )
    thu_road = f"{cx._heal_kind},weekdays,Thursday"
    cx.edit_idea_attr(
        road=work_road,
...
def test_contract_requiredunits_del_required_sufffact_UncoupledMethod2():
    ...
    cx = example_contracts_get_contract_with_4_levels()
    work_road = f"{cx._heal_kind},work"
    weekdays_road = f"{cx._heal_kind},weekdays"
    work_idea1 = cx.get_idea_kid(road=work_road)
...
def test_contract_edit_idea_attr_contractIsAbleToEdit_suff_idea_active_status_AnyIdeaIfInvaildThrowsError():
    ...
    commute_road = f"{cx._heal_kind},{commute_text}"
    cx.add_idea(idea_kid=IdeaKid(_label=commute_text), walk=cx._heal_kind)
