...
def test_contract_idearoot_meld_IdeaRootAttrCorrectlyMelded():
    ...
    assert (
        str(excinfo.value)
        == f"Meld fail idea=None,{cx1._goal_kind} _uid:1 with None,{cx2._goal_kind} _uid:4"
    )
...
def test_contract_idearoot_meld_Add4IdeasScenario():
    ...
    tech_text = "tech"
    tech_road = f"{cx1._goal_kind},{tech_text}"
    bowl_text = "bowl"
    bowl_road = f"{cx1._goal_kind},{tech_text},{bowl_text}"
    swim_text = "swim"
    swim_road = f"{cx1._goal_kind},{swim_text}"
    free_text = "freestyle"
    free_road = f"{cx1._goal_kind},{swim_text},{free_text}"

    cx2 = ContractUnit(_owner=spirit_text)
    cx2.add_idea(walk=cx2._goal_kind, idea_kid=IdeaKid(_label=tech_text))
...
def test_contract_idearoot_meld_2SameIdeasScenario():
    ...
    cx1.add_idea(walk=cx1._goal_kind, idea_kid=IdeaKid(_label=tech_text))
...
def test_contract_acptfactunits_meld_AcptFactsAttributeCorrectlySet():
    ...
    swim_road = f"{cx1._goal_kind},{swim_text}"
    free_text = "freestyle"
    free_road = f"{cx1._goal_kind},{free_text}"
    cx1.add_idea(walk=swim_road, idea_kid=IdeaKid(_label=free_text))
...
def test_contract_meld_OriginUnitsCorrectlySet():
    ...
    swim_text = "swim"
    swim_road = f"{bob_cx._goal_kind},{swim_text}"
    free_text = "freestyle"
