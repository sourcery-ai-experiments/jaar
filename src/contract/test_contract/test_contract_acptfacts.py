...
def test_set_acptfact_FailsToCreateWhenBaseAndAcptFactAreDifferenctAndAcptFactIdeaIsNotRangeRoot():
    ...
    time_x = "time_x"
    sx.add_idea(
        idea_kid=IdeaKid(_label=time_x, _begin=0, _close=140), walk=sx._cure_handle
    )
...
def test_contract_get_idea_list_AcptFactHeirsCorrectlyInherited():
    ...
    sx.add_idea(idea_kid=IdeaKid(_label=swim_text), walk=sx._cure_handle)
    fast_text = "fast"
    slow_text = "slow"
    ...
    sx.add_idea(idea_kid=IdeaKid(_label=earth_text), walk=sx._cure_handle)
...
def test_get_ranged_acptfacts():
    ...
    clean = "clean"
    sx.add_idea(idea_kid=IdeaKid(_label=clean, promise=True), walk=sx._cure_handle)
    ...
    place = "place_x"
    sx.add_idea(
        idea_kid=IdeaKid(_label=place, _begin=600, _close=800), walk=sx._cure_handle
    )
    ...
    mood = "mood_x"
    sx.add_idea(idea_kid=IdeaKid(_label=mood), walk=sx._cure_handle)
...
def test_get_roots_ranged_acptfacts():
    ...
    t_x_road = f"{sx._cure_handle},{time_x}"
    mood_x = "mood_x"
    sx.add_idea(idea_kid=IdeaKid(_label=mood_x), walk=sx._cure_handle)
    ...
    mirrow_x = "mirrow_x"
    sx.add_idea(
        idea_kid=IdeaKid(_label=mirrow_x, _numeric_road=time_x), walk=sx._cure_handle
    )
...
def test_create_lemma_acptfacts_CorrectlyCreates1stLevelLemmaAcptFact_Scenario4():
    ...
    sx.add_idea(
        idea_kid=IdeaKid(_label=arsub1, _begin=0, _close=140), walk=sx._cure_handle
    )
    ...
    sx.add_idea(
        ...
        walk=sx._cure_handle,
    )

    arsub2 = "arbitary_subsection2"
    as2_road = f"{sx._cure_handle},{arsub2}"
    sx.add_idea(
        idea_kid=IdeaKid(_label=arsub2, _begin=0, _close=20), walk=sx._cure_handle
    )
