...
def test_contract_acptfact_exists():
    contract_x = examples_get_contract_with_4_levels()
    ...
    contract_x._idearoot._acptfactunits = {sunday_cx_acptfact.base: sunday_cx_acptfact}
    assert contract_x._idearoot._acptfactunits != None
    contract_x._idearoot._acptfactunits = None
    assert contract_x._idearoot._acptfactunits is None
    contract_x.set_acptfact(base=weekday_road, pick=sunday_road)
    assert contract_x._idearoot._acptfactunits == {
    ...
    contract_x._idearoot._acptfactunits = {usa_week_unit.base: usa_week_unit}

    contract_x._idearoot._acptfactunits = None
    assert contract_x._idearoot._acptfactunits is None
    contract_x.set_acptfact(base=usa_week_road, pick=usa_week_road, open=608, nigh=610)
    assert contract_x._idearoot._acptfactunits != None
    assert contract_x._idearoot._acptfactunits == {usa_week_unit.base: usa_week_unit}
...
def test_contract_acptfact_create():
    ...
    # When
    contract_x.del_acptfact(base=weekday_road)
...
def test_contract_get_idea_list_AcptFactHeirsCorrectlyInherited():
    ...
    contract_x.add_idea(idea_kid=IdeaKid(_label=swim_text), walk=root_label())
    fast_text = "fast"
    slow_text = "slow"
    fast_road = Road(f"{swim_road},{fast_text}")
    slow_road = Road(f"{swim_road},{slow_text}")
    contract_x.add_idea(idea_kid=IdeaKid(_label=fast_text), walk=swim_road)
    contract_x.add_idea(idea_kid=IdeaKid(_label=slow_text), walk=swim_road)

    earth_text = "earth"
    earth_road = Road(f"{root_label()},{earth_text}")
    contract_x.add_idea(idea_kid=IdeaKid(_label=earth_text), walk=root_label())

    swim_idea = contract_x.get_idea_kid(road=swim_road)
    fast_idea = contract_x.get_idea_kid(road=fast_road)
    slow_idea = contract_x.get_idea_kid(road=slow_road)
    ...
    # WHEN
    contract_x.set_acptfact(base=earth_road, pick=earth_road, open=1.0, nigh=5.0)
...
def test_contract_get_idea_list_AcptFactUnitCorrectlyTransformsacptfactheir_shop():
    ...
    contract_x.set_acptfact(base=earth_road, pick=earth_road, open=3.0, nigh=5.0)
...
def test_contract_get_idea_list_AcptFactHeirCorrectlyDeletesAcptFactUnit():
    ...
    # WHEN
    contract_x.set_acptfact(base=earth_road, pick=earth_road, open=200.0, nigh=500.0)
