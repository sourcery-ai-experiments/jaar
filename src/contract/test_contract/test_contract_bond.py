...
#     cx = ContractUnit(_healer=jessi_text)
#     casa_text = "case"
#     casa_road = Road(f"{cx._healing_handle},{casa_text}")
#     cx.add_idea(idea_kid=IdeaKid(_label=casa_text), walk=jessi_text)
#     clean_cookery_text = "clean cookery"
#     clean_cookery_road = Road(f"{cx._healing_handle},{casa_text},{clean_cookery_text}")
...
#     # WHEN/THEN
#     water_text = "water"
#     water_road = Road(f"{cx._healing_handle},{water_text}")
#     cx.add_idea(idea_kid=IdeaKid(_label=water_text), walk=jessi_text)
#     assert cx.get_bond_status() == False

#     rain_text = "rain"
#     rain_road = Road(f"{cx._healing_handle},{water_text},{rain_text}")
...
def test_contractunit_get_contract_sprung_from_single_idea_ReturnsCorrectContractScenario1():
    ...
    water_text = "water"
    water_road = Road(f"{cx._healing_handle},{water_text}")
    cx.add_idea(idea_kid=IdeaKid(_label=water_text), walk=cx._healing_handle)
    assert cx.get_bond_status() == False
    ...
    b_src_idea = bond_contract.get_idea_kid(road=cx._healing_handle)
    source_x_idea = cx.get_idea_kid(road=cx._healing_handle)
...
def test_contractunit_get_meld_of_contract_files_MeldsIntoSourceContract_Scenario1(
    ...
    primary_cx.add_idea(idea_kid=idea_kid_work, walk=f"{primary_cx._healing_handle}")

    cat = "feed cat"
    idea_kid_feedcat = IdeaKid(_weight=20, _label=cat, promise=True)
    primary_cx.add_idea(idea_kid=idea_kid_feedcat, walk=f"{primary_cx._healing_handle}")
