...
def test_contract_get_idea_list_SetsSatiateStatusCorrectlyWhenAcptFactSaysNo():
    ...
    cx = get_contract_with_4_levels_and_2requireds()
    week_text = "weekdays"
    week_road = f"{cx._goal_tag},{week_text}"
    sun_text = "Sunday"
...
def test_contract_get_idea_list_SetsSatiateStatusCorrectlyWhenAcptFactChanges():
    ...
    states_road = f"{cx._goal_tag},{states_text}"
    usa_text = "USA"
...
def test_contract_get_idea_list_returns_correct_list():
    ...
    state_road = f"{cx._goal_tag},{state_text}"
    france_text = "France"
    ...
    assert work_idea._walk == f"{cx._goal_tag}"
...
def test_contract_get_idea_list_CorrectlyCalculatesIdeaAttr_contract_coin():
    ...
    cx.add_idea(idea_kid=auto_idea, walk=cx._goal_tag)

    barn_text = "barn"
    barn_road = f"{cx._goal_tag},{barn_text}"
    barn_idea = IdeaKid(_label=barn_text, _weight=60)
    cx.add_idea(idea_kid=barn_idea, walk=cx._goal_tag)
    lamb_text = "lambs"
    ...
    cx.add_idea(idea_kid=coal_idea, walk=cx._goal_tag)
...
def test_contract_get_idea_list_CorrectlyCalculatesRangeAttributes():
    ...
    house_road = f"{cx._goal_tag},{house_text}"
...
def test_exammple_idea_list_HasCorrectData():
    ...
    laundry_road = f"{cx._goal_tag},casa,cleaning,{laundry_text}"
...
def test_exammple_idea_list_OptionWeekdaysCorrectlyWork():
    ...
    day_hr_road = f"{cx._goal_tag},{day_hr_text}"
    ...
    casa_text = "casa"
    casa_road = f"{cx._goal_tag},{casa_text}"
    ...
    # cx.set_acptfact(base=f"{cx._goal_tag},weekdays", pick=f"{cx._goal_tag},weekdays,Tuesday")
    # idea_list = cx.get_idea_list()
    # assert YR.get_active_status(road=bird_idea, idea_list=idea_list) == True

    # cx.set_acptfact(base=f"{cx._goal_tag},weekdays", pick=f"{cx._goal_tag},weekdays,Wednesday")
    # idea_list = cx.get_idea_list()
...
def test_exammple_idea_list_Every6WeeksRequired():
    ...
    cx = contract_v001()
    day_text = "day_hour"
    day_road = f"{cx._goal_tag},{day_text}"
    min_text = "day_minute"
    min_road = f"{cx._goal_tag},{day_text}"
    ...
    # THEN
    ced_week_base = f"{cx._goal_tag},ced_week"
    ...
    clean_sheet_road = f"{cx._goal_tag},casa,cleaning,clean sheets couch blankets"
    ...
    clean_couch_road = f"{cx._goal_tag},casa,cleaning,{clean_couch_text}"
...
def test_exammple_idea_list_EveryOtherMonthWorks():
    ...
    internet_road = f"{cx._goal_tag},{internet_text}"
    cx.set_acptfact(base=internet_road, pick=internet_road)
    weekdays_text = "weekdays"
    weekdays_road = f"{cx._goal_tag},{weekdays_text}"
