...
def test_contract_yoke_dict_isDictionaryAndIsFullyPopulated():
    # GIVEN
    contract_x = get_contract_with_4_levels_and_2requireds()

    # WHEN
    contract_x.set_contract_metrics()

    # THEN
    assert len(contract_x._idea_dict) == 17
...
def test_contract_get_idea_list_SetsSatiateStatusCorrectlyWhenAcptFactSaysNo():
    ...
    # WHEN
    contract_x.set_acptfact(base=week_road, pick=sun_road)
    ...
    # for idea in contract_x._idea_dict.values():
    #     print(f"{work_road=} {idea.get_road()=}")
    work_text = "work"
    work_road = f"{root_label()},{work_text}"
    assert contract_x._idea_dict.get(work_road)._active_status == False
...
def test_contract_get_idea_list_SetsSatiateStatusCorrectlyWhenAcptFactChanges():
    ...
    contract_x.set_acptfact(base=states_road, pick=usa_road)

    # THEN
    idea_list = contract_x.get_idea_list()
    assert idea_list
    assert len(idea_list) == 17
    assert contract_x._idea_dict.get(work_road)._active_status

    # WHEN
    france_text = "France"
    france_road = f"{states_road},{france_text}"
    contract_x.set_acptfact(base=states_road, pick=france_road)
...
def test_contract_get_idea_list_returns_correct_list():
    ...
    state_road = f"{root_label()},{state_text}"
    france_text = "France"
    france_road = f"{state_road},{france_text}"
    contract_x.set_acptfact(base=week_road, pick=wed_road)
    contract_x.set_acptfact(base=state_road, pick=france_road)

    work_text = "work"
    work_road = f"{root_label()},{work_text}"
    work_idea = contract_x.get_idea_kid(work_road)
    print(f"{contract_x._owner=} {len(work_idea._requiredunits)=}")
    # print(f"{work_idea._requiredunits=}")
    print(f"{contract_x._owner=} {len(contract_x._idearoot._acptfactunits)=}")
    # print(f"{contract_x._idearoot._acptfactunits=}")
    ...
    # WHEN
    contract_x.set_acptfact(base=state_road, pick=oregon_road)

    # THEN
    work_idea = contract_x._idea_dict.get(work_road)
    print(f"\nlook at {work_idea.get_road()=}")
    assert work_idea._walk == f"{root_label()}"
...
def test_contract_get_idea_list_without_root_CorrectlyCalculatesIdeaAttributes():
    # GIVEN
    contract_x = get_contract_with7amCleanTableRequired()

    # WHEN
    idea_list_without_idearoot = contract_x.get_idea_list_without_idearoot()
    idea_list_with_idearoot = contract_x.get_idea_list()

    # THEN
    assert len(idea_list_without_idearoot) == 28
    assert len(idea_list_without_idearoot) + 1 == len(idea_list_with_idearoot)

    # for idea in contract_x.get_idea_list_without_idearoot():
...
def test_contract_get_idea_list_CorrectlyCalculatesRangeAttributes():
    ...
    assert contract_x._idea_dict.get(clean_road)._active_status == False
    ...
    # WHEN
    contract_x.set_acptfact(
        base=day24hr_base, pick=day24hr_pick, open=day24hr_open, nigh=day24hr_nigh
    )

    # THEN
    contract_x.set_contract_metrics()
    assert contract_x._idea_dict.get(clean_road)._active_status

    # WHEN
    # set acptfacts as 8am to 10am
    day24hr_open = 8.0
    day24hr_nigh = 10.0
    print(contract_x._idearoot._acptfactunits[day24hr_road])
    contract_x.set_acptfact(
        base=day24hr_base, pick=day24hr_pick, open=day24hr_open, nigh=day24hr_nigh
    )
    print(contract_x._idearoot._acptfactunits[day24hr_road])
    print(contract_x._idearoot._kids[house_text]._kids[clean_text]._requiredunits)
    # contract_x._idearoot._kids["housework"]._kids[clean_text]._active_status = None
...
def test_get_agenda_items():
    ...
    # WHEN
    promise_items = contract_x.get_agenda_items()
...
def test_exammple_idea_list_HasCorrectData():
    contract_x = contract_v001()
    print(f"{contract_x.get_required_bases()=}")
    # day_hour = f"{root_label()},day_hour"
    # contract_x.set_acptfact(base=day_hour, pick=day_hour, open=0, nigh=23)
    day_min_text = "day_minute"
    day_min_road = f"{root_label()},{day_min_text}"
    contract_x.set_acptfact(base=day_min_road, pick=day_min_road, open=0, nigh=1439)

    mood_text = "Moods"
    mood_road = f"{root_label()},{mood_text}"
    contract_x.set_acptfact(base=mood_road, pick=mood_road)
    print(f"{contract_x.get_required_bases()=}")

    yr_mon_text = "year_month"
    yr_mon_road = f"{root_label()},{yr_mon_text}"
    contract_x.set_acptfact(base=yr_mon_road, pick=yr_mon_road)
    inter_text = "Internet"
    inter_road = f"{root_label()},{inter_text}"
    contract_x.set_acptfact(base=inter_road, pick=inter_road)
    assert contract_x != None
    # print(f"{contract_x._owner=}")
    # print(f"{len(contract_x._idearoot._kids)=}")
    ulty_text = "Ultimate Frisbee"
    ulty_road = f"{root_label()},{ulty_text}"

    # if contract_x._idearoot._kids["Ultimate Frisbee"]._label == "Ultimate Frisbee":
    assert contract_x._idearoot._kids[ulty_text]._requiredunits != None
    assert contract_x._owner != None

    # for acptfact in contract_x._idearoot._acptfactunits.values():
    #     print(f"{acptfact=}")
    ...
    laundry_road = f"{root_label()},casa,cleaning,{laundry_text}"
    ...
    assert contract_x._idea_dict.get(laundry_road)._active_status == False
    ...
    contract_x.set_acptfact(base=week_road, pick=mon_road)
...
def test_exammple_idea_list_OptionWeekdaysCorrectlyWork():
    ...
    day_hr_road = f"{root_label()},{day_hr_text}"
    contract_x.set_acptfact(base=day_hr_road, pick=day_hr_road, open=0, nigh=23)
    day_min_text = "day_minute"
    day_min_road = f"{root_label()},{day_min_text}"
    contract_x.set_acptfact(base=day_min_road, pick=day_min_road, open=0, nigh=59)
    mon_wk_text = "month_week"
    mon_wk_road = f"{root_label()},{mon_wk_text}"
    contract_x.set_acptfact(base=mon_wk_road, pick=mon_wk_road)
    nation_text = "Nation-States"
    nation_road = f"{root_label()},{nation_text}"
    contract_x.set_acptfact(base=nation_road, pick=nation_road)
    mood_text = "Moods"
    mood_road = f"{root_label()},{mood_text}"
    contract_x.set_acptfact(base=mood_road, pick=mood_road)
    aaron_text = "Aaron Donald sphere"
    aaron_road = f"{root_label()},{aaron_text}"
    contract_x.set_acptfact(base=aaron_road, pick=aaron_road)
    inter_text = "Internet"
    inter_road = f"{root_label()},{inter_text}"
    contract_x.set_acptfact(base=inter_road, pick=inter_road)
    yr_mon_text = "year_month"
    yr_mon_road = f"{root_label()},{yr_mon_text}"
    contract_x.set_acptfact(base=yr_mon_road, pick=yr_mon_road, open=0, nigh=1000)

    idea_list = contract_x.get_idea_list()
    missing_acptfacts = contract_x.get_missing_acptfact_bases()
    ...
    contract_x._idearoot.set_required_unit(required=mt_required)
    # print(f"{contract_x._requiredunits[week_road].base=}")
    # print(f"{contract_x._requiredunits[week_road].sufffacts[mon_road].need=}")
    # print(f"{contract_x._requiredunits[week_road].sufffacts[tue_road].need=}")
    print(f"{contract_x._idearoot._requiredunits[week_road].sufffacts=}")
    sufffact_mon = contract_x._idearoot._requiredunits[week_road].sufffacts.get(
        mon_road
    )
    sufffact_tue = contract_x._idearoot._requiredunits[week_road].sufffacts.get(
        tue_road
    )
    assert sufffact_mon
    ...
    assert contract_x._idearoot._requiredunits[week_road] == mt_required
    ...
    assert contract_x._idearoot._requiredheirs[week_road] == mt_required_x
    ...
    contract_x.set_acptfact(base=week_road, pick=mon_road)
    idea_list = contract_x.get_idea_list()
    casa_idea = contract_x._idearoot._kids[casa_text]
    twee_idea = casa_idea._kids[bird_text]
    print(f"{len(contract_x._idearoot._requiredheirs)=}")
    ...
    # contract_x.set_acptfact(base=f"{root_label()},weekdays", pick=f"{root_label()},weekdays,Tuesday")
    # idea_list = contract_x.get_idea_list()
    # assert YR.get_active_status(road=bird_idea, idea_list=idea_list) == True

    # contract_x.set_acptfact(base=f"{root_label()},weekdays", pick=f"{root_label()},weekdays,Wednesday")
    # idea_list = contract_x.get_idea_list()
...
def test_exammple_idea_list_Every6WeeksRequired():
    ...
    min_text = "day_minute"
    min_road = f"{root_label()},{day_text}"

    # WHEN
    contract_x.set_acptfact(base=day_road, pick=day_road, open=0, nigh=23)
    contract_x.set_acptfact(base=min_road, pick=min_road, open=0, nigh=59)
    idea_list = contract_x.get_idea_list()

    # THEN
    ced_week_base = f"{root_label()},ced_week"
    ...
    clean_sheet_road = f"{root_label()},casa,cleaning,clean sheets couch blankets"
    clean_sheet_idea = contract_x.get_idea_kid(road=clean_sheet_road)
    ...
    clean_couch_road = f"{root_label()},casa,cleaning,{clean_couch_text}"
    clean_couch_idea = contract_x.get_idea_kid(road=clean_couch_road)
...
def test_exammple_idea_list_EveryOtherMonthWorks():
    ...
    minute_road = f"{root_label()},{minute_text}"
    contract_x.set_acptfact(base=minute_road, pick=minute_road, open=0, nigh=1399)
    month_text = "month_week"
    month_road = f"{root_label()},{month_text}"
    contract_x.set_acptfact(base=month_road, pick=month_road)
    nations_text = "Nation-States"
    nations_road = f"{root_label()},{nations_text}"
    contract_x.set_acptfact(base=nations_road, pick=nations_road)
    mood_text = "Moods"
    mood_road = f"{root_label()},{mood_text}"
    contract_x.set_acptfact(base=mood_road, pick=mood_road)
    aaron_text = "Aaron Donald sphere"
    ...
    contract_x.set_acptfact(base=internet_road, pick=internet_road)
    weekdays_text = "weekdays"
    weekdays_road = f"{root_label()},{weekdays_text}"
    contract_x.set_acptfact(base=weekdays_road, pick=weekdays_road)
    idea_list = contract_x.get_idea_list()
    print(f"{len(idea_list)=}")
    ...
    year_month_base = f"{root_label()},year_month"
    print(f"{year_month_base=}, {year_month_base=}")

    # WHEN
    contract_x.set_acptfact(base=year_month_base, pick=year_month_base, open=0, nigh=8)
    ced_week = f"{root_label()},ced_week"
    contract_x.set_acptfact(base=ced_week, pick=ced_week, open=0, nigh=4)

    # THEN
    idea_list = contract_x.get_idea_list()
    print(f"{len(idea_list)=}")
    print(f"{len(contract_x._idearoot._acptfactunits)=}")
