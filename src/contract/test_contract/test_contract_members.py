from src.contract.member import MemberName, memberlink_shop, memberunit_shop
...
from src.contract.contract import ContractUnit, get_intersection_of_members
from src.contract.idea import IdeaKid
from pytest import raises as pytest_raises
from src.economy.bank_sqlstr import RiverTmemberUnit


def test_contract_members_exists():
    # GIVEN / WHEN
    cx = ContractUnit()
...
def test_contract_set_member_correctly_sets_members_1():
    # GIVEN
...
def test_contract_set_member_correctly_sets_members_2():
    # GIVEN
    cx = ContractUnit(_owner="prom")
    rico_text = "rico"
    carm_text = "carmen"
...
def test_contract_get_member_CorrectlyGetsMember():
    # GIVEN
    cx = ContractUnit(_owner="prom")
    rico_text = "rico"
    carm_text = "carmen"
...
def test_contract_get_members_depotlink_count_GetsCorrectCount():
    # GIVEN
    sue_text = "sue"
...
def test_contract_get_idea_list_CorrectlySetsMemberLinkContractCreditAndDebt():
    # GIVEN
    prom_text = "prom"
    ...
    rico_memberlink._contract_credit is None
    rico_memberlink._contract_debt is None
    carm_memberlink._contract_credit is None
    carm_memberlink._contract_debt is None
    patr_memberlink._contract_credit is None
    patr_memberlink._contract_debt is None

    # for group in cx._groups.values():
    #     for memberlink in group._members.values():
    #         assert memberlink._contract_credit is None
    #         assert memberlink._contract_debt is None
    ...
    assert rico_memberlink._contract_credit == 0.5
    assert rico_memberlink._contract_debt == 0.8
    assert carm_memberlink._contract_credit == 0.25
    assert carm_memberlink._contract_debt == 0.1
    assert patr_memberlink._contract_credit == 0.25
    assert patr_memberlink._contract_debt == 0.1

    # memberlink_contract_credit_sum = 0.0
    # memberlink_contract_debt_sum = 0.0
    # for group in cx._groups.values():
    #     # print(f"{group.name=} {group._members=}")

    #     for memberlink in group._members.values():
    #         assert memberlink._contract_credit != None
    #         assert memberlink._contract_credit in [0.25, 0.5]
    #         assert memberlink._contract_debt != None
    #         assert memberlink._contract_debt in [0.8, 0.1]
    #         # print(
    #         #     f"{group.name=} {memberlink._contract_importance=} {group._contract_importance=}"
    #         # )
    #         memberlink_contract_credit_sum += memberlink._contract_credit
    #         memberlink_contract_debt_sum += memberlink._contract_debt

    #         # print(f"{memberlink_contract_importance_sum=}")
    # assert memberlink_contract_credit_sum == 1.0
    # assert memberlink_contract_debt_sum == 1.0

    assert (
        rico_memberlink._contract_credit
        + carm_memberlink._contract_credit
        + patr_memberlink._contract_credit
        == 1.0
    )
    assert (
        rico_memberlink._contract_debt
        + carm_memberlink._contract_debt
        + patr_memberlink._contract_debt
        == 1.0
    )
    ...
    assert rico_memberlink._contract_credit != 0.25
    assert rico_memberlink._contract_debt != 0.8
    assert carm_memberlink._contract_credit != 0.25
    assert carm_memberlink._contract_debt != 0.1
    assert patr_memberlink._contract_credit != 0.5
    assert patr_memberlink._contract_debt != 0.1
    assert selena_memberlink._contract_credit != None
    assert selena_memberlink._contract_debt != None
    ...
    #     for memberlink in group._members.values():
    #         assert memberlink._contract_credit != None
    #         assert memberlink._contract_credit not in [0.25, 0.5]
    #         assert memberlink._contract_debt != None
    #         assert memberlink._contract_debt not in [0.8, 0.1]
    #         # print(
    #         #     f"{group.name=} {memberlink._contract_importance=} {group._contract_importance=}"
    #         # )
    #         memberlink_contract_credit_sum += memberlink._contract_credit
    ...
    #         # print(f"{memberlink_contract_importance_sum=}")
    # assert memberlink_contract_credit_sum == 1.0
    # assert memberlink_contract_debt_sum > 0.9999999
    # assert memberlink_contract_debt_sum < 1.00000001

    assert (
        rico_memberlink._contract_credit
        + carm_memberlink._contract_credit
        + patr_memberlink._contract_credit
        + selena_memberlink._contract_credit
        == 1.0
    )
    assert (
        rico_memberlink._contract_debt
        + carm_memberlink._contract_debt
        + patr_memberlink._contract_debt
        + selena_memberlink._contract_debt
        > 0.9999999
    )
    assert (
        rico_memberlink._contract_debt
...
def test_contract_get_idea_list_CorrectlySetsMemberUnitContractImportance():
    # GIVEN
    prom_text = "prom"
    cx = ContractUnit(_owner=prom_text)
    swim_text = "swim"
    ...
    assert rico_memberunit._contract_credit is None
    assert rico_memberunit._contract_debt is None
    assert carm_memberunit._contract_credit is None
    assert carm_memberunit._contract_debt is None
    assert patr_memberunit._contract_credit is None
    assert patr_memberunit._contract_debt is None

    # WHEN
    cx.set_contract_metrics()

    # THEN
    memberunit_contract_credit_sum = 0.0
    memberunit_contract_debt_sum = 0.0

    assert rico_memberunit._contract_credit == 0.5
    assert rico_memberunit._contract_debt == 0.8
    assert carm_memberunit._contract_credit == 0.25
    assert carm_memberunit._contract_debt == 0.1
    assert patr_memberunit._contract_credit == 0.25
    assert patr_memberunit._contract_debt == 0.1

    assert (
        rico_memberunit._contract_credit
        + carm_memberunit._contract_credit
        + patr_memberunit._contract_credit
        == 1.0
    )
    assert (
        rico_memberunit._contract_debt
        + carm_memberunit._contract_debt
        + patr_memberunit._contract_debt
        == 1.0
    )

    # for memberunit in cx._members.values():
    #     assert memberunit._contract_credit != None
    #     assert memberunit._contract_credit in [0.25, 0.5]
    #     assert memberunit._contract_debt != None
    #     assert memberunit._contract_debt in [0.8, 0.1]
    #     # print(
    #     #     f"{group.name=} {memberunit._contract_creditor=} {group._contract_creditor=}"
    #     # )
    #     print(f"{memberunit.name=} {memberunit._contract_credit=} {memberunit._contract_debt=} ")
    #     # print(f"{memberunit_contract_credit_sum=}")
    #     # print(f"{memberunit_contract_debt_sum=}")
    #     memberunit_contract_credit_sum += memberunit._contract_credit
    #     memberunit_contract_debt_sum += memberunit._contract_debt

    # assert memberunit_contract_credit_sum == 1.0
    # assert memberunit_contract_debt_sum > 0.9999999
    # assert memberunit_contract_debt_sum < 1.00000001
    ...
    assert patr_memberunit._contract_credit != 0.25
    assert patr_memberunit._contract_debt != 0.1
    assert selena_memberunit._contract_credit != None
    assert selena_memberunit._contract_debt != None
    ...
    assert (
        ...
        + carm_memberunit._contract_credit
        + patr_memberunit._contract_credit
        + selena_memberunit._contract_credit
        == 1.0
    )
    assert (
        rico_memberunit._contract_debt
    ...
    assert (
        ...
        + carm_memberunit._contract_debt
        + patr_memberunit._contract_debt
        + selena_memberunit._contract_debt
        == 1.0
    )
    ...
    # for memberunit in cx._members.values():
    #     assert memberunit._contract_credit != None
    #     assert memberunit._contract_credit not in [0.25, 0.5]
    #     assert memberunit._contract_debt != None
    #     assert memberunit._contract_debt not in [0.8, 0.1]
    #     # print(
    #     #     f"{group.name=} {memberunit._contract_creditor=} {group._contract_creditor=}"
    #     # )
    #     print(f"{memberunit.name=} {memberunit._contract_credit=} {memberunit._contract_debt=} ")
...
def test_contract_get_idea_list_CorrectlySetsPartGroupedLWMemberUnitContractImportance():
    # GIVEN
    prom_text = "prom"
    cx = ContractUnit(_owner=prom_text)
    swim_text = "swim"
    ...
    assert rico_memberunit._contract_credit == 0.375
    assert rico_memberunit._contract_debt == 0.45
    assert carm_memberunit._contract_credit == 0.3125
    assert carm_memberunit._contract_debt == 0.275
    assert patr_memberunit._contract_credit == 0.3125
    assert patr_memberunit._contract_debt == 0.275
...
def test_contract_get_idea_list_WithAllMembersWeighted():
    # GIVEN
    cx = ContractUnit(_owner="prom")
...
def clear_all_memberunits_groupunits_contract_agenda_credit_debt(cx: ContractUnit):
    ...
    for groupunit_x in cx._groups.values():
        ...
        #     print(f"{groupunit_x.name=} {memberlink_x.creditor_weight=}  {memberlink_x._contract_credit:.6f} {memberlink_x.debtor_weight=} {memberlink_x._contract_debt:.6f} {memberlink_x.name=} ")

    # DELETE contract_agenda_debt and contract_agenda_credit
    for memberunit_x in cx._members.values():
        memberunit_x.reset_contract_credit_debt()
...
def test_contract_agenda_credit_debt_IsCorrectlySet():
    ...
    clear_all_memberunits_groupunits_contract_agenda_credit_debt(cx=cx)
    ...
    sum_memberlink_contract_agenda_credit = 0
    sum_memberlink_contract_agenda_debt = 0
    for groupunit_x in cx._groups.values():
        ...
        for memberlink_x in groupunit_x._members.values():
            sum_memberlink_contract_agenda_credit += (
                memberlink_x._contract_agenda_credit
            )
            sum_memberlink_contract_agenda_debt += memberlink_x._contract_agenda_debt

    assert sum_groupunit_contract_agenda_credit == 0
    assert sum_groupunit_contract_agenda_debt == 0
    assert sum_memberlink_contract_agenda_credit == 0
    assert sum_memberlink_contract_agenda_debt == 0

    # TEST contract_agenda_debt and contract_agenda_credit are empty
    sum_memberunit_contract_agenda_credit = 0
    sum_memberunit_contract_agenda_debt = 0
    sum_memberunit_contract_agenda_ratio_credit = 0
    sum_memberunit_contract_agenda_ratio_debt = 0
    for memberunit in cx._members.values():
        # print(f"{memberunit.name=}")
        sum_memberunit_contract_agenda_credit += memberunit._contract_agenda_credit
        sum_memberunit_contract_agenda_debt += memberunit._contract_agenda_debt
        sum_memberunit_contract_agenda_ratio_credit += (
            memberunit._contract_agenda_ratio_credit
        )
        sum_memberunit_contract_agenda_ratio_debt += (
            memberunit._contract_agenda_ratio_debt
        )

    assert sum_memberunit_contract_agenda_credit == 0
    assert sum_memberunit_contract_agenda_debt == 0
    assert sum_memberunit_contract_agenda_ratio_credit == 0
    assert sum_memberunit_contract_agenda_ratio_debt == 0
    ...
    assert sum_memberlink_contract_agenda_credit == x_sum
    assert sum_memberlink_contract_agenda_debt == x_sum
    assert (
    ...
    for memberunit in cx._members.values():
        ...
        assert memberunit._contract_credit not in [0.25, 0.5]
        assert memberunit._contract_debt != None
        assert memberunit._contract_debt not in [
            0.8,
            0.1,
        ]  # print(f"{memberunit.name=}")
        sum_memberunit_contract_agenda_credit += memberunit._contract_agenda_credit
    ...
    assert (
        abs(sum_memberunit_contract_agenda_credit - sum_contract_agenda_importance)
        < e10
    )
    assert (
        abs(sum_memberunit_contract_agenda_debt - sum_contract_agenda_importance) < e10
    )
    assert abs(sum_memberunit_contract_agenda_ratio_credit - 1) < e10
    assert abs(sum_memberunit_contract_agenda_ratio_debt - 1) < e10
...
def test_contract_agenda_ratio_credit_debt_IsCorrectlySetWhenAgendaIsEmpty():
    ...
    assert cx_rico_member._contract_agenda_credit in [0, None]
    assert cx_rico_member._contract_agenda_debt in [0, None]
    assert cx_carm_member._contract_agenda_credit in [0, None]
    assert cx_carm_member._contract_agenda_debt in [0, None]
    assert cx_patr_member._contract_agenda_credit in [0, None]
    assert cx_patr_member._contract_agenda_debt in [0, None]
    assert cx_rico_member._contract_agenda_ratio_credit != 0.05
    assert cx_rico_member._contract_agenda_ratio_debt != 0.2
    assert cx_carm_member._contract_agenda_ratio_credit != 0.15
    assert cx_carm_member._contract_agenda_ratio_debt != 0.3
    assert cx_patr_member._contract_agenda_ratio_credit != 0.8
    assert cx_patr_member._contract_agenda_ratio_debt != 0.5

    # WHEN
    cx.set_contract_metrics()

    # THEN
    assert cx_rico_member._contract_agenda_credit == 0
    assert cx_rico_member._contract_agenda_debt == 0
    assert cx_carm_member._contract_agenda_credit == 0
    assert cx_carm_member._contract_agenda_debt == 0
    assert cx_patr_member._contract_agenda_credit == 0
    assert cx_patr_member._contract_agenda_debt == 0
    assert cx_rico_member._contract_agenda_ratio_credit == 0.05
    assert cx_rico_member._contract_agenda_ratio_debt == 0.2
    assert cx_carm_member._contract_agenda_ratio_credit == 0.15
    assert cx_carm_member._contract_agenda_ratio_debt == 0.3
...
def test_contract_get_member_groups_returnsCorrectData():
    cx = ContractUnit(_owner="prom")
    cx.add_idea(idea_kid=IdeaKid(_label="swim"), walk="prom")
    rico_text = "rico"
    carm_text = "carmen"
...
def test_contract_MemberUnit_CorrectlyCreatesNewName():
    # GIVEN
    cx = ContractUnit(_owner="prom")
    rico_text = "rico"
...
def test_contract_MemberUnit_raiseErrorNewNamePreviouslyExists():
    # GIVEN
...
def test_contract_MemberUnit_CorrectlyChangesGroupUnitMemberLinks():
    # GIVEN
    prom_text = "prom"
    cx = ContractUnit(_owner=prom_text)
    rico_text = "rico"
...
def test_contract_MemberUnit_CorrectlyMergesNames():
    # GIVEN
    prom_text = "prom"
    cx = ContractUnit(_owner=prom_text)
    rico_text = "rico"
...
def test_contract_MemberUnit_CorrectlyMergesGroupUnitMemberLinks():
    # GIVEN
    # GIVEN
    prom_text = "prom"
    cx = ContractUnit(_owner=prom_text)
...
def test_contract_MemberUnit_raiseErrorNewNameGroupUnitPreviouslyExists():
    # GIVEN
    sx = ContractUnit(_owner="prom")
    rico_text = "rico"
    sx.add_memberunit(name=rico_text, uid=61, creditor_weight=13)
...
def test_contract_MemberUnit_CorrectlyOverwriteNewNameGroupUnit():
    # GIVEN
    sx = ContractUnit(_owner="prom")
    rico_text = "rico"
    sx.add_memberunit(name=rico_text, uid=61, creditor_weight=13)
...
def test_contract_set_all_memberunits_uids_unique_CorrectlySetsEmptyGroupUIDs():
    # GIVEN
    owner_text = "Noa"
...
def test_contract_set_all_memberunits_uids_unique_CorrectlySetsChangesSameGroupUIDs():
    # GIVEN
    owner_text = "Noa"
    sx = ContractUnit(_owner=owner_text)
    sx.set_members_empty_if_null()
...
def test_contract_all_memberunits_uids_are_unique_ReturnsCorrectBoolean():
    # GIVEN
    owner_text = "Noa"
    sx = ContractUnit(_owner=owner_text)
    sx.set_members_empty_if_null()
...
def test_contract_get_memberunits_name_list_CorrectlyReturnsListOfMemberUnits():
    # GIVEN
    owner_text = "Noa"
    sx = ContractUnit(_owner=owner_text)
    sx.set_members_empty_if_null()
...
def test_contract_set_banking_data_memberunits_CorrectlySetsMemberUnitBankingAttr():
    # GIVEN
    bob_text = "bob"
