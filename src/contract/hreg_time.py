from src.contract.required_idea import Road, SuffFactUnit
from src.contract.idea import IdeaBare as YB
...
class SuffFactUnitHregTime:
    ...
    _every_x_months: int = None  # builds mycontract,time,months
    _on_x_monthday: int = None  # " build mycontract,time,month,monthday
    _every_x_years: int = None  # builds mycontract,time,years
...
def get_jajatime_legible_from_dt(dt: datetime) -> str:
    ...
    return f"{weekday_text[:3]} {monthtitle_text[:3]} {monthday_text}, {year_text} at {convert1440toReadableTime(min1440)}"
