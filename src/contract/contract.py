...
class ContractUnit:
    ...
    def set_goal_tag(self, goal_tag: str):
        old_goal_tag = copy_deepcopy(self._goal_tag)
        self._goal_tag = goal_tag
        self.edit_idea_label(old_road=old_goal_tag, new_label=self._goal_tag)
    ...
    def get_time_c400_from_min(self, min: int) -> int:
        c400_idea = self.get_idea_kid(f"{self._goal_tag},time,tech,400 year cycle")
    ...
    def get_time_c400yr_from_min(self, min: int):
        ...
        if c100_4_96y._close - c100_4_96y._begin in (
            ...
            yr4_1461 = self.get_idea_kid(f"{self._goal_tag},time,tech,4year with leap")
            ...
            yr4_1460 = self.get_idea_kid(f"{self._goal_tag},time,tech,4year wo leap")
            yr4_cycles = 0
    ...
    def get_time_month_from_min(self, min: int):
        ...
        if yr1_idea._close - yr1_idea._begin == 525600:
            yrx = self.get_idea_kid(f"{self._goal_tag},time,tech,365 year")
        elif yr1_idea._close - yr1_idea._begin == 527040:
            yrx = self.get_idea_kid(f"{self._goal_tag},time,tech,366 year")
        ...
        day_x = self.get_idea_kid(f"{self._goal_tag},time,tech,day")
    ...
    def _get_jajatime_week_legible_text(self, open: int, divisor: int) -> str:
        ...
        week_road = f"{self._goal_tag},time,tech,week"
    ...
    def set_time_acptfacts(self, open: datetime = None, nigh: datetime = None) -> None:
        ...
        minutes_acptfact = f"{self._goal_tag},time,jajatime"
        self.set_acptfact(
    ...
    def _is_idea_rangeroot(self, idea_road: Road) -> bool:
        ...
        parent_road = self._goal_tag if len(anc_roads) == 1 else anc_roads[1]
    ...
    def add_idea(
        ...
        if temp_road == self._goal_tag and walk_nodes == []:
            idea_kid.set_walk(parent_road=Road(self._goal_tag))
        else:
        ...
        if old_road != new_road:
            ...
            if walk == "":
                self._idearoot.set_idea_label(
                    new_label, contract_goal_tag=self._goal_tag
                )
    ...
    def set_time_hreg_ideas(self, c400_count):
        ...
        while len(ideabase_list) != 0:
            ...
            if yb.sr != None:
                range_source_road_x = f"{self._goal_tag},{yb.sr}"
            ...
            road_x = f"{self._goal_tag},{yb.rr}"
            self.add_idea(idea_kid=idea_x, walk=road_x)

            numeric_road_x = None
            if yb.nr != None:
                numeric_road_x = f"{self._goal_tag},{yb.nr}"
                self.edit_idea_attr(
...
def get_from_dict(cx_dict: dict) -> ContractUnit:
    c_x = ContractUnit()
    c_x.set_goal_tag(cx_dict["_goal_tag"])
    ...
    c_x._idearoot.set_idea_label(c_x._goal_tag, c_x._goal_tag)
