...
from src.contract.x_func import x_get_json
from src.contract.idea import IdeaCore, IdeaKid, IdeaRoot, IdeaAttrHolder
...
from src.contract.origin import originunit_get_from_dict, originunit_shop, OriginUnit
...
class ContractUnit:
    ...
    def __init__(
        ...
        if _healer is None:
            _healer = ""
        self._healing_handle = root_label()
    ...
    def set_healing_handle(self, healing_handle: str):
        old_healing_handle = copy_deepcopy(self._healing_handle)
        self._healing_handle = healing_handle
        self.edit_idea_label(
            old_road=old_healing_handle, new_label=self._healing_handle
        )
    ...
    def export_all_bonds(self, dir: str):
        ...
        for yx in self.get_idea_list():
            if yx.promise:
                cx = self.get_contract_sprung_from_single_idea(yx.get_road())
    ...
    def get_contract_sprung_from_single_idea(self, road: Road):
        self.set_contract_metrics()
    ...
    def get_time_c400_from_min(self, min: int) -> int:
        c400_idea = self.get_idea_kid(
            f"{self._healing_handle},time,tech,400 year cycle"
        )
    ...
    def get_time_c400yr_from_min(self, min: int):
        ...
        if c100_4_96y._close - c100_4_96y._begin in (
            ...
            yr4_1461 = self.get_idea_kid(
                f"{self._healing_handle},time,tech,4year with leap"
            )
            ...
            yr4_1460 = self.get_idea_kid(
                f"{self._healing_handle},time,tech,4year wo leap"
            )
            yr4_cycles = 0
    ...
    def get_time_month_from_min(self, min: int):
        ...
        if yr1_idea._close - yr1_idea._begin == 525600:
            yrx = self.get_idea_kid(f"{self._healing_handle},time,tech,365 year")
        elif yr1_idea._close - yr1_idea._begin == 527040:
            yrx = self.get_idea_kid(f"{self._healing_handle},time,tech,366 year")
        ...
        day_x = self.get_idea_kid(f"{self._healing_handle},time,tech,day")
    ...
    def _get_jajatime_week_legible_text(self, open: int, divisor: int) -> str:
        ...
        week_road = f"{self._healing_handle},time,tech,week"
        ...
        for partyunit in self._partys.values():
            if partyunit.title == partyunit_title:
                partyunit.add_contract_credit_debt(
                    ...
                    contract_agenda_debt=contract_agenda_debt,
                )
    ...
    def set_time_acptfacts(self, open: datetime = None, nigh: datetime = None) -> None:
        ...
        minutes_acptfact = f"{self._healing_handle},time,jajatime"
        self.set_acptfact(
    ...
    def _is_idea_rangeroot(self, idea_road: Road) -> bool:
        ...
        parent_road = self._healing_handle if len(anc_roads) == 1 else anc_roads[1]
    ...
    def add_idea(
        ...
        if temp_road == self._healing_handle and walk_nodes == []:
            idea_kid.set_walk(parent_road=Road(self._healing_handle))
        else:
        ...
        if old_road != new_road:
            ...
            if walk == "":
                self._idearoot.set_idea_label(
                    new_label, contract_healing_handle=self._healing_handle
                )
    ...
    def _set_kids_attributes(
        ...
        idea_kid.set_contract_importance(
            ...
            parent_contract_importance=parent_idea._contract_importance,
    ...
    def get_contract_importance(
        ...
        return parent_contract_importance * sibling_ratio

    def get_idea_list(self):
        self.set_contract_metrics()
        return list(self._idea_dict.values())

    def set_contract_metrics(self):
        self._set_acptfacts_empty_if_null()
    ...
    def _execute_tree_traverse(self):
        ...
        coin_onset = self._idearoot._contract_coin_onset
        parent_coin_cease = self._idearoot._contract_coin_cease
        ...
        for idea_kid in self._idearoot._kids.values():
            ...
            coin_onset += idea_kid._contract_importance
        ...
        while cache_idea_list != []:
            ...
            if parent_idea._kids != None:
                coin_onset = parent_idea._contract_coin_onset
                parent_coin_cease = parent_idea._contract_coin_cease
    ...
    def _run_after_idea_all_tree_traverses(self):
        self._distribute_contract_agenda_importance()
        self._distribute_groups_contract_importance()
        self._set_contract_agenda_ratio_credit_debt()
    ...
    def get_dict(self):
        ...
        return {
            ...
            "_healing_handle": self._healing_handle,
    ...
    def set_time_hreg_ideas(self, c400_count):
        ...
        while len(ideabase_list) != 0:
            ...
            if yb.sr != None:
                range_source_road_x = f"{self._healing_handle},{yb.sr}"
            ...
            road_x = f"{self._healing_handle},{yb.rr}"
            self.add_idea(idea_kid=idea_x, walk=road_x)

            numeric_road_x = None
            if yb.nr != None:
                numeric_road_x = f"{self._healing_handle},{yb.nr}"
                self.edit_idea_attr(
    ...
    def get_contract4party(
        ...
        if not_included_contract_importance > 0:
            y4a_other = IdeaKid(
                _label="__other__",
    ...
    def _meld_acptfacts(self, other_contract):
        self._set_acptfacts_empty_if_null()
        other_contract._set_acptfacts_empty_if_null()
        for hx in other_contract._idearoot._acptfactunits.values():
    ...
    def get_assignment(
        ...
        assignor_promises = self._get_assignor_promise_ideas(contract_x, assignor_title)
        relevant_roads = self._get_relevant_roads(assignor_promises)
        self._set_assignment_ideas(contract_x, relevant_roads)
        return contract_x
    ...
    def _set_assignment_partys(
        ...
        if self._partys.get(assignor_title) != None:
            ...
            for partytitle_x in partys_set:
                contract_x.set_partyunit(partyunit=self._partys.get(partytitle_x))
        return contract_x
    ...
    def _set_assignment_groups(self, contract_x):
        ...
        for group_title, group_partys in revelant_groups.items():
            if contract_x._groups.get(group_title) is None:
    ...
    def _get_assignor_promise_ideas(
        self, contract_x, assignor_title: GroupBrand
    ) -> dict[Road:int]:
        contract_x.set_groupunits_empty_if_null()
        assignor_groups = get_party_relevant_groups(contract_x._groups, assignor_title)
        return {
            idea_road: -1
...
def get_from_dict(cx_dict: dict) -> ContractUnit:
    c_x = ContractUnit()
    c_x.set_healing_handle(cx_dict["_healing_handle"])
    ...
    c_x._idearoot.set_idea_label(c_x._healing_handle, c_x._healing_handle)
...
def get_dict_of_contract_from_dict(x_dict: dict[str:dict]) -> dict[str:ContractUnit]:
    contractunits = {}
...
def get_meld_of_contract_files(
    cx_primary: ContractUnit, meldees_dir: str
) -> ContractUnit:
    cx_primary.set_contract_metrics()
    for meldee_file_x in x_func_dir_files(dir_path=meldees_dir):
        ...
        cx_primary.meld(other_contract=meldee_x)
