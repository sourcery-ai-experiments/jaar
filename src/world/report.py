...
from src.world.person import get_gut_file_name, get_life_file_name
...
def get_world_lifes_partys_dataframe(x_world: WorldUnit) -> DataFrame:
    # get list of all person paths
    person_paths = x_world.get_person_paths()
    # for all persons get life
    life_dfs = []
    for person_path in person_paths:
        life_agenda = agenda_get_from_json(
...
def get_world_lifes_partys_plotly_fig(x_world: WorldUnit) -> plotly_Figure:
    ...
    fig = plotly_Figure(data=[x_table])
    fig_title = f"World '{x_world.world_id}', life partys metrics"
    fig.update_xaxes(showgrid=False)
    fig.update_yaxes(showgrid=False, zeroline=True, showticklabels=False)
    fig.update_layout(plot_bgcolor="white", title=fig_title, title_font_size=20)
...
def get_world_lifes_intent_dataframe(x_world: WorldUnit) -> DataFrame:
    # get list of all person paths
    person_paths = x_world.get_person_paths()
    # for all persons get life
    life_dfs = []
    for person_path in person_paths:
        ...
        life_agenda.set_agenda_metrics()
        life_df = get_agenda_intent_dataframe(life_agenda)
        life_dfs.append(life_df)
    return pandas_concat(life_dfs, ignore_index=True)


def get_world_lifes_intent_plotly_fig(x_world: WorldUnit) -> plotly_Figure:
    column_header_list = [
        "worker_id",
        "agenda_importance",
        "_label",
    ...
    fig = plotly_Figure(data=[x_table])
    fig_title = f"World '{x_world.world_id}', life intent metrics"
    fig.update_xaxes(showgrid=False)
    fig.update_yaxes(showgrid=False, zeroline=True, showticklabels=False)
    fig.update_layout(plot_bgcolor="white", title=fig_title, title_font_size=20)
