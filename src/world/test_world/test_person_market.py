...
def test_PersonUnit_create_person_marketunits_RaisesErrorWhen__markets_justified_IsFalse(
    worlds_dir_setup_cleanup,
):
    # GIVEN
    sue_text = "Sue"
    ...
    assert sue_gut_agenda._markets_justified == False
    sue_person._save_agenda_to_gut_path(sue_gut_agenda)

    # WHEN / THEN
    with pytest_raises(Exception) as excinfo:
        sue_person.create_person_marketunits(market_exceptions=False)
    assert (
        str(excinfo.value)
        == f"Cannot set '{sue_text}' gut agenda marketunits because 'AgendaUnit._markets_justified' is False."
    )


def test_PersonUnit_create_person_marketunits_RaisesErrorWhen__markets_buildable_IsFalse(
    worlds_dir_setup_cleanup,
):
    # GIVEN
    sue_text = "Sue"
    ...
    sue_gut_agenda.edit_idea_attr(texas_road, healerhold=healerhold_shop({sue_text}))
    sue_gut_agenda.set_agenda_metrics()
    assert sue_gut_agenda._markets_justified
    assert sue_gut_agenda._markets_buildable == False
    sue_person._save_agenda_to_gut_path(sue_gut_agenda)

    # WHEN / THEN
    with pytest_raises(Exception) as excinfo:
        sue_person.create_person_marketunits()
    assert (
        str(excinfo.value)
        == f"Cannot set '{sue_text}' gut agenda marketunits because 'AgendaUnit._markets_buildable' is False."
    )


def test_PersonUnit_create_person_marketunits_CreatesMarketUnits(
    worlds_dir_setup_cleanup,
):
    # GIVEN
    sue_text = "Sue"
    ...
    dallas_dir = sue_person._get_market_path(dallas_nodes)
    elpaso_dir = sue_person._get_market_path(elpaso_nodes)
    dallas_db_path = f"{dallas_dir}/bank.db"
    elpaso_db_path = f"{elpaso_dir}/bank.db"
    print(f"{dallas_dir=}")
    ...
    assert sue_person._market_objs != {}
    assert sue_person._market_objs.get(dallas_road) != None
    assert sue_person._market_objs.get(elpaso_road) != None
