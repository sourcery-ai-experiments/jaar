...
import src.world.examples.example_persons as example_persons
...
def test_world_set_agent_CreatesAgentFile(env_dir_setup_cleanup):
    # GIVEN
...
def test_world_get_agents_dir_list_of_obj_CreatesAgentFilesList(
    env_dir_setup_cleanup,
):
    # GIVEN
    world_name = get_temp_env_name()
...
def test_world_get_agent_currentlyGetsAgent(env_dir_setup_cleanup):
    # GIVEN
    world_name = get_temp_env_name()
...
def test_world_rename_agent_in_agents_dir_ChangesAgentName(env_dir_setup_cleanup):
    # GIVEN
    world_name = get_temp_env_name()
    e5 = WorldUnit(name=world_name, worlds_dir=get_test_worlds_dir())
    e5.create_dirs_if_null(in_memory_bank=True)
...
def test_personunit_refresh_agentlinks_CorrectlyPullsAllPublicAgents(
    ...
    world_name = get_temp_env_name()
    e1 = WorldUnit(name=world_name, worlds_dir=env_dir)
    ...
    px_ernie = e1.get_person_obj_from_world(name=ernie_text)
    px_jessi = e1.get_person_obj_from_world(name=jessi_text)
    # px_steve = e1.get_person_obj_from_world(name=steve_text)
