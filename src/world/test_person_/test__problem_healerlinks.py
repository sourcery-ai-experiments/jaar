...
def test_ProblemUnit_marketlink_exists_ReturnsCorrectObj():
    # GIVEN
    knee_text = "knee discomfort"
    knee_problemunit = problemunit_shop(problem_id=knee_text)
    yao_text = "yao"
    ...
    # WHEN / THEN
    diet_text = "diet"
    assert knee_problemunit.marketlink_exists(diet_text) == False

    # WHEN / THEN
    yao_healerlink.set_marketlink(marketlink_shop(market_id=diet_text))
    assert len(yao_healerlink._marketlinks) != 0
    assert knee_problemunit.marketlink_exists(diet_text)
