...
def test_MoneyUnit_exists():
    ...
    # WHEN
    x_money = MoneyUnit(filehub=texas_filehub)

    # THEN
    assert x_money.filehub == texas_filehub
...
def test_moneyunit_shop_ReturnsObj(env_dir_setup_cleanup):
    ...
    # WHEN
    texas_money = moneyunit_shop(sue_texas_filehub)

    # THEN
    assert texas_money != None
    assert texas_money.filehub.real_id != None
    assert texas_money._treasury_db != None
    assert texas_money.filehub.person_id == sue_text
    assert texas_money.filehub.road_delimiter == default_road_delimiter_if_none()
    assert texas_money.filehub == sue_texas_filehub
...
def test_MoneyUnitcreate_treasury_db_CreatesTreasuryDBIfItDoesNotExist(
    ...
    # THEN
    assert os_path_exists(x_money.filehub.treasury_db_path())
...
def test_MoneyUnitcreate_treasury_db_DoesNotOverWriteDBIfItExists(
    ...
    save_file(
        x_money.filehub.econ_dir(),
        file_name=db_file,
        file_text=x_file_text,
        replace=True,
    )
    assert os_path_exists(x_money.filehub.treasury_db_path())
    assert open_file(x_money.filehub.econ_dir(), file_name=db_file) == x_file_text
