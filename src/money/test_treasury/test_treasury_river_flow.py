...
def get_truth_otherunit_table_treasurying_attr_set_count_sqlstr():
    # def get_truth_otherunit_table_treasurying_attr_set_count_sqlstr(cash_master:):
    #     return f"""
    # SELECT COUNT(*)
...
def test_MoneyUnit_set_cred_flow_for_truth_CorrectlyPopulatesothertreasuryunitTable01(
    ...
    sal_agentunit = truthunit_shop(_owner_id=sal_text)
    ...
    othertreasuryunit_count_sqlstr = (
        get_truth_otherunit_table_treasurying_attr_set_count_sqlstr()
    )
...
def test_MoneyUnit_set_cred_flow_for_truth_CorrectlyPopulatesothertreasuryunitTable03(
    env_dir_setup_cleanup,
):
    # GIVEN 4 truths, 85% of river blocks to sal
    x_money = moneyunit_shop(get_texas_userhub())
...
def test_MoneyUnit_set_cred_flow_for_truth_CorrectlyPopulatesothertreasuryunitTable04(
    env_dir_setup_cleanup,
):
    # GIVEN 5 truths, 85% of river blocks to sal, left over %15 goes on endless loop
    x_money = moneyunit_shop(get_texas_userhub())
    ...
    elu_truth.add_otherunit(other_id=ava_text, credor_weight=2)
...
def test_MoneyUnit_set_cred_flow_for_truth_CorrectlyPopulatesothertreasuryunitTable05_v1(
    env_dir_setup_cleanup,
):
    # GIVEN 5 truths, 85% of river blocks to sal, left over %15 goes on endless loop that slowly bleeds to sal
    x_money = moneyunit_shop(get_texas_userhub())
...
def test_MoneyUnit_set_cred_flow_for_truth_CorrectlyUsesMaxblocksCount(
    env_dir_setup_cleanup,
):
    # GIVEN 5 truths, 85% of river blocks to sal, left over %15 goes on endless loop that slowly bleeds to sal
    x_money = moneyunit_shop(get_texas_userhub())
    ...
    # WHEN
    mbc = 13
    x_money.set_cred_flow_for_truth(owner_id=sal_text, max_blocks_count=mbc)
...
def test_MoneyUnit_set_cred_flow_for_truth_CorrectlyPopulatesothertreasuryunitTable05(
    env_dir_setup_cleanup,
):
    # GIVEN 5 truths, 85% of river blocks to sal, left over %15 goes on endless loop that slowly bleeds to sal
    x_money = moneyunit_shop(get_texas_userhub())
...
def test_MoneyUnit_set_cred_flow_for_truth_CorrectlyBuildsASingle_ContinuousRange(
    env_dir_setup_cleanup,
):
    # GIVEN 5 truths, 85% of river blocks to sal, left over %15 goes on endless loop that slowly bleeds to sal
    x_money = moneyunit_shop(get_texas_userhub())
...
def test_MoneyUnit_set_cred_flow_for_truth_CorrectlyUpatesTruthOtherUnits(
    env_dir_setup_cleanup,
):
    """GIVEN 5 truths, 85% of river blocks to sal, left over %15 goes on endless loop that slowly bleeds to sal"""
    # GIVEN
    ...
    sal_truth_src = truthunit_shop(_owner_id=sal_text)
    sal_truth_src.add_otherunit(other_id=bob_text, credor_weight=2, debtor_weight=2)
    sal_truth_src.add_otherunit(other_id=tom_text, credor_weight=2, debtor_weight=1)
    sal_truth_src.add_otherunit(other_id=ava_text, credor_weight=2, debtor_weight=2)
    x_money.userhub.save_job_truth(sal_truth_src)
    ...
    tom_truth = truthunit_shop(_owner_id=tom_text)
    tom_truth.add_otherunit(other_id=sal_text)
    x_money.userhub.save_job_truth(tom_truth)
    ...
    elu_truth = truthunit_shop(_owner_id=elu_text)
    elu_truth.add_otherunit(other_id=ava_text, credor_weight=8)
    elu_truth.add_otherunit(other_id=sal_text, credor_weight=2)
    x_money.userhub.save_job_truth(elu_truth)

    x_money.refresh_treasury_job_truths_data()
    sal_truth_before = x_money.userhub.get_job_truth(owner_id=sal_text)

    x_money.set_cred_flow_for_truth(owner_id=sal_text, max_blocks_count=100)
    assert len(sal_truth_before._others) == 3
    print(f"{len(sal_truth_before._others)=}")
    bob_other = sal_truth_before._others.get(bob_text)
    tom_other = sal_truth_before._others.get(tom_text)
    ava_other = sal_truth_before._others.get(ava_text)
    ...
    sal_truth_after = x_money.userhub.get_job_truth(owner_id=sal_text)
    bob_other = sal_truth_after._others.get(bob_text)
    tom_other = sal_truth_after._others.get(tom_text)
    ava_other = sal_truth_after._others.get(ava_text)
    elu_other = sal_truth_after._others.get(elu_text)
    ...
    #     x_otherunit = sal_truth_after._others.get(sal_othertreasuryunit.due_owner_id)
    #     if x_otherunit != None:
    #         # print(
    ...
    # for x_otherunit in sal_truth_after._others.values():
    #     print(f"sal_truth_after {x_otherunit.other_id=} {x_otherunit._treasury_due_paid=}")
    ...
    # assert sal_truth_after != sal_truth_before
