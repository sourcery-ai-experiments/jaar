from src._world.world import worldunit_shop, charunit_shop
...
from src.money.treasury_sqlstr import (
    ...
    get_world_charunit_table_update_treasury_due_paid_sqlstr,
    CharTreasuryUnit,
...
def test_MoneyUnit_get_world_charunit_table_insert_sqlstr_CorrectlyPopulatesTable01(
    ...
    tim_charunit = charunit_shop(
        char_id=tim_text,
        _credor_operational=True,
        _debtor_operational=False,
    )
    tim_charunit._world_cred = 0.9
    tim_charunit._world_debt = 0.8
    tim_charunit._world_agenda_cred = 0.7
    tim_charunit._world_agenda_debt = 0.6
    tim_charunit._world_agenda_ratio_cred = 0.5
    tim_charunit._world_agenda_ratio_debt = 0.4

    tim_due_paid = 0.5151
    tim_cred_score = 0.5252
    tim_voice_rank = 33
    tim_charunit.set_treasury_attr(tim_due_paid, None, tim_cred_score, tim_voice_rank)
    assert tim_charunit._treasury_due_paid == tim_due_paid
    assert tim_charunit._treasury_cred_score == tim_cred_score
    assert tim_charunit._treasury_voice_rank == tim_voice_rank

    insert_sqlstr = get_world_charunit_table_insert_sqlstr(bob_world, tim_charunit)
    print(insert_sqlstr)
    ...
    assert tim_ledger.char_id == tim_text
...
def test_MoneyUnit_get_river_ledger_unit_ReturnsRiverLedgerUnit(env_dir_setup_cleanup):
    ...
    sal_text = "Sal"
    bob_world = worldunit_shop(_owner_id=bob_text)
    sal_charunit = charunit_shop(
        sal_text, _credor_operational=True, _debtor_operational=False
    )
    sal_charunit._world_cred = 0.9
    sal_charunit._world_debt = 0.8
    sal_charunit._world_agenda_cred = 0.7
    sal_charunit._world_agenda_debt = 0.6
    sal_charunit._world_agenda_ratio_cred = 0.5
    sal_charunit._world_agenda_ratio_debt = 0.4

    insert_sqlstr_sal = get_world_charunit_table_insert_sqlstr(bob_world, sal_charunit)

    tim_text = "Tim"
    tim_charunit = charunit_shop(
        tim_text, _credor_operational=True, _debtor_operational=False
    )
    tim_charunit._world_cred = 0.012
    tim_charunit._world_debt = 0.017
    tim_charunit._world_agenda_cred = 0.077
    tim_charunit._world_agenda_debt = 0.066
    tim_charunit._world_agenda_ratio_cred = 0.051
    tim_charunit._world_agenda_ratio_debt = 0.049

    insert_sqlstr_tim = get_world_charunit_table_insert_sqlstr(bob_world, tim_charunit)

    with x_money.get_treasury_conn() as treasury_conn:
        treasury_conn.execute(insert_sqlstr_sal)
        treasury_conn.execute(insert_sqlstr_tim)
        charview_dict_x = get_charview_dict(
    ...
    assert river_ledger_x._charviews == charview_dict_x
...
def test_RiverLedgerUnit_Exists():
    ...
    x_charview_dict = {
        x1_chardbunit.char_id: x1_chardbunit,
        x2_chardbunit.char_id: x2_chardbunit,
    }
    # WHEN
    ...
    assert river_ledger_unit._charviews == x_charview_dict
    assert abs(river_ledger_unit.get_range() - 0.2) < 0.00000001


def test_CharTreasuryUnit_exists():
    # GIVEN
    ...
    # WHEN
    x_chartreasury = CharTreasuryUnit(
    ...
    # THEN
    assert x_chartreasury.cash_master == x_cash_master
    assert x_chartreasury.due_owner_id == x_due_owner_id
    assert x_chartreasury.due_total == x_due_total
    assert x_chartreasury.debt == x_debt
    assert x_chartreasury.due_diff == x_due_diff
    assert x_chartreasury.cred_score == x_cred_score
    assert x_chartreasury.voice_rank == x_voice_rank


def test_world_set_treasury_attr_charunits_CorrectlySetsCharUnitTreasuryingAttr():
    # GIVEN
    bob_text = "Bob"
    ...
    fry_text = "fry"
    elu_text = "Elu"
    x_world.set_charunit(charunit=charunit_shop(char_id=sam_text))
    x_world.set_charunit(charunit=charunit_shop(char_id=wil_text))
    x_world.set_charunit(charunit=charunit_shop(char_id=fry_text))
    assert x_world._chars.get(sam_text)._treasury_due_paid is None
    assert x_world._chars.get(sam_text)._treasury_due_diff is None
    assert x_world._chars.get(wil_text)._treasury_due_paid is None
    assert x_world._chars.get(wil_text)._treasury_due_diff is None
    assert x_world._chars.get(fry_text)._treasury_due_paid is None
    assert x_world._chars.get(fry_text)._treasury_due_diff is None
    elu_charunit = charunit_shop(char_id=elu_text)
    elu_charunit._treasury_due_paid = 0.003
    elu_charunit._treasury_due_diff = 0.007
    x_world.set_charunit(charunit=elu_charunit)
    assert x_world._chars.get(elu_text)._treasury_due_paid == 0.003
    assert x_world._chars.get(elu_text)._treasury_due_diff == 0.007

    chartreasuryunit_sam = CharTreasuryUnit(
        bob_text, sam_text, 0.209, 0, 0.034, None, None
    )
    chartreasuryunit_wil = CharTreasuryUnit(
        bob_text, wil_text, 0.501, 0, 0.024, None, None
    )
    chartreasuryunit_fry = CharTreasuryUnit(
        bob_text, fry_text, 0.111, 0, 0.006, None, None
    )
    chartreasuryunits = {
        chartreasuryunit_sam.due_owner_id: chartreasuryunit_sam,
        chartreasuryunit_wil.due_owner_id: chartreasuryunit_wil,
        chartreasuryunit_fry.due_owner_id: chartreasuryunit_fry,
    }
    # WHEN
    set_treasury_chartreasuryunits_to_world_charunits(
        x_world, chartreasuryunits=chartreasuryunits
    )

    # THEN
    assert x_world._chars.get(sam_text)._treasury_due_paid == 0.209
    assert x_world._chars.get(sam_text)._treasury_due_diff == 0.034
    assert x_world._chars.get(wil_text)._treasury_due_paid == 0.501
    assert x_world._chars.get(wil_text)._treasury_due_diff == 0.024
    assert x_world._chars.get(fry_text)._treasury_due_paid == 0.111
    assert x_world._chars.get(fry_text)._treasury_due_diff == 0.006
    assert x_world._chars.get(elu_text)._treasury_due_paid is None
    assert x_world._chars.get(elu_text)._treasury_due_diff is None


def test_MoneyUnit_get_world_charunit_table_update_treasury_due_paid_sqlstr_CorrectlyPopulatesTable01(
    env_dir_setup_cleanup,
):
    # GIVEN Create example econ with 4 Healers, each with 3 CharUnits = 12 ledger rows
    x_money = moneyunit_shop(get_texas_userhub())
    ...
    bob_world = worldunit_shop(_owner_id=bob_text)
    tom_charunit = charunit_shop(
        tom_text, _credor_operational=True, _debtor_operational=False
    )
    tom_charunit._world_cred = 0.9
    tom_charunit._world_debt = 0.8
    tom_charunit._world_agenda_cred = 0.7
    tom_charunit._world_agenda_debt = 0.6
    tom_charunit._world_agenda_ratio_cred = 0.5
    tom_charunit._world_agenda_ratio_debt = 0.411

    insert_sqlstr_tom = get_world_charunit_table_insert_sqlstr(bob_world, tom_charunit)
    sal_charunit = charunit_shop(
        sal_text, _credor_operational=True, _debtor_operational=False
    )
    sal_charunit._world_cred = 0.9
    ...
    sal_charunit._world_agenda_debt = 0.6
    sal_charunit._world_agenda_ratio_cred = 0.5
    sal_charunit._world_agenda_ratio_debt = 0.455
    ...
    # WHEN
    mstr_sqlstr = get_world_charunit_table_update_treasury_due_paid_sqlstr(
        cash_owner_id=bob_text
    )
    with x_money.get_treasury_conn() as treasury_conn:
        print(mstr_sqlstr)
    ...
    # THEN
    with x_money.get_treasury_conn() as treasury_conn:
        chartreasuryunits = get_chartreasuryunit_dict(
            treasury_conn, cash_owner_id=bob_text
        )
        print(f"{chartreasuryunits=}")

    assert len(chartreasuryunits) == 2

    bob_tom_x = chartreasuryunits.get(tom_text)
    ...
    bob_sal_x = chartreasuryunits.get(sal_text)
    ...
    # for value in chartreasuryunits.values():
