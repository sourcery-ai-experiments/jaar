...
from src.money.treasury_sqlstr import (
    ...
    get_world_charunit_table_create_sqlstr,
    get_world_charunit_table_insert_sqlstr as char_insert_sqlstr,
    get_world_charunit_table_update_cred_score_sqlstr,
    get_world_charunit_table_update_treasury_voice_rank_sqlstr,
)
...
def test_get_world_charunit_table_update_cred_score_sqlstr_UpdatesWithoutError():
    # GIVEN
    x_db = sqlite3_connect(":memory:")
    charunit_text = "world_charunit"
    reach_text = "river_reach"
    with x_db as x_conn:
        x_conn.execute(get_world_charunit_table_create_sqlstr())
        x_conn.execute(get_river_reach_table_create_sqlstr())
        assert 0 == get_single_result(x_conn, get_row_count_sqlstr(charunit_text))
        assert 0 == get_single_result(x_conn, get_row_count_sqlstr(reach_text))
    ...
    cal_charunit = charunit_shop(cal_text)
    dee_charunit = charunit_shop(dee_text)
    bob_s = 0.78
    cal_s = 0.1
    dee_s1 = 0.2
    dee_s2 = 0.6
    ...
    with x_db as x_conn:
        x_conn.execute(char_insert_sqlstr(yao_world, bob_charunit))
        x_conn.execute(char_insert_sqlstr(yao_world, cal_charunit))
        x_conn.execute(char_insert_sqlstr(yao_world, dee_charunit))
        ...
        assert 3 == get_single_result(x_conn, get_row_count_sqlstr(charunit_text))
        assert 4 == get_single_result(x_conn, get_row_count_sqlstr(reach_text))

    charunit_select_str = f"""
SELECT 
  owner_id
, char_id
...
"""
    with x_db as x_conn:
        results = x_conn.execute(charunit_select_str)
    ...
    # WHEN
    with x_db as x_conn:
        x_conn.execute(get_world_charunit_table_update_cred_score_sqlstr(yao_text))
...
def test_get_world_charunit_table_update_treasury_voice_rank_sqlstr_UpdatesWithoutError():
    # GIVEN
    yao_text = "Yao"
    yao_world = worldunit_shop(yao_text)
    bob_text = "Bob"
    ...
    cal_charunit = charunit_shop(cal_text)
    dee_charunit = charunit_shop(dee_text)
    bob_charunit.set_treasury_attr(None, None, bob_cred_score, None)
    cal_charunit.set_treasury_attr(None, None, cal_cred_score, None)
    dee_charunit.set_treasury_attr(None, None, dee_cred_score, None)
    print(f"{bob_charunit._treasury_cred_score=}")
    print(f"{cal_charunit._treasury_cred_score=}")
    print(f"{dee_charunit._treasury_cred_score=}")
...
"""
    ...
    assert x_rows[1][1] == cal_text
    assert x_rows[2][1] == dee_text
    assert x_rows[0][2] - bob_charunit._treasury_cred_score < 0.000001
    assert x_rows[1][2] == cal_charunit._treasury_cred_score
    assert x_rows[2][2] == dee_charunit._treasury_cred_score
    ...
    with x_db as x_conn:
        x_conn.execute(
            get_world_charunit_table_update_treasury_voice_rank_sqlstr(yao_text)
        )
