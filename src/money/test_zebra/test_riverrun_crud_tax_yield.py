...
def test_RiverRun_set_char_tax_yield_SetsAttr():
    ...
    # WHEN
    yao_tax_yield = 7
    bob_riverrun.set_char_tax_yield(yao_text, yao_tax_yield)
...
def test_RiverRun_tax_yields_is_empty_ReturnsObj():
    ...
    yao_text = "Yao"
    yao_tax_yield = 500
    x_riverrun.set_char_tax_yield(yao_text, yao_tax_yield)
    # THEN
    ...
    bob_text = "Yao"
    bob_tax_yield = 300
    x_riverrun.set_char_tax_yield(bob_text, bob_tax_yield)
    x_riverrun.set_char_tax_yield(yao_text, yao_tax_yield)
    # THEN
    assert x_riverrun.tax_yields_is_empty() == False
...
def test_RiverRun_reset_tax_yields_CorrectlySetsAttr():
    ...
    sue_tax_yield = 56
    yao_tax_yield = 6
    bob_riverrun.set_char_tax_yield(bob_text, bob_tax_yield)
    bob_riverrun.set_char_tax_yield(sue_text, sue_tax_yield)
    bob_riverrun.set_char_tax_yield(yao_text, yao_tax_yield)
...
def test_RiverRun_char_has_tax_yield_ReturnsCorrectBool():
    ...
    bob_riverrun.set_char_tax_yield(sue_text, sue_tax_yield)
    bob_riverrun.set_char_tax_yield(yao_text, yao_tax_yield)
    assert bob_riverrun.char_has_tax_yield(bob_text)
    assert bob_riverrun.char_has_tax_yield(sue_text)
    assert bob_riverrun.char_has_tax_yield(yao_text)
    assert bob_riverrun.char_has_tax_yield(zia_text) == False

    # WHEN
    bob_riverrun.reset_tax_yields()

    # THEN
    assert bob_riverrun.char_has_tax_yield(bob_text) == False
    assert bob_riverrun.char_has_tax_yield(sue_text) == False
    assert bob_riverrun.char_has_tax_yield(yao_text) == False
    assert bob_riverrun.char_has_tax_yield(zia_text) == False
...
def test_RiverRun_delete_tax_yield_SetsAttr():
    ...
    bob_riverrun = riverrun_shop(bob_userhub)
    yao_text = "Yao"
    bob_riverrun.set_char_tax_yield(yao_text, 5)
    assert bob_riverrun.char_has_tax_yield(yao_text)
...
def test_RiverRun_get_char_tax_yield_ReturnsCorrectObj():
    ...
    bob_riverrun.set_char_tax_yield(yao_text, yao_tax_yield)
    assert bob_riverrun.char_has_tax_yield(bob_text)
    assert bob_riverrun.get_char_tax_yield(bob_text) == bob_tax_yield
    assert bob_riverrun.char_has_tax_yield(zia_text) == False
    ...
    # THEN
    assert bob_riverrun.char_has_tax_yield(bob_text) == False
    assert bob_riverrun.get_char_tax_yield(bob_text) == 0
    assert bob_riverrun.char_has_tax_yield(zia_text) == False
    assert bob_riverrun.get_char_tax_yield(zia_text) == 0
...
def test_RiverRun_add_char_tax_yield_ReturnsCorrectObj():
    ...
    bob_riverrun.set_char_tax_yield(yao_text, yao_tax_yield)
    assert bob_riverrun.get_char_tax_yield(bob_text) == bob_tax_yield
    assert bob_riverrun.get_char_tax_yield(sue_text) == sue_tax_yield
    assert bob_riverrun.get_char_tax_yield(zia_text) == 0

    # WHEN
    bob_riverrun.add_char_tax_yield(sue_text, 5)
    bob_riverrun.add_char_tax_yield(zia_text, 10)

    # THEN
    assert bob_riverrun.get_char_tax_yield(bob_text) == bob_tax_yield
    assert bob_riverrun.get_char_tax_yield(sue_text) == sue_tax_yield + 5
    assert bob_riverrun.get_char_tax_yield(zia_text) == 10
...
def test_RiverRun_levy_tax_due_SetsAttr():
    ...
    bob_world.add_charunit(bob_text, 2, bob_tax_yield)
    bob_world.add_charunit(sue_text, 2, sue_tax_yield)
    bob_world.add_charunit(yao_text, 2, yao_tax_yield)
    ...
    assert bob_riverrun.get_char_tax_yield(bob_text) == 5
    ...
    assert bob_riverrun.get_char_tax_yield(yao_text) == 60
