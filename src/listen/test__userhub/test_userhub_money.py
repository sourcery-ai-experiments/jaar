...
def test_UserHub_get_econ_roads_RaisesErrorWhen__econs_justified_IsFalse(
    ...
    sue_duty_agenda.add_l1_fact(factunit_shop(texas_text, _problem_bool=True))
    sue_duty_agenda.add_fact(factunit_shop(dallas_text), texas_road)
    sue_duty_agenda.edit_fact_attr(texas_road, healerhold=healerhold_shop({sue_text}))
    sue_duty_agenda.edit_fact_attr(dallas_road, healerhold=healerhold_shop({sue_text}))
...
def test_UserHub_get_econ_roads_CreatesMoneyUnits(env_dir_setup_cleanup):
    ...
    dallas_fact = factunit_shop(dallas_text, _healerhold=healerhold_shop({sue_text}))
    elpaso_fact = factunit_shop(elpaso_text, _healerhold=healerhold_shop({sue_text}))
    sue_duty_agenda.add_fact(dallas_fact, texas_road)
    sue_duty_agenda.add_fact(elpaso_fact, texas_road)
    sue_duty_agenda.calc_agenda_metrics()
    # display_facttree(sue_duty_agenda, mode="Econ").show()
    sue_userhub.save_duty_agenda(sue_duty_agenda)
