...
def test_UserHub_get_econ_roads_RaisesErrorWhen__econs_justified_IsFalse(
    ...
    sue_duty_agenda.add_oath(oathunit_shop(dallas_text), texas_road)
    sue_duty_agenda.edit_oath_attr(texas_road, healerhold=healerhold_shop({sue_text}))
    sue_duty_agenda.edit_oath_attr(dallas_road, healerhold=healerhold_shop({sue_text}))
...
def test_UserHub_get_econ_roads_CreatesMoneyUnits(env_dir_setup_cleanup):
    ...
    dallas_oath = oathunit_shop(dallas_text, _healerhold=healerhold_shop({sue_text}))
    elpaso_oath = oathunit_shop(elpaso_text, _healerhold=healerhold_shop({sue_text}))
    sue_duty_agenda.add_oath(dallas_oath, texas_road)
    sue_duty_agenda.add_oath(elpaso_oath, texas_road)
    sue_duty_agenda.calc_agenda_metrics()
    # display_oathtree(sue_duty_agenda, mode="Econ").show()
    sue_userhub.save_duty_agenda(sue_duty_agenda)
