...
def create_listen_basis(x_role: AgendaUnit) -> AgendaUnit:
    ...
    if x_role._guy_credor_pool != None:
        x_listen.set_guy_credor_pool(x_role._guy_credor_pool)
    if x_role._guy_debtor_pool != None:
        x_listen.set_guy_debtor_pool(x_role._guy_debtor_pool)
    for x_guyunit in x_listen._guys.values():
        x_guyunit.reset_listen_calculated_attrs()
    return x_listen
...
def get_default_work_agenda(duty: AgendaUnit) -> AgendaUnit:
    ...
    default_work_agenda._guy_credor_pool = None
    default_work_agenda._guy_debtor_pool = None
