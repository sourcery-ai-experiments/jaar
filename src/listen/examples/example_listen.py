...
def get_example_zia_speaker() -> AgendaUnit:
    ...
    zia_speaker.add_fact(factunit_shop(cook_text(), pledge=True), casa_road())
    zia_speaker.add_fact(factunit_shop(hungry_text()), eat_road())
    zia_speaker.add_fact(factunit_shop(full_text()), eat_road())
    yao_text = "Yao"
    zia_speaker.add_partyunit(yao_text, debtor_weight=12)
    cook_factunit = zia_speaker.get_fact_obj(cook_road())
    cook_factunit._assignedunit.set_suffidea(yao_text)
    zia_speaker.edit_fact_attr(
...
def get_example_bob_speaker() -> AgendaUnit:
    ...
    bob_speaker.add_fact(factunit_shop(cook_text(), pledge=True), casa_road())
    bob_speaker.add_fact(factunit_shop(hungry_text()), eat_road())
    bob_speaker.add_fact(factunit_shop(full_text()), eat_road())
    yao_text = "Yao"
    bob_speaker.add_partyunit(yao_text, debtor_weight=12)
    cook_factunit = bob_speaker.get_fact_obj(cook_road())
    cook_factunit._assignedunit.set_suffidea(yao_text)
    bob_speaker.edit_fact_attr(
        cook_road(), reason_base=eat_road(), reason_premise=hungry_road()
    )
