...
def test_FileHub_create_initial_atom_files_from_default_CorrectlySavesAtomUnitFiles(
    env_dir_setup_cleanup,
):
    # GIVEN
    sue_text = "Sue"
    ...
    init_atom_file_name = sue_filehub.atom_file_name(init_atom_id())
    init_atom_file_path = f"{sue_filehub.atoms_dir()}/{init_atom_file_name}"
    assert os_path_exists(init_atom_file_path) is False
    assert sue_filehub.duty_file_exists() is False

    # WHEN
    sue_filehub._create_initial_atom_files_from_default()
...
def test_FileHub_create_duty_from_atoms_CreatesDutyFileFromAtomFiles(
    env_dir_setup_cleanup,
):
    # GIVEN
    sue_text = "Sue"
    ...
    # WHEN
    sue_filehub._create_duty_from_atoms()

    # THEN
    assert sue_filehub.duty_file_exists()
    static_sue_duty = sue_filehub._merge_any_atoms(sue_filehub.default_duty_agenda())
    assert sue_filehub.get_duty_agenda().get_dict() == static_sue_duty.get_dict()


def test_FileHub_create_initial_atom_and_duty_files_CreatesAtomFilesAndDutyFile(
    env_dir_setup_cleanup,
):
    # GIVEN
    sue_text = "Sue"
    ...
    # WHEN
    sue_filehub._create_initial_atom_and_duty_files()
...
def test_FileHub_create_initial_atom_files_from_duty_SavesOnlyAtomFiles(
    env_dir_setup_cleanup,
):
    # GIVEN
    sue_text = "Sue"
    ...
    assert sue_filehub.duty_file_exists()
    init_atom_file_path = f"{sue_filehub.atoms_dir()}/{init_atom_id()}.json"
    assert os_path_exists(init_atom_file_path) is False

    # WHEN
    sue_filehub._create_initial_atom_files_from_duty()

    # THEN
    assert os_path_exists(init_atom_file_path)


def test_FileHub_initialize_atom_duty_files_CorrectlySavesDutyFileAndAtomFile(
    env_dir_setup_cleanup,
):
    # GIVEN
    sue_text = "Sue"
...
def test_FileHub_initialize_atom_duty_files_CorrectlySavesOnlyDutyFile(
    env_dir_setup_cleanup,
):
    # GIVEN
    sue_text = "Sue"
...
def test_FileHub_initialize_atom_duty_files_CorrectlySavesOnlyatomFile(
    env_dir_setup_cleanup,
):
    # GIVEN
    sue_text = "Sue"
...
def test_FileHub_append_atoms_to_duty_file_AddsatomsToDutyFile(
    env_dir_setup_cleanup,
):
    # GIVEN
    sue_text = "Sue"
    ...
    # WHEN
    new_agenda = sue_filehub.append_atoms_to_duty_file()
