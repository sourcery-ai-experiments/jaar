...
from src.listen.basis_truths import (
    create_empty_truth,
    create_listen_basis,
    get_default_live_truth,
)


def test_create_empty_truth_ReturnsCorrectObj():
    # GIVEN
    yao_text = "Yao"
    slash_text = "/"
    penny_float = 0.7
    yao_same = truthunit_shop(yao_text, _road_delimiter=slash_text, _penny=penny_float)
    ...
    # WHEN
    yao_empty_job = create_empty_truth(yao_same, x_owner_id=zia_text)
    ...
    yao_empty_job.calc_truth_metrics()
...
def test_create_listen_basis_ReturnsCorrectObj():
    ...
    yao_text = "Yao"
    slash_text = "/"
    yao_role = truthunit_shop(yao_text, _road_delimiter=slash_text)
    ...
    yao_basis_job.calc_truth_metrics()
...
def test_get_default_live_truth_ReturnsCorrectObj():
    # GIVEN
    sue_text = "Sue"
    blue_text = "blue"
    slash_text = "/"
    ...
    sue_truthunit = truthunit_shop(sue_text, blue_text, slash_text, five_pixel)
    sue_truthunit.set_last_atom_id(last_atom_id)
    sue_truthunit.add_otherunit(bob_text, 3, 4)
    swim_text = "/swimmers"
    swim_beliefunit = beliefunit_shop(swim_text, _road_delimiter=slash_text)
    swim_beliefunit.edit_otherlink(bob_text)
    sue_truthunit.set_beliefunit(swim_beliefunit)
    sue_truthunit.set_other_pool(sue_other_pool)
    sue_truthunit.add_l1_idea(ideaunit_shop(casa_text))
    sue_truthunit.set_max_tree_traverse(sue_max_tree_traverse)

    # WHEN
    default_live_truth = get_default_live_truth(sue_truthunit)

    # THEN
    default_live_truth.calc_truth_metrics()
    assert default_live_truth._owner_id == sue_truthunit._owner_id
    assert default_live_truth._owner_id == sue_text
    assert default_live_truth._real_id == sue_truthunit._real_id
    assert default_live_truth._real_id == blue_text
    assert default_live_truth._road_delimiter == slash_text
    assert default_live_truth._pixel == five_pixel
    assert default_live_truth._other_credor_pool is None
    assert default_live_truth._other_debtor_pool is None
    assert default_live_truth._max_tree_traverse == sue_max_tree_traverse
    assert len(default_live_truth.get_others_dict()) == 1
    assert len(default_live_truth.get_beliefunits_dict()) == 1
    assert len(default_live_truth._idea_dict) == 1
