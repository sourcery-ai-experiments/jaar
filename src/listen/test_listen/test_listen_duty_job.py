...
def get_example_yao_think_with_3_healers():
    yao_think = get_example_yao_world()
    yao_text = yao_think.get_char("Yao").char_id
    bob_text = yao_think.get_char("Bob").char_id
    zia_text = yao_think.get_char("Zia").char_id
    ...
    yao_think.add_idea(iowa_idea, get_usa_road())
    yao_think.add_idea(ohio_idea, get_usa_road())
    yao_think.add_idea(utah_idea, get_usa_road())
...
def test_listen_to_owner_jobs_Pipeline_Scenario0(env_dir_setup_cleanup):
    # GIVEN
    # yao0_think with 3 debotors of different credor_weights
    ...
    yao_think0 = get_example_yao_think_with_3_healers()
    yao_think0.del_idea_obj(run_road())
    yao_think0.add_l1_idea(ideaunit_shop(get_location_text()))
    yao_think0.add_idea(ideaunit_shop(get_in_ocean_text()), get_location_road())
    yao_think0.add_idea(ideaunit_shop(get_on_land_text()), get_location_road())
    yao_think0.add_l1_idea(ideaunit_shop(get_swim_text(), pledge=True))
    yao_think0.edit_reason(get_swim_road(), get_location_road(), get_in_ocean_road())
    yao_think0.calc_world_metrics()
    assert yao_think0._econ_dict.get(get_iowa_road())
    assert yao_think0._econ_dict.get(get_ohio_road())
    assert yao_think0._econ_dict.get(get_utah_road())
    assert len(yao_think0._econ_dict) == 3
    print(f"{yao_think0._idea_dict.keys()=}")

    yao_text = yao_think0._owner_id
    ...
    assert yao_iowa_hubunit.think_file_exists() is False
    ...
    assert yao_action._chars.keys() == yao_think0._chars.keys()
    assert yao_action.get_char(yao_text)._irrational_debtor_weight == 0
    assert yao_action.get_beliefunits_dict() == yao_think0.get_beliefunits_dict()
    ...
    assert yao_action != yao_think0


def test_listen_to_owner_jobs_Pipeline_Scenario1_yao_think_CanOnlyReferenceItself(
    env_dir_setup_cleanup,
):
    # GIVEN
    # yao0_think with 3 debotors of different credor_weights
    ...
    yao_think0.add_l1_idea(ideaunit_shop(get_swim_text(), pledge=True))
    yao_think0.edit_reason(get_swim_road(), get_location_road(), get_in_ocean_road())
    yao_think0.set_fact(get_location_road(), get_in_ocean_road())
    print(f"{yao_think0.get_fact(get_location_road())=}")
    yao_think0.del_idea_obj(run_road())
    assert yao_think0._econ_dict.get(get_iowa_road())
    assert yao_think0._econ_dict.get(get_ohio_road())
    assert yao_think0._econ_dict.get(get_utah_road())
