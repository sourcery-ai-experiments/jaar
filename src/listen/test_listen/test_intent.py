...
def test_listen_to_speaker_intent_RaisesErrorIfPoolIsNotSet():
    ...
    assert (
        str(excinfo.value)
        == f"listener '{yao_text}' agenda is assumed to have {zia_agendaunit._owner_id} otherunit."
    )
...
def test_listen_to_speaker_intent_ReturnsEqualAgenda():
    ...
    yao_agendaunit = agendaunit_shop(yao_text)
    zia_text = "Zia"
    yao_agendaunit.add_otherunit(zia_text)
    yao_agendaunit.set_other_pool(100)
    zia_agendaunit = agendaunit_shop(zia_text)
...
def test_listen_to_speaker_intent_ReturnsSingleTaskAgenda():
    ...
    zia_text = "Zia"
    before_yao_agendaunit.add_otherunit(zia_text)
    yao_other_debtor_weight = 77
    before_yao_agendaunit.set_other_pool(yao_other_debtor_weight)
    clean_text = "clean"
    zia_clean_ideaunit = ideaunit_shop(clean_text, pledge=True)
    zia_clean_ideaunit._assignedunit.set_suffbelief(yao_text)
    zia_agendaunit = agendaunit_shop(zia_text)
    zia_agendaunit.add_otherunit(yao_text)
...
def test_listen_to_speaker_intent_ReturnsLevel2TaskAgenda():
    ...
    before_yao_agendaunit.set_other_pool(yao_debtor_weight)
    zia_agendaunit = agendaunit_shop(zia_text)
    zia_agendaunit.add_otherunit(yao_text)
    clean_text = "clean"
    zia_clean_ideaunit = ideaunit_shop(clean_text, pledge=True)
