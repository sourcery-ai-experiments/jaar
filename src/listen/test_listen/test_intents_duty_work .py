...
def test_listen_to_intents_duty_work_AddsTasksToAgendaWhenNo_suffideaIsSet(
    ...
    zia_work.add_oath(oathunit_shop(clean_text(), pledge=True), casa_road())
    zia_work.add_oath(oathunit_shop(cook_text(), pledge=True), casa_road())
    ...
    # WHEN
    print(f"{len(new_yao_work.get_oath_dict())=}")
...
def test_listen_to_intents_duty_work_AddsTasksToAgenda(env_dir_setup_cleanup):
    ...
    zia_work.add_oath(oathunit_shop(cook_text(), pledge=True), casa_road())
    zia_work.add_partyunit(yao_text, debtor_weight=12)
    clean_oathunit = zia_work.get_oath_obj(clean_road())
    cook_oathunit = zia_work.get_oath_obj(cook_road())
...
def test_listen_to_intents_duty_work_AddsTasksToAgendaWithDetailsDecidedBy_debtor_weight(
    ...
    bob_work.edit_oath_attr(
        road=cook_road(),
        reason_del_premise_base=eat_road(),
        reason_del_premise_need=hungry_road(),
    )
    bob_cook_oathunit = bob_work.get_oath_obj(cook_road())
    zia_cook_oathunit = zia_work.get_oath_obj(cook_road())
...
def test_listen_to_intents_duty_work_ProcessesIrrationalAgenda(env_dir_setup_cleanup):
    ...
    sue_work.add_l1_oath(oathunit_shop(vacuum_text, pledge=True))
    vacuum_oathunit = sue_work.get_oath_obj(vacuum_road)
    vacuum_oathunit._assignedunit.set_suffidea(yao_text)

    egg_text = "egg first"
    egg_road = sue_work.make_l1_road(egg_text)
    sue_work.add_l1_oath(oathunit_shop(egg_text))
    chicken_text = "chicken first"
    chicken_road = sue_work.make_l1_road(chicken_text)
    sue_work.add_l1_oath(oathunit_shop(chicken_text))
    # set egg pledge is True when chicken first is False
    sue_work.edit_oath_attr(
        road=egg_road,
        pledge=True,
...
def test_listen_to_intents_duty_work_ListensToOwner_duty_AndNotOwner_work(
    ...
    yao_old_work.add_l1_oath(oathunit_shop(vacuum_text, pledge=True))
    vacuum_oathunit = yao_old_work.get_oath_obj(vacuum_road)
    vacuum_oathunit._assignedunit.set_suffidea(yao_text)
