...
def test_listen_to_agendas_same_live_AddsTasksToWorldWhenNo_suffbeliefIsSet(
    ...
    zia_pool = 87
    yao_same.add_charunit(zia_text, zia_credor_weight, zia_debtor_weight)
    yao_same.set_char_pool(zia_pool)
    ...
    zia_live.add_charunit(yao_text, debtor_weight=12)
...
def test_listen_to_agendas_same_live_AddsTasksToWorldWithDetailsDecidedBy_debtor_weight(
    ...
    yao_same.add_charunit(zia_text, None, yao_zia_debtor_weight)
    yao_same.add_charunit(bob_text, None, yao_bob_debtor_weight)
    yao_same.set_char_pool(100)
...
def test_listen_to_agendas_same_live_ProcessesIrrationalWorld(env_dir_setup_cleanup):
    ...
    yao_same.add_charunit(zia_text, zia_credor_weight, zia_debtor_weight)
    yao_same.add_charunit(sue_text, sue_credor_weight, sue_debtor_weight)
    yao_pool = 92
    yao_same.set_char_pool(yao_pool)
    yao_userhub = userhub_shop(env_dir(), None, yao_text)
    yao_userhub.save_same_world(yao_same)
    ...
    assert len(new_yao_live.get_agenda_dict()) == 2
    zia_charunit = new_yao_live.get_char(zia_text)
    sue_charunit = new_yao_live.get_char(sue_text)
...
def test_listen_to_agendas_same_live_ListensToOwner_same_AndNotOwner_live(
    ...
    yao_same.add_charunit(yao_text, yao_credor_weight, yao_debtor_weight)
    zia_text = "Zia"
    zia_credor_weight = 47
    zia_debtor_weight = 41
    yao_same.add_charunit(zia_text, zia_credor_weight, zia_debtor_weight)
