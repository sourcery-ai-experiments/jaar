...
from src.listen.listen import create_listen_basis, listen_to_agendas_mind_being
...
def test_listen_to_agendas_mind_being_AddsTasksToWorldWhenNo_heldbeliefIsSet(
    env_dir_setup_cleanup,
):
    # GIVEN
    yao_text = "Yao"
    yao_mind = worldunit_shop(yao_text)
    zia_text = "Zia"
    zia_credor_weight = 47
    zia_debtor_weight = 41
    zia_pool = 87
    yao_mind.add_charunit(zia_text, zia_credor_weight, zia_debtor_weight)
    yao_mind.set_char_pool(zia_pool)
    yao_hubunit = hubunit_shop(env_dir(), None, yao_text)
    yao_hubunit.save_mind_world(yao_mind)
    ...
    new_yao_being = create_listen_basis(yao_mind)
    assert len(new_yao_being.get_agenda_dict()) == 0

    # WHEN
    print(f"{len(new_yao_being.get_idea_dict())=}")
    listen_to_agendas_mind_being(new_yao_being, yao_hubunit)

    # THEN
    assert len(new_yao_being.get_agenda_dict()) == 2


def test_listen_to_agendas_mind_being_AddsTasksToWorld(env_dir_setup_cleanup):
    # GIVEN
    yao_text = "Yao"
    yao_mind = worldunit_shop(yao_text)
    zia_text = "Zia"
...
def test_listen_to_agendas_mind_being_AddsTasksToWorldWithDetailsDecidedBy_debtor_weight(
    env_dir_setup_cleanup,
):
    # GIVEN
    ...
    yao_mind = get_example_yao_speaker()
    yao_text = yao_mind._owner_id
    yao_hubunit = hubunit_shop(env_dir(), None, yao_text)
    yao_hubunit.save_mind_world(yao_mind)

    new_yao_being1 = create_listen_basis(yao_mind)
    assert new_yao_being1.idea_exists(cook_road()) is False

    # WHEN
    listen_to_agendas_mind_being(new_yao_being1, yao_hubunit)
    ...
    yao_mind.add_charunit(zia_text, None, yao_zia_debtor_weight)
    yao_mind.add_charunit(bob_text, None, yao_bob_debtor_weight)
    yao_mind.set_char_pool(100)
    new_yao_being2 = create_listen_basis(yao_mind)
    assert new_yao_being2.idea_exists(cook_road()) is False

    # WHEN
    listen_to_agendas_mind_being(new_yao_being2, yao_hubunit)
...
def test_listen_to_agendas_mind_being_ProcessesIrrationalWorld(env_dir_setup_cleanup):
    # GIVEN
    yao_text = "Yao"
    yao_mind = worldunit_shop(yao_text)
    zia_text = "Zia"
    ...
    yao_mind.add_charunit(zia_text, zia_credor_weight, zia_debtor_weight)
    yao_mind.add_charunit(sue_text, sue_credor_weight, sue_debtor_weight)
    yao_pool = 92
    yao_mind.set_char_pool(yao_pool)
    yao_hubunit = hubunit_shop(env_dir(), None, yao_text)
    yao_hubunit.save_mind_world(yao_mind)
...
def test_listen_to_agendas_mind_being_ProcessesMissingDebtorWorld(
    env_dir_setup_cleanup,
):
    # GIVEN
    yao_text = "Yao"
    yao_hubunit = hubunit_shop(env_dir(), None, yao_text)
    delete_dir(yao_hubunit.mind_file_path())  # don't know why I have to do this...
    print(f"{os_path_exists(yao_hubunit.mind_file_path())=}")
    yao_mind = worldunit_shop(yao_text)
    zia_text = "Zia"
    sue_text = "Sue"
    zia_credor_weight = 47
...
def test_listen_to_agendas_mind_being_ListensToOwner_mind_AndNotOwner_being(
    env_dir_setup_cleanup,
):
    # GIVEN
    yao_text = "Yao"
    ...
    yao_mind.add_charunit(yao_text, yao_credor_weight, yao_debtor_weight)
    zia_text = "Zia"
    zia_credor_weight = 47
    zia_debtor_weight = 41
    yao_mind.add_charunit(zia_text, zia_credor_weight, zia_debtor_weight)
