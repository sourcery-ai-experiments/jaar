...
def get_example_yao_agenda() -> AgendaUnit:
    ...
    yao_speaker.add_fact(factunit_shop(run_text()), casa_road())
...
def get_example_yao_job1_speaker() -> AgendaUnit:
    ...
    yao_speaker.del_fact_obj(run_road())
    yao_speaker.set_party_pool(40)
    yao_speaker.add_fact(factunit_shop(cook_text(), pledge=True), casa_road())
    yao_speaker.add_fact(factunit_shop(hungry_text()), eat_road())
    yao_speaker.add_fact(factunit_shop(full_text()), eat_road())
    cook_factunit = yao_speaker.get_fact_obj(cook_road())
...
def get_example_yao_job2_speaker() -> AgendaUnit:
    ...
    yao_speaker.add_fact(factunit_shop(dirty_text()), sanitation_road())
    yao_speaker.add_fact(factunit_shop(clean_text()), sanitation_road())
...
def get_example_yao_duty_with_3_healers():
    ...
    iowa_fact = factunit_shop(get_iowa_text(), _problem_bool=True)
    ohio_fact = factunit_shop(get_ohio_text(), _problem_bool=True)
    utah_fact = factunit_shop(get_utah_text(), _problem_bool=True)
    iowa_fact._healerhold.set_idea_id(get_yao_iowa_userhub().person_id)
    ohio_fact._healerhold.set_idea_id(get_yao_ohio_userhub().person_id)
    utah_fact._healerhold.set_idea_id(get_zia_utah_userhub().person_id)
    yao_duty.add_fact(iowa_fact, get_usa_road())
    yao_duty.add_fact(ohio_fact, get_usa_road())
    yao_duty.add_fact(utah_fact, get_usa_road())
...
def test_listen_to_person_jobs_Pipeline_Scenario0(env_dir_setup_cleanup):
    ...
    yao_duty0.del_fact_obj(run_road())
    yao_duty0.add_l1_fact(factunit_shop(get_location_text()))
    yao_duty0.add_fact(factunit_shop(get_in_ocean_text()), get_location_road())
    yao_duty0.add_fact(factunit_shop(get_on_land_text()), get_location_road())
    yao_duty0.add_l1_fact(factunit_shop(get_swim_text(), pledge=True))
    ...
    print(f"{yao_duty0._fact_dict.keys()=}")
    ...
    assert len(yao_work._fact_dict) == 10
    print(f"{yao_work._fact_dict.keys()=}")
    print(f"{yao_work.get_beliefunits_dict().keys()=}")
    assert yao_work.fact_exists(cook_road())
    assert yao_work.fact_exists(clean_road())
    assert yao_work.fact_exists(run_road()) is False
    assert len(yao_work._factroot._beliefunits) == 2
...
def test_listen_to_person_jobs_Pipeline_Scenario1_yao_duty_CanOnlyReferenceItself(
    ...
    assert yao_work.get_party(yao_text)._irrational_debtor_weight == 0
    assert yao_work.get_ideaunits_dict() == yao_duty0.get_ideaunits_dict()
    assert len(yao_work._fact_dict) == 4
    print(f"{yao_work._fact_dict.keys()=}")
    print(f"{yao_work.get_beliefunits_dict().keys()=}")
    assert yao_work.fact_exists(cook_road()) is False
    assert yao_work.fact_exists(clean_road()) is False
    assert yao_work.fact_exists(run_road()) is False
    assert yao_work.fact_exists(get_swim_road())
    assert yao_work.fact_exists(get_in_ocean_road())
    assert yao_work.fact_exists(get_on_land_road()) is False
    ...
    assert len(yao_work._factroot._beliefunits) == 1
    assert yao_work != yao_duty0
