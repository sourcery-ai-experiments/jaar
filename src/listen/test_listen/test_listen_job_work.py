...
def get_example_yao_agenda() -> AgendaUnit:
    ...
    yao_speaker.add_guyunit(yao_text, debtor_weight=10)
    yao_speaker.add_guyunit(zia_text, debtor_weight=30)
    yao_speaker.add_guyunit(bob_text, debtor_weight=40)
    yao_speaker.set_guy_pool(80)
    return yao_speaker
...
def get_example_yao_duty_with_3_healers():
    ...
    yao_text = yao_duty.get_guy("Yao").guy_id
    bob_text = yao_duty.get_guy("Bob").guy_id
    zia_text = yao_duty.get_guy("Zia").guy_id
...
def test_listen_to_person_jobs_Pipeline_Scenario0(env_dir_setup_cleanup):
    ...
    assert yao_work._guys.keys() == yao_duty0._guys.keys()
    assert yao_work.get_guy(yao_text)._irrational_debtor_weight == 0
