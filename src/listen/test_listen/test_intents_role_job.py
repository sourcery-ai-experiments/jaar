...
def test_listen_to_intent_role_job_intent_AddsTasksToJobAgendaWhenNo_suffideaIsSet(
    env_dir_setup_cleanup,
):
    # GIVEN
    yao_text = "Yao"
    ...
    zia_job.add_fact(factunit_shop(clean_text(), pledge=True), casa_road())
    zia_job.add_fact(factunit_shop(cook_text(), pledge=True), casa_road())
    ...
    # WHEN
    print(f"{len(new_yao_job.get_fact_dict())=}")
...
def test_listen_to_intent_role_job_intent_AddsTasksToJobAgenda(env_dir_setup_cleanup):
    ...
    zia_job.add_fact(factunit_shop(cook_text(), pledge=True), casa_road())
    zia_job.add_partyunit(yao_text, debtor_weight=12)
    clean_factunit = zia_job.get_fact_obj(clean_road())
    cook_factunit = zia_job.get_fact_obj(cook_road())
    clean_factunit._assignedunit.set_suffidea(yao_text)
    cook_factunit._assignedunit.set_suffidea(yao_text)
    yao_dakota_userhub = userhub_shop(env_dir(), None, yao_text, get_dakota_road())
    yao_dakota_userhub.save_job_agenda(zia_job)
...
def test_listen_to_intent_role_job_intent_AddsTasksToJobAgendaWithDetailsDecidedBy_debtor_weight(
    ...
    bob_job.edit_fact_attr(
        road=cook_road(),
        reason_del_premise_base=eat_road(),
        reason_del_premise_need=hungry_road(),
    )
    bob_cook_factunit = bob_job.get_fact_obj(cook_road())
    zia_cook_factunit = zia_job.get_fact_obj(cook_road())
    assert bob_cook_factunit != zia_cook_factunit
    assert len(zia_cook_factunit._reasonunits) == 1
    assert len(bob_cook_factunit._reasonunits) == 0
    ...
    assert new_yao_work1.fact_exists(cook_road()) is False

    # WHEN
    listen_to_intents_role_job(new_yao_work1, sue_dakota_userhub)

    # THEN
    assert new_yao_work1.fact_exists(cook_road())
    new_cook_fact = new_yao_work1.get_fact_obj(cook_road())
    ...
    assert new_cook_fact.get_reasonunit(eat_road()) is None
    ...
    assert new_yao_work2.fact_exists(cook_road()) is False

    # WHEN
    listen_to_intents_role_job(new_yao_work2, sue_dakota_userhub)

    # THEN
    assert new_yao_work2.fact_exists(cook_road())
    new_cook_fact = new_yao_work2.get_fact_obj(cook_road())
    zia_partyunit = new_yao_work2.get_party(zia_text)
    bob_partyunit = new_yao_work2.get_party(bob_text)
    assert zia_partyunit.debtor_weight > bob_partyunit.debtor_weight
    zia_eat_reasonunit = zia_cook_factunit.get_reasonunit(eat_road())
    assert new_cook_fact.get_reasonunit(eat_road()) == zia_eat_reasonunit
...
def test_listen_to_intent_role_job_intent_ProcessesIrrationalAgenda(
    ...
    sue_job.add_l1_fact(factunit_shop(vacuum_text, pledge=True))
    vacuum_factunit = sue_job.get_fact_obj(vacuum_road)
    vacuum_factunit._assignedunit.set_suffidea(yao_text)

    egg_text = "egg first"
    egg_road = sue_job.make_l1_road(egg_text)
    sue_job.add_l1_fact(factunit_shop(egg_text))
    chicken_text = "chicken first"
    chicken_road = sue_job.make_l1_road(chicken_text)
    sue_job.add_l1_fact(factunit_shop(chicken_text))
    # set egg pledge is True when chicken first is False
    sue_job.edit_fact_attr(
        road=egg_road,
        pledge=True,
...
def test_listen_to_intent_role_job_intent_ListensToOwner_role_AndNotOwner_job(
    ...
    yao_old_job.add_l1_fact(factunit_shop(vacuum_text, pledge=True))
    vacuum_factunit = yao_old_job.get_fact_obj(vacuum_road)
    vacuum_factunit._assignedunit.set_suffidea(yao_text)
...
def test_listen_to_intent_role_job_intent_GetsIntentFromSrcAgendaNotSpeakerSelf(
    ...
    yao_role = get_example_yao_speaker()
    assert yao_role.fact_exists(run_road()) is False
    assert yao_role.fact_exists(clean_road()) is False
    yao_role.add_fact(factunit_shop(run_text(), pledge=True), casa_road())
    sue_dakota_userhub = get_dakota_userhub()
    sue_dakota_userhub.save_role_agenda(yao_role)

    yao_old_job = get_example_yao_speaker()
    assert yao_old_job.fact_exists(run_road()) is False
    assert yao_old_job.fact_exists(clean_road()) is False
    yao_old_job.add_fact(factunit_shop(clean_text(), pledge=True), casa_road())
    sue_dakota_userhub.save_job_agenda(yao_old_job)

    yao_new_job = create_listen_basis(yao_role)
    assert yao_new_job.fact_exists(run_road()) is False
    assert yao_new_job.fact_exists(clean_road()) is False
    ...
    # THEN
    assert yao_new_job.fact_exists(clean_road()) is False
    assert yao_new_job.fact_exists(run_road())
