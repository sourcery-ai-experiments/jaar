...
def test_add_same_pledge_Addspledgeatom(env_dir_setup_cleanup):
    ...
    old_sue_same = sue_userhub.get_same_agenda()
    clean_text = "clean"
    ...
    new_sue_same = sue_userhub.get_same_agenda()
    assert new_sue_same.idea_exists(clean_road)


def test_add_same_pledge_SetsSameAgendapledgeIdea_suffbelief(env_dir_setup_cleanup):
    # GIVEN
    sue_text = "Sue"
    sue_userhub = userhub_shop(env_dir(), root_label(), sue_text)
    sue_userhub.initialize_atom_same_files()
