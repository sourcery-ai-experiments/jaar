from src.calendar.member import MemberName, memberlink_shop
...
def test_groupunit_exists():
    ...
    swimmers_group = groupunit_shop(
        name=swimmers,
        _calendar_credit=0.33,
        _calendar_debt=0.44,
        _calendar_agenda_credit=0.66,
        _calendar_agenda_debt=0.77,
    ...
    assert swimmers_group._calendar_credit != None
    assert swimmers_group._calendar_debt != None
    assert swimmers_group._calendar_agenda_credit != None
    assert swimmers_group._calendar_agenda_debt != None
...
def test_Groupunit_reset_calendar_importance_WorkCorrectly():
    # GIVEN
    maria_name = "maria"
    maria_group = groupunit_shop(
        ...
        _calendar_credit=0.33,
        _calendar_debt=0.44,
        _calendar_agenda_credit=0.13,
        _calendar_agenda_debt=0.23,
    )
    print(f"{maria_group}")
    assert maria_group._calendar_credit == 0.33
    assert maria_group._calendar_debt == 0.44
    assert maria_group._calendar_agenda_credit == 0.13
    assert maria_group._calendar_agenda_debt == 0.23

    # WHEN
    maria_group.reset_calendar_credit_debt()

    # THEN
    assert maria_group._calendar_credit == 0
    assert maria_group._calendar_debt == 0
    assert maria_group._calendar_agenda_credit == 0
    assert maria_group._calendar_agenda_debt == 0


def test_Groupunit_reset_calendar_importance_reset_memberlinks():
    # GIVEN
    todd_text = "Todd"
    mery_text = "Merry"
    todd_member = memberlink_shop(
        name=todd_text,
        _calendar_credit=0.13,
        _calendar_debt=0.7,
        _calendar_agenda_credit=0.53,
        _calendar_agenda_debt=0.77,
    )
    mery_member = memberlink_shop(
        name=mery_text,
        _calendar_credit=0.23,
        _calendar_debt=0.5,
        _calendar_agenda_credit=0.54,
        _calendar_agenda_debt=0.57,
    )
    ...
    bikers_group = groupunit_shop(
        ...
        _calendar_credit=0.33,
        _calendar_debt=0.44,
        _calendar_agenda_credit=0.1,
        _calendar_agenda_debt=0.2,
    )
    ...
    assert biker_memberlink_todd._calendar_credit == 0.13
    assert biker_memberlink_todd._calendar_debt == 0.7
    assert biker_memberlink_todd._calendar_agenda_credit == 0.53
    assert biker_memberlink_todd._calendar_agenda_debt == 0.77

    biker_memberlink_mery = bikers_group._members.get(mery_text)
    assert biker_memberlink_mery._calendar_credit == 0.23
    assert biker_memberlink_mery._calendar_debt == 0.5
    assert biker_memberlink_mery._calendar_agenda_credit == 0.54
    assert biker_memberlink_mery._calendar_agenda_debt == 0.57

    # WHEN
    bikers_group.reset_calendar_credit_debt()

    # THEN
    assert biker_memberlink_todd._calendar_credit == 0
    assert biker_memberlink_todd._calendar_debt == 0
    assert biker_memberlink_todd._calendar_agenda_credit == 0
    assert biker_memberlink_todd._calendar_agenda_debt == 0
    assert biker_memberlink_mery._calendar_credit == 0
    assert biker_memberlink_mery._calendar_debt == 0
    assert biker_memberlink_mery._calendar_agenda_credit == 0
    assert biker_memberlink_mery._calendar_agenda_debt == 0
...
def test_brankLink_set_calendar_importanceCorrectly():
    # GIVEN
    bikers_name = GroupName("bikers")
    bikers_creditor_weight = 3.0
    ...
    # WHEN
    group_heir_x.set_calendar_credit_debt(
        idea_calendar_importance=idea_calendar_importance,
        groupheirs_creditor_weight_sum=grouplinks_sum_creditor_weight,
        groupheirs_debtor_weight_sum=grouplinks_sum_debtor_weight,
    )

    # THEN
    assert group_heir_x._calendar_credit == 0.05
    assert group_heir_x._calendar_debt == 0.1
...
def test_Groupline_exists():
    ...
    groupline_x = Groupline(
        name=bikers_name, _calendar_credit=0.33, _calendar_debt=0.55
    )
    assert groupline_x.name == bikers_name
    assert groupline_x._calendar_credit == 0.33
    assert groupline_x._calendar_debt == 0.55

    # WHEN
    groupline_x.add_calendar_credit_debt(calendar_credit=0.11, calendar_debt=0.2)

    # THEN
    assert groupline_x._calendar_credit == 0.44
    assert groupline_x._calendar_debt == 0.75
