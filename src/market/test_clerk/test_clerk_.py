...
def test_ClerkUnit_exists(clerk_dir_setup_cleanup):
    ...
    assert x_clerk._market_id is None
...
def test_clerkunit_shop_exists(clerk_dir_setup_cleanup):
    ...
    assert x_clerk._market_id != None
...
def test_clerkunit_auto_output_to_forum_SavesAgendaToForumDirWhenTrue(
    ...
    # GIVEN
    env_dir = get_temp_clerkunit_dir()
    x_market_id = get_temp_market_id()
    tim_text = "Tim"
    forum_text = "forum"
...
def test_clerkunit_auto_output_to_forum_DoesNotSaveAgendaToForumDirWhenFalse(
    ...
    print(f"{forum_file_path=}")
    # forum_file_path = f"src/market/examples/ex_env/agendas/{forum_file_name}"
    x_clerk = clerkunit_shop(tim_text, env_dir, x_market_id, False)
    x_clerk.create_core_dir_and_files()
    assert os_path.exists(forum_file_path) is False
