...
def test_market_set_agenda_bank_attrs_CorrectlyPopulatesAgenda_partylinks(
    ...
    tom_agenda = agendaunit_shop(tom_text, x_market_id)
    ava_agenda = agendaunit_shop(ava_text, x_market_id)

    swim_text = "swimming"
    sports_text = "sports"
    sal_sports_road = create_road(x_market_id, sports_text)
    bob_sports_road = create_road(x_market_id, sports_text)
    tom_sports_road = create_road(x_market_id, sports_text)
...
def test_market_get_agenda_ideaunit_table_insert_sqlstr_CorrectlyPopulatesTable01(
    env_dir_setup_cleanup,
):
    # GIVEN
