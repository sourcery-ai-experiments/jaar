...
from src.market.clerk import clerkunit_shop, ClerkUnit
from src.market.examples.clerk_env_kit import get_temp_market_id
...
def get_2node_agenda(market_id: MarketID = None) -> AgendaUnit:
    if market_id is None:
        market_id = get_temp_market_id()
    a_text = "A"
    b_text = "B"
    x_agenda = agendaunit_shop(_agent_id=a_text)
    x_agenda.set_market_id(market_id)
    idea_b = ideaunit_shop(b_text)
    x_agenda.add_idea(idea_b, parent_road=get_temp_market_id())
    x_agenda.set_agenda_metrics()
    return x_agenda
...
def get_clerkunit_2agenda(env_dir, market_id) -> ClerkUnit:
    yao_text = "Xio"
    yao_clerkunit = clerkunit_shop(yao_text, env_dir, market_id)
