...
def test_market_ChangingOneHealersBeliefChangesAnotherAgenda(env_dir_setup_cleanup):
    ...
    casa_road = x_market.build_market_road(casa_text)
    ...
    # print(f"{x_market.get_forum_agenda(amer_text)._idearoot._beliefunits.keys()=}")
    amer_output = x_market.get_forum_agenda(amer_text)

    # create assignment for Cali
    cali_text = "Cali"
    x_market.create_new_clerkunit(clerk_id=cali_text)
    cali_clerk = x_market.get_clerkunit(cid=cali_text)
    cali_clerk.set_depot_agenda(amer_output, "assignment")
    old_cali_agenda = x_market.get_output_agenda(cali_text)
    ...
    # new_forum_amer = x_market.get_forum_agenda(amer_text)
...
def test_market_clerk_MeldOrderChangesOutputBelief(env_dir_setup_cleanup):
    ...
    beto_text = "Beto"
    x_market.create_new_clerkunit(clerk_id=amer_text)
    x_market.create_new_clerkunit(clerk_id=beto_text)
    amer_clerk = x_market.get_clerkunit(cid=amer_text)
    ...
    casa_text = "casa"
    casa_road = create_road(x_market.market_id, casa_text)
    basket_text = "laundry basket status"
    basket_road = create_road(casa_road, basket_text)
    b_full_text = "full"
    b_full_road = create_road(basket_road, b_full_text)
    ...
    amer_output = x_market.get_forum_agenda(amer_text)
    beto_output = x_market.get_forum_agenda(beto_text)

    cali_text = "Cali"
    x_market.create_new_clerkunit(cali_text)
    cali_kichen = x_market.get_clerkunit(cali_text)
    ...
    # THEN
    old_cali_output = x_market.get_forum_agenda(cali_text)
    ...
    new_cali_output = x_market.get_forum_agenda(cali_text)
