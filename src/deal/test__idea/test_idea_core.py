from src.deal.idea import IdeaCore
from src.deal.group import GroupBrand, balancelink_shop, balanceheir_shop
...
def test_idea_core_exists():
    ...
    assert new_obj._deal_importance is None
    assert new_obj._deal_coin_onset is None
    assert new_obj._deal_coin_cease is None
...
def test_idea_core_get_inherited_balanceheirs_weight_sum_WorksCorrectlyWithValues():
    ...
    assert swimmer_balanceheir._deal_credit is None
    assert swimmer_balanceheir._deal_debt is None
    biker_balanceheir = idea_core._balanceheirs.get(biker_text)
    assert biker_balanceheir._deal_credit is None
    assert biker_balanceheir._deal_debt is None

    # WHEN
    idea_core._deal_importance = 0.25
    idea_core.set_balanceheirs_deal_credit_debit()

    # THEN
    print(f"{len(idea_core._balanceheirs)=}")
    swimmer_balanceheir = idea_core._balanceheirs.get(swimmer_text)
    assert swimmer_balanceheir._deal_credit != None
    assert swimmer_balanceheir._deal_debt != None
    biker_balanceheir = idea_core._balanceheirs.get(biker_text)
    assert biker_balanceheir._deal_credit != None
    assert biker_balanceheir._deal_debt != None
...
def test_idea_core_set_requiredheirsCorrectlyTakesFromOutside():
    ...
    idea.set_requiredheirs(requiredheirs=requiredheirs, deal_idea_dict=None)
...
def test_idea_core_set_requiredheirsCorrectlyTakesFromSelf():
    ...
    assert idea._requiredheirs is None
    idea.set_requiredheirs(requiredheirs=None, deal_idea_dict=None)
    assert idea._requiredheirs == requiredheirs
...
def test_idea_get_requiredheir_correctlyReturnsRequiredHeir():
    ...
    idea_x.set_requiredheirs(requiredheirs=required_heirs_x, deal_idea_dict=None)
...
def test_idea_set_assignedunit_empty_if_null():
    ...
    # WHEN
    idea_x.set_assignedheir(parent_assignheir=None, deal_groups=None)
    ...
    assigned_heir_x.set_suffgroups(
        assignunit=assigned_unit_x, parent_assignheir=None, deal_groups=None
    )
