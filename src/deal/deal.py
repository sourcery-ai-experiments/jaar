...
from src.deal.required_idea import (
    AcptFactCore,
    AcptFactUnit,
    AcptFactUnit,
    RequiredHeir,
...
from src.deal.x_func import x_get_json
from src.deal.idea import IdeaCore, IdeaKid, IdeaRoot, IdeaAttrHolder
...
from src.deal.road import (
    ...
    get_ancestor_roads,
    get_default_project_root_label as root_label,
    get_road_from_nodes,
    get_all_road_nodes,
    get_forefather_roads,
)
from src.deal.origin import originunit_get_from_dict, originunit_shop, OriginUnit
...
class DealUnit:
    ...
    def set_project_handle(self, project_handle: str):
        ...
        self.edit_idea_label(
            old_road=old_project_handle, new_label=self._project_handle
        )
    ...
    def get_deal_sprung_from_single_idea(self, road: Road):
        self.set_deal_metrics()
        idea_x = self.get_idea_kid(road=road)
        new_weight = self._weight * idea_x._deal_importance
    ...
    def _are_all_partys_groups_are_in_idea_kid(self, road: Road) -> bool:
        ...
        # check all deal._partys are in balanceheir_partyunits
    ...
    def get_time_c400_from_min(self, min: int) -> int:
        c400_idea = self.get_idea_kid(
            f"{self._project_handle},time,tech,400 year cycle"
        )
    ...
    def get_time_c400yr_from_min(self, min: int):
        ...
        if c100_4_96y._close - c100_4_96y._begin in (
            ...
            yr4_1461 = self.get_idea_kid(
                f"{self._project_handle},time,tech,4year with leap"
            )
            ...
            yr4_1460 = self.get_idea_kid(
                f"{self._project_handle},time,tech,4year wo leap"
            )
            yr4_cycles = 0
    ...
    def get_time_month_from_min(self, min: int):
        ...
        if yr1_idea._close - yr1_idea._begin == 525600:
            yrx = self.get_idea_kid(f"{self._project_handle},time,tech,365 year")
        elif yr1_idea._close - yr1_idea._begin == 527040:
            yrx = self.get_idea_kid(f"{self._project_handle},time,tech,366 year")
        ...
        day_x = self.get_idea_kid(f"{self._project_handle},time,tech,day")
    ...
    def _get_jajatime_week_legible_text(self, open: int, divisor: int) -> str:
        ...
        week_road = f"{self._project_handle},time,tech,week"
    ...
    def add_to_group_deal_credit_debt(
        self,
        groupbrand: GroupBrand,
        balanceheir_deal_credit: float,
        balanceheir_deal_debt: float,
    ):
        for group in self._groups.values():
            if group.brand == groupbrand:
                group.set_empty_deal_credit_debt_to_zero()
                group._deal_credit += balanceheir_deal_credit
                group._deal_debt += balanceheir_deal_debt

    def add_to_group_deal_agenda_credit_debt(
        self,
        groupbrand: GroupBrand,
        balanceline_deal_credit: float,
        balanceline_deal_debt: float,
    ):
        for group in self._groups.values():
            if (
                group.brand == groupbrand
                and balanceline_deal_credit != None
                and balanceline_deal_debt != None
            ):
                group.set_empty_deal_credit_debt_to_zero()
                group._deal_agenda_credit += balanceline_deal_credit
                group._deal_agenda_debt += balanceline_deal_debt

    def add_to_partyunit_deal_credit_debt(
        self,
        partyunit_title: PartyTitle,
        deal_credit,
        deal_debt: float,
        deal_agenda_credit: float,
        deal_agenda_debt: float,
    ):
        for partyunit in self._partys.values():
            if partyunit.title == partyunit_title:
                partyunit.add_deal_credit_debt(
                    deal_credit=deal_credit,
                    deal_debt=deal_debt,
                    deal_agenda_credit=deal_agenda_credit,
                    deal_agenda_debt=deal_agenda_debt,
                )
    ...
    def set_time_acptfacts(self, open: datetime = None, nigh: datetime = None) -> None:
        ...
        minutes_acptfact = f"{self._project_handle},time,jajatime"
        self.set_acptfact(
    ...
    def _is_idea_rangeroot(self, idea_road: Road) -> bool:
        ...
        parent_road = self._project_handle if len(anc_roads) == 1 else anc_roads[1]
    ...
    def add_idea(
        ...
        if temp_road == self._project_handle and pad_nodes == []:
            idea_kid.set_pad(parent_road=Road(self._project_handle))
        else:
        ...
        if old_road != new_road:
            ...
            if pad == "":
                self._idearoot.set_idea_label(
                    new_label, deal_project_handle=self._project_handle
                )
    ...
    def _add_to_partyunits_deal_credit_debt(self, idea_deal_importance: float):
        ...
        for partyunit_x in self._partys.values():
            au_deal_credit = (
                idea_deal_importance * partyunit_x.get_creditor_weight()
            ) / sum_partyunit_creditor_weight

            au_deal_debt = (
                idea_deal_importance * partyunit_x.get_debtor_weight()
            ) / sum_partyunit_debtor_weight

            partyunit_x.add_deal_credit_debt(
                deal_credit=au_deal_credit,
                deal_debt=au_deal_debt,
                deal_agenda_credit=0,
                deal_agenda_debt=0,
            )

    def _add_to_partyunits_deal_agenda_credit_debt(self, idea_deal_importance: float):
        sum_partyunit_creditor_weight = self.get_partyunit_total_creditor_weight()
        sum_partyunit_debtor_weight = self.get_partyunit_total_debtor_weight()

        for partyunit_x in self._partys.values():
            au_deal_agenda_credit = (
                idea_deal_importance * partyunit_x.get_creditor_weight()
            ) / sum_partyunit_creditor_weight
            ...
            partyunit_x.add_deal_credit_debt(
                deal_credit=0,
                deal_debt=0,
                deal_agenda_credit=au_deal_agenda_credit,
                deal_agenda_debt=au_deal_agenda_debt,
            )

    def _set_partyunits_deal_agenda_importance(self, deal_agenda_importance: float):
        sum_partyunit_creditor_weight = self.get_partyunit_total_creditor_weight()
        sum_partyunit_debtor_weight = self.get_partyunit_total_debtor_weight()

        for partyunit_x in self._partys.values():
            au_deal_agenda_credit = (
                deal_agenda_importance * partyunit_x.get_creditor_weight()
            ) / sum_partyunit_creditor_weight

            au_deal_agenda_debt = (
                deal_agenda_importance * partyunit_x.get_debtor_weight()
            ) / sum_partyunit_debtor_weight

            partyunit_x.add_deal_agenda_credit_debt(
                deal_agenda_credit=au_deal_agenda_credit,
                deal_agenda_debt=au_deal_agenda_debt,
            )

    def _reset_groupunits_deal_credit_debt(self):
        self.set_groupunits_empty_if_null()
        for balancelink_obj in self._groups.values():
            balancelink_obj.reset_deal_credit_debt()

    def _set_groupunits_deal_importance(
        self, balanceheirs: dict[GroupBrand:Balancelink]
    ):
        self.set_groupunits_empty_if_null()
        for balancelink_obj in balanceheirs.values():
            self.add_to_group_deal_credit_debt(
                groupbrand=balancelink_obj.brand,
                balanceheir_deal_credit=balancelink_obj._deal_credit,
                balanceheir_deal_debt=balancelink_obj._deal_debt,
            )

    def _distribute_deal_agenda_importance(self):
        for idea in self._idea_dict.values():
            # If there are no balancelines associated with idea
            # distribute deal_importance via general partyunit
            # credit ratio and debt ratio
            # if idea.is_agenda_item() and idea._balancelines == {}:
            if idea.is_agenda_item():
                if idea._balancelines == {}:
                    self._add_to_partyunits_deal_agenda_credit_debt(
                        idea._deal_importance
                    )
                else:
                    for balanceline_x in idea._balancelines.values():
                        self.add_to_group_deal_agenda_credit_debt(
                            groupbrand=balanceline_x.brand,
                            balanceline_deal_credit=balanceline_x._deal_credit,
                            balanceline_deal_debt=balanceline_x._deal_debt,
                        )

    def _distribute_groups_deal_importance(self):
        for group_obj in self._groups.values():
            group_obj._set_partylink_deal_credit_debt()
            for partylink in group_obj._partys.values():
                self.add_to_partyunit_deal_credit_debt(
                    partyunit_title=partylink.title,
                    deal_credit=partylink._deal_credit,
                    deal_debt=partylink._deal_debt,
                    deal_agenda_credit=partylink._deal_agenda_credit,
                    deal_agenda_debt=partylink._deal_agenda_debt,
                )

    def _set_deal_agenda_ratio_credit_debt(self):
        deal_agenda_ratio_credit_sum = 0
        deal_agenda_ratio_debt_sum = 0

        for partyunit_x in self._partys.values():
            deal_agenda_ratio_credit_sum += partyunit_x._deal_agenda_credit
            deal_agenda_ratio_debt_sum += partyunit_x._deal_agenda_debt

        for partyunit_x in self._partys.values():
            partyunit_x.set_deal_agenda_ratio_credit_debt(
                deal_agenda_ratio_credit_sum=deal_agenda_ratio_credit_sum,
                deal_agenda_ratio_debt_sum=deal_agenda_ratio_debt_sum,
                deal_partyunit_total_creditor_weight=self.get_partyunit_total_creditor_weight(),
                deal_partyunit_total_debtor_weight=self.get_partyunit_total_debtor_weight(),
            )
    ...
    def _reset_partyunit_deal_credit_debt(self):
        self.set_partys_empty_if_null()
        for partyunit in self._partys.values():
            partyunit.reset_deal_credit_debt()
    ...
    def _set_root_attributes(self):
        ...
        self._idearoot.set_requiredheirs(deal_idea_dict=self._idea_dict)
        self._idearoot.set_assignedheir(
            parent_assignheir=None, deal_groups=self._groups
        )
        ...
        self._idearoot.set_deal_importance(coin_onset_x=0, parent_coin_cease=1)
        self._idearoot.set_balanceheirs_deal_credit_debit()
        ...
        if self._idearoot.is_kidless():
            ...
            self._distribute_deal_importance(idea=self._idearoot)
    ...
    def _set_kids_attributes(
        ...
        idea_kid.set_active_status(
            ...
            deal_groups=self._groups,
            deal_healer=self._healer,
        )
        idea_kid.set_sibling_total_weight(parent_idea._kids_total_weight)
        idea_kid.set_deal_importance(
            coin_onset_x=coin_onset,
            parent_deal_importance=parent_idea._deal_importance,
        ...
        if idea_kid.is_kidless():
            # set idea's ancestor metrics using deal root as common reference
            self._set_ancestor_metrics(road=idea_kid.get_road())
            self._distribute_deal_importance(idea=idea_kid)

    def _distribute_deal_importance(self, idea: IdeaCore):
        ...
        if idea.is_balanceheirless() == False:
            self._set_groupunits_deal_importance(idea._balanceheirs)
        elif idea.is_balanceheirless():
            self._add_to_partyunits_deal_credit_debt(idea._deal_importance)

    def get_deal_importance(
        self, parent_deal_importance: float, weight: int, sibling_total_weight: int
    ):
        sibling_ratio = weight / sibling_total_weight
        return parent_deal_importance * sibling_ratio
    ...
    def _execute_tree_traverse(self):
        ...
        coin_onset = self._idearoot._deal_coin_onset
        parent_coin_cease = self._idearoot._deal_coin_cease
        ...
        for idea_kid in self._idearoot._kids.values():
            ...
            coin_onset += idea_kid._deal_importance
        ...
        while cache_idea_list != []:
            ...
            if parent_idea._kids != None:
                coin_onset = parent_idea._deal_coin_onset
                parent_coin_cease = parent_idea._deal_coin_cease
    ...
    def _run_after_idea_all_tree_traverses(self):
        self._distribute_deal_agenda_importance()
        self._distribute_groups_deal_importance()
        self._set_deal_agenda_ratio_credit_debt()

    def _run_before_idea_tree_traverse(self):
        self._reset_groupunits_deal_credit_debt()
        self._reset_groupunits_deal_credit_debt()
        self._reset_partyunit_deal_credit_debt()
    ...
    def get_dict(self):
        ...
        return {
            ...
            "_project_handle": self._project_handle,
    ...
    def set_time_hreg_ideas(self, c400_count):
        ...
        while len(ideabase_list) != 0:
            ...
            if yb.sr != None:
                range_source_road_x = f"{self._project_handle},{yb.sr}"
            ...
            road_x = f"{self._project_handle},{yb.rr}"
            self.add_idea(idea_kid=idea_x, pad=road_x)

            numeric_road_x = None
            if yb.nr != None:
                numeric_road_x = f"{self._project_handle},{yb.nr}"
                self.edit_idea_attr(
    ...
    def get_deal4party(
        ...
        # set deal4party by traversing the idea tree and selecting associated groups
        # set root
        not_included_deal_importance = 0
        deal4party._idearoot._kids = {}
        for ykx in self._idearoot._kids.values():
            ...
            if y4a_included:
                y4a_new = IdeaKid(
                    _label=ykx._label,
                    _deal_importance=ykx._deal_importance,
                ...
                deal4party._idearoot._kids[ykx._label] = y4a_new
            else:
                not_included_deal_importance += ykx._deal_importance

        if not_included_deal_importance > 0:
            y4a_other = IdeaKid(
                _label="__other__",
                _deal_importance=not_included_deal_importance,
            )
            deal4party._idearoot._kids[y4a_other._label] = y4a_other
    ...
    def meld(self, other_deal, party_weight: float = None):
        self._meld_groups(other_deal)
        self._meld_partys(other_deal)
        self._meld_ideas(other_deal, party_weight)
        self._meld_acptfacts(other_deal)
        self._weight = get_meld_weight(
            src_weight=self._weight,
            src_on_meld_weight_action="default",
            other_weight=other_deal._weight,
            other_on_meld_weight_action="default",
        )
        self._meld_originlinks(other_deal._healer, party_weight)

    def _meld_ideas(self, other_deal, party_weight: float):
        # meld idearoot
        self._idearoot.meld(other_idea=other_deal._idearoot, _idearoot=True)

        # meld all other ideas
        party_title = other_deal._healer
        o_idea_list = other_deal.get_idea_list_without_idearoot()
    ...
    def _meld_partys(self, other_deal):
        self.set_partys_empty_if_null()
        other_deal.set_partys_empty_if_null()
        for partyunit in other_deal._partys.values():
            if self._partys.get(partyunit.title) is None:
                self.set_partyunit(partyunit=partyunit)
            else:
                self._partys.get(partyunit.title).meld(partyunit)

    def _meld_groups(self, other_deal):
        self.set_groupunits_empty_if_null()
        other_deal.set_groupunits_empty_if_null()
        for brx in other_deal._groups.values():
            if self._groups.get(brx.brand) is None:
                self.set_groupunit(groupunit=brx)
            else:
                self._groups.get(brx.brand).meld(brx)

    def _meld_acptfacts(self, other_deal):
        self._set_acptfacts_empty_if_null()
        other_deal._set_acptfacts_empty_if_null()
        for hx in other_deal._idearoot._acptfactunits.values():
    ...
    def get_assignment(
        ...
        self.set_deal_metrics()
        self._set_assignment_partys(deal_x, assignor_partys, assignor_title)
        self._set_assignment_groups(deal_x)
        assignor_promises = self._get_assignor_promise_ideas(deal_x, assignor_title)
        relevant_roads = self._get_relevant_roads(assignor_promises)
        self._set_assignment_ideas(deal_x, relevant_roads)
        return deal_x

    def _set_assignment_ideas(self, deal_x, relevant_roads: dict[Road:str]):
        ...
        for relevant_road in sorted_relevants:
            ...
            #     relevant_idea._kids = {}
            #     deal_x.add_idea(idea_kid=relevant_idea, pad=relevant_idea._pad)
    ...
    def _set_assignment_partys(
        ...
        deal_x.set_partys_empty_if_null()
        if self._partys.get(assignor_title) != None:
            ...
            for partytitle_x in partys_set:
                deal_x.set_partyunit(partyunit=self._partys.get(partytitle_x))
        return deal_x

    def _set_assignment_groups(self, deal_x):
        revelant_groups = get_partys_relevant_groups(self._groups, deal_x._partys)
        for group_title, group_partys in revelant_groups.items():
            if deal_x._groups.get(group_title) is None:
                ...
                deal_x.set_groupunit(group_x)

    def _get_assignor_promise_ideas(
        self, deal_x, assignor_title: GroupBrand
    ) -> dict[Road:int]:
        deal_x.set_groupunits_empty_if_null()
        assignor_groups = get_party_relevant_groups(deal_x._groups, assignor_title)
        return {
            idea_road: -1
...
def get_dict_of_deal_from_dict(x_dict: dict[str:dict]) -> dict[str:DealUnit]:
    dealunits = {}
    for dealunit_dict in x_dict.values():
        x_deal = get_from_dict(cx_dict=dealunit_dict)
        dealunits[x_deal._healer] = x_deal
    return dealunits


def get_meld_of_deal_files(primary_deal: DealUnit, meldees_dir: str) -> DealUnit:
    primary_deal.set_deal_metrics()
    for meldee_file_x in x_func_dir_files(dir_path=meldees_dir):
        meldee_x = get_from_json(
            x_deal_json=x_func_open_file(meldees_dir, meldee_file_x)
        )
        primary_deal.meld(other_deal=meldee_x)
