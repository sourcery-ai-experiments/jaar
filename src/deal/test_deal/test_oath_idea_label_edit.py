...
def test_idea_label_fails_when_idea_does_not_exist():
    ...
    swim_text = "swim"
    x_deal.add_idea(pad=x_deal._cure_handle, idea_kid=IdeaKid(_label=work_text))
    x_deal.add_idea(pad=work_road, idea_kid=IdeaKid(_label=swim_text))

    # When/Then
    no_idea_road = Road(f"{x_deal._cure_handle},bees")
...
def test_Deal_level0_idea_edit_idea_label_RaisesError_cure_handle_IsNone():
    # GIVEN
    healer_text = "Tim"
    x_deal = DealUnit(_healer=healer_text)
    ...
    work_road = f"{x_deal._cure_handle},{work_text}"
    swim_text = "swim"
    swim_road = f"{x_deal._cure_handle},{work_text},{swim_text}"
    x_deal.add_idea(pad=x_deal._cure_handle, idea_kid=IdeaKid(_label=work_text))
    x_deal.add_idea(pad=work_road, idea_kid=IdeaKid(_label=swim_text))
    assert x_deal._healer == healer_text
    assert x_deal._cure_handle == x_deal._cure_handle
    assert x_deal._idearoot._label == x_deal._cure_handle
    work_idea = x_deal.get_idea_kid(road=work_road)
    assert work_idea._pad == x_deal._cure_handle
    ...
    with pytest_raises(Exception) as excinfo:
        moon_text = "moon"
        x_deal.edit_idea_label(old_road=x_deal._cure_handle, new_label=moon_text)
    assert (
        str(excinfo.value)
        == f"Cannot set idearoot to string other than '{x_deal._cure_handle}'"
    )
...
def test_Deal_level0_idea_edit_idea_label_RaisesError_cure_handle_IsDifferent():
    # GIVEN
    healer_text = "Tim"
    x_deal = DealUnit(_healer=healer_text)
    work_text = "work"
    ...
    x_deal.add_idea(pad=work_road, idea_kid=IdeaKid(_label=swim_text))
    sun_text = "sun"
    x_deal._cure_handle = sun_text
    assert x_deal._healer == healer_text
    assert x_deal._cure_handle == sun_text
...
def test_deal_set_cure_handle_CorrectlySetsAttr():
    # GIVEN
    healer_text = "Tim"
    x_deal = DealUnit(_healer=healer_text)
    work_text = "work"
    old_work_road = f"{x_deal._cure_handle},{work_text}"
    swim_text = "swim"
    ...
    x_deal.set_cure_handle(cure_handle=cure_handle_text)

    # THEN
    new_work_road = f"{cure_handle_text},{work_text}"
    swim_text = "swim"
    new_swim_road = f"{new_work_road},{swim_text}"
    assert x_deal._cure_handle == cure_handle_text
    assert x_deal._idearoot._label == cure_handle_text
...
def test_idea_find_replace_road_Changes_kids_scenario1():
    ...
    old_healer_road = Road(f"{x_deal._cure_handle},{old_healer_text}")
    bloomers_text = "bloomers"
    old_bloomers_road = Road(f"{x_deal._cure_handle},{old_healer_text},{bloomers_text}")
    roses_text = "roses"
    old_roses_road = Road(
        f"{x_deal._cure_handle},{old_healer_text},{bloomers_text},{roses_text}"
    )
    red_text = "red"
    old_red_road = Road(
        f"{x_deal._cure_handle},{old_healer_text},{bloomers_text},{roses_text},{red_text}"
    )

    x_deal.add_idea(pad=x_deal._cure_handle, idea_kid=IdeaKid(_label=old_healer_text))
    ...
    new_healer_road = Road(f"{x_deal._cure_handle},{new_healer_text}")
    ...
    new_bloomers_road = Road(f"{x_deal._cure_handle},{new_healer_text},{bloomers_text}")
    assert r_idea_roses._pad == new_bloomers_road
    assert r_idea_roses._kids.get(red_text) != None
    r_idea_red = r_idea_roses._kids.get(red_text)
    new_roses_road = Road(
        f"{x_deal._cure_handle},{new_healer_text},{bloomers_text},{roses_text}"
    )
...
def test_deal_edit_idea_label_Changes_acptfactunits():
    ...
    healer = "healer"
    bloomers_text = "bloomers"
    bloomers_road = f"{x_deal._cure_handle},{healer},{bloomers_text}"
    roses_text = "roses"
    roses_road = f"{x_deal._cure_handle},{healer},{bloomers_text},{roses_text}"
    old_water_text = "water"
    old_water_road = f"{x_deal._cure_handle},{old_water_text}"
    rain_text = "rain"
    old_rain_road = f"{x_deal._cure_handle},{old_water_text},{rain_text}"

    x_deal.add_idea(pad=x_deal._cure_handle, idea_kid=IdeaKid(_label=healer))
    ...
    # WHEN
    new_water_text = "h2o"
    new_water_road = f"{x_deal._cure_handle},{new_water_text}"
    x_deal.add_idea(pad=x_deal._cure_handle, idea_kid=IdeaKid(_label=new_water_text))
    ...
    new_rain_road = f"{x_deal._cure_handle},{new_water_text},{rain_text}"
...
def test_deal_edit_idea_label_ChangesIdeaRoot_range_source_road():
    ...
    old_healer_road = Road(f"{x_deal._cure_handle},{old_healer_text}")
    x_deal.add_idea(pad=x_deal._cure_handle, idea_kid=IdeaKid(_label=old_healer_text))
    x_deal.edit_idea_attr(road=x_deal._cure_handle, range_source_road=old_healer_road)
...
def test_deal_edit_idea_label_ChangesIdeaKidN_range_source_road():
    ...
    healer_text = "healer"
    healer_road = Road(f"{x_deal._cure_handle},{healer_text}")
    old_water_text = "water"
    old_water_road = f"{x_deal._cure_handle},{healer_text},{old_water_text}"
    rain_text = "rain"
    old_rain_road = f"{x_deal._cure_handle},{healer_text},{old_water_text},{rain_text}"
    mood_text = "mood"
    mood_road = Road(f"{x_deal._cure_handle},{mood_text}")
    x_deal.add_idea(pad=x_deal._cure_handle, idea_kid=IdeaKid(_label=healer_text))
    x_deal.add_idea(pad=healer_road, idea_kid=IdeaKid(_label=old_water_text))
    x_deal.add_idea(pad=old_water_road, idea_kid=IdeaKid(_label=rain_text))
    x_deal.add_idea(pad=x_deal._cure_handle, idea_kid=IdeaKid(_label=mood_text))
    ...
    # WHEN
    new_water_text = "h2o"
    new_rain_road = f"{x_deal._cure_handle},{healer_text},{new_water_text},{rain_text}"
    x_deal.edit_idea_label(old_road=old_water_road, new_label=new_water_text)
...
def test_deal_edit_idea_label_ChangesIdeaRequiredUnitsScenario1():
    ...
    old_weekday_road = f"{x_deal._cure_handle},{old_weekday_text}"
    wednesday_text = "Wednesday"
    old_wednesday_road = f"{x_deal._cure_handle},{old_weekday_text},{wednesday_text}"
    work_idea = x_deal.get_idea_kid(f"{x_deal._cure_handle},work")
    usa = f"{x_deal._cure_handle},nation-state,USA"
    nationstate = f"{x_deal._cure_handle},nation-state"
    ...
    new_weekday_road = f"{x_deal._cure_handle},{new_weekday_text}"
    new_wednesday_road = f"{x_deal._cure_handle},{new_weekday_text},{wednesday_text}"
