...
def test_deal_get_dict_ReturnsDictObject():
    ...
    day_hour_road = f"{x_deal._cure_handle},{day_hour_text}"
    ...
    time_minute = f"{x_deal._cure_handle},day_minute"
    ...
    assert x_dict["_cure_handle"] == x_deal._cure_handle
    ...
    assert x_idearoot._label == x_deal._cure_handle
    ...
    assert month_week_special_dict == f"{x_deal._cure_handle},ced_week"
    ...
    num1_road = f"{x_deal._cure_handle},{num1_text}"
...
def test_deal_get_dict_ReturnsDictWith_ideakid_AssignedUnit():
    ...
    morn_text = "morning"
    morn_road = f"{x_deal._cure_handle},{morn_text}"
    x_deal.add_idea(idea_kid=IdeaKid(_label=morn_text), pad=x_deal._cure_handle)
    assigned_unit_x = assigned_unit_shop()
    assigned_unit_x.set_suffgroup(title=run_text)
...
def test_export_to_JSON_simple_example_works():
    ...
    x_deal.set_cure_handle(cure_handle_text)
...
def test_export_to_JSON_BigExampleCorrectlyReturnsValues():
    ...
    cont_road = f"{x_deal._cure_handle},{cont_text}"
    ulti_road = f"{x_deal._cure_handle},{ulti_text}"
...
def test_deal_get_json_CorrectlyWorksForSimpleExample():
    ...
    cure_handle_text = "tiger_econ"
    y_deal.set_cure_handle(cure_handle_text)

    shave_text = "shave"
    shave_road = f"{y_deal._cure_handle},{shave_text}"
    ...
    y_deal.edit_idea_attr(road=y_deal._cure_handle, assignedunit=run_assigned_unit)
    ...
    y_deal.edit_idea_attr(
        road=y_deal._cure_handle, balancelink=balancelink_shop(brand=sue_text)
    )
    ...
    assert x_deal._cure_handle == y_deal._cure_handle
    ...
    weekday_road = f"{y_deal._cure_handle},{weekday_text}"
