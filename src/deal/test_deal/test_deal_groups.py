...
def test_example_has_groups():
    ...
    # THEN
    db_idea = idea_dict.get(f"{cx._project_handle},D&B")
...
def test_DealUnit__get_filtered_balancelinks_idea_CorrectlyFiltersIdea_balancelinks():
    ...
    work_text = "work"
    work_road = f"{cx1._project_handle},{work_text}"
    swim_text = "swim"
    swim_road = f"{cx1._project_handle},{swim_text}"
    cx1.add_idea(IdeaKid(_label=work_text), pad=cx1._project_handle)
    cx1.add_idea(IdeaKid(_label=swim_text), pad=cx1._project_handle)
