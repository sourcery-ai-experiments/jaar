...
from src.deal.examples.example_deals import (
    get_deal_with_4_levels_and_2requireds,
    get_deal_with7amCleanTableRequired,
    deal_v001,
)
...
def test_deal_idea_dict_isDictionaryAndIsFullyPopulated():
    ...
    # THEN
    assert len(x_deal._idea_dict) == 17


def test_deal_get_idea_list_SetsSatiateStatusCorrectlyWhenAcptFactSaysNo():
    # GIVEN
    x_deal = get_deal_with_4_levels_and_2requireds()
    week_text = "weekdays"
...
def test_deal_get_idea_list_SetsSatiateStatusCorrectlyWhenAcptFactChanges():
    # GIVEN
    x_deal = get_deal_with_4_levels_and_2requireds()
    week_text = "weekdays"
    week_road = f"{x_deal._project_handle},{week_text}"
    ...
    assert idea_list
    assert len(idea_list) == 17
    assert x_deal._idea_dict.get(work_road)._active_status

    # WHEN
    france_text = "France"
    france_road = f"{states_road},{france_text}"
    x_deal.set_acptfact(base=states_road, pick=france_road)
...
def test_deal_get_idea_list_returns_correct_list():
    # GIVEN
    x_deal = get_deal_with_4_levels_and_2requireds()
    week_text = "weekdays"
    week_road = f"{x_deal._project_handle},{week_text}"
    ...
    state_road = f"{x_deal._project_handle},{state_text}"
    france_text = "France"
    ...
    print(f"{x_deal._healer=} {len(x_deal._idearoot._acptfactunits)=}")
    # print(f"{x_deal._idearoot._acptfactunits=}")
    ...
    # WHEN
    x_deal.set_acptfact(base=state_road, pick=oregon_road)

    # THEN
    work_idea = x_deal._idea_dict.get(work_road)
    print(f"\nlook at {work_idea.get_road()=}")
    assert work_idea._pad == f"{x_deal._project_handle}"
    ...
    #             x_dict=required.sufffacts, type_str="src.deal.required.SuffFactUnit"
    #         )


def test_deal_set_deal_metrics_CorrectlyClears_deal_coin():
    # GIVEN
    x_deal = get_deal_with7amCleanTableRequired()
    work_text = "work"
    catt_text = "feed cat"
    week_text = "weekdays"
    x_deal._idearoot._deal_coin_onset = 13
    x_deal._idearoot._deal_coin_cease = 13
    x_deal._idearoot._kids.get(work_text)._deal_coin_onset = 13
    x_deal._idearoot._kids.get(work_text)._deal_coin_cease = 13
    x_deal._idearoot._kids.get(catt_text)._deal_coin_onset = 13
    x_deal._idearoot._kids.get(catt_text)._deal_coin_cease = 13
    x_deal._idearoot._kids.get(week_text)._deal_coin_onset = 13
    x_deal._idearoot._kids.get(week_text)._deal_coin_cease = 13

    assert x_deal._idearoot._deal_coin_onset == 13
    assert x_deal._idearoot._deal_coin_cease == 13
    assert x_deal._idearoot._kids.get(work_text)._deal_coin_onset == 13
    assert x_deal._idearoot._kids.get(work_text)._deal_coin_cease == 13
    assert x_deal._idearoot._kids.get(catt_text)._deal_coin_onset == 13
    assert x_deal._idearoot._kids.get(catt_text)._deal_coin_cease == 13
    assert x_deal._idearoot._kids.get(week_text)._deal_coin_onset == 13
    assert x_deal._idearoot._kids.get(week_text)._deal_coin_cease == 13
...
def test_deal_get_idea_list_CorrectlyCalculatesIdeaAttr_deal_coin():
    ...
    x_deal.add_idea(idea_kid=auto_idea, pad=x_deal._project_handle)

    barn_text = "barn"
    barn_road = f"{x_deal._project_handle},{barn_text}"
    barn_idea = IdeaKid(_label=barn_text, _weight=60)
    x_deal.add_idea(idea_kid=barn_idea, pad=x_deal._project_handle)
    lamb_text = "lambs"
    lamb_road = f"{barn_road},{lamb_text}"
    lamb_idea = IdeaKid(_label=lamb_text, _weight=1)
    x_deal.add_idea(idea_kid=lamb_idea, pad=barn_road)
    duck_text = "ducks"
    duck_road = f"{barn_road},{duck_text}"
    duck_idea = IdeaKid(_label=duck_text, _weight=2)
    x_deal.add_idea(idea_kid=duck_idea, pad=barn_road)

    coal_text = "coal"
    coal_idea = IdeaKid(_label=coal_text, _weight=30)
    x_deal.add_idea(idea_kid=coal_idea, pad=x_deal._project_handle)

    assert x_deal._idearoot._deal_coin_onset is None
    assert x_deal._idearoot._deal_coin_cease is None
    assert x_deal._idearoot._kids.get(auto_text)._deal_coin_onset is None
    assert x_deal._idearoot._kids.get(auto_text)._deal_coin_cease is None
    assert x_deal._idearoot._kids.get(barn_text)._deal_coin_onset is None
    assert x_deal._idearoot._kids.get(barn_text)._deal_coin_cease is None
    assert x_deal._idearoot._kids.get(coal_text)._deal_coin_onset is None
    assert x_deal._idearoot._kids.get(coal_text)._deal_coin_cease is None
    lamb_before = x_deal.get_idea_kid(road=lamb_road)
    assert lamb_before._deal_coin_onset is None
    assert lamb_before._deal_coin_cease is None
    duck_before = x_deal.get_idea_kid(road=duck_road)
    assert duck_before._deal_coin_onset is None
    assert duck_before._deal_coin_cease is None

    # WHEN
    x_deal.set_deal_metrics()

    # THEN
    assert x_deal._idearoot._deal_coin_onset == 0.0
    assert x_deal._idearoot._deal_coin_cease == 1.0
    assert x_deal._idearoot._kids.get(auto_text)._deal_coin_onset == 0.0
    assert x_deal._idearoot._kids.get(auto_text)._deal_coin_cease == 0.1
    assert x_deal._idearoot._kids.get(barn_text)._deal_coin_onset == 0.1
    assert x_deal._idearoot._kids.get(barn_text)._deal_coin_cease == 0.7
    assert x_deal._idearoot._kids.get(coal_text)._deal_coin_onset == 0.7
    assert x_deal._idearoot._kids.get(coal_text)._deal_coin_cease == 1.0

    duck_after = x_deal.get_idea_kid(road=duck_road)
    assert duck_after._deal_coin_onset == 0.1
    assert duck_after._deal_coin_cease == 0.5
    lamb_after = x_deal.get_idea_kid(road=lamb_road)
    assert lamb_after._deal_coin_onset == 0.5
    assert lamb_after._deal_coin_cease == 0.7


def test_deal_get_idea_list_without_root_CorrectlyCalculatesIdeaAttributes():
    # GIVEN
    x_deal = get_deal_with7amCleanTableRequired()

    # WHEN
    idea_list_without_idearoot = x_deal.get_idea_list_without_idearoot()
    idea_list_with_idearoot = x_deal.get_idea_list()

    # THEN
    assert len(idea_list_without_idearoot) == 28
    assert len(idea_list_without_idearoot) + 1 == len(idea_list_with_idearoot)

    # for idea in x_deal.get_idea_list_without_idearoot():
...
def test_deal_get_idea_list_CorrectlyCalculatesRangeAttributes():
    # GIVEN
    x_deal = get_deal_with7amCleanTableRequired()
    idea_list = x_deal.get_idea_list()
    house_text = "housework"
    house_road = f"{x_deal._project_handle},{house_text}"
    ...
    # THEN
    x_deal.set_deal_metrics()
    assert x_deal._idea_dict.get(clean_road)._active_status
    ...
    print(x_deal._idearoot._acptfactunits[day24hr_road])
    print(x_deal._idearoot._kids[house_text]._kids[clean_text]._requiredunits)
    # x_deal._idearoot._kids["housework"]._kids[clean_text]._active_status = None
...
def test_get_agenda_items():
    ...
    # WHEN
    promise_items = x_deal.get_agenda_items()
...
def test_exammple_idea_list_HasCorrectData():
    ...
    # day_hour = f"{x_deal._project_handle},day_hour"
    ...
    yr_mon_road = f"{x_deal._project_handle},{yr_mon_text}"
    x_deal.set_acptfact(base=yr_mon_road, pick=yr_mon_road)
    inter_text = "Internet"
    inter_road = f"{x_deal._project_handle},{inter_text}"
    ...
    ulty_road = f"{x_deal._project_handle},{ulty_text}"
    ...
    laundry_road = f"{x_deal._project_handle},casa,cleaning,{laundry_text}"
    ...
    assert x_deal._idea_dict.get(laundry_road)._active_status == False
...
def test_exammple_idea_list_OptionWeekdaysCorrectlyWork():
    ...
    day_hr_road = f"{x_deal._project_handle},{day_hr_text}"
    ...
    mon_wk_road = f"{x_deal._project_handle},{mon_wk_text}"
    x_deal.set_acptfact(base=mon_wk_road, pick=mon_wk_road)
    nation_text = "Nation-States"
    nation_road = f"{x_deal._project_handle},{nation_text}"
    ...
    idea_list = x_deal.get_idea_list()
    missing_acptfacts = x_deal.get_missing_acptfact_bases()
    # for missing_acptfact, count in missing_acptfacts.items():
    #     print(f"{missing_acptfact=} {count=}")

    week_road = f"{x_deal._project_handle},weekdays"
    ...
    x_deal._idearoot.set_required_unit(required=mt_required)
    # print(f"{x_deal._requiredunits[week_road].base=}")
    # print(f"{x_deal._requiredunits[week_road].sufffacts[mon_road].need=}")
    # print(f"{x_deal._requiredunits[week_road].sufffacts[tue_road].need=}")
    print(f"{x_deal._idearoot._requiredunits[week_road].sufffacts=}")
    sufffact_mon = x_deal._idearoot._requiredunits[week_road].sufffacts.get(mon_road)
    sufffact_tue = x_deal._idearoot._requiredunits[week_road].sufffacts.get(tue_road)
    assert sufffact_mon
    assert sufffact_mon == mt_required.sufffacts[sufffact_mon.need]
    assert sufffact_tue
    assert sufffact_tue == mt_required.sufffacts[sufffact_tue.need]
    assert x_deal._idearoot._requiredunits[week_road] == mt_required
    ...
    x_deal.set_acptfact(base=week_road, pick=mon_road)
    idea_list = x_deal.get_idea_list()
    casa_idea = x_deal._idearoot._kids[casa_text]
    twee_idea = casa_idea._kids[bird_text]
    print(f"{len(x_deal._idearoot._requiredheirs)=}")
    ...
    # x_deal.set_acptfact(base=f"{x_deal._project_handle},weekdays", pick=f"{x_deal._project_handle},weekdays,Tuesday")
    # idea_list = x_deal.get_idea_list()
    # assert YR.get_active_status(road=bird_idea, idea_list=idea_list) == True

    # x_deal.set_acptfact(base=f"{x_deal._project_handle},weekdays", pick=f"{x_deal._project_handle},weekdays,Wednesday")
    # idea_list = x_deal.get_idea_list()
...
def test_exammple_idea_list_Every6WeeksRequired():
    ...
    min_text = "day_minute"
    min_road = f"{x_deal._project_handle},{day_text}"

    # WHEN
    x_deal.set_acptfact(base=day_road, pick=day_road, open=0, nigh=23)
    x_deal.set_acptfact(base=min_road, pick=min_road, open=0, nigh=59)
    idea_list = x_deal.get_idea_list()

    # THEN
    ced_week_base = f"{x_deal._project_handle},ced_week"
    ...
    clean_sheet_road = (
        f"{x_deal._project_handle},casa,cleaning,clean sheets couch blankets"
    )
    ...
    clean_couch_road = f"{x_deal._project_handle},casa,cleaning,{clean_couch_text}"
...
def test_exammple_idea_list_EveryOtherMonthWorks():
    ...
    minute_road = f"{x_deal._project_handle},{minute_text}"
    x_deal.set_acptfact(base=minute_road, pick=minute_road, open=0, nigh=1399)
    month_text = "month_week"
    month_road = f"{x_deal._project_handle},{month_text}"
