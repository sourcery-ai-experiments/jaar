...
from src.deal.required_idea import RequiredUnit, SuffFactStatusFinder
...
from src.deal.examples.example_deals import (
    ...
    get_deal_with_4_levels_and_2requireds as example_deals_get_deal_with_4_levels_and_2requireds,
    get_deal_with7amCleanTableRequired as example_deals_get_deal_with7amCleanTableRequired,
    get_deal_with_4_levels_and_2requireds_2acptfacts as example_deals_get_deal_with_4_levels_and_2requireds_2acptfacts,
    deal_v001 as example_deals_deal_v001,
    deal_v001_with_large_agenda as example_deals_deal_v001_with_large_agenda,
    deal_v002 as example_deals_deal_v002,
)
from src.deal.x_func import yr_explanation, open_file as x_func_open_file
...
def test_get_agenda_returns_agenda_with_deal_importance():
    # GIVEN
    x_deal = example_deals_get_deal_with_4_levels_and_2requireds_2acptfacts()
    ...
    assert agenda_list[0]._deal_importance

    print(f"{agenda_list[1]._label=}")
    assert agenda_list[1]._deal_importance
...
def test_get_agenda_with_7amItem():
    ...
    x_deal = example_deals_get_deal_with7amCleanTableRequired()
    print(f"{len(x_deal.get_agenda_items())=}")
    assert len(x_deal.get_agenda_items()) == 1

    # WHEN
    day24hr_road = f"{x_deal._cure_handle},timetech,24hr day"
    day24hr_open = 0.0
    day24hr_nigh = 8.0
    ...
    print(x_deal._idearoot._acptfactunits[day24hr_road])
    print(x_deal._idearoot._kids[housework_text]._kids[clean_text]._requiredunits)
    print(x_deal._idearoot._kids[housework_text]._kids[clean_text]._active_status)
...
def test_get_agenda_does_not_return_promise_items_outside_range():
    ...
    x_deal.add_idea(idea_kid=c_idea, pad=x_deal._cure_handle)
    c_road = f"{x_deal._cure_handle},{c_label}"
    jajatime = f"{x_deal._cure_handle},time,jajatime"
    jajaday = f"{x_deal._cure_handle},time,jajatime,day"
    ...
    open_x = 1063971180
    nigh_x1 = 2063971523
    x_deal.set_acptfact(base=jajatime, pick=jajaday, open=open_x, nigh=nigh_x1)
    ...
    nigh_x2 = 1063971923
    x_deal.set_acptfact(base=jajatime, pick=jajaday, open=open_x, nigh=nigh_x2)
...
def test_exammple_agenda_exists():
    ...
    min_text = "day_minute"
    min_road = f"{x_deal._cure_handle},{min_text}"
...
def test_exammple_AgendaHasCorrectAttributes():
    ...
    month_week_road = f"{x_deal._cure_handle},{month_week_text}"
    ...
    year_month_road = f"{x_deal._cure_handle},{year_month_text}"
    ...
    x_deal.set_acptfact(base=weekday_road, pick=monday_road)
    idea_action_list = x_deal.get_agenda_items()
    assert len(idea_action_list) == 39

    x_deal.set_acptfact(base=weekday_road, pick=weekday_road)
    idea_action_list = x_deal.get_agenda_items()
...
def test_exammple_AgendaCanFiltersOnBase():
    ...
    # for agenda_item in x_deal.get_agenda_items():
    #     print(
    ...
    # x_deal.edit_idea_attr(
    #     road="{x_deal._cure_handle},sufffacts,cleaning,laundry wednesday",
    #     required_del_sufffact_base=weekdays,
    #     required_del_sufffact_need=weekdays,
    # )
    assert len(x_deal.get_agenda_items()) == 68

    # When
    action_list = x_deal.get_agenda_items(base=week_road)
...
def test_set_agenda_task_as_complete_RangeWorksCorrectly():
    ...
    x_deal.add_idea(
        idea_kid=IdeaKid(_label=run_text, promise=True), pad=x_deal._cure_handle
    )
    x_deal.add_idea(
    ...
    x_deal.set_acptfact(base=day_road, pick=day_road, open=30, nigh=87)
    x_deal.get_agenda_items()
    run_requiredunits = x_deal._idearoot._kids[run_text]._requiredunits[day_road]
    ...
    assert len(x_deal.get_idea_list()) == 4
    assert len(x_deal.get_agenda_items()) == 1
    assert x_deal.get_agenda_items()[0]._task == True

    # When
    x_deal.set_agenda_task_complete(task_road=run_road, base=day_road)
...
def test_set_agenda_task_as_complete_DivisionWorksCorrectly():
    ...
    x_deal.set_acptfact(base=day_road, pick=day_road, open=1, nigh=2)
    assert len(x_deal.get_agenda_items()) == 1
    x_deal.set_acptfact(base=day_road, pick=day_road, open=2, nigh=2)
    assert len(x_deal.get_agenda_items()) == 0
    x_deal.set_acptfact(base=day_road, pick=day_road, open=400, nigh=400)
    assert len(x_deal.get_agenda_items()) == 0
    x_deal.set_acptfact(base=day_road, pick=day_road, open=401, nigh=402)
    assert len(x_deal.get_agenda_items()) == 1
    # print(f"{run_idea._acptfactheirs=}")
...
def test_deal_get_from_json_LoadsActionFromJSONCorrectly():
    # GIVEN
    file_dir = deal_env()
    file_title = "example_deal1.json"
    x_deal_json = x_func_open_file(dest_dir=file_dir, file_title=file_title)

    # WHEN
    x_deal = get_from_json(x_deal_json=x_deal_json)
    ...
    veg_idea = x_deal.get_idea_kid(road=veg_road)
...
def test_weekdayAgendaItemsCorrectlyReturned():
    ...
    x_deal.add_idea(IdeaKid(_label=things_text), pad=x_deal._cure_handle)
    t_road = f"{x_deal._cure_handle},{things_text}"
    clean = "clean"
    run = "run"
    swim = "swim"
    jog = "jog"
    veg = "veg"
    lift = "life"
    x_deal.add_idea(IdeaKid(_label=clean, promise=True), pad=t_road)
    x_deal.add_idea(IdeaKid(_label=run, promise=True), pad=t_road)
    x_deal.add_idea(IdeaKid(_label=swim, promise=True), pad=t_road)
    x_deal.add_idea(IdeaKid(_label=jog, promise=True), pad=t_road)
    ...
    c_idea = x_deal.get_idea_kid(road=c_road)
    ...
    # for base, count_x in x_deal.get_required_bases().items():
    ...
    mon_min = x_deal.get_time_min_from_dt(dt=mon_dt)
    tue_min = x_deal.get_time_min_from_dt(dt=tue_dt)
    wed_min = x_deal.get_time_min_from_dt(dt=wed_dt)
    thu_min = x_deal.get_time_min_from_dt(dt=thu_dt)
    fri_min = x_deal.get_time_min_from_dt(dt=fri_dt)
    sat_min = x_deal.get_time_min_from_dt(dt=sat_dt)
    sun_min = x_deal.get_time_min_from_dt(dt=sun_dt)
    assert x_deal._idearoot._acptfactunits.get(jaja_road) is None

    # When
    print("\nset acptfact for Sunday")
    x_deal.set_acptfact(base=jaja_road, pick=jaja_road, open=sun_min, nigh=sun_min)
    # for acptfact in x_deal._idearoot._acptfactunits.values():
    #     print(f"{acptfact.base=} (H: {acptfact.acptfact}) {acptfact.active=} {acptfact.open=} {acptfact.nigh=}")

    # Then
    assert len(x_deal._idearoot._acptfactunits) == 7
    print(x_deal._idearoot._acptfactunits[jaja_road])
    print(x_deal._idearoot._acptfactunits[sat_road])
    print(x_deal._idearoot._acptfactunits[sun_road])
    print(x_deal._idearoot._acptfactunits[tue_road])
    print(x_deal._idearoot._acptfactunits[wed_road])
    print(x_deal._idearoot._acptfactunits[thu_road])
    print(x_deal._idearoot._acptfactunits[fri_road])
    assert x_deal._idearoot._acptfactunits[sun_road]
    assert x_deal._idearoot._acptfactunits[sun_road].open == 1440
    assert x_deal._idearoot._acptfactunits[sun_road].nigh == 1440
    # assert x_deal._idearoot._acptfactunits[sun_road].active == True

    # assert x_deal._idearoot._acptfactunits[tue_road].active == False
    # assert x_deal._idearoot._acptfactunits[wed_road].active == False
    # assert x_deal._idearoot._acptfactunits[thu_road].active == False
    # assert x_deal._idearoot._acptfactunits[fri_road].active == False
    # assert x_deal._idearoot._acptfactunits[sat_road].active == False

    # When
    print("\nset acptfact for Sat through Monday")
    x_deal.set_acptfact(base=jaja_road, pick=jaja_road, open=sat_min, nigh=mon_min)
    # for acptfact in x_deal._idearoot._acptfactunits.values():
    #     print(f"{acptfact.base=} (H: {acptfact.acptfact}) {acptfact.active=} {acptfact.open=} {acptfact.nigh=}")

    # Then
    assert x_deal._idearoot._acptfactunits[sat_road]
    # assert x_deal._idearoot._acptfactunits[sat_road].active == True
    # assert x_deal._idearoot._acptfactunits[sun_road].active == True
    # assert x_deal._idearoot._acptfactunits[tue_road].active == False
    # assert x_deal._idearoot._acptfactunits[wed_road].active == False
    # assert x_deal._idearoot._acptfactunits[thu_road].active == False
    ...
    assert x_deal._idearoot._acptfactunits[sat_road].open == 0
    assert x_deal._idearoot._acptfactunits[sat_road].nigh == 1440
    assert x_deal._idearoot._acptfactunits[sun_road].open == 1440
    assert x_deal._idearoot._acptfactunits[sun_road].nigh == 2880

    # When
    print("\nset acptfacts for Sunday through Friday")
    x_deal.set_acptfact(base=jaja_road, pick=jaja_road, open=sun_min, nigh=fri_min)
    # for acptfact in x_deal._idearoot._acptfactunits.values():
    #     print(f"{acptfact.base=} (H: {acptfact.acptfact}) {acptfact.active=} {acptfact.open=} {acptfact.nigh=}")
    ...
    # assert x_deal._idearoot._acptfactunits[sat_road].active == False
    # assert x_deal._idearoot._acptfactunits[sun_road].active == True
    # assert x_deal._idearoot._acptfactunits[tue_road].active == True
    # assert x_deal._idearoot._acptfactunits[wed_road].active == True
    # assert x_deal._idearoot._acptfactunits[thu_road].active == True
    # assert x_deal._idearoot._acptfactunits[fri_road].active == False
    ...
    dayzero_min = x_deal.get_time_min_from_dt(dt=dayzero_dt)
    dayten_min = x_deal.get_time_min_from_dt(dt=dayten_dt)
    x_deal.set_acptfact(
    ...
    # assert x_deal._idearoot._acptfactunits[wed_road].active == True
    # assert x_deal._idearoot._acptfactunits[thu_road].active == True
    # assert x_deal._idearoot._acptfactunits[fri_road].active == True


def test_deal_create_agenda_item_CorrectlyCreatesAllDealAttributes():
    ...
    clean_things_road = Road(f"{x_deal._cure_handle},{clean_things_text}")
    ...
    house_text = "house"
    house_road = Road(f"{x_deal._cure_handle},{house_text}")
    ...
    x_deal.set_time_hreg_ideas(c400_count=7)
    daytime_road = Road(f"{x_deal._cure_handle},time,jajatime,day")
    open_8am = 480
    nigh_8am = 480
    ...
    assert len(x_deal._partys) == 0
    assert len(x_deal._groups) == 0
    assert len(x_deal._idearoot._kids) == 1
    assert x_deal.get_idea_kid(road=daytime_road)._begin == 0
    assert x_deal.get_idea_kid(road=daytime_road)._close == 1440
    print(f"{clean_cookery_idea.get_road()=}")

    # GIVEN
    x_deal.set_dominate_promise_idea(idea_kid=clean_cookery_idea)

    # THEN
    # for idea_kid in x_deal._idearoot._kids.keys():
    #     print(f"  {idea_kid=}")

    print(f"{clean_cookery_idea.get_road()=}")
    assert x_deal.get_idea_kid(road=clean_cookery_road) != None
    assert x_deal.get_idea_kid(road=clean_cookery_road)._label == clean_cookery_text
    assert x_deal.get_idea_kid(road=clean_cookery_road).promise
    assert len(x_deal.get_idea_kid(road=clean_cookery_road)._requiredunits) == 2
    assert x_deal.get_idea_kid(road=clean_things_road) != None
    assert x_deal.get_idea_kid(road=cookery_room_road) != None
    ...
    assert len(x_deal._idearoot._kids) == 3
...
def test_Issue116Resolved_correctlySetsTaskAsTrue():
    ...
    jajatime_road = f"{x_deal._cure_handle},time,jajatime"

    # WHEN
    x_deal.set_acptfact(
        base=jajatime_road, pick=jajatime_road, open=1063998720, nigh=1064130373
    )
    action_idea_list = x_deal.get_agenda_items()

    # THEN
    assert len(action_idea_list) == 66
    jajatime_road = f"{x_deal._cure_handle},time,jajatime"
    night_text = "late_night_go_to_sleep"
    night_road = f"{x_deal._cure_handle},D&B,{night_text}"
    ...
    print(f"      {x_deal.get_jajatime_repeating_legible_text(open=1063998720)}")
    print(f"      {x_deal.get_jajatime_repeating_legible_text(open=1064130373)}")
