...
def test_get_idea_kid_CorrectlyReturnsIdea():
    ...
    brazil = f"{x_deal._project_handle},nation-state,Brazil"
    idea_kid = x_deal.get_idea_kid(road=brazil)
    assert idea_kid != None
    assert idea_kid._label == "Brazil"

    weekdays = f"{x_deal._project_handle},weekdays"
    idea_kid = x_deal.get_idea_kid(road=weekdays)
    assert idea_kid != None
    assert idea_kid._label == "weekdays"

    # with pytest.raises(Exception) as excinfo:
    #     x_deal.get_idea_kid(road=x_deal._project_handle)
    # assert str(excinfo.value) == f"Cannot return root '{x_deal._project_handle}'"
    idea_root = x_deal.get_idea_kid(road=x_deal._project_handle)
    assert idea_root != None
    assert idea_root._label == x_deal._project_handle

    wrong_road = f"{x_deal._project_handle},bobdylan"
...
def test_set_deal_metrics_NLevelCorrectlySetsDescendantAttributes_1():
    ...
    x_deal.add_idea(idea_kid=x_idea, pad=f"{x_deal._project_handle},work")
...
def test_set_deal_metrics_NLevelCorrectlySetsDescendantAttributes_2():
    ...
    x_deal.add_idea(idea_kid=x1_idea, pad=f"{x_deal._project_handle},work")
    x2_idea = IdeaKid(_label="sweep", promise=True)
    x_deal.add_idea(idea_kid=x2_idea, pad=f"{x_deal._project_handle},work")
...
def test_TreeTraverseSetsBalancelineestorFromRootCorrectly():
    ...
    sum_x = 0
    cat_road = f"{x_deal._project_handle},feed cat"
    ...
    nation_road = f"{x_deal._project_handle},nation-state"
...
def test_deal_get_orderd_node_list_WorksCorrectly():
    ...
    assert x_deal.get_idea_tree_ordered_road_list()[0] == f"{x_deal._project_handle}"
    assert (
        x_deal.get_idea_tree_ordered_road_list()[8]
...
def test_deal_get_orderd_node_list_CorrectlyFiltersRangedIdeaRoads():
    ...
    t_road = f"{x_deal._project_handle},{time}"
    week = "weeks"
...
#     x_deal.add_idea(idea_kid=IdeaKid(_label="D", _weight=7), pad=f"{x_deal._project_handle},A,C")
...
#     x_deal.add_idea(idea_kid=IdeaKid(_label="I"), pad=f"{x_deal._project_handle},G")
#     x_deal.add_idea(idea_kid=IdeaKid(_label="H"), pad=f"{x_deal._project_handle},G")
#     x_deal.add_idea(idea_kid=IdeaKid(_label="J"), pad=f"{x_deal._project_handle},G,I")
#     x_deal.add_idea(idea_kid=IdeaKid(_label="K"), pad=f"{x_deal._project_handle},G,I")
#     x_deal.add_idea(idea_kid=IdeaKid(_label="M"), pad=f"{x_deal._project_handle},G,H")

#     billy_title = PartyTitle("billy")
#     x_deal.add_partyunit(title=billy_title)
#     billy_bl = Balancelink(brand=billy_title)
#     x_deal.edit_idea_attr(road=f"{x_deal._project_handle},G", balancelink=billy_bl)
#     x_deal.edit_idea_attr(road=f"{x_deal._project_handle},G,H,M", balancelink=billy_bl)

#     sandy_title = PartyTitle("sandy")
#     x_deal.add_partyunit(title=sandy_title)
#     sandy_bl = Balancelink(brand=sandy_title)
#     x_deal.edit_idea_attr(road=f"{x_deal._project_handle},A", balancelink=sandy_bl)
#     x_deal.edit_idea_attr(road=f"{x_deal._project_handle},B", balancelink=sandy_bl)
...
#     assert gen_sandy.get_idea_kid(road=f"{x_deal._project_handle},A")._deal_importance == 0.07
#     assert gen_sandy.get_idea_kid(road=f"{x_deal._project_handle},A,C")._deal_importance == 0.07
#     assert gen_sandy.get_idea_kid(road=f"{x_deal._project_handle},A,C,E")._deal_importance == 0.5
#     assert gen_sandy.get_idea_kid(road=f"{x_deal._project_handle},B")._deal_importance == 0.13
#     assert (
#         gen_sandy.get_idea_kid(road=f"{x_deal._project_handle},A")._deal_importance
#         == exp_sandy.get_idea_kid(road=f"{x_deal._project_handle},A")._deal_importance
#     )
#     assert (
#         gen_sandy.get_idea_kid(road=f"{x_deal._project_handle},A,C")._deal_importance
#         == exp_sandy.get_idea_kid(road=f"{x_deal._project_handle},A,C")._deal_importance
#     )
#     assert (
#         gen_sandy.get_idea_kid(road=f"{x_deal._project_handle},A,C,E")._deal_importance
#         == exp_sandy.get_idea_kid(road=f"{x_deal._project_handle},A,C,E")._deal_importance
#     )
#     assert (
#         gen_sandy.get_idea_kid(road=f"{x_deal._project_handle},B")._deal_importance
#         == exp_sandy.get_idea_kid(road=f"{x_deal._project_handle},B")._deal_importance
#     )
