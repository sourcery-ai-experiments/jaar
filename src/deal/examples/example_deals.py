...
def deal_v001_with_large_agenda() -> DealUnit:
    ...
    day_minute_road = f"{a1._fix_handle},{day_minute_text}"
    month_week_text = "month_week"
    month_week_road = f"{a1._fix_handle},{month_week_text}"
    nations_text = "Nation-States"
    nations_road = f"{a1._fix_handle},{nations_text}"
    mood_text = "Moods"
    mood_road = f"{a1._fix_handle},{mood_text}"
    aaron_text = "Aaron Donald sphere"
    aaron_road = f"{a1._fix_handle},{aaron_text}"
    # internet_text = "Internet"
    # internet_road = f"{a1._fix_handle},{internet_text}"
    year_month_text = "year_month"
    year_month_road = f"{a1._fix_handle},{year_month_text}"
    season_text = "Seasons"
    season_road = f"{a1._fix_handle},{season_text}"
    ced_week_text = "ced_week"
    ced_week_road = f"{a1._fix_handle},{ced_week_text}"
    # water_text = "WaterBeing"
    # water_road = f"{a1._fix_handle},{water_text}"
    weekdays_text = "weekdays"
    weekdays_road = f"{a1._fix_handle},{weekdays_text}"
    # movie_text = "No Movie playing"
    # movie_road = f"{a1._fix_handle},{movie_text}"
...
def get_deal_with_4_levels() -> DealUnit:
    ...
    a1.add_idea(idea_kid=idea_kid_work, pad=a1._fix_handle)

    cat = "feed cat"
    idea_kid_feedcat = IdeaKid(_weight=30, _label=cat, promise=True)
    a1.add_idea(idea_kid=idea_kid_feedcat, pad=a1._fix_handle)

    week_text = "weekdays"
    week_road = f"{a1._fix_handle},{week_text}"
    idea_kid_weekdays = IdeaKid(_weight=40, _label=week_text)
    a1.add_idea(idea_kid=idea_kid_weekdays, pad=a1._fix_handle)
    ...
    states_road = f"{a1._fix_handle},{states_text}"
    idea_kid_states = IdeaKid(_weight=30, _label=states_text)
    a1.add_idea(idea_kid=idea_kid_states, pad=f"{a1._fix_handle}")
...
def get_deal_with_4_levels_and_2requireds() -> DealUnit:
    ...
    nation_road = f"{a1._fix_handle},{nation_text}"
    usa_text = "USA"
    ...
    work_text = "work"
    work_road = f"{a1._fix_handle},{work_text}"
...
def get_deal_with7amCleanTableRequired() -> DealUnit:
    ...
    time_text = "timetech"
    time_road = f"{a1._fix_handle},{time_text}"
    ...
    a1.add_idea(time_idea, a1._fix_handle)
    ...
    house_road = f"{a1._fix_handle},{house_text}"
    ...
    a1.add_idea(idea_kid=house_idea, pad=a1._fix_handle)
...
def get_deal_1Task_1CE0MinutesRequired_1AcptFact() -> DealUnit:
    ...
    ced_road = f"{a1._fix_handle},{ced_min_label}"
    a1.add_idea(idea_kid=ced_minutes, pad=a1._fix_handle)
    mail_label = "obtain mail"
    mail_task = IdeaKid(_label=mail_label, promise=True)
    a1.add_idea(idea_kid=mail_task, pad=a1._fix_handle)
    ...
    mail_road = f"{a1._fix_handle},{mail_label}"
...
def get_deal_x1_3levels_1required_1acptfacts() -> DealUnit:
    ...
    a1 = DealUnit(_healer=healer_text, _weight=10)
    shave_text = "shave"
    shave_road = f"{a1._fix_handle},{shave_text}"
    idea_kid_shave = IdeaKid(_weight=30, _label=shave_text, promise=True)
    a1.add_idea(idea_kid=idea_kid_shave, pad=a1._fix_handle)
    week_text = "weekdays"
    week_road = f"{a1._fix_handle},{week_text}"
    week_idea = IdeaKid(_weight=40, _label=week_text)
    a1.add_idea(idea_kid=week_idea, pad=a1._fix_handle)
...
def get_deal_irrational_example() -> DealUnit:
    ...
    egg_text = "egg first"
    egg_road = f"{a1._fix_handle},{egg_text}"
    a1.add_idea(idea_kid=IdeaKid(_label=egg_text), pad=a1._fix_handle)

    chicken_text = "chicken first"
    chicken_road = f"{a1._fix_handle},{chicken_text}"
    a1.add_idea(idea_kid=IdeaKid(_label=chicken_text), pad=a1._fix_handle)
...
def get_assignment_deal_example1():
    ...
    a1 = DealUnit(_healer=healer_text)
    casa_text = "casa"
    casa_road = f"{a1._fix_handle},{casa_text}"
    ...
    unim_road = f"{a1._fix_handle},{unim_text}"
    unim_idea = IdeaKid(_label=unim_text)
    a1.add_idea(idea_kid=unim_idea, pad=a1._fix_handle)
