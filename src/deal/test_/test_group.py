...
def test_groupunit_exists():
    ...
    swimmers_group = groupunit_shop(
        ...
        _partylinks_set_by_fix_road=usa_road,
    )
    ...
    assert swimmers_group._partylinks_set_by_fix_road == usa_road
...
def test_groupunit_set_attr_WorksCorrectly():
    ...
    swim_text = "swimmers"
    swim_group = groupunit_shop(brand=swim_text)
    assert swim_group._partylinks_set_by_fix_road is None

    # WHEN
    water_road = f"{root_label()},sports,water"
    swim_group.set_attr(_partylinks_set_by_fix_road=water_road)

    # THEN
    assert swim_group._partylinks_set_by_fix_road == water_road


def test_groupunit_shop_WhenSinglePartyCorrectlyRemoves_partylinks_set_by_fix_road():
    # GIVEN
    swimmers = "swimmers"
    usa_road = f"{root_label()},nation-states,USA"
    ...
    assert (
        str(excinfo.value)
        == f"_partylinks_set_by_fix_road cannot be '{usa_road}' for a single_party GroupUnit. It must have no value."
    )
...
def test_groupUnit_get_dict_ReturnsDictWithNecessaryDataForJSON():
    ...
    assert ee_dict == {
        ...
        "_single_party": False,
        "_partys": {},
        "_partylinks_set_by_fix_road": None,
    }
    ...
    teachers_group = groupunit_shop(
        ...
        _partylinks_set_by_fix_road=swim_road,
    )
...
def test_groupunit_get_from_JSON_SimpleExampleWorks():
    ...
    _partylinks_set_by_fix_road_text = "_partylinks_set_by_fix_road"
    print(f"{teacher_dict.get(_partylinks_set_by_fix_road_text)=}")
    groups_dict = {"teachers": teacher_dict}

    teachers_json = x_get_json(dict_x=groups_dict)
    print(f"{teachers_json.find(_partylinks_set_by_fix_road_text)=}")
    ...
    partylinks_set_by_fix_road_text = "_partylinks_set_by_fix_road"
    print(f"{teachers_obj_check_dict.get(partylinks_set_by_fix_road_text)=}")
