from src.deal.party import PartyTitle, partylink_shop
...
from src.deal.x_func import x_is_json, x_get_json
...
def test_groupunit_exists():
    ...
    swimmers_group = groupunit_shop(
        ...
        _deal_credit=0.33,
        _deal_debt=0.44,
        _deal_agenda_credit=0.66,
        _deal_agenda_debt=0.77,
        _partylinks_set_by_project_road=usa_road,
    )
    ...
    assert swimmers_group._deal_credit != None
    assert swimmers_group._deal_debt != None
    assert swimmers_group._deal_agenda_credit != None
    assert swimmers_group._deal_agenda_debt != None
    assert swimmers_group._partylinks_set_by_project_road == usa_road
...
def test_groupunit_set_attr_WorksCorrectly():
    ...
    swim_text = "swimmers"
    swim_group = groupunit_shop(brand=swim_text)
    assert swim_group._partylinks_set_by_project_road is None

    # WHEN
    water_road = f"{root_label()},sports,water"
    swim_group.set_attr(_partylinks_set_by_project_road=water_road)

    # THEN
    assert swim_group._partylinks_set_by_project_road == water_road


def test_groupunit_shop_WhenSinglePartyCorrectlyRemoves_partylinks_set_by_project_road():
    # GIVEN
    swimmers = "swimmers"
    usa_road = f"{root_label()},nation-states,USA"
    ...
    assert (
        str(excinfo.value)
        == f"_partylinks_set_by_project_road cannot be '{usa_road}' for a single_party GroupUnit. It must have no value."
    )
...
def test_groupunit_set_partylink_worksCorrectly():
    ...
    todd_text = "Todd"
    mery_text = "Merry"
    todd_party = partylink_shop(title=todd_text, creditor_weight=13, debtor_weight=7)
    mery_party = partylink_shop(title=mery_text, creditor_weight=23, debtor_weight=5)
    ...
    # THEN
    swimmers_partys = {todd_party.title: todd_party, mery_party.title: mery_party}
...
def test_groupunit_del_partylink_worksCorrectly():
    ...
    # WHEN
    swimmers_group.del_partylink(title=todd_text)
...
def test_Groupunit_reset_deal_importance_WorkCorrectly():
    # GIVEN
    maria_brand = "maria"
    maria_group = groupunit_shop(
        ...
        _deal_credit=0.33,
        _deal_debt=0.44,
        _deal_agenda_credit=0.13,
        _deal_agenda_debt=0.23,
    )
    print(f"{maria_group}")
    assert maria_group._deal_credit == 0.33
    assert maria_group._deal_debt == 0.44
    assert maria_group._deal_agenda_credit == 0.13
    assert maria_group._deal_agenda_debt == 0.23

    # WHEN
    maria_group.reset_deal_credit_debt()

    # THEN
    assert maria_group._deal_credit == 0
    assert maria_group._deal_debt == 0
    assert maria_group._deal_agenda_credit == 0
    assert maria_group._deal_agenda_debt == 0


def test_Groupunit_reset_deal_importance_reset_partylinks():
    # GIVEN
    todd_text = "Todd"
    mery_text = "Merry"
    ...
    mery_party = partylink_shop(
        ...
        _deal_credit=0.23,
        _deal_debt=0.5,
        _deal_agenda_credit=0.54,
        _deal_agenda_debt=0.57,
    )
    bikers_partys = {todd_party.title: todd_party, mery_party.title: mery_party}
    bikers_brand = "bikers"
    bikers_group = groupunit_shop(
        ...
        _deal_credit=0.33,
        _deal_debt=0.44,
        _deal_agenda_credit=0.1,
        _deal_agenda_debt=0.2,
    )
    ...
    assert biker_partylink_todd._deal_credit == 0.13
    assert biker_partylink_todd._deal_debt == 0.7
    assert biker_partylink_todd._deal_agenda_credit == 0.53
    assert biker_partylink_todd._deal_agenda_debt == 0.77

    biker_partylink_mery = bikers_group._partys.get(mery_text)
    assert biker_partylink_mery._deal_credit == 0.23
    assert biker_partylink_mery._deal_debt == 0.5
    assert biker_partylink_mery._deal_agenda_credit == 0.54
    assert biker_partylink_mery._deal_agenda_debt == 0.57

    # WHEN
    bikers_group.reset_deal_credit_debt()

    # THEN
    assert biker_partylink_todd._deal_credit == 0
    assert biker_partylink_todd._deal_debt == 0
    assert biker_partylink_todd._deal_agenda_credit == 0
    assert biker_partylink_todd._deal_agenda_debt == 0
    assert biker_partylink_mery._deal_credit == 0
    assert biker_partylink_mery._deal_debt == 0
    assert biker_partylink_mery._deal_agenda_credit == 0
    assert biker_partylink_mery._deal_agenda_debt == 0


def test_GroupUnit_partylink_meld_BaseScenarioWorks():
    # GIVEN
    todd_party = partylink_shop(title="Todd")
    merry_party = partylink_shop(title="Merry")
    x1_brand = "bikers"
...
def test_GroupUnit_meld_RaiseSameTitleException():
    # GIVEN
    todd_text = "Todd"
...
def test_groupUnit_get_dict_ReturnsDictWithNecessaryDataForJSON():
    ...
    assert ee_dict == {
        ...
        "_single_party": False,
        "_partys": {},
        "_partylinks_set_by_project_road": None,
    }

    # GIVEN
    str_title = "Marie"
    marie_title = PartyTitle(str_title)
    marie_partylink = partylink_shop(
        title=marie_title, creditor_weight=29, debtor_weight=3
    )
    partylinks_dict = {marie_partylink.title: marie_partylink}
    marie_json_dict = {
        marie_title: {"title": marie_title, "creditor_weight": 29, "debtor_weight": 3}
    }
    ...
    teachers_group = groupunit_shop(
        ...
        _partylinks_set_by_project_road=swim_road,
    )
...
def test_groupunit_get_from_JSON_SimpleExampleWorks():
    ...
    _partylinks_set_by_project_road_text = "_partylinks_set_by_project_road"
    print(f"{teacher_dict.get(_partylinks_set_by_project_road_text)=}")
    groups_dict = {"teachers": teacher_dict}

    teachers_json = x_get_json(dict_x=groups_dict)
    print(f"{teachers_json.find(_partylinks_set_by_project_road_text)=}")
    ...
    partylinks_set_by_project_road_text = "_partylinks_set_by_project_road"
    print(f"{teachers_obj_check_dict.get(partylinks_set_by_project_road_text)=}")
...
def test_brandLink_set_deal_importanceCorrectly():
    # GIVEN
    bikers_brand = GroupBrand("bikers")
    bikers_creditor_weight = 3.0
    ...
    # WHEN
    group_heir_x.set_deal_credit_debt(
        idea_deal_importance=idea_deal_importance,
    ...
    # THEN
    assert group_heir_x._deal_credit == 0.05
    assert group_heir_x._deal_debt == 0.1
...
def test_Balanceline_exists():
    # GIVEN
    bikers_brand = GroupBrand("bikers")
    balanceline_x = Balanceline(brand=bikers_brand, _deal_credit=0.33, _deal_debt=0.55)
    assert balanceline_x.brand == bikers_brand
    assert balanceline_x._deal_credit == 0.33
    assert balanceline_x._deal_debt == 0.55

    # WHEN
    balanceline_x.add_deal_credit_debt(deal_credit=0.11, deal_debt=0.2)

    # THEN
    assert balanceline_x._deal_credit == 0.44
    assert balanceline_x._deal_debt == 0.75
