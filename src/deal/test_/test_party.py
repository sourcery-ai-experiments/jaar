...
def test_PartyUnit_exists():
    ...
    assert bob_party._deal_credit is None
    assert bob_party._deal_debt is None
    assert bob_party._deal_agenda_credit is None
    assert bob_party._deal_agenda_debt is None
...
def test_PartyUnit_set_empty_deal_credit_debt_to_zero_CorrectlySetsZero():
    # GIVEN
    bob_title = "bob"
    bob_party = partyunit_shop(title=bob_title)
    assert bob_party._deal_credit is None
    ...
    assert bob_party._deal_agenda_credit is None
    assert bob_party._deal_agenda_debt is None
    assert bob_party._deal_agenda_ratio_credit is None
    assert bob_party._deal_agenda_ratio_debt is None

    # WHEN
    bob_party.set_empty_deal_credit_debt_to_zero()

    # THEN
    assert bob_party._deal_credit == 0
    assert bob_party._deal_debt == 0
    assert bob_party._deal_agenda_credit == 0
    assert bob_party._deal_agenda_debt == 0
    assert bob_party._deal_agenda_ratio_credit == 0
    assert bob_party._deal_agenda_ratio_debt == 0

    # GIVEN
    bob_party._deal_credit = 0.27
    bob_party._deal_debt = 0.37
    bob_party._deal_agenda_credit = 0.41
    bob_party._deal_agenda_debt = 0.51
    bob_party._deal_agenda_ratio_credit = 0.23
    bob_party._deal_agenda_ratio_debt = 0.87
    assert bob_party._deal_credit == 0.27
    assert bob_party._deal_debt == 0.37
    assert bob_party._deal_agenda_credit == 0.41
    assert bob_party._deal_agenda_debt == 0.51
    assert bob_party._deal_agenda_ratio_credit == 0.23
    assert bob_party._deal_agenda_ratio_debt == 0.87
...
def test_PartyUnit_reset_deal_credit_debt_MethodWorkCorrectly():
    # GIVEN
    bob_title = "bob"
    bob_party = partyunit_shop(title=bob_title)
    bob_party._deal_credit = 0.27
    ...
    bob_party._deal_agenda_credit = 0.41
    bob_party._deal_agenda_debt = 0.51
    bob_party._deal_agenda_ratio_credit = 0.433
    bob_party._deal_agenda_ratio_debt = 0.533
    assert bob_party._deal_credit == 0.27
    assert bob_party._deal_debt == 0.37
    assert bob_party._deal_agenda_credit == 0.41
    assert bob_party._deal_agenda_debt == 0.51
    assert bob_party._deal_agenda_ratio_credit == 0.433
    assert bob_party._deal_agenda_ratio_debt == 0.533

    # WHEN
    bob_party.reset_deal_credit_debt()
...
def test_PartyUnit_add_deal_credit_debt_MethodWorkCorrectly():
    # GIVEN
    bob_title = "bob"
    bob_party = partyunit_shop(title=bob_title)
    bob_party._deal_credit = 0.4106
    bob_party._deal_debt = 0.1106
    bob_party._deal_agenda_credit = 0.41
    bob_party._deal_agenda_debt = 0.51
    assert bob_party._deal_agenda_credit == 0.41
    assert bob_party._deal_agenda_debt == 0.51
    ...
    # WHEN
    bob_party.add_deal_credit_debt(
        deal_credit=0.33,
        deal_debt=0.055,
        deal_agenda_credit=0.3,
        deal_agenda_debt=0.05,
    )

    # THEN
    assert bob_party._deal_credit == 0.7406
    assert bob_party._deal_debt == 0.1656
    assert bob_party._deal_agenda_credit == 0.71
    assert bob_party._deal_agenda_debt == 0.56


def test_PartyUnit_set_deal_agenda_ratio_credit_debt_MethodWorkCorrectly():
    # GIVEN
    bob_title = "bob"
    bob_party = partyunit_shop(
        ...
        _deal_credit=0.4106,
        _deal_debt=0.1106,
        _deal_agenda_credit=0.041,
        _deal_agenda_debt=0.051,
        _deal_agenda_ratio_credit=0,
        _deal_agenda_ratio_debt=0,
    )
    assert bob_party._deal_agenda_ratio_credit == 0
    assert bob_party._deal_agenda_ratio_debt == 0

    # WHEN
    bob_party.set_deal_agenda_ratio_credit_debt(
        deal_agenda_ratio_credit_sum=0.2,
        deal_agenda_ratio_debt_sum=0.5,
        deal_partyunit_total_creditor_weight=20,
        deal_partyunit_total_debtor_weight=14,
    )

    # THEN
    assert bob_party._deal_agenda_ratio_credit == 0.205
    assert bob_party._deal_agenda_ratio_debt == 0.102
    ...
    # THEN
    assert bob_party._deal_agenda_ratio_credit == 0.75
    assert bob_party._deal_agenda_ratio_debt == 0.5
...
def test_PartyLink_exists():
    ...
    party_link_x = partylink_shop(
        ...
        _deal_agenda_debt=0.55,
    )

    # THEN
    assert party_link_x.creditor_weight == bikers_creditor_weight
    assert party_link_x.debtor_weight == bikers_debtor_weight
    assert party_link_x._deal_credit != None
    assert party_link_x._deal_credit == 0.7
    assert party_link_x._deal_debt == 0.51
    assert party_link_x._deal_agenda_credit == 0.66
    assert party_link_x._deal_agenda_debt == 0.55


def test_partylink_shop_set_deal_credit_debt_CorrectlyWorks():
    # GIVEN
    bikers_title = PartyTitle("Marie")
    bikers_creditor_weight = 3.0
    partylinks_sum_creditor_weight = 60
    group_deal_credit = 0.5
    group_deal_agenda_credit = 0.98

    bikers_debtor_weight = 13.0
    partylinks_sum_debtor_weight = 26.0
    group_deal_debt = 0.9
    group_deal_agenda_debt = 0.5151
    ...
    assert party_link_x._deal_credit is None
    assert party_link_x._deal_debt is None
    assert party_link_x._deal_agenda_credit is None
    assert party_link_x._deal_agenda_debt is None

    # WHEN
    party_link_x.set_deal_credit_debt(
        partylinks_creditor_weight_sum=partylinks_sum_creditor_weight,
        partylinks_debtor_weight_sum=partylinks_sum_debtor_weight,
        group_deal_credit=group_deal_credit,
        group_deal_debt=group_deal_debt,
        group_deal_agenda_credit=group_deal_agenda_credit,
        group_deal_agenda_debt=group_deal_agenda_debt,
    )

    # THEN
    assert party_link_x._deal_credit == 0.025
    assert party_link_x._deal_debt == 0.45
    assert party_link_x._deal_agenda_credit == 0.049
    assert party_link_x._deal_agenda_debt == 0.25755


def test_partylink_shop_reset_deal_credit_debt():
    # GIVEN
    biker_title = "maria"
    biker_party = partylink_shop(title=biker_title, _deal_credit=0.04, _deal_debt=0.7)
    print(f"{biker_party}")

    assert biker_party._deal_credit != None
    assert biker_party._deal_debt != None

    # WHEN
    biker_party.reset_deal_credit_debt()

    # THEN
    assert biker_party._deal_credit == 0
    assert biker_party._deal_debt == 0
