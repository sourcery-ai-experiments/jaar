...
from src.deal.origin import OriginUnit
...
class IdeaCore:
    ...
    _deal_importance: float = None
    _deal_coin_onset: float = None
    _deal_coin_cease: float = None
    _task: bool = None
    ...
    def set_deal_importance(
        self,
        coin_onset_x: float,
        parent_deal_importance: float = None,
        parent_coin_cease: float = None,
    ):
        parent_deal_importance = x_func_return1ifnone(parent_deal_importance)
        self.set_kids_total_weight()
        self._deal_importance = None
        self._deal_coin_onset = None
        self._deal_coin_cease = None
        sibling_ratio = self._weight / self._sibling_total_weight
        self._deal_importance = parent_deal_importance * sibling_ratio
        self._deal_coin_onset = coin_onset_x
        self._deal_coin_cease = self._deal_coin_onset + self._deal_importance
        self._deal_coin_cease = min(self._deal_coin_cease, parent_coin_cease)
        self.set_balanceheirs_deal_credit_debit()
    ...
    def set_kidless_balancelines(self):
        ...
        for bh in self._balanceheirs.values():
            balanceline_x = Balanceline(
                brand=bh.brand,
                _deal_credit=bh._deal_credit,
                _deal_debt=bh._deal_debt,
            )
    ...
    def set_balancelines(self, child_balancelines: dict[GroupBrand:Balanceline] = None):
        ...
        for bl in child_balancelines.values():
            ...
            self._balancelines[bl.brand].add_deal_credit_debt(
                deal_credit=bl._deal_credit, deal_debt=bl._deal_debt
            )
    ...
    def set_balanceheirs_deal_credit_debit(self):
        ...
        for balanceheir_x in self._balanceheirs.values():
            balanceheir_x.set_deal_credit_debt(
                idea_deal_importance=self._deal_importance,
    ...
    def set_active_status(
        self,
        tree_traverse_count: int,
        deal_groups: dict[GroupBrand:GroupUnit] = None,
        deal_healer: str = None,
    ):
        self.set_acptfactheirs_empty_if_null()
        ...
        if (
            self._active_status
            and deal_groups != None
            and deal_healer != None
            and self._assignedheir._suffgroups != {}
        ):
            self._assignedheir.set_group_party(deal_groups, deal_healer)
    ...
    def set_requiredheirs(
        ...
        for required in coalesced_requireds.values():
            ...
            if deal_idea_dict != None:
                base_idea = deal_idea_dict.get(required.base)
                if base_idea != None:
    ...
    def set_assignedheir(
        ...
        deal_groups: dict[GroupBrand:GroupUnit],
    ):
...
class IdeaRoot(IdeaCore):
    ...
    def set_idea_label(self, _label: str, deal_project_handle: str = None):
        if _label != root_label() and deal_project_handle is None:
            raise IdeaRootLabelNotEmptyException(
                f"Cannot set idearoot to string other than '{root_label()}'"
            )
        elif _label != deal_project_handle != None:
            raise IdeaRootLabelNotEmptyException(
                f"Cannot set idearoot to string other than '{deal_project_handle}'"
            )
        elif _label != root_label() and deal_project_handle == _label:
            self._label = _label
        else:
