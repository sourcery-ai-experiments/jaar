import dataclasses
from src.deal.group import groupunit_shop, GroupUnit, GroupBrand
...
class AssignedHeir:
    ...
    def _get_all_partys(
        ...
        for groupbrand_x in groupbrand_dict:
            dict_x |= deal_groups.get(groupbrand_x)._partys
        return dict_x

    def _get_all_suff_partys(self, deal_groups: dict[GroupBrand:GroupUnit]):
        return self._get_all_partys(deal_groups, self._suffgroups)

    def set_group_party(
        self, deal_groups: dict[GroupBrand:GroupUnit], deal_healer: PartyTitle
    ):
        self._group_party = False
        if self._suffgroups == {}:
            self._group_party = True
        else:
            all_suff_partys_x = self._get_all_suff_partys(deal_groups)
            if all_suff_partys_x.get(deal_healer) != None:
                self._group_party = True
