...
from src._truth.belief import BeliefID, beliefunit_shop
...
def test_AssignedHeir_get_all_suff_others_ReturnsSingleDictWithAllOthers_v1():
    # GIVEN
    jim_text = "Jim"
    sue_text = "Sue"
    x_truth = truthunit_shop(_owner_id=jim_text)
    x_truth.add_otherunit(other_id=jim_text)
    ...
    # WHEN
    all_others = assigned_heir_x._get_all_suff_others(truth_beliefs=x_truth._beliefs)

    # THEN
    assert len(all_others) == 1


def test_AssignedHeir_get_all_suff_others_ReturnsSingleDictWithAllOthers_v2():
    # GIVEN
    jim_text = "Jim"
    sue_text = "Sue"
    bob_text = "Bob"
    ...
    swim_belief.set_otherlink(otherlink=otherlink_shop(other_id=jim_text))
    swim_belief.set_otherlink(otherlink=otherlink_shop(other_id=sue_text))
...
def test_AssignedHeir_set_owner_id_assigned_CorrectlySetsAttribute_Empty_suffbeliefs_x():
    ...
    assigned_heir_x.set_owner_id_assigned(
        truth_beliefs=truth_beliefs, truth_owner_id=""
    )
...
def test_AssignedHeir_set_owner_id_assigned_CorrectlySetsAttribute_NonEmpty_suffbeliefs_x_v1():
    ...
    x_truth.add_otherunit(other_id=jim_text)
    x_truth.add_otherunit(other_id=sue_text)
    truth_owner_id = x_truth._owner_id
    truth_beliefs = x_truth._beliefs
    print(f"{len(truth_beliefs)=}")
    # print(f"{truth_beliefs.get(jim_text)=}")
    # print(f"{truth_beliefs.get(sue_text)=}")
    ...
    # WHEN
    assigned_heir_x.set_owner_id_assigned(truth_beliefs, truth_owner_id)
...
def test_AssignedHeir_set_owner_id_assigned_CorrectlySetsAttribute_NonEmpty_suffbeliefs_x_v3():
    ...
    assigned_heir_x = assigned_heir_shop(_suffbeliefs=_suffbeliefs_x)
    assert assigned_heir_x._owner_id_assigned is False
    assigned_heir_x.set_owner_id_assigned(x_truth._beliefs, x_truth._owner_id)
    assert assigned_heir_x._owner_id_assigned

    # WHEN
    swim_belief.del_otherlink(other_id=jim_text)
    x_truth.set_beliefunit(y_beliefunit=swim_belief)
    assigned_heir_x.set_owner_id_assigned(x_truth._beliefs, x_truth._owner_id)
...
def test_AssignedHeir_set_suffbelief_AssignedUnitNotEmpty_ParentAssignedHeirEmpty():
    ...
    parent_assignheir_empty = assigned_heir_shop()
    assigned_heir_x.set_suffbeliefs(
        parent_assignheir_empty, assignunit=assignedunit_x, truth_beliefs=None
    )
...
def test_AssignedHeir_set_suffbelief_AssignedUnitEmpty_ParentAssignedHeirNotEmpty():
    ...
    parent_assigned_heir.set_suffbeliefs(
        empty_assigned_heir, assignedunit_swim, truth_beliefs=None
    )
    ...
    assigned_heir_x.set_suffbeliefs(
        parent_assigned_heir, assignunit=assignedunit_empty, truth_beliefs=None
    )
...
def test_AssignedHeir_set_suffbelief_AssignedUnitEqualParentAssignedHeir_NonEmpty():
    ...
    assert assigned_heir_x._suffbeliefs == {}
    assigned_heir_x.set_suffbeliefs(
        parent_assigned_heir, assignunit=assignedunit_swim, truth_beliefs=None
    )
...
def test_AssignedHeir_set_suffbelief_AssignedUnit_NotEqual_ParentAssignedHeir_NonEmpty():
    ...
    x_truth.add_otherunit(other_id=sue_text)
    x_truth.add_otherunit(other_id=bob_text)
    x_truth.add_otherunit(other_id=tom_text)

    swim2_text = ",swim2"
    swim2_belief = beliefunit_shop(belief_id=swim2_text)
    swim2_belief.set_otherlink(otherlink=otherlink_shop(other_id=jim_text))
    swim2_belief.set_otherlink(otherlink=otherlink_shop(other_id=sue_text))
    x_truth.set_beliefunit(y_beliefunit=swim2_belief)

    swim3_text = ",swim3"
    swim3_belief = beliefunit_shop(belief_id=swim3_text)
    swim3_belief.set_otherlink(otherlink=otherlink_shop(other_id=jim_text))
    swim3_belief.set_otherlink(otherlink=otherlink_shop(other_id=sue_text))
    swim3_belief.set_otherlink(otherlink=otherlink_shop(other_id=tom_text))
    x_truth.set_beliefunit(y_beliefunit=swim3_belief)

    parent_assignedunit = assignedunit_shop()
    parent_assignedunit.set_suffbelief(belief_id=swim3_text)
    parent_assigned_heir = assigned_heir_shop()
    parent_assigned_heir.set_suffbeliefs(
        parent_assignheir=None, assignunit=parent_assignedunit, truth_beliefs=None
    )
    ...
    assigned_heir_x = assigned_heir_shop()
    assigned_heir_x.set_suffbeliefs(
        parent_assigned_heir, assignedunit_swim2, truth_beliefs=x_truth._beliefs
    )
...
def test_AssignedHeir_set_suffbelief_AssignedUnit_NotEqualParentAssignedHeir_RaisesError():
    ...
    # WHEN / THEN
    assigned_heir_x = assigned_heir_shop()
    all_parent_assignedheir_others = {jim_text, sue_text}
    all_assignedunit_others = {jim_text, sue_text, tom_text}
    with pytest_raises(Exception) as excinfo:
        assigned_heir_x.set_suffbeliefs(
            parent_assigned_heir, assignedunit_swim3, truth_beliefs=x_truth._beliefs
        )
    assert (
        str(excinfo.value)
