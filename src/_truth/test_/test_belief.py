from src._truth.other import OtherID, otherlink_shop
...
def test_BeliefUnit_reset_truth_importance_SetsAttrCorrectly():
    # GIVEN
...
def test_BalanceHeir_set_truth_importance_CorrectlySetsAttr():
    # GIVEN
    bikers_belief_id = BeliefID("bikers")
    bikers_credor_weight = 3.0
    ...
    # WHEN
    belief_heir_x.set_truth_cred_debt(
        idea_truth_importance=idea_truth_importance,
    ...
    # THEN
    assert belief_heir_x._truth_cred == 0.05
    assert belief_heir_x._truth_debt == 0.1
...
def test_BalanceLine_exists():
    # GIVEN
    bikers_belief_id = BeliefID("bikers")
    bikers_truth_cred = 0.33
    bikers_truth_debt = 0.55

    # WHEN
    bikers_balanceline = BalanceLine(
        belief_id=bikers_belief_id,
        _truth_cred=bikers_truth_cred,
        _truth_debt=bikers_truth_debt,
    )

    # THEN
    assert bikers_balanceline.belief_id == bikers_belief_id
    assert bikers_balanceline._truth_cred == bikers_truth_cred
    assert bikers_balanceline._truth_debt == bikers_truth_debt
...
def test_balanceline_shop_ReturnsCorrectObj_exists():
    ...
    assert biker_balanceline._truth_cred == bikers_truth_cred
    assert biker_balanceline._truth_debt == bikers_truth_debt


def test_BalanceLine_add_truth_cred_debt_CorrectlyModifiesAttr():
    # GIVEN
    bikers_belief_id = BeliefID("bikers")
    bikers_balanceline = balanceline_shop(
        belief_id=bikers_belief_id, _truth_cred=0.33, _truth_debt=0.55
    )
    assert bikers_balanceline.belief_id == bikers_belief_id
    assert bikers_balanceline._truth_cred == 0.33
    assert bikers_balanceline._truth_debt == 0.55

    # WHEN
    bikers_balanceline.add_truth_cred_debt(truth_cred=0.11, truth_debt=0.2)

    # THEN
    assert bikers_balanceline._truth_cred == 0.44
    assert bikers_balanceline._truth_debt == 0.75
