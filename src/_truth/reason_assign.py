...
class AssignedHeir:
    ...
    def _get_all_others(
        ...
        for belief_id_x in belief_id_dict:
            dict_x |= truth_beliefs.get(belief_id_x)._others
        return dict_x

    def _get_all_suff_others(
        self, truth_beliefs: dict[BeliefID:BeliefUnit]
    ) -> dict[BeliefID:BeliefUnit]:
        return self._get_all_others(truth_beliefs, self._suffbeliefs)

    def set_owner_id_assigned(
        self, truth_beliefs: dict[BeliefID:BeliefUnit], truth_owner_id: OtherID
    ):
        self._owner_id_assigned = False
        if self._suffbeliefs == {}:
            self._owner_id_assigned = True
        else:
            all_suff_others_x = self._get_all_suff_others(truth_beliefs)
            if all_suff_others_x.get(truth_owner_id) != None:
                self._owner_id_assigned = True
