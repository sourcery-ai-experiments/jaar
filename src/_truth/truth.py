...
from src._truth.healer import HealerHold
...
from src._truth.reason_assign import AssignedUnit
from src._truth.tree_metrics import TreeMetrics, treemetrics_shop
from src._truth.hreg_time import HregTimeIdeaSource as HregIdea
from src._truth.lemma import lemmas_shop, Lemmas
from src._truth.origin import originunit_get_from_dict, originunit_shop, OriginUnit
from src._truth.idea import (
    IdeaUnit,
    ideaunit_shop,
    ideaattrfilter_shop,
    IdeaAttrFilter,
...
class InvalidTruthException(Exception):
    pass
...
class TruthUnit:
    ...
    _originunit: OriginUnit = None  # In job truths this shows source
    # calc_truth_metrics Calculated field begin
    ...
    # calc_truth_metrics Calculated field end
    ...
    def set_other_credor_pool(
        ...
        if (new_other_credor_pool / self._pixel).is_integer() is False:
            raise _pixel_RatioException(
                f"Truth '{self._owner_id}' cannot set _other_credor_pool='{new_other_credor_pool}'. It is not divisible by pixel '{self._pixel}'"
            )
    ...
    def set_other_debtor_pool(
        ...
        if (new_other_debtor_pool / self._pixel).is_integer() is False:
            raise _pixel_RatioException(
                f"Truth '{self._owner_id}' cannot set _other_debtor_pool='{new_other_debtor_pool}'. It is not divisible by pixel '{self._pixel}'"
            )
    ...
    def set_others_output_truth_meld_order(self):
        ...
        for count_x, x_otherunit in enumerate(sort_others_list):
            x_otherunit.set_output_truth_meld_order(count_x)

    def clear_others_output_truth_meld_order(self):
        for x_otherunit in self._others.values():
            x_otherunit.clear_output_truth_meld_order()
    ...
    def set_real_id(self, real_id: str):
        ...
        for idea_obj in self._idea_dict.values():
            idea_obj._truth_real_id = self._real_id
    ...
    def set_max_tree_traverse(self, int_x: int):
        if int_x < 2:
            raise InvalidTruthException(
                f"set_max_tree_traverse: input '{int_x}' must be number that is 2 or greater"
            )
        else:
            self._max_tree_traverse = int_x

    def get_truth_sprung_from_single_idea(self, road: RoadUnit) -> any:
        self.calc_truth_metrics()
        x_idea = self.get_idea_obj(road)
        new_weight = self._weight * x_idea._truth_importance
        x_truth = truthunit_shop(_owner_id=self._idearoot._label, _weight=new_weight)
        ...
        for road_assc in sorted(list(self._get_relevant_roads({road}))):
            ...
            if new_yx._parent_road != "":
                x_truth.add_idea(new_yx, parent_road=new_yx._parent_road)
    ...
    def _are_all_others_beliefs_are_in_idea_kid(self, road: RoadUnit) -> bool:
        ...
        # check all truth._others are in balanceheir_otherunits
    ...
    def add_to_belief_truth_cred_debt(
        self,
        belief_id: BeliefID,
        balanceheir_truth_cred: float,
        balanceheir_truth_debt: float,
    ):
    ...
    def _get_lemma_factunits(self) -> dict[RoadUnit:FactUnit]:
        ...
        while count_x < 10000 and x_lemmas.is_lemmas_evaluated() is False:
            count_x += 1
            if count_x == 9998:
                raise InvalidTruthException("lemma loop failed")
    ...
    def add_idea(
        ...
        if idea_kid._truth_real_id != self._real_id:
            idea_kid._truth_real_id = self._real_id
    ...
    def del_idea_obj(self, road: RoadUnit, del_children: bool = True):
        if road == self._idearoot.get_road():
            raise InvalidTruthException("Idearoot cannot be deleted")
        parent_road = get_parent_road(road)
    ...
    def edit_idea_label(
        ...
        if self.idea_exists(old_road) is False:
            raise InvalidTruthException(f"Idea {old_road=} does not exist")
    ...
    def _set_ideaattrfilter_begin_close(
        ...
        if (
            ...
            raise InvalidTruthException("Root Idea cannot have numor denom reest.")
            ...
            for x_belief_id in healerhold._belief_ids:
                if self._beliefs.get(x_belief_id) is None:
                    raise healerhold_belief_id_Exception(
                        f"Idea cannot edit healerhold because belief_id '{x_belief_id}' does not exist as belief in Truth"
                    )
    ...
    def _add_to_otherunits_truth_cred_debt(self, idea_truth_importance: float):
    ...
    def _reset_beliefunits_truth_cred_debt(self):
        for balancelink_obj in self._beliefs.values():
            balancelink_obj.reset_truth_cred_debt()

    def _set_beliefunits_truth_importance(
        self, balanceheirs: dict[BeliefID:BalanceLink]
    ):
        for balancelink_obj in balanceheirs.values():
            self.add_to_belief_truth_cred_debt(
                belief_id=balancelink_obj.belief_id,
                balanceheir_truth_cred=balancelink_obj._truth_cred,
    ...
    def _allot_beliefs_truth_importance(self):
        for belief_obj in self._beliefs.values():
            belief_obj._set_otherlink_truth_cred_debt()
            for otherlink in belief_obj._others.values():
                self.add_to_otherunit_truth_cred_debt(
    ...
    def _reset_otherunit_truth_cred_debt(self):
        for otherunit in self._others.values():
            otherunit.reset_truth_cred_debt()
    ...
    def get_idea_obj(self, road: RoadUnit, if_missing_create: bool = False) -> IdeaUnit:
        if road is None:
            raise InvalidTruthException("get_idea_obj received road=None")
        if self.idea_exists(road) is False and not if_missing_create:
            raise InvalidTruthException(f"get_idea_obj failed. no item at '{road}'")
    ...
    def _set_ancestors_metrics(self, road: RoadUnit, econ_exceptions: bool = False):
        ...
        while ancestor_roads != []:
            ...
            if x_idea_obj._healerhold.any_belief_id_exists():
                ...
                healerhold_count += 1
                self._sum_healerhold_importance += x_idea_obj._truth_importance
    ...
    def _set_root_attributes(self, econ_exceptions: bool):
        ...
        x_idearoot.set_assignedheir(parent_assignheir=None, truth_beliefs=self._beliefs)
        ...
        x_idearoot.set_active(
            ...
            truth_beliefunits=self._beliefs,
            truth_owner_id=self._owner_id,
        )
        x_idearoot.set_truth_importance(fund_onset_x=0, parent_fund_cease=1)
        x_idearoot.set_balanceheirs_truth_cred_debt()
        ...
        if x_idearoot.is_kidless():
            ...
            self._allot_truth_importance(idea=self._idearoot)
    ...
    def _set_kids_attributes(
        ...
        idea_kid.set_truth_importance(
            fund_onset_x=fund_onset,
            parent_truth_importance=parent_idea._truth_importance,
        ...
        if idea_kid.is_kidless():
            # set idea's ancestor metrics using truth root as common reference
            self._set_ancestors_metrics(idea_kid.get_road(), econ_exceptions)
            self._allot_truth_importance(idea=idea_kid)

    def _allot_truth_importance(self, idea: IdeaUnit):
        ...
        if idea.is_balanceheirless() is False:
            self._set_beliefunits_truth_importance(idea._balanceheirs)
        elif idea.is_balanceheirless():
            self._add_to_otherunits_truth_cred_debt(idea._truth_importance)

    def get_truth_importance(
        self, parent_truth_importance: float, weight: int, sibling_total_weight: int
    ) -> float:
        sibling_ratio = weight / sibling_total_weight
        return parent_truth_importance * sibling_ratio
    ...
    def _clear_truth_base_metrics(self):
    ...
    def calc_truth_metrics(self, econ_exceptions: bool = False):
        ...
        while not self._rational and self._tree_traverse_count < max_count:
            self._clear_truth_base_metrics()
    ...
    def _execute_tree_traverse(self, econ_exceptions: bool = False):
        ...
        fund_onset = self._idearoot._truth_fund_onset
        parent_fund_cease = self._idearoot._truth_fund_cease
        ...
        for idea_kid in self._idearoot._kids.values():
            ...
            fund_onset += idea_kid._truth_importance
        ...
        while cache_idea_list != []:
            ...
            if parent_idea._kids != None:
                fund_onset = parent_idea._truth_fund_onset
                parent_fund_cease = parent_idea._truth_fund_cease
                for idea_kid in parent_idea._kids.values():
                    self._set_kids_attributes(
                        idea_kid=idea_kid,
                        fund_onset=fund_onset,
    ...
    def _set_econ_dict(self):
        ...
        for x_idea in self._idea_dict.values():
            if self._sum_healerhold_importance == 0:
                ...
                x_idea._healerhold_importance = x_idea._truth_importance / x_sum
    ...
    def _pre_tree_traverse_cred_debt_reset(self):
        ...
        self._reset_beliefunits_truth_cred_debt()
        self._reset_beliefunits_truth_cred_debt()
        self._reset_otherunit_truth_cred_debt()
    ...
    def get_truth4other(self, other_id: OtherID, facts: dict[RoadUnit:FactCore]):
        self.calc_truth_metrics()
        truth4other = truthunit_shop(_owner_id=other_id)
        truth4other._idearoot._truth_importance = self._idearoot._truth_importance
        # get other's others: otherzone

        # get otherzone beliefs
        other_beliefs = self.get_other_belief_ids(other_id=other_id)

        # set truth4other by traversing the idea tree and selecting associated beliefs
        # set root
        not_included_truth_importance = 0
        truth4other._idearoot.clear_kids()
        for ykx in self._idearoot._kids.values():
            ...
            if y4a_included:
                y4a_new = ideaunit_shop(
                    _label=ykx._label,
                    _truth_importance=ykx._truth_importance,
                ...
                truth4other._idearoot._kids[ykx._label] = y4a_new
            else:
                not_included_truth_importance += ykx._truth_importance

        if not_included_truth_importance > 0:
            y4a_exterior = ideaunit_shop(
                _label="__truth4other__",
                _truth_importance=not_included_truth_importance,
            )
            truth4other._idearoot._kids[y4a_exterior._label] = y4a_exterior
    ...
    def meld(
        ...
        self._meld_beliefs(exterior_truth)
        if not ignore_otherunits:
            self._meld_others(exterior_truth)
        self._meld_ideas(exterior_truth, other_weight)
        self._meld_facts(exterior_truth)
        self._weight = get_meld_weight(
            src_weight=self._weight,
            src_meld_strategy="default",
            exterior_weight=exterior_truth._weight,
            exterior_meld_strategy="default",
        )
        self._meld_originlinks(exterior_truth._owner_id, other_weight)

    def _meld_ideas(self, exterior_truth, other_weight: float):
        # meld idearoot
        self._idearoot.meld(exterior_idea=exterior_truth._idearoot, _idearoot=True)

        # meld all exterior ideas
        other_id = exterior_truth._owner_id
        o_idea_list = exterior_truth.get_idea_list_without_idearoot()
    ...
    def _meld_others(self, exterior_truth):
        for otherunit in exterior_truth._others.values():
            if self.get_other(otherunit.other_id) is None:
                self.set_otherunit(otherunit=otherunit)
            else:
                self.get_other(otherunit.other_id).meld(otherunit)

    def _meld_beliefs(self, exterior_truth):
        for brx in exterior_truth._beliefs.values():
    ...
    def _meld_facts(self, exterior_truth):
        for hx in exterior_truth._idearoot._factunits.values():
    ...
    def get_assignment(
        ...
        self.calc_truth_metrics()
        self._set_assignment_others(truth_x, assignor_others, assignor_other_id)
        self._set_assignment_beliefs(truth_x)
        assignor_pledges = self._get_assignor_pledge_ideas(truth_x, assignor_other_id)
        relevant_roads = self._get_relevant_roads(assignor_pledges)
        self._set_assignment_ideas(truth_x, relevant_roads)
        return truth_x

    def _set_assignment_ideas(self, x_truth, relevant_roads: dict[RoadUnit:str]):
        ...
        for relevant_road in sorted_relevants:
            ...
            relevant_idea.find_replace_road(
                ...
                new_road=x_truth._real_id,
            )
            relevant_idea.clear_kids()
            x_truth.add_idea(relevant_idea, parent_road=relevant_idea._parent_road)

        for afu in self._idearoot._factunits.values():
            if relevant_roads.get(afu.base) != None:
                x_truth.set_fact(
                    base=rebuild_road(afu.base, self._real_id, x_truth._real_id),
                    pick=rebuild_road(afu.pick, self._real_id, x_truth._real_id),
                    open=afu.open,
                    nigh=afu.nigh,
                )
    ...
    def _set_assignment_others(
        ...
        if self.other_exists(assignor_other_id):
            ...
            for other_id_x in others_set:
                truth_x.set_otherunit(otherunit=self.get_other(other_id_x))
        return truth_x

    def _set_assignment_beliefs(self, truth_x):
        revelant_beliefs = get_others_relevant_beliefs(self._beliefs, truth_x._others)
        for belief_belief_id, belief_others in revelant_beliefs.items():
            if truth_x._beliefs.get(belief_belief_id) is None:
                ...
                truth_x.set_beliefunit(belief_x)

    def _get_assignor_pledge_ideas(
        self, truth_x, assignor_other_id: BeliefID
    ) -> dict[RoadUnit:int]:
        assignor_beliefs = get_other_relevant_beliefs(
            truth_x._beliefs, assignor_other_id
        )
        return {
            idea_road: -1
...
def truthunit_shop(
    ...
    x_truth._idearoot = ideaunit_shop(
        _root=True, _uid=1, _level=0, _truth_real_id=x_truth._real_id
    )
    x_truth._idearoot._road_delimiter = x_truth._road_delimiter
    x_truth.set_max_tree_traverse(3)
    x_truth._rational = False
    x_truth._originunit = originunit_shop()
    return x_truth


def get_from_json(x_truth_json: str) -> TruthUnit:
    return get_from_dict(get_dict_from_json(x_truth_json))


def get_from_dict(truth_dict: dict) -> TruthUnit:
    x_truth = truthunit_shop()
    x_truth.set_owner_id(get_obj_from_truth_dict(truth_dict, "_owner_id"))
    x_truth._weight = get_obj_from_truth_dict(truth_dict, "_weight")
    x_truth.set_max_tree_traverse(
        get_obj_from_truth_dict(truth_dict, "_max_tree_traverse")
    )
    x_truth.set_real_id(get_obj_from_truth_dict(truth_dict, "_real_id"))
    x_truth._road_delimiter = default_road_delimiter_if_none(
        get_obj_from_truth_dict(truth_dict, "_road_delimiter")
    )
    x_truth._pixel = default_pixel_if_none(
        get_obj_from_truth_dict(truth_dict, "_pixel")
    )
    x_truth._penny = default_penny_if_none(
        get_obj_from_truth_dict(truth_dict, "_penny")
    )
    x_truth._other_credor_pool = get_obj_from_truth_dict(
        truth_dict, "_other_credor_pool"
    )
    x_truth._other_debtor_pool = get_obj_from_truth_dict(
        truth_dict, "_other_debtor_pool"
    )
    if get_obj_from_truth_dict(truth_dict, "_meld_strategy") is None:
        x_truth._meld_strategy = get_meld_default()
    else:
        x_truth._meld_strategy = get_obj_from_truth_dict(truth_dict, "_meld_strategy")
    x_truth._last_atom_id = get_obj_from_truth_dict(truth_dict, "_last_atom_id")
    for x_otherunit in get_obj_from_truth_dict(
        truth_dict, dict_key="_others", _road_delimiter=x_truth._road_delimiter
    ).values():
        x_truth.set_otherunit(x_otherunit)
    for x_beliefunit in get_obj_from_truth_dict(
        truth_dict, dict_key="_beliefs", _road_delimiter=x_truth._road_delimiter
    ).values():
        x_truth.set_beliefunit(x_beliefunit)
    x_truth._originunit = get_obj_from_truth_dict(truth_dict, "_originunit")

    set_idearoot_from_truth_dict(x_truth, truth_dict)
    x_truth.calc_truth_metrics()  # clean up tree traverse defined fields
    return x_truth


def set_idearoot_from_truth_dict(x_truth: TruthUnit, truth_dict: dict):
    idearoot_dict = truth_dict.get("_idearoot")
    x_truth._idearoot = ideaunit_shop(
        _root=True,
        _label=x_truth._real_id,
        ...
        _truth_real_id=x_truth._real_id,
    )
    set_idearoot_kids_from_dict(x_truth, idearoot_dict)


def set_idearoot_kids_from_dict(x_truth: TruthUnit, idearoot_dict: dict):
    ...
    for x_dict in get_obj_from_idea_dict(idearoot_dict, "_kids").values():
        x_dict[parent_road_text] = x_truth._real_id
    ...
    while to_evaluate_idea_dicts != []:
        ...
        for kid_dict in get_obj_from_idea_dict(idea_dict, "_kids").values():
            ...
            kid_dict[parent_road_text] = x_truth.make_road(parent_road, kid_label)
        ...
        x_truth.add_idea(x_ideakid, parent_road=idea_dict[parent_road_text])


def get_obj_from_truth_dict(
...
def get_dict_of_truth_from_dict(x_dict: dict[str:dict]) -> dict[str:TruthUnit]:
    truthunits = {}
    for truthunit_dict in x_dict.values():
        x_truth = get_from_dict(truth_dict=truthunit_dict)
        truthunits[x_truth._owner_id] = x_truth
    return truthunits
