...
from src._truth.origin import OriginUnit, originunit_get_from_dict
from src._truth.origin import originunit_shop
from src._truth.meld import get_meld_weight, validate_meld_strategy
...
class IdeaUnit:
    ...
    _root: bool = None
    _kids: dict = None
    _truth_real_id: RealID = None
    _uid: int = None  # Calculated field?
    _balancelinks: dict[BeliefID:BalanceLink] = None
    _balanceheirs: dict[BeliefID:BalanceHeir] = None  # Calculated field
    _balancelines: dict[BeliefID:BalanceLine] = None  # Calculated field
    ...
    _truth_importance: float = None
    _truth_fund_onset: float = None
    _truth_fund_cease: float = None
    _task: bool = None
    _active: bool = None
    ...
    def set_truth_importance(
        self,
        fund_onset_x: float,
        parent_truth_importance: float = None,
        parent_fund_cease: float = None,
    ):
        parent_truth_importance = get_1_if_None(parent_truth_importance)
        self.set_kids_total_weight()
        self._truth_importance = None
        self._truth_fund_onset = None
        self._truth_fund_cease = None
        sibling_ratio = self._weight / self._sibling_total_weight
        self._truth_importance = parent_truth_importance * sibling_ratio
        self._truth_fund_onset = fund_onset_x
        self._truth_fund_cease = self._truth_fund_onset + self._truth_importance
        self._truth_fund_cease = min(self._truth_fund_cease, parent_fund_cease)
        self.set_balanceheirs_truth_cred_debt()
    ...
    def inherit_balanceheirs(
        self, parent_balanceheirs: dict[BeliefID:BalanceHeir] = None
    ):
        ...
        for ib in parent_balanceheirs.values():
            ...
            self._balanceheirs[balanceheir.belief_id] = balanceheir
    ...
    def set_kidless_balancelines(self):
        ...
        for bh in self._balanceheirs.values():
            x_balanceline = balanceline_shop(
                ...
                _truth_cred=bh._truth_cred,
                _truth_debt=bh._truth_debt,
            )
            self._balancelines[x_balanceline.belief_id] = x_balanceline

    def set_balancelines(self, child_balancelines: dict[BeliefID:BalanceLine] = None):
        if child_balancelines is None:
            child_balancelines = {}

        # get balancelines from child
        for bl in child_balancelines.values():
            if self._balancelines.get(bl.belief_id) is None:
                self._balancelines[bl.belief_id] = balanceline_shop(
                    belief_id=bl.belief_id,
                    _truth_cred=0,
                    _truth_debt=0,
                )

            self._balancelines[bl.belief_id].add_truth_cred_debt(
                truth_cred=bl._truth_cred, truth_debt=bl._truth_debt
            )
    ...
    def set_balanceheirs_truth_cred_debt(self):
        ...
        for balanceheir_x in self._balanceheirs.values():
            balanceheir_x.set_truth_cred_debt(
                idea_truth_importance=self._truth_importance,
    ...
    def set_idea_label(self, _label: str):
        if (
            self._root
            and _label != None
            and _label != self._truth_real_id
            and self._truth_real_id != None
        ):
            raise Idea_root_LabelNotEmptyException(
                f"Cannot set idearoot to string different than '{self._truth_real_id}'"
            )
        elif self._root and self._truth_real_id is None:
    ...
    def _set_idea_attr(self, idea_attr: IdeaAttrFilter):
        ...
        if idea_attr.balancelink_del != None:
            self.del_balancelink(belief_id=idea_attr.balancelink_del)
    ...
    def set_balancelink(self, balancelink: BalanceLink):
        self._balancelinks[balancelink.belief_id] = balancelink

    def del_balancelink(self, belief_id: BeliefID):
        try:
            self._balancelinks.pop(belief_id)
        except KeyError as e:
            raise (f"Cannot delete balancelink '{belief_id}'.") from e
    ...
    def set_active(
        self,
        tree_traverse_count: int,
        truth_beliefunits: dict[BeliefID:BeliefUnit] = None,
        truth_owner_id: OtherID = None,
    ):
        prev_to_now_active = deepcopy(self._active)
        self._active = self._create_active(
            truth_beliefunits=truth_beliefunits, truth_owner_id=truth_owner_id
        )
        self._set_idea_task()
    ...
    def _create_active(
        self, truth_beliefunits: dict[BeliefID:BeliefUnit], truth_owner_id: OtherID
    ) -> bool:
        self.set_reasonheirs_status()
        x_bool = self._are_all_reasonheir_active_true()
        if (
            x_bool
            and truth_beliefunits != {}
            and truth_owner_id != None
            and self._assignedheir._suffbeliefs != {}
        ):
            self._assignedheir.set_owner_id_assigned(truth_beliefunits, truth_owner_id)
    ...
    def set_reasonheirs(
        ...
        for old_reasonheir in coalesced_reasons.values():
            ...
            # if truth_idea_dict != None:
            base_idea = truth_idea_dict.get(old_reasonheir.base)
            if base_idea != None:
    ...
    def get_balancelinks_dict(self):
        return {
            x_belief_id: balancelink.get_dict()
            for x_belief_id, balancelink in self._balancelinks.items()
        }
    ...
    def set_assignedheir(
        ...
        truth_beliefs: dict[BeliefID:BeliefUnit],
    ):
...
def ideaunit_shop(
    ...
    _balancelinks: dict[BeliefID:BalanceLink] = None,
    _balanceheirs: dict[BeliefID:BalanceHeir] = None,  # Calculated field
    _balancelines: dict[BeliefID:BalanceLink] = None,  # Calculated field
    _reasonunits: dict[RoadUnit:ReasonUnit] = None,
    _reasonheirs: dict[RoadUnit:ReasonHeir] = None,  # Calculated field
    _assignedunit: AssignedUnit = None,
    _assignedheir: AssignedHeir = None,  # Calculated field
    ...
    if _truth_real_id is None:
        _truth_real_id = root_label()
    ...
    x_ideakid = IdeaUnit(
        ...
        _truth_real_id=_truth_real_id,
        # Calculated fields
        _level=_level,
        _kids_total_weight=get_0_if_None(_kids_total_weight),
        _truth_importance=_truth_importance,
        _truth_fund_onset=_truth_fund_onset,
        _truth_fund_cease=_truth_fund_cease,
        _task=_task,
        _active=_active,
    ...
    if x_ideakid._root:
        x_ideakid.set_idea_label(_label=_truth_real_id)
    else:
