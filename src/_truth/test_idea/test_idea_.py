...
from src._truth.belief import BeliefID, balancelink_shop, balanceheir_shop
...
from src._truth.reason_assign import assignedunit_shop, assigned_heir_shop
from src._truth.origin import originunit_shop
from src._truth.idea import IdeaUnit, ideaunit_shop, get_obj_from_idea_dict
...
def test_IdeaUnit_exists():
    ...
    assert x_ideaunit._all_other_cred is None
    assert x_ideaunit._all_other_debt is None
    ...
    assert x_ideaunit._truth_importance is None
    assert x_ideaunit._truth_fund_onset is None
    assert x_ideaunit._truth_fund_cease is None
    ...
    assert x_ideaunit._truth_real_id is None
...
def test_ideaunit_shop_NoParametersReturnsCorrectObj():
    ...
    assert x_ideaunit._truth_real_id == root_label()
...
def test_IdeaUnit_balancelinks_exist():
    ...
    # WHEN
    sport_text = "sport"
    sport_idea = ideaunit_shop(_label=sport_text, _balancelinks=belief_links)

    # THEN
    assert sport_idea._balancelinks == belief_links
...
def test_IdeaUnit_get_inherited_balanceheirs_weight_sum_SetsAttrCorrectly_WithValues():
    ...
    # WHEN
    sport_text = "sport"
    sport_idea = ideaunit_shop(_label=sport_text, _balanceheirs=belief_links)
    ...
    assert swimmer_balanceheir._truth_cred is None
    assert swimmer_balanceheir._truth_debt is None
    biker_balanceheir = sport_idea._balanceheirs.get(biker_text)
    assert biker_balanceheir._truth_cred is None
    assert biker_balanceheir._truth_debt is None

    # WHEN
    sport_idea._truth_importance = 0.25
    sport_idea.set_balanceheirs_truth_cred_debt()

    # THEN
    print(f"{len(sport_idea._balanceheirs)=}")
    swimmer_balanceheir = sport_idea._balanceheirs.get(swimmer_text)
    assert swimmer_balanceheir._truth_cred != None
    assert swimmer_balanceheir._truth_debt != None
    biker_balanceheir = sport_idea._balanceheirs.get(biker_text)
    assert biker_balanceheir._truth_cred != None
    assert biker_balanceheir._truth_debt != None
...
def test_IdeaUnit_set_reasonheirsCorrectlySourcesFromOutside():
    ...
    # WHEN
    ball_idea.set_reasonheirs(reasonheirs=reasonheirs, truth_idea_dict={})
...
def test_IdeaUnit_set_reasonheirsCorrectlySourcesFromSelf():
    ...
    # WHEN
    ball_idea.set_reasonheirs(reasonheirs=None, truth_idea_dict={})
...
def test_IdeaUnit_clear_all_other_cred_debt_ClearsCorrectly():
    # GIVEN
    ball_text = "ball"
    ball_idea = ideaunit_shop(_label=ball_text, _all_other_cred=55, _all_other_debt=33)
    assert ball_idea._all_other_cred == 55
    assert ball_idea._all_other_debt == 33

    # WHEN
    ball_idea.clear_all_other_cred_debt()

    # THEN
    assert ball_idea._all_other_cred is None
    assert ball_idea._all_other_debt is None
...
def test_IdeaUnit_get_dict_ReturnsCorrectCompleteDict():
    ...
    x1_balancelinks = {biker_belief_id: biker_get_dict, flyer_belief_id: flyer_get_dict}
    sue_text = "Sue"
    yao_text = "Yao"
    ...
    casa_idea._originunit.set_originlink(other_id="Ray", weight=None)
    casa_idea._originunit.set_originlink(other_id="Lei", weight=4)
    x_begin = 11
    x_close = 12
    x_addin = 13
    x_denom = 14
...
def test_IdeaUnit_get_reasonheir_ReturnsCorrectObj():
    ...
    clean_idea.set_reasonheirs(reasonheirs=reason_heirs_x, truth_idea_dict={})
...
def test_IdeaUnit_set_assignedheir_CorrectlySetsAttr():
    ...
    sport_text = "sports"
    sport_idea = ideaunit_shop(_label=sport_text)
    sport_idea._assignedunit.set_suffbelief(belief_id=swim_text)
    assert sport_idea._assignedheir is None

    # WHEN
    sport_idea.set_assignedheir(parent_assignheir=None, truth_beliefs=None)

    # THEN
    assert sport_idea._assignedheir != None
    swim_assignedunit = assignedunit_shop()
    swim_assignedunit.set_suffbelief(belief_id=swim_text)
    swim_assigned_heir = assigned_heir_shop()
    swim_assigned_heir.set_suffbeliefs(
        assignunit=swim_assignedunit, parent_assignheir=None, truth_beliefs=None
    )
