...
def test_time_get_time_min_from_dt_ReturnsCorrectObj():
    kia_truth = truthunit_shop(_owner_id="Kia")
    kia_truth.set_time_hreg_ideas(c400_count=6)
    assert kia_truth.get_time_min_from_dt(dt=datetime(2000, 1, 1, 0, 0))
    assert kia_truth.get_time_min_from_dt(dt=datetime(1, 1, 1, 0, 0)) == 527040
    assert kia_truth.get_time_min_from_dt(dt=datetime(1, 1, 2, 0, 0)) == 527040 + 1440
    assert kia_truth.get_time_min_from_dt(dt=datetime(400, 1, 1, 0, 0)) == 210379680
    assert kia_truth.get_time_min_from_dt(dt=datetime(800, 1, 1, 0, 0)) == 420759360
    assert kia_truth.get_time_min_from_dt(dt=datetime(1200, 1, 1, 0, 0)) == 631139040


def test_get_time_400Yearsegment_from_min_ReturnsCorrectObj():
    kia_truth = truthunit_shop("Kia")
    kia_truth.set_time_hreg_ideas(c400_count=6)
    assert kia_truth.get_time_c400_from_min(min=0)[0] == 0
    assert kia_truth.get_time_c400_from_min(min=210379680)[0] == 1
    assert kia_truth.get_time_c400_from_min(min=210379681)[0] == 1
    assert kia_truth.get_time_c400_from_min(min=841518720)[0] == 4


def test_get_time_c400year_from_min_ReturnsCorrectObj():
    kia_truth = truthunit_shop("Kia")
    kia_truth.set_time_hreg_ideas(c400_count=6)
    assert kia_truth.get_time_c400yr_from_min(min=0)[0] == 0
    assert kia_truth.get_time_c400yr_from_min(min=1)[0] == 0
    assert kia_truth.get_time_c400yr_from_min(min=1)[2] == 1
    assert kia_truth.get_time_c400yr_from_min(min=210379680)[0] == 0
    assert kia_truth.get_time_c400yr_from_min(min=210379680)[0] == 0
    assert kia_truth.get_time_c400yr_from_min(min=210379681)[0] == 0
    assert kia_truth.get_time_c400yr_from_min(min=841518720)[0] == 0
    assert kia_truth.get_time_c400yr_from_min(min=576000)[0] == 1
    assert kia_truth.get_time_c400yr_from_min(min=4608000)[0] == 8
    assert kia_truth.get_time_c400yr_from_min(min=157785120)[0] == 300


def _check_time_conversion_with_random_inputs(x_truth: TruthUnit):
    py_dt = datetime(
        year=randint(1, 2800),
        month=randint(1, 12),
        day=randint(1, 28),
    ...
    assert py_dt == x_truth.get_time_dt_from_min(x_truth.get_time_min_from_dt(py_dt))


def test_get_time_dt_from_min_ReturnsCorrectObj():
    kia_truth = truthunit_shop(_owner_id="Kia")
    kia_truth.set_time_hreg_ideas(c400_count=6)
    assert kia_truth.get_time_dt_from_min(min=5000000)
    # assert kia_truth.get_time_dt_from_min(
    #     min=kia_truth.get_time_min_from_dt(dt=datetime(2000, 1, 1, 0, 0))
    # ) == datetime(2000, 1, 1, 0, 0)
    assert kia_truth.get_time_dt_from_min(min=420759360) == datetime(800, 1, 1, 0, 0)
    assert kia_truth.get_time_dt_from_min(min=631139040) == datetime(1200, 1, 1, 0, 0)
    assert kia_truth.get_time_dt_from_min(min=631751040) == datetime(1201, 3, 1, 0, 0)
    assert kia_truth.get_time_dt_from_min(min=631751060) == datetime(1201, 3, 1, 0, 20)

    x_minutes = 1063903680
    assert kia_truth.get_time_dt_from_min(min=x_minutes) == datetime(2022, 10, 29, 0, 0)
    x_next_day = x_minutes + 1440
    assert kia_truth.get_time_dt_from_min(min=x_next_day) == datetime(
        2022, 10, 30, 0, 0
    )

    _check_time_conversion_with_random_inputs(kia_truth)
    _check_time_conversion_with_random_inputs(kia_truth)
    _check_time_conversion_with_random_inputs(kia_truth)
    ...
    #     jaja_min = x_truth.get_time_min_from_dt(dt=py_dt)
    #     # print(f"assert for {year=} {month=} {day=} {jaja_min}")

    #     jaja_dt = x_truth.get_time_dt_from_min(min=jaja_min)
    #     print(
    ...
    #             assert x_truth.get_time_dt_from_min(
    #                 min=x_truth.get_time_min_from_dt(dt=datetime(year, month, day, 0, 0))
...
def test_get_time_():
    ...
    # WHEN
    kia_truth.set_time_facts(
        open=datetime(2000, 1, 1, 0, 0), nigh=datetime(2003, 11, 15, 4, 0)
    )

    # THEN
    time_text = "time"
    time_road = kia_truth.make_l1_road(time_text)
    jaja_text = "jajatime"
    jaja_road = kia_truth.make_road(time_road, jaja_text)
    assert kia_truth._idearoot._factunits[jaja_road]
    assert kia_truth._idearoot._factunits[jaja_road].open == 1051898400  # - 1440
    assert kia_truth._idearoot._factunits[jaja_road].nigh == 1053934800  # - 1440


# def test_time_hreg_set_exists():
#     x_truth = truthunit_shop(_owner_id=bob_text)
#     x_truth.set_time_hreg_ideas(c400_count=6)
#     idea_x = x_truth.get_idea_obj(x_truth.make_l1_road("hreg")
#     assert idea_x != None
#     assert x_truth.get_kid("hreg"]
#     for kid in x_truth.get_kid("hreg"]._kids.values():
#         print(f"hreg kid= {kid._label=}")

#     assert len(x_truth.get_kid("hreg"]._kids) > 0


# def test_time_hreg_set_creates_idea():
#     x_truth = examples.get_truth_base_time_example()

#     hreg_label = "hreg"
#     with pytest.raises(KeyError) as excinfo:
#         x_truth.get_kid(hreg_label]
#     assert str(excinfo.value) == f"'{hreg_label}'"
#     print(f"added {hreg_label}")
#     x_truth.set_time_hreg_ideas(c400_count=6)
#     hreg_idea = x_truth.get_kid(hreg_label]
...
#     weekday = x_truth.get_idea_obj(x_truth.make_l1_road("hreg,{weekday_label}")
#     assert weekday != None
...
#     x_truth = examples.get_truth_base_time_example()
#     c400_count = 6
#     x_truth.set_time_hreg_ideas(c400_count=c400_count)

#     timetech_label = "400 year segment"
#     timetech_road = x_truth.make_l1_road("hreg,{timetech_label}"
#     print(f"{timetech_road=}")
#     timetech = x_truth.get_idea_obj(timetech_road)
...
#     h_x_truth = examples.get_truth_base_time_example()
#     c400_count = 6
#     h_x_truth.set_time_hreg_ideas(c400_count=c400_count)

#     hy400_label = "segment400year_years"
#     hy400_road = x_truth.make_l1_road("hreg,{hy400_label}"
#     print(f"{hy400_road=}")
#     hy400_idea = h_x_truth.get_idea_obj(hy400_road)
...
#     x_truth = examples.get_truth_base_time_example()
#     x_truth.set_time_hreg_ideas(c400_count=6)
#     timetech = x_truth.get_idea_obj(x_truth.make_l1_road("hreg,day_range")
#     assert timetech != None
#     assert timetech._begin == 0
#     assert timetech._close == 876582

# x_x_truth = truthunit_shop()
# x_truth.get_idea_obj({x_truth.make_l1_road("hreg,weekday"})
...
# truth_x.edit_idea_attr(road=woork_road, reason=woork_wk_reason)
# woork_idea = truth_x.get_kid("woork"]
...
# x_truth = examples.get_truth_gregorian_years()


def test_get_jajatime_repeating_legible_text_correctlyText():
    # GIVEN
    noa_truth = truthunit_shop(_owner_id="Noa")
    noa_truth.set_time_hreg_ideas(c400_count=7)

    # WHEN / THEN
    every_day_8am_text = noa_truth.get_jajatime_repeating_legible_text(
        open=480, nigh=480, divisor=1440
    )
    print(f"ReturnsDailyText {every_day_8am_text=}")
    assert every_day_8am_text == "every day at 8am"

    every_2nd_day_8_10am_text = noa_truth.get_jajatime_repeating_legible_text(
    ...
    ReturnsEvery6DaysText = noa_truth.get_jajatime_repeating_legible_text(
    ...
    every_saturday_8am_text = noa_truth.get_jajatime_repeating_legible_text(
    ...
    sat_2nd_8am_text = noa_truth.get_jajatime_repeating_legible_text(
    ...
    sat_6th_8am_text = noa_truth.get_jajatime_repeating_legible_text(
    ...
    feb_1st_9am_text = noa_truth.get_jajatime_repeating_legible_text(
    ...
    feb_1st_7pm_text = noa_truth.get_jajatime_repeating_legible_text(
        open=1064041620.0, nigh=1064041620.0
    )
    print(f"ReturnsOneTimeEventCorrectlyMorning: {feb_1st_9am_text=}")
    assert feb_1st_7pm_text == "Wed Feb 1st, 2023 at 7pm"

    feb_2nd_12am_text = noa_truth.get_jajatime_repeating_legible_text(
