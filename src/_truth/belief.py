...
from src._truth.meld import get_meld_weight
...
class BeliefUnit(BeliefCore):
    _other_mirror: bool = None  # set by TruthUnit.set_otherunit()
    _others: dict[OtherID:OtherLink] = None  # set by TruthUnit.set_otherunit()
    _road_delimiter: str = None  # calculated by TruthUnit.set_beliefunit
    ...
    def _set_otherlink_truth_cred_debt(self):
...
class BalanceLine(BeliefCore):
    ...
    def add_truth_cred_debt(self, truth_cred: float, truth_debt: float):
        self.set_truth_cred_debt_zero_if_null()
        self._truth_cred += truth_cred
        self._truth_debt += truth_debt

    def set_truth_cred_debt_zero_if_null(self):
        if self._truth_cred is None:
            self._truth_cred = 0
        if self._truth_debt is None:
            self._truth_debt = 0


def balanceline_shop(belief_id: BeliefID, _truth_cred: float, _truth_debt: float):
    return BalanceLine(
        belief_id=belief_id, _truth_cred=_truth_cred, _truth_debt=_truth_debt
    )
