...
from src.change.agendanox import agendanox_shop, pipeline_role_job_text as role_job
...
def test_listen_to_speakers_intent_AddsTasksToJobAgendaWhenNo_suffgroupIsSet(
    ...
    listen_to_speakers_intent(new_agenda, agendanox, yao_src_listener)
...
def test_listen_to_speakers_intent_AddsTasksToJobAgenda(env_dir_setup_cleanup):
    ...
    agendanox = agendanox_shop(None, None, yao_text, get_texas_road(), role_job())
    save_file(agendanox.jobs_dir(), get_file_name(zia_text), zia_agendaunit.get_json())
...
def test_listen_to_speakers_intent_ProcessesIrrationalAgenda(env_dir_setup_cleanup):
    ...
    save_file(agendanox.jobs_dir(), get_file_name(sue_text), sue_agendaunit.get_json())
...
def test_listen_to_speakers_intent_ProcessesMissingDebtorJobAgenda(
    ...
    yao_text = "Yao"
    agendanox = agendanox_shop(None, None, yao_text, get_texas_road(), role_job())
    sue_file_path = f"{agendanox.jobs_dir()}/Sue.json"
    ...
    save_file(agendanox.roles_dir(), get_file_name(yao_text), yao_json)
...
def test_listen_to_speakers_intent_ListensToOwner_role_AndNotOwner_job(
    ...
    agendanox = agendanox_shop(None, None, yao_text, get_texas_road(), role_job())
    save_file(agendanox.roles_dir(), yao_text, yao_src_listener.get_json())
    ...
    vacuum_ideaunit._assignedunit.set_suffgroup(yao_text)
    save_file(agendanox.jobs_dir(), get_file_name(yao_text), yao_job.get_json())
