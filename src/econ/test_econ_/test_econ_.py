...
def test_EconUnit_exists():
    ...
    # WHEN
    x_econ = EconUnit(x_real_id, econ_dir=get_test_econ_dir())
...
def test_econunit_shop_ReturnsObj(env_dir_setup_cleanup):
    ...
    econ_dir = f"src/econ/examples/econs/{x_real_id}"
    sue_text = "Sue"
...
def test_econunit_shop_ReturnsObj_WithTempNames(env_dir_setup_cleanup):
    ...
    # WHEN
    x_econ = econunit_shop(real_id=x_real_id, econ_dir=get_test_econ_dir())
...
def test_econunit_shop_RaisesErrorIfParameterContains_road_delimiter():
    ...
    with pytest_raises(Exception) as excinfo:
        econunit_shop(real_id=texas_text, _road_delimiter=slash_text)
    assert (
        str(excinfo.value)
...
def test_EconUnit_set_road_delimiter_CorrectSetsAttribute(
    ...
    econ_dir = f"src/econ/examples/econs/{x_real_id}"
    sue_text = "Sue"
    x_econ = econunit_shop(x_real_id, get_test_econ_dir(), _manager_person_id=sue_text)
    assert x_econ._road_delimiter == default_road_delimiter_if_none()
...
def test_modify_real_id_example_econ_CorrectlyModifiesDirAndFiles(
    ...
    x_econ = econunit_shop(real_id=old_x_real_id, econ_dir=get_test_econ_dir())
    # delete_dir(x_econ.get_object_root_dir())
...
def test_copy_evaluation_econ_CorrectlyCopiesDirAndFiles(env_dir_setup_cleanup):
    ...
    x_econ = econunit_shop(old_x_real_id, get_test_econ_dir())
...
def test_EconUnit_get_road_ReturnsCorrectObj(env_dir_setup_cleanup):
    ...
    bob_text = "Bob"
    econ_road_with_root = create_road(x_econ.real_id, bob_text)
    ...
    # WHEN / THEN
    assert x_econ.real_id == x_econ.build_econ_road()
