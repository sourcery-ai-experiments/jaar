...
def test_get_river_circle_table_insert_sqlstr_CorrectlyPopulatesTable01(
    ...
    with x_econ.get_treasury_conn() as treasury_conn:
        ...
        assert len(get_river_circle_dict(treasury_conn, cash_worker_id=sal_text)) == 0

    # WHEN / THEN
    mstr_sqlstr = get_river_circle_table_insert_sqlstr(cash_worker_id=sal_text)
    with x_econ.get_treasury_conn() as treasury_conn:
        print(mstr_sqlstr)
    ...
    # THEN
    with x_econ.get_treasury_conn() as treasury_conn:
        river_circles = get_river_circle_dict(treasury_conn, cash_worker_id=sal_text)
    ...
    assert circle_0.dst_worker_id == sal_text
    ...
    assert circle_1.dst_worker_id == sal_text
    ...
    #     assert value.dst_worker_id == sal_text
