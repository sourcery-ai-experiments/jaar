...
def test_econ_get_agenda_partyunit_table_insert_sqlstr_CorrectlyPopulatesTable01(
    ...
    ledger_dict = get_partyview_dict(
        db_conn=x_econ.get_treasury_conn(), payer_agent_id=bob_text
    )
    # tim_ledger = None
...
def test_river_block_insert_CorrectlyPopulatesTable01(
    ...
    with x_econ.get_treasury_conn() as treasury_conn:
        treasury_conn.execute(insert_sqlstr)
        river_blocks = get_river_block_dict(treasury_conn, cash_agent_id=bob_text)
...
def test_PartyBankUnit_exists():
    ...
    assert x_partytreasury.due_agent_id == x_due_agent_id
...
def test_agenda_set_treasurying_data_partyunits_CorrectlySetsPartyUnitBankingAttr():
    ...
    partytreasuryunits = {
        partytreasuryunit_sam.due_agent_id: partytreasuryunit_sam,
        partytreasuryunit_wil.due_agent_id: partytreasuryunit_wil,
        partytreasuryunit_fry.due_agent_id: partytreasuryunit_fry,
    }
    # WHEN
    set_treasury_partytreasuryunits_to_agenda_partyunits(
...
def test_get_agenda_partyunit_table_update_treasury_due_paid_sqlstr_CorrectlyPopulatesTable01(
    ...
    with x_econ.get_treasury_conn() as treasury_conn:
        partytreasuryunits = get_partytreasuryunit_dict(
            treasury_conn, cash_agent_id=bob_text
        )
