...
def test_econ_set_credit_flow_for_agenda_CorrectlyPopulatespartytreasuryunitTable01(
    ...
    # WHEN
    x_econ.set_credit_flow_for_agenda(agent_id=sal_text)
    ...
    with x_econ.get_treasury_conn() as treasury_conn:
        river_blocks = get_river_block_dict(treasury_conn, cash_agent_id=sal_text)
...
def test_econ_set_credit_flow_for_agenda_CorrectlyUsesMaxblocksCount(
    ...
    # WHEN
    mbc = 13
    x_econ.set_credit_flow_for_agenda(agent_id=sal_text, max_blocks_count=mbc)
...
def test_econ_set_credit_flow_for_agenda_CorrectlyUpatesAgendaPartyUnits(
    ...
    x_econ.save_forum_agenda(sal_agenda_src)
    ...
    x_econ.refresh_treasury_forum_agendas_data()
    sal_agenda_before = x_econ.get_forum_agenda(agent_id=sal_text)
    ...
    # THEN
    sal_partytreasuryunits = x_econ.get_partytreasuryunits(agent_id=sal_text)
    ...
    assert bob_partytreasury.due_agent_id == bob_text
    assert tom_partytreasury.due_agent_id == tom_text
    assert bob_partytreasury.cash_master == sal_text
    assert tom_partytreasury.cash_master == sal_text

    sal_agenda_after = x_econ.get_forum_agenda(agent_id=sal_text)
    ...
    #     assert sal_partytreasuryunit.due_agent_id in [bob_text, tom_text, elu_text]
    #     x_partyunit = sal_agenda_after._partys.get(sal_partytreasuryunit.due_agent_id)
    #     if x_partyunit != None:
    #         # print(
    #         #     f"{sal_partytreasuryunit.cash_master=} {sal_partytreasuryunit.due_agent_id=} {x_partyunit.party_id=} due_total: {sal_partytreasuryunit.due_total} _Due_ Paid: {x_partyunit._treasury_due_paid}"
    #         # )
    #         # print(
    #         #     f"{sal_partytreasuryunit.cash_master=} {sal_partytreasuryunit.due_agent_id=} {x_partyunit.party_id=} due_diff:  {sal_partytreasuryunit.due_diff} _Due_ Paid: {x_partyunit._treasury_due_diff}"
    #         # )
