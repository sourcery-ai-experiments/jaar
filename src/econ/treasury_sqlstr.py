...
# river_block
def get_river_block_table_delete_sqlstr(cash_worker_id: str) -> str:
    return f"""
DELETE FROM river_block
WHERE cash_master = '{cash_worker_id}' 
;
"""


def get_river_block_table_create_sqlstr() -> str:
    """Table that stores each block of cash from src_worker_id to dst_worker_id.
    cash_master: every cash starts with a worker_id as credit source
        All river blocks with destination cash worker_id stop. For that cash range
        there is no more block
    src_worker_id: worker_id that is source of credit
    dst_worker_id: worker_id that is destination of credit.
...
CREATE TABLE IF NOT EXISTS river_block (
  cash_master VARCHAR(255) NOT NULL
, src_worker_id VARCHAR(255) NOT NULL
, dst_worker_id VARCHAR(255) NOT NULL
...
, FOREIGN KEY(cash_master) REFERENCES agendaunit(worker_id)
, FOREIGN KEY(src_worker_id) REFERENCES agendaunit(worker_id)
, FOREIGN KEY(dst_worker_id) REFERENCES agendaunit(worker_id)
)
;
"""
...
class RiverBlockUnit:
    ...
    def block_returned(self) -> bool:
        return self.cash_worker_id == self.dst_worker_id
...
VALUES (
  '{river_block_x.cash_worker_id}'
, '{river_block_x.src_worker_id}'
, '{river_block_x.dst_worker_id}'
...
def get_river_block_dict(db_conn: str, cash_worker_id: str) -> dict[str:RiverBlockUnit]:
    sqlstr = f"""
SELECT 
  cash_master
, src_worker_id
...
# river_circle
def get_river_circle_table_delete_sqlstr(cash_worker_id: str) -> str:
    return f"""
...
def get_river_circle_table_insert_sqlstr(cash_worker_id: str) -> str:
    """Table that stores discontinuous cash ranges that circle back from source (cash_master)
    to final destination (cash_master)
    Columns
    cash_master: every cash starts with a worker_id as credit source
    dst_worker_id: worker_id that is destination of credit.
        All river blocks with destination worker_id are summed into ranges called circles
    circle_num: all destination worker_id circles have a unique number. (sequential 0, 1, 2...)
...
FROM  (
    ...
    , *
    FROM  river_block
    WHERE cash_master = '{cash_worker_id}' and dst_worker_id = cash_master 
    ) b
) c
GROUP BY cash_master, dst_worker_id, cash_circle_num
...
"""
    ...
    for row in results.fetchall():
        ...
        dict_x[partytreasuryunit_x.due_worker_id] = partytreasuryunit_x
    return dict_x


# agenda
def get_agendaunit_table_create_sqlstr() -> str:
    """Create table that references the person_id of every agenda. The worker_id pip of the one running that agenda's clerk."""
    return """
CREATE TABLE IF NOT EXISTS agendaunit (
  worker_id VARCHAR(255) PRIMARY KEY ASC
, rational INT NULL
, UNIQUE(worker_id)
)
;
...
WHERE worker_id = '{agenda._worker_id}'
;
"""
...
SET _treasury_due_paid = (
    ...
    FROM river_block block
    WHERE block.cash_master='{cash_worker_id}' 
...
WHERE EXISTS (
    ...
    FROM river_block block
    WHERE agenda_partyunit.worker_id='{cash_worker_id}' 
...
SET _treasury_voice_rank = 
    ...
    FROM (
        ...
        WHERE p2.worker_id = '{worker_id}'
    ) p3
    WHERE p3.party_id = agenda_partyunit.party_id AND agenda_partyunit.worker_id = '{worker_id}'
    )
WHERE agenda_partyunit.worker_id = '{worker_id}'
;
"""
...
def get_partyview_dict(
    db_conn: Connection, payer_worker_id: PersonID
...
, _treasury_voice_hx_lowest_rank
FROM agenda_partyunit
WHERE worker_id = '{payer_worker_id}' 
;
"""
    dict_x = {}
    results = db_conn.execute(sqlstr)
...
def get_river_ledger_unit(
    ...
    partyview_x = get_partyview_dict(db_conn, river_block_x.dst_worker_id)
    return RiverLedgerUnit(
        worker_id=river_block_x.dst_worker_id,
...
def get_agenda_ideaunit_table_create_sqlstr() -> str:
    """table that holds every road and its worker_id"""
    return """
...
def get_agenda_ideaunit_table_insert_sqlstr(
    ...
    # return f"""INSERT INTO agenda_ideaunit (worker_id, idea_road) VALUES ('{agenda_ideaunit.worker_id}', '{agenda_ideaunit.idea_road}');"""
    return f"""
...
"""
    ...
    dict_x = {}
    for row in results.fetchall():
        agenda_ideaunit_x = IdeaCatalog(worker_id=row[0], idea_road=row[1])
        dict_key = f"{agenda_ideaunit_x.worker_id} {agenda_ideaunit_x.idea_road}"
...
VALUES (
  '{agenda_idea_beliefunit.worker_id}'
...
"""
    ...
    for row in results.fetchall():
        ...
        dict_key = (
            f"{agenda_groupunit_x.worker_id} {agenda_groupunit_x.groupunit_group_id}"
        )
...
, task)
VALUES (
  '{x_obj.calendarreport.worker_id}'
...
def get_calendar_table_delete_sqlstr(calendar_worker_id: str) -> str:
    return f"""
DELETE FROM calendar
WHERE worker_id = '{calendar_worker_id}' 
;
"""
