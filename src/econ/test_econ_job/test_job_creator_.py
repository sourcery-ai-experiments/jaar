...
def test_get_role_file_agenda_ReturnsCorrectAgendaWhenFileExists(env_dir_setup_cleanup):
    ...
    save_role_file_agenda(texas_agendanox, src_yao_agenda)

    # WHEN
    yao_role = get_role_file_agenda(texas_agendanox, yao_text)
...
def test_get_job_file_ReturnsCorrectAgendaWhenFileExists(env_dir_setup_cleanup):
    ...
    # WHEN / THEN
    assert get_job_file(texas_agendanox, yao_text) is None

    # GIVEN
    save_job_file(texas_agendanox, agendaunit_shop(yao_text))
    # WHEN
    yao_job = get_job_file(texas_agendanox, yao_text)
    # THEN
...
def test_create_job_file_from_role_file_CreatesEmptyJob(env_dir_setup_cleanup):
    ...
    texas_agendanox = get_texas_agendanox()
    save_role_file_agenda(texas_agendanox, yao_role)
    yao_job_file_path = texas_agendanox.job_path(yao_text)
    assert os_path_exists(yao_job_file_path) == False

    # WHEN
    yao_job = create_job_file_from_role_file(texas_agendanox, yao_text)
