...
def test_get_role_agenda_ReturnsCorrectAgendaWhenFileExists(env_dir_setup_cleanup):
    ...
    texas_filehub.save_role_agenda(src_sue_agenda)

    # WHEN
    sue_role = texas_filehub.get_role_agenda(sue_text)
...
def test_get_job_agenda_ReturnsCorrectAgendaWhenFileExists(env_dir_setup_cleanup):
    ...
    # WHEN / THEN
    assert texas_filehub.get_role_agenda(sue_text) is None

    # GIVEN
    texas_filehub.save_role_agenda(agendaunit_shop(sue_text))
    # WHEN
    sue_job = texas_filehub.get_role_agenda(sue_text)
    # THEN
    print(f"{sue_job=}")
