...
from src.x_econ.deal import DealUnit
from src.x_econ.problem import problemunit_shop, healerlink_shop, marketlink_shop
from src.x_econ.person import PersonUnit, personunit_shop
...
class EconMark(str):  # Created to help track the concept
    pass
...
class EconUnit:
    ...
    def _set_econ_dirs(self):
        self._econ_dir = f"{self.econs_dir}/{self.mark}"
        self._persons_dir = f"{self._econ_dir}/persons"
        self._deals_dir = f"{self._econ_dir}/deals"
    ...
    def get_econ_agenda(self, person_id: PersonID):
        x_personunit = self.get_personunit_from_memory(person_id)
        econ_agenda = agendaunit_shop(person_id)
        for person_problemunit in x_personunit._problems.values():
            for person_healerlink in person_problemunit._healerlinks.values():
                ...
                for x_marketlink in person_healerlink._marketlinks.values():
                    ...
                    forum_agenda.set_market_id(econ_agenda._market_id)
                    econ_agenda.meld(forum_agenda)
        return econ_agenda
...
def econunit_shop(
    mark: EconMark, econs_dir: str, _road_delimiter: str = None
) -> EconUnit:
    econ_x = EconUnit(
        mark=mark,
        econs_dir=econs_dir,
    ...
    econ_x.set_max_deal_uid()
    econ_x._set_econ_dirs()
    econ_x._road_delimiter = default_road_delimiter_if_none(_road_delimiter)
    return econ_x
