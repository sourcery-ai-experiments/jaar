...
from src.x_econ.examples.example_deals import get_no_topiclinks_yao_sue_dealunit
from os import path as os_path


def test_EconUnit_set_dealunit_CorrectSetsAttr(econs_dir_setup_cleanup):
    # GIVEN
    oregon_econ = econunit_shop("Oregon", get_test_econs_dir())
    assert oregon_econ._dealunits == {}

    # WHEN
    yao_sue_dealunit = get_no_topiclinks_yao_sue_dealunit()
    yao_sue_uid = oregon_econ.set_dealunit(x_dealunit=yao_sue_dealunit)

    # THEN
    assert oregon_econ._dealunits != {}
    assert oregon_econ._dealunits == {1: yao_sue_dealunit}
    assert yao_sue_uid == 1


def test_EconUnit_get_dealunit_ReturnsCorrectObj(econs_dir_setup_cleanup):
    # GIVEN
    oregon_econ = econunit_shop("Oregon", get_test_econs_dir())
    yao_sue_uid = oregon_econ.set_dealunit(get_no_topiclinks_yao_sue_dealunit())

    # WHEN
    yao_sue_dealunit = oregon_econ.get_dealunit(yao_sue_uid)

    # THEN
    assert yao_sue_dealunit == get_no_topiclinks_yao_sue_dealunit()


def test_EconUnit_dealunit_exists_ReturnsCorrectObj(econs_dir_setup_cleanup):
    # GIVEN
    oregon_econ = econunit_shop("Oregon", get_test_econs_dir())
    static_yao_sue_uid = 1
    assert oregon_econ.dealunit_exists(static_yao_sue_uid) == False

    # WHEN
    gen_yao_sue_uid = oregon_econ.set_dealunit(get_no_topiclinks_yao_sue_dealunit())
    assert static_yao_sue_uid == gen_yao_sue_uid

    # THEN
    assert oregon_econ.dealunit_exists(static_yao_sue_uid)


def test_EconUnit_del_dealunit_CorrectChangesAttr(econs_dir_setup_cleanup):
    # GIVEN
    oregon_econ = econunit_shop("Oregon", get_test_econs_dir())
    yao_sue_uid = oregon_econ.set_dealunit(get_no_topiclinks_yao_sue_dealunit())
    assert oregon_econ.dealunit_exists(yao_sue_uid)

    # WHEN
    oregon_econ.del_dealunit(yao_sue_uid)

    # THEN
    assert oregon_econ.dealunit_exists(yao_sue_uid) == False


# def test_EconUnit_apply_requestunit_CorrectlyCreates_contract_agendas(
#     econs_dir_setup_cleanup,
# ):
#     # GIVEN requester and requestee contract_agendas does not exist
#     w1_text = "w1"
#     x_econ = econunit_shop(w1_text, get_test_econs_dir())
#     yao_text = "Yao"
#     x_econ.set_personunit(yao_text)
#     yao_person = x_econ.get_personunit_from_memory(yao_text)
#     texas_text = "Texas"
...
#     )
#     assert x_econ.get_personunit_from_memory(tim_text) is None
#     assert x_econ.get_personunit_from_memory(xio_text) is None
...
#     # WHEN
#     x_econ.apply_requestunit(highway_requestunit)

#     # THEN
#     assert x_econ.get_personunit_from_memory(tim_text) != None
#     assert x_econ.get_personunit_from_memory(xio_text) != None
...
# def test_EconUnit_apply_requestunit_CorrectlyAddsTaskTo_requester_contract_agenda(
#     econs_dir_setup_cleanup,
# ):
#     x_econ = econunit_shop("w1", get_test_econs_dir())
#     yao_text = "Yao"
#     x_econ.set_personunit(yao_text)
#     yao_person = x_econ.get_personunit_from_memory(yao_text)
#     texas_text = "Texas"
...
# def test_EconUnit_apply_requestunit_CorrectlyAppliesGroup(econs_dir_setup_cleanup):
#     x_econ = econunit_shop("w1", get_test_econs_dir())
#     yao_text = "Yao"
#     x_econ.set_personunit(yao_text)
#     yao_person = x_econ.get_personunit_from_memory(yao_text)
...
# # def test_EconUnit_apply_requestunit_Multiple_requestunitsCreateMultiple_intent_items(
# #     econs_dir_setup_cleanup,
# # ):
# #     x_econ = econunit_shop("w1", get_test_econs_dir())
# #     yao_text = "Yao"
...
# #     # WHEN
# #     x_econ.apply_requestunit(fly_requestunit)
# #     x_econ.apply_requestunit(fly_requestunit)
