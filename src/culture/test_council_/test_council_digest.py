...
def test_healeropen_seed_agenda_WhenStartingAgendaFileDoesNotExists(
    ...
    ux = councilunit_shop(
        pid=tim_text, env_dir=env_dir, culture_title=culture_title_text
    )
...
def test_healer_save_seed_agenda_seedPersonPIDMustBeHealer(
    ...
    p_pid = "Game1"
    env_dir = get_temp_councilunit_dir()
    ux = councilunit_shop(p_pid, env_dir, get_temp_culture_title())
...
def test_councilunit_save_agenda_to_digest_SavesFileCorrectly(
    ...
    council_cid = "healer1"
    env_dir = get_temp_councilunit_dir()
    ux = councilunit_shop(council_cid, env_dir, get_temp_culture_title())
...
def test_healer_get_remelded_output_agenda_withEmptyDigestDict(
    ...
    ux = councilunit_shop(
        council_cid_x, get_temp_councilunit_dir(), get_temp_culture_title()
    )
    ux.create_core_dir_and_files()
