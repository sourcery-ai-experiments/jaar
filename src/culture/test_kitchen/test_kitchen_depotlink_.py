...
from src.culture.examples.example_kitchens import (
    ...
    get_agenda_assignment_laundry_example1 as get_america_assign_ex,
)
...
def test_kitchenunit_set_depotlink_RaisesErrorWhenDealDoesNotExist(
    ...
    # WHEN / THEN
    file_path_x = f"{sue_cx._admin._agendas_depot_dir}/{tim_text}.json"
    print(f"{file_path_x=}")
...
def test_kitchenunit_set_depotlink_CorrectlySetsAssignment(kitchen_dir_setup_cleanup):
    ...
    joachim_ux._admin.save_agenda_to_depot(america_cx)
    assert joachim_ux.get_seed().get_party(america_cx._healer) is None
    america_digest_path = (
        f"{joachim_ux._admin._agendas_digest_dir}/{america_cx._healer}.json"
    )
    ...
    digest_cx = agenda_get_from_json(
        x_func_open_file(
            dest_dir=joachim_ux._admin._agendas_digest_dir,
            file_title=f"{america_cx._healer}.json",
        )
    )
    print(f"{digest_cx._healer=}")
    print(f"{len(digest_cx._idea_dict)=}")
    digest_cx.set_agenda_metrics()
...
def test_kitchenunit_del_depot_agenda_CorrectlyDeletesObj(kitchen_dir_setup_cleanup):
    ...
    bob_text = "Bob"
    env_dir = get_temp_kitchenunit_dir()
    bob_cx = kitchenunit_shop(bob_text, env_dir, get_temp_culture_handle())
    yao_text = "Yao"
    create_agenda_file(bob_cx._admin._agendas_depot_dir, yao_text)
    assignment_text = "assignment"
    ...
    # WHEN
    bob_cx.del_depot_agenda(agenda_healer=yao_text)
...
def test_kitchenunit_del_depot_agenda_CorrectlyDeletesBlindTrustFile(
    ...
    bob_cx = kitchenunit_shop(bob_text, env_dir, get_temp_culture_handle())
    lai_text = "Lai"
    create_agenda_file(bob_cx._admin._agendas_depot_dir, lai_text)
    bob_cx.set_seed_if_empty()
    bob_cx._set_depotlink(lai_text, link_type="blind_trust")
    assert x_func_count_files(dir_path=bob_cx._admin._agendas_depot_dir) == 1
    assert x_func_count_files(dir_path=bob_cx._admin._agendas_digest_dir) == 1

    # WHEN
    bob_cx.del_depot_agenda(agenda_healer=lai_text)

    # THEN
    assert x_func_count_files(dir_path=bob_cx._admin._agendas_depot_dir) == 0
    assert x_func_count_files(dir_path=bob_cx._admin._agendas_digest_dir) == 0
...
def test_kitchenunit_set_depot_agenda_SavesFileCorrectly(
    ...
    assert (
        x_func_count_files(bob_cx._admin._agendas_depot_dir) is None
    )  # dir does not exist

    # WHEN
    bob_cx.set_seed_if_empty()
    bob_cx.set_depot_agenda(x_agenda=cal1, depotlink_type="blind_trust")

    # THEN
    print(f"Saving to {bob_cx._admin._agendas_depot_dir=}")
    # for path_x in os_scandir(ux._admin._agendas_depot_dir):
    #     print(f"{path_x=}")
    assert x_func_count_files(bob_cx._admin._agendas_depot_dir) == 1
...
def test_kitchenunit_del_depot_agenda_CorrectlyDoesNotDeletesIgnoreFile(
    ...
    bob_cx = kitchenunit_shop(bob_text, env_dir, get_temp_culture_handle())
    zia_text = "Zia"
    create_agenda_file(bob_cx._admin._agendas_depot_dir, zia_text)
    bob_cx.set_seed_if_empty()
    bob_cx._set_depotlink(zia_text, link_type="ignore")
    assert x_func_count_files(dir_path=bob_cx._admin._agendas_depot_dir) == 1
    assert x_func_count_files(dir_path=bob_cx._admin._agendas_digest_dir) == 1
    assert x_func_count_files(dir_path=bob_cx._admin._agendas_ignore_dir) == 1

    # WHEN
    bob_cx.del_depot_agenda(agenda_healer=zia_text)
...
def test_kitchenunit_refresh_depotlinks_CorrectlyPullsAllPublicDeals(
    ...
    #     print(f"{bob_cx._admin._agendas_public_dir=} {file_title=}")

    # for file_title in x_func_dir_files(dir_path=bob_cx._admin._agendas_public_dir):
    #     print(f"{bob_cx._admin._agendas_public_dir=} {file_title=}")
