...
from src.culture.examples.council_env_kit import get_temp_culture_qid
...
def get_Jnode2node_agenda() -> AgendaUnit:
    ...
    x_agenda.set_culture_qid(get_temp_culture_qid())
    x_agenda.add_idea(ideacore_shop("A"), pad=x_agenda._culture_qid)
    x_agenda.set_agenda_metrics()
    return x_agenda
...
def get_3node_agenda() -> AgendaUnit:
    ...
    x_agenda.set_culture_qid(get_temp_culture_qid())
    x_agenda.add_idea(ideacore_shop("B"), pad=x_agenda._culture_qid)
    x_agenda.add_idea(ideacore_shop("C"), pad=x_agenda._culture_qid)
    x_agenda.set_agenda_metrics()
    return x_agenda
...
def get_3node_D_E_F_agenda() -> AgendaUnit:
    ...
    x_agenda.set_culture_qid(get_temp_culture_qid())
    x_agenda.add_idea(ideacore_shop("E"), pad=x_agenda._culture_qid)
    x_agenda.add_idea(ideacore_shop("F"), pad=x_agenda._culture_qid)
    x_agenda.set_agenda_metrics()
    return x_agenda
...
def get_healer_2agenda(env_dir, culture_qid) -> CouncilUnit:
    yao_text = "Xio"
    yao_healer = councilunit_shop(yao_text, env_dir, culture_qid)
...
def get_agenda_3CleanNodesRandomWeights(_healer: str = None) -> AgendaUnit:
    ...
    x_agenda = agendaunit_shop(healer_text)
    casa_text = "casa"
    x_agenda.add_idea(ideacore_shop(casa_text), pad=x_agenda._culture_qid)
    casa_road = f"{x_agenda._culture_qid},{casa_text}"
    cookery_text = "clean cookery"
    bedroom_text = "clean bedroom"
