from src.agenda.agenda import DealUnit, agendaunit_shop, IdeaKid, assigned_unit_shop
...
def get_agenda_2CleanNodesRandomWeights(_healer: str = None) -> DealUnit:
    ...
    x_agenda = agendaunit_shop(_healer=healer_text)
    casa_text = "casa"
    x_agenda.add_idea(idea_kid=IdeaKid(_label=casa_text), pad="")
...
def get_agenda_assignment_laundry_example1() -> DealUnit:
    ...
    america_agenda.add_idea(IdeaKid(_label=casa_text), pad=root_road)

    basket_text = "laundry basket status"
    basket_road = f"{casa_road},{basket_text}"
    america_agenda.add_idea(IdeaKid(_label=basket_text), pad=casa_road)

    b_full_text = "full"
    b_full_road = f"{basket_road},{b_full_text}"
    america_agenda.add_idea(IdeaKid(_label=b_full_text), pad=basket_road)

    b_smel_text = "smelly"
    b_smel_road = f"{basket_road},{b_smel_text}"
    america_agenda.add_idea(IdeaKid(_label=b_smel_text), pad=basket_road)

    b_bare_text = "bare"
    b_bare_road = f"{basket_road},{b_bare_text}"
    america_agenda.add_idea(IdeaKid(_label=b_bare_text), pad=basket_road)

    b_fine_text = "fine"
    b_fine_road = f"{basket_road},{b_fine_text}"
    america_agenda.add_idea(IdeaKid(_label=b_fine_text), pad=basket_road)

    b_half_text = "half full"
    b_half_road = f"{basket_road},{b_half_text}"
    america_agenda.add_idea(IdeaKid(_label=b_half_text), pad=basket_road)
