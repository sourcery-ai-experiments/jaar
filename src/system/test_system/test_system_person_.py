...
def test_system_create_personunit_from_public_RaisesErrorWhenPersonDoesNotExist(
    env_dir_setup_cleanup,
):
    # GIVEN
    system_name = get_temp_env_name()
    ...
    with pytest_raises(Exception) as excinfo:
        sx.create_personunit_from_public(name=bobs_text)
    assert (
        str(excinfo.value)
...
def test_system_rename_personunit_WorksCorrectly(env_dir_setup_cleanup):
    ...
    old_person_x = e5.get_person_obj(name=old_bob_text)
    assert e5.get_person_obj(name=new_bob_text) is None
    ...
    assert e5.get_person_obj(name=old_bob_text) is None
    new_person_x = e5.get_person_obj(name=new_bob_text)
