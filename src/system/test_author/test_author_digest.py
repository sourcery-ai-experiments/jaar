...
# def test_author_save_isol_calendar_CreateStartingCalendarFile(
#     author_dir_setup_cleanup,
# ):
#     # GIVEN
#     lai_name = "Lai"
#     env_dir = get_temp_author_dir()
#     lai_calendar = authorunit_shop(name=lai_name, env_dir=env_dir)
#     lai_isol_file_name = lai_calendar._admin._isol_file_name
#     with pytest_raises(Exception) as excinfo:
#         x_func_open_file(lai_calendar._admin._author_dir, lai_isol_file_name)
#     assert (
#         str(excinfo.value)
...
#     # THEN
#     assert x_func_open_file(lai_calendar._admin._author_dir, lai_isol_file_name) != None


def test_authoropen_isol_calendar_WhenStartingCalendarFileDoesNotExists(
    author_dir_setup_cleanup,
):
    # GIVEN
    tim_text = "Tim"
    env_dir = get_temp_author_dir()
    ux = authorunit_shop(name=tim_text, env_dir=env_dir)
...
def test_author_save_isol_calendar_IsolCalendarOwnerMustBeAuthor(
    author_dir_setup_cleanup,
):
    # GIVEN
    p_name = "Game1"
    env_dir = get_temp_author_dir()
    ux = authorunit_shop(name=p_name, env_dir=env_dir)
    ...
    # THEN
    assert ux._admin.open_isol_calendar()._owner == ux._admin._author_name


def test_author_open_isol_calendar_WhenStartingCalendarFileExists(
    author_dir_setup_cleanup,
):
    # GIVEN
    p_name = "Game1"
    ...
    assert isol_calendar._owner == ux._admin._author_name


def test_author_erase_isol_calendar_file_DeletesFileCorrectly(
    author_dir_setup_cleanup,
):
    # GIVEN
    p_name = "Game1"
    ...
    assert x_func_open_file(ux._admin._author_dir, file_name) != None
    ...
    # THEN
    with pytest_raises(Exception) as excinfo:
        x_func_open_file(ux._admin._author_dir, file_name)
    assert (
        str(excinfo.value)
        == f"Could not load file {ux._admin._author_dir}/isol_calendar.json (2, 'No such file or directory')"
    )


def test_authorunit_save_calendar_to_digest_SavesFileCorrectly(
    author_dir_setup_cleanup,
):
    # GIVEN
    author_name = "author1"
    env_dir = get_temp_author_dir()
    ux = authorunit_shop(name=author_name, env_dir=env_dir)
    ux.create_core_dir_and_files()
    cx = example_authors.get_2node_calendar()
...
def test_author_get_remelded_output_calendar_withEmptyDigestDict(
    author_dir_setup_cleanup,
):
    # GIVEN
    author_name_x = "boots3"
    ux = authorunit_shop(name=author_name_x, env_dir=get_temp_author_dir())
    ux.create_core_dir_and_files()
    sx_output_before = ux._admin.get_remelded_output_calendar()
    assert str(type(sx_output_before)).find(".calendar.CalendarUnit'>")
    assert sx_output_before._owner == author_name_x
    ...
    # THEN
    author_calendar_x = CalendarUnit(_owner=author_name_x, _weight=0.0)
    author_calendar_x._idearoot._walk = ""
    author_calendar_x.set_calendar_metrics()

    assert str(type(sx_output_after)).find(".calendar.CalendarUnit'>")
    assert sx_output_after._weight == author_calendar_x._weight
    assert sx_output_after._idearoot._walk == author_calendar_x._idearoot._walk
    assert (
        sx_output_after._idearoot._acptfactunits
        == author_calendar_x._idearoot._acptfactunits
    )
    assert sx_output_after._idearoot == author_calendar_x._idearoot


def test_author_get_remelded_output_calendar_with1DigestedCalendar(
    author_dir_setup_cleanup,
):
    # GIVEN
    yao_text = "Yao"
    env_dir = get_temp_author_dir()
    ux = authorunit_shop(name=yao_text, env_dir=env_dir)
    ux.create_core_dir_and_files()
    ...
    input_calendar = example_authors.get_2node_calendar()
...
# def test_author_set_digested_calendar_with2Groups(author_dir_setup_cleanup):
#     # GIVEN
#     env_dir = get_temp_author_dir()
#     ux = authorunit_shop(name="test8", env_dir=env_dir)
#     sx_output_old = ux._admin.get_remelded_output_calendar()
#     assert str(type(sx_output_old)).find(".calendar.CalendarUnit'>")
...
def test_author_isol_calendar_CorrectlysHasOriginLinksWithOwnerAsSource(
    author_dir_setup_cleanup,
):
    # GIVEN
    # authorunit with isol_calendar and no other depot calendars
    yao_text = "Yao"
    isol_origin_weight = 1
    yao_originunit = originunit_shop()
    yao_originunit.set_originlink(name=yao_text, weight=isol_origin_weight)
    isol_calendar_x = example_authors.get_7nodeJRoot_calendar()
    ...
    ux = authorunit_shop(name=yao_text, env_dir=get_temp_author_dir())
    ux.create_core_dir_and_files()
