...
def test_authorunit_set_depotlink_CorrectlySetsAssignment(author_dir_setup_cleanup):
    ...
    america_digest_path = (
        f"{joachim_ux._admin._calendars_digest_dir}/{america_cx._owner}.json"
    )
    ...
    digest_cx = calendar_get_from_json(
        x_func_open_file(
            dest_dir=joachim_ux._admin._calendars_digest_dir,
...
def test_authorunit_set_depot_calendar_SavesFileCorrectly(
    ...
    # for path_x in os_scandir(ux._admin._calendars_depot_dir):
    #     print(f"{path_x=}")
...
def test_authorunit_set_ignore_calendar_file_CorrectlyUpdatesIgnoreFile(
    ...
    cx1 = bob_ux._admin.open_ignore_calendar(owner=zia_text)
    ...
    bob_ux.set_ignore_calendar_file(zia_calendar, src_calendar_owner=None)

    # THEN
    cx2 = bob_ux._admin.open_ignore_calendar(owner=zia_text)
