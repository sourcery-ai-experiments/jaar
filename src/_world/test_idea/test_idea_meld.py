...
from src._world.beliefunit import CashLink, BeliefID, cashlink_shop
...
def test_IdeaUnit_meld_ReturnsCorrectObj_BaseScenario_cashlinkWhen_meld_strategyEquals_default():
    # GIVEN
    casa_text = "casa"
    ...
    arbitrarily_set_idea_attr(
        idea=x1_idea, cashlink=cashlink_shop(belief_id=br1, credor_weight=2)
    )
    x2_idea = ideaunit_shop("Swimming")
    arbitrarily_set_idea_attr(idea=x2_idea, meld_strategy=default_text)
    arbitrarily_set_idea_attr(
        idea=x2_idea, cashlink=cashlink_shop(belief_id=br1, credor_weight=3)
    )
    ...
    # THEN
    bl_x = cashlink_shop(belief_id=br1, credor_weight=2)
    assert x1_idea._cashlinks[br1] == bl_x


def test_IdeaUnit_meld_ReturnsCorrectObj_BaseScenario_cashlinkWhen_meld_strategyEquals_sum():
    # GIVEN
    sum_text = "sum"
    casa_text = "casa"
    ...
    arbitrarily_set_idea_attr(
        idea=x1_idea,
        cashlink=cashlink_shop(belief_id=br1, credor_weight=2, debtor_weight=3),
    )
    x2_idea = ideaunit_shop("Swimming")
    ...
    # THEN
    lu_x = cashlink_shop(belief_id=br1, credor_weight=4, debtor_weight=6)
    assert x1_idea._cashlinks[br1] == lu_x


def test_IdeaUnit_meld_ReturnsCorrectObj_TwoBeliefsScenario_cashlink():
    # GIVEN
    sum_text = "sum"
    casa_text = "casa"
    ...
    arbitrarily_set_idea_attr(idea=x1_idea, meld_strategy=sum_text)
    arbitrarily_set_idea_attr(
        idea=x2_idea, cashlink=cashlink_shop(belief_id=br2, credor_weight=2)
    )
    ...
    # THEN
    lu_x1 = cashlink_shop(belief_id=br1, credor_weight=2)
    lu_x2 = cashlink_shop(belief_id=br2, credor_weight=2)
    assert x1_idea._cashlinks[br1] == lu_x1
    assert x1_idea._cashlinks[br2] == lu_x2
...
def test_IdeaUnit_meld_CorrectlyMeldsRangeAttributesWhen_meld_strategyEquals_default():
    ...
    x_uid = "uid1xx"
    x_all_char_cred = "am_cx"
    x_all_char_debt = "am_dx"

    label1_text = "clean"
    texas_text = "texas"
    x1_idea = ideaunit_shop(label1_text, _world_real_id=texas_text)
    arbitrarily_set_idea_attr(
        idea=x1_idea,
        uid=x_uid,
        weight=7,
        ...
        pledge=True,
        all_char_cred=x_all_char_cred,
        all_char_debt=x_all_char_debt,
        is_expanded=True,
    )

    label2_text = "cook"
    x2_idea = ideaunit_shop(label2_text, _world_real_id=texas_text)
    arbitrarily_set_idea_attr(
        idea=x2_idea,
        uid=x_uid,
        weight=7,
    ...
    assert x1_idea._all_char_cred == x_all_char_cred
    assert x1_idea._all_char_debt == x_all_char_debt
    assert x1_idea._is_expanded == True
    assert x1_idea._world_real_id == texas_text
...
# def test_IdeaUnit_meld_FailRaisesError_all_char_cred():
# def test_IdeaUnit_meld_FailRaisesError_all_char_debt():
#     x_attr = "_all_char_cred"
#     x_attr = "_all_char_debt"
#     x_val = "test_all_char_cred1"
#     x_val = "test_all_char_debt1"
#     x1_idea = ideaunit_shop("clean", _parent_road=casa_text)
#     arbitrarily_set_idea_attr(idea=x1_idea, all_char_cred=x_val)
#     arbitrarily_set_idea_attr(idea=x1_idea, all_char_debt=x_val)
#     x2_idea = ideaunit_shop("cook", _parent_road=casa_text)
...
def test_IdeaUnit_meld_CorrectlyCreatesOriginUnitWithOriginLink():
    ...
    sue_text = "Sue"
    sue_weight = 5
    x1_idea.meld(exterior_idea=x2_idea, char_id=sue_text, char_weight=sue_weight)
...
def test_IdeaUnit_meld_IdeaMeldingItselfCreatesOriginUnitWithCorrectOriginLink():
    ...
    # WHEN
    x1_idea.meld(exterior_idea=x1_idea, char_id=sue_text, char_weight=sue_weight)
    assert x1_idea._originunit == ex_x1_idea_originunit
