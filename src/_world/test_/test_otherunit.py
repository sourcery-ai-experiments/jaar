...
def test_OtherUnit_exists():
    ...
    assert bob_otherunit._world_cred is None
    assert bob_otherunit._world_debt is None
    assert bob_otherunit._world_agenda_cred is None
    assert bob_otherunit._world_agenda_debt is None
    ...
    assert bob_otherunit._output_world_meld_order is None
...
def test_otherunit_shop_CorrectlySetsAttributes():
    ...
    assert todd_otherunit._world_cred == 0
    assert todd_otherunit._world_debt == 0
    assert todd_otherunit._world_agenda_cred == 0
    assert todd_otherunit._world_agenda_debt == 0
    assert todd_otherunit._world_agenda_ratio_cred == 0
    assert todd_otherunit._world_agenda_ratio_debt == 0
...
def test_OtherUnit_set_output_world_meld_order_CorrectlySetsAttribute():
    # GIVEN
    bob_otherunit = otherunit_shop("Bob")
    assert bob_otherunit._output_world_meld_order is None
    ...
    bob_otherunit.set_output_world_meld_order(x_output_world_meld_order)

    # THEN
    assert bob_otherunit._output_world_meld_order == x_output_world_meld_order


def test_OtherUnit_clear_output_world_meld_order_CorrectlySetsAttribute():
    # GIVEN
    bob_otherunit = otherunit_shop("Bob")
    x_output_world_meld_order = 5
    bob_otherunit.set_output_world_meld_order(x_output_world_meld_order)
    ...
    # WHEN
    bob_otherunit.clear_output_world_meld_order()
...
def test_OtherUnit_reset_world_cred_debt_SetsAttrCorrectly():
    # GIVEN
    bob_otherunit = otherunit_shop("Bob")
    bob_otherunit._world_cred = 0.27
    bob_otherunit._world_debt = 0.37
    bob_otherunit._world_agenda_cred = 0.41
    bob_otherunit._world_agenda_debt = 0.51
    bob_otherunit._world_agenda_ratio_cred = 0.433
    bob_otherunit._world_agenda_ratio_debt = 0.533
    assert bob_otherunit._world_cred == 0.27
    assert bob_otherunit._world_debt == 0.37
    assert bob_otherunit._world_agenda_cred == 0.41
    assert bob_otherunit._world_agenda_debt == 0.51
    assert bob_otherunit._world_agenda_ratio_cred == 0.433
    assert bob_otherunit._world_agenda_ratio_debt == 0.533

    # WHEN
    bob_otherunit.reset_world_cred_debt()

    # THEN
    assert bob_otherunit._world_cred == 0
    assert bob_otherunit._world_debt == 0
    assert bob_otherunit._world_agenda_cred == 0
    assert bob_otherunit._world_agenda_debt == 0
    assert bob_otherunit._world_agenda_ratio_cred == 0
    assert bob_otherunit._world_agenda_ratio_debt == 0


def test_OtherUnit_add_world_cred_debt_SetsAttrCorrectly():
    # GIVEN
    bob_otherunit = otherunit_shop("Bob")
    bob_otherunit._world_cred = 0.4106
    bob_otherunit._world_debt = 0.1106
    bob_otherunit._world_agenda_cred = 0.41
    bob_otherunit._world_agenda_debt = 0.51
    assert bob_otherunit._world_agenda_cred == 0.41
    assert bob_otherunit._world_agenda_debt == 0.51
    ...
    # WHEN
    bob_otherunit.add_world_cred_debt(
        world_cred=0.33,
        world_debt=0.055,
    ...
    # THEN
    assert bob_otherunit._world_cred == 0.7406
    assert bob_otherunit._world_debt == 0.1656
    assert bob_otherunit._world_agenda_cred == 0.71
    assert bob_otherunit._world_agenda_debt == 0.56


def test_OtherUnit_set_world_agenda_ratio_cred_debt_SetsAttrCorrectly():
    # GIVEN
    bob_otherunit = otherunit_shop("Bob", credor_weight=15, debtor_weight=7)
    bob_otherunit._world_cred = 0.4106
    bob_otherunit._world_debt = 0.1106
    bob_otherunit._world_agenda_cred = 0.041
    bob_otherunit._world_agenda_debt = 0.051
    bob_otherunit._world_agenda_ratio_cred = 0
    bob_otherunit._world_agenda_ratio_debt = 0
    assert bob_otherunit._world_agenda_ratio_cred == 0
    assert bob_otherunit._world_agenda_ratio_debt == 0

    # WHEN
    bob_otherunit.set_world_agenda_ratio_cred_debt(
        world_agenda_ratio_cred_sum=0.2,
        world_agenda_ratio_debt_sum=0.5,
        world_otherunit_total_credor_weight=20,
        world_otherunit_total_debtor_weight=14,
    )

    # THEN
    assert bob_otherunit._world_agenda_ratio_cred == 0.205
    assert bob_otherunit._world_agenda_ratio_debt == 0.102
    ...
    # THEN
    assert bob_otherunit._world_agenda_ratio_cred == 0.75
    assert bob_otherunit._world_agenda_ratio_debt == 0.5
...
def test_OtherUnit_set_treasury_attr_SetsAttrCorrectly():
    ...
    bob_otherunit = otherunit_shop("Bob")
    bob_otherunit._world_agenda_ratio_cred = x_world_agenda_ratio_cred
    bob_otherunit._world_agenda_ratio_debt = x_world_agenda_ratio_debt
    assert bob_otherunit._world_agenda_ratio_cred == 0.077
    assert bob_otherunit._world_agenda_ratio_debt == 0.066
    assert bob_otherunit._treasury_due_paid is None
    assert bob_otherunit._treasury_due_diff is None
    assert bob_otherunit._treasury_cred_score is None
    assert bob_otherunit._treasury_voice_rank is None
    ...
    # THEN
    assert bob_otherunit._world_agenda_ratio_cred == x_world_agenda_ratio_cred
    assert bob_otherunit._world_agenda_ratio_debt == x_world_agenda_ratio_debt
...
def test_OtherUnit_clear_treasurying_data_SetsAttrCorrectly_Method():
    # GIVEN
    bob_otherunit = otherunit_shop("Bob")
    bob_otherunit._world_agenda_ratio_cred = 0.355
    bob_otherunit._world_agenda_ratio_debt = 0.066
    x_treasury_cred_score = 900
    x_treasury_voice_rank = 45
    bob_otherunit.set_treasury_attr(
...
def test_OtherUnit_get_dict_ReturnsDictWithAllAttrDataForJSON():
    ...
    bob_otherunit._world_cred = bob_world_cred
    bob_otherunit._world_debt = bob_world_debt
    bob_otherunit._world_agenda_cred = bob_world_agenda_cred
    bob_otherunit._world_agenda_debt = bob_world_agenda_debt
    bob_otherunit._world_agenda_ratio_cred = bob_world_agenda_ratio_cred
    bob_otherunit._world_agenda_ratio_debt = bob_world_agenda_ratio_debt
    bob_otherunit._output_world_meld_order = bob_output_world_meld_order
