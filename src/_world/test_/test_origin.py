...
def test_OriginLink_exists():
    ...
    # WHEN
    originlink_x = OriginLink(person_id=roy_text, weight=roy_weight)

    # THEN
    assert originlink_x.person_id == roy_text
...
def test_originlink_shop_ReturnsCorrectObj():
    ...
    # WHEN
    originlink_x = originlink_shop(person_id=roy_text, weight=roy_weight)
...
def test_originlink_shop_WeightIsNot_Reason():
    # GIVEN
    roy_text = "Roy"
    # roy_person_id = PersonID(roy_text)

    # WHEN
    originlink_x = originlink_shop(person_id=roy_text)
...
def test_OriginLink_get_dict_ReturnsDictWithNecessaryDataForJSON():
    # GIVEN
    roy_text = "Roy"
    roy_originlink = originlink_shop(person_id=roy_text)
    ...
    print(f"{x_dict=}")
    assert x_dict != None
    assert x_dict == {"person_id": roy_text, "weight": 1}
...
def test_originunit_set_originlink_CorrectlySetsOriginLink():
    ...
    tim_text = "Tim"
    tim_weight = 3
    originunit_x.set_originlink(person_id=tim_text, weight=tim_weight)

    # THEN
    assert originunit_x._links.get(tim_text) != None
    assert originunit_x._links.get(tim_text).person_id == tim_text
...
def test_originunit_del_originlink_CorrectlyDeletesOriginLink():
    ...
    # WHEN
    originunit_x.del_originlink(person_id=tim_text)
...
def test_OriginUnit_get_dict_ReturnsDictWithNecessaryDataForJSON():
    ...
    sue_text = "Sue"
    sue_weight = 4
    sue_originlink = originlink_shop(person_id=sue_text, weight=sue_weight)
    sue_ol_dict = sue_originlink.get_dict()

    originunit_x = originunit_shop()
    originunit_x.set_originlink(person_id=roy_text, weight=None)
    originunit_x.set_originlink(person_id=sue_text, weight=sue_weight)
