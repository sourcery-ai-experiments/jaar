...
def test_PremiseUnitUnit_set_status_CorrectlySets_status_WhenFactUnitIsNull():
    ...
    # WHEN
    premise_2.set_status(x_factheir=world_fact_2)
...
def test_PremiseUnitUnit_set_status_CorrectlySetsStatusOfSimple():
    ...
    world_fact = factheir_shop(base=wkday_road, pick=wed_road)
    assert wed_premise._status is None

    # WHEN
    wed_premise.set_status(x_factheir=world_fact)
...
def test_PremiseUnit_set_status_CorrectlySetsTimeRangeStatusFalse():
    ...
    # WHEN
    world_fact = factheir_shop(base=hr24_road, pick=hr24_road, open=8, nigh=10)
    hr24_premise.set_status(x_factheir=world_fact)
...
def test_PremiseUnit_set_status_CorrectlySetCEDWeekStatusTrue():
    ...
    world_fact = factheir_shop(base=week_road, pick=week_road, open=7, nigh=7)
    assert week_premise._status is None

    # WHEN
    week_premise.set_status(x_factheir=world_fact)
