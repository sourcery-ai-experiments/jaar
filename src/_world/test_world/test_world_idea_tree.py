...
from src._world.other import OtherID
...
def test_WorldUnit_calc_world_metrics_RootOnlyCorrectlySetsDescendantAttributes():
    ...
    assert tim_world._idearoot._all_other_cred is None
    assert tim_world._idearoot._all_other_debt is None
    ...
    assert tim_world._idearoot._all_other_cred == True
    assert tim_world._idearoot._all_other_debt == True
...
def test_WorldUnit_calc_world_metrics_NLevelCorrectlySetsDescendantAttributes_2():
    ...
    x_world.add_otherunit(other_id=sue_text)
    ...
    # THEN
    assert x_world._idearoot._all_other_cred is False
    assert x_world._idearoot._all_other_debt is False
...
def test_world4other_Exists():
    ...
    x_world.add_otherunit(other_id=sue_other_id)
    ...
    # WHEN
    sue_world4other = x_world.get_world4other(facts=None, other_id=sue_other_id)

    # THEN
    assert sue_world4other
    assert str(type(sue_world4other)).find(".world.WorldUnit'>")
    assert sue_world4other._owner_id == sue_other_id


def test_world4other_hasCorrectLevel1StructureNoBelieflessAncestors():
    # GIVEN
    x_world = example_worlds_get_world_with_4_levels()
    email_text = "email"
    casa_text = "casa"
    ...
    x_world.add_otherunit(other_id=yao_other_id)
    ...
    # WHEN
    sue_world4other = x_world.get_world4other(sue_other_id, facts=None)

    # THEN
    assert len(sue_world4other._idearoot._kids) > 0
    print(f"{len(sue_world4other._idearoot._kids)=}")

    casa_idea = sue_world4other.get_idea_obj(casa_road)
    ...
    assert sue_world4other._idearoot._kids.get(feed_text) is None
    assert sue_world4other._idearoot._world_importance == 1
    assert casa_idea._world_importance == yrx._kids[casa_text]._world_importance
    world4other_road = sue_world4other.make_l1_road("__world4other__")
    assert sue_world4other.get_idea_obj(world4other_road) != None

    y4a_exteriors = sue_world4other.get_idea_obj(world4other_road)
