...
def test_WorldUnit_calc_world_metrics_CorrectlySetsCharLinkWorldCredAndDebt():
    ...
    bl_rico = cashlink_shop(belief_id=rico_text, credor_weight=20, debtor_weight=40)
    bl_carm = cashlink_shop(belief_id=carm_text, credor_weight=10, debtor_weight=5)
    bl_patr = cashlink_shop(belief_id=patr_text, credor_weight=10, debtor_weight=5)
    yao_world._idearoot.set_cashlink(cashlink=bl_rico)
    yao_world._idearoot.set_cashlink(cashlink=bl_carm)
    yao_world._idearoot.set_cashlink(cashlink=bl_patr)
    ...
    # for cashlink in yao_world._cashheirs.values():
    #     print(
    #         f"{yao_world._world_importance=} {cashlink.belief_id=} {cashlink._world_cred=} {cashlink._world_debt=}"
    #     )
    ...
    yao_world._idearoot.set_cashlink(
        cashlink=cashlink_shop(
...
def test_WorldUnit_calc_world_metrics_CorrectlySetsCharUnitWorldImportance():
    ...
    bl_carm = cashlink_shop(belief_id=carm_text, credor_weight=10, debtor_weight=5)
    bl_patr = cashlink_shop(belief_id=patr_text, credor_weight=10, debtor_weight=5)
    yao_world._idearoot._kids.get(swim_text).set_cashlink(cashlink=bl_rico)
    yao_world._idearoot._kids.get(swim_text).set_cashlink(cashlink=bl_carm)
    yao_world._idearoot._kids.get(swim_text).set_cashlink(cashlink=bl_patr)
...
def test_WorldUnit_agenda_cred_debt_IsCorrectlySet():
    ...
    x_cashagendametrics = CashAgendaMetrics()
    x_cashagendametrics.set_sums(agenda_dict=agenda_dict)
    # print(f"{sum_world_agenda_importance=}")
    assert x_cashagendametrics.agenda_no_count == 14
    assert x_cashagendametrics.agenda_yes_count == 49
    assert x_cashagendametrics.agenda_no_world_i_sum == 0.0037472680016539662
    assert x_cashagendametrics.agenda_yes_world_i_sum == 0.0027965049894874455
    assert are_equal(
        x_cashagendametrics.agenda_no_world_i_sum
        + x_cashagendametrics.agenda_yes_world_i_sum,
        x_cashagendametrics.sum_world_agenda_importance,
    )
    assert x_cashagendametrics.sum_world_agenda_importance == 0.006543772991141412
