...
def test_WorldUnit_clear_output_world_meld_orders_CorrectlyClearsAttrs():
    ...
    yao_world.set_charunit(charunit=charunit_shop(CharID(patr_text)))
    rico_charunit = yao_world.get_char(rico_text)
    carm_charunit = yao_world.get_char(carm_text)
    patr_charunit = yao_world.get_char(patr_text)
    rico_charunit.set_output_world_meld_order(3)
    carm_charunit.set_output_world_meld_order(4)
    patr_charunit.set_output_world_meld_order(5)

    assert rico_charunit._output_world_meld_order != None
    assert carm_charunit._output_world_meld_order != None
    assert patr_charunit._output_world_meld_order != None

    # WHEN
    yao_world.clear_chars_output_world_meld_order()

    # THEN
    assert rico_charunit._output_world_meld_order is None
    assert carm_charunit._output_world_meld_order is None
    assert patr_charunit._output_world_meld_order is None
...
def test_WorldUnit_clear_output_world_meld_orders_WithNoArgsCorrectlySetOrder():
    ...
    # WHEN
    yao_world.set_chars_output_world_meld_order()

    # THEN
    assert rico_charunit._output_world_meld_order != None
    assert carm_charunit._output_world_meld_order != None
    assert patr_charunit._output_world_meld_order != None
    print(f"{rico_charunit._output_world_meld_order=}")
    print(f"{carm_charunit._output_world_meld_order=}")
    print(f"{patr_charunit._output_world_meld_order=}")
    assert rico_charunit._output_world_meld_order == 2
    assert carm_charunit._output_world_meld_order == 0
    assert patr_charunit._output_world_meld_order == 1
...
def test_WorldUnit_set_charunit_external_metrics_SetsAttrs_credor_operational_debtor_operational():
    # GIVEN
    x_world = worldunit_shop("Yao")
    jane_text = "Jane Randolph"
    x_world.add_charunit(jane_text)

    jane_char = x_world._chars.get(jane_text)
    print(f"Before Char {jane_char.char_id} {jane_char._debtor_operational=} ")
    assert jane_char._debtor_operational is None
    assert jane_char._credor_operational is None

    assert sum(
        char_x._credor_operational is None for char_x in x_world._chars.values()
    ) == len(x_world._chars)
    assert sum(
        char_x._debtor_operational is None for char_x in x_world._chars.values()
    ) == len(x_world._chars)

    # WHEN
    jane_debtor_status = True
    jane_credor_status = True
    jane_metr = CharUnitExternalMetrics(
        internal_char_id=jane_text,
        debtor_operational=jane_debtor_status,
        credor_operational=jane_credor_status,
    )
    x_world.set_charunit_external_metrics(jane_metr)

    # THEN
    assert jane_char._debtor_operational == jane_debtor_status
    assert jane_char._credor_operational == jane_credor_status

    assert (
        sum(char_x._credor_operational is None for char_x in x_world._chars.values())
        == len(x_world._chars) - 1
    )
    assert (
        sum(char_x._debtor_operational is None for char_x in x_world._chars.values())
        == len(x_world._chars) - 1
    )
    assert (
        sum(char_x._credor_operational != None for char_x in x_world._chars.values())
        == 1
    )
    assert (
        sum(char_x._debtor_operational != None for char_x in x_world._chars.values())
        == 1
    )
