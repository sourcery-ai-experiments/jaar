...
from src._world.beliefunit import cashlink_shop
...
def test_WorldUnit_edit_idea_attr_IsAbleToEditAnyAncestor_Idea():
    ...
    # _cashlink: dict = None,
    sue_world._idearoot._kids[casa_text]._cashlinks = {
        "fun": cashlink_shop(belief_id="fun", credor_weight=1, debtor_weight=7)
    }
    _cashlinks = sue_world._idearoot._kids[casa_text]._cashlinks
    assert _cashlinks == {
        "fun": cashlink_shop(belief_id="fun", credor_weight=1, debtor_weight=7)
    }
    sue_world.edit_idea_attr(
        road=casa_road,
        cashlink=cashlink_shop(belief_id="fun", credor_weight=4, debtor_weight=8),
    )
    assert sue_world._idearoot._kids[casa_text]._cashlinks == {
        "fun": cashlink_shop(belief_id="fun", credor_weight=4, debtor_weight=8)
    }
