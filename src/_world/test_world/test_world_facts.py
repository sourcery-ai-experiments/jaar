...
def test_WorldUnit_set_fact_CorrectlyModifiesAttr_1():
    ...
    sunday_world_fact = factunit_shop(base=weekday_road, pick=sunday_road)
    print(sunday_world_fact)
    x_idearoot = x_world._idearoot
    x_idearoot._factunits = {sunday_world_fact.base: sunday_world_fact}
    ...
    # WHEN
    x_world.set_fact(base=usa_week_road, pick=usa_week_road, open=608, nigh=610)
...
def test_WorldUnit_set_fact_CorrectlyModifiesAttrWhen_pick_IsNone():
    ...
    # THEN
    sunday_world_fact = factunit_shop(weekday_road, weekday_road, 5, 7)
    x_idearoot = x_world._idearoot
    assert x_idearoot._factunits == {sunday_world_fact.base: sunday_world_fact}
...
def test_WorldUnit_set_fact_CorrectlyModifiesAttrWhen_open_IsNone():
    ...
    # WHEN
    x_world.set_fact(base=weekday_road, nigh=10)
...
def test_WorldUnit_set_fact_FailsToCreateWhenBaseAndFactAreDifferenctAndFactIdeaIsNotRangeRoot():
    ...
    bob_world.add_l1_idea(time_idea)
    time_road = bob_world.make_l1_road(time_text)
    a1st = "age1st"
    a1st_road = bob_world.make_road(time_road, a1st)
    a1st_idea = ideaunit_shop(a1st, _begin=0, _close=20)
    bob_world.add_idea(a1st_idea, parent_road=time_road)
    a1e1st_text = "a1_era1st"
    a1e1st_idea = ideaunit_shop(a1e1st_text, _begin=20, _close=30)
    bob_world.add_idea(a1e1st_idea, parent_road=a1st_road)
    a1e1_road = bob_world.make_road(a1st_road, a1e1st_text)
    assert bob_world._idearoot._factunits in (None, {})

    # WHEN / THEN
    with pytest_raises(Exception) as excinfo:
        bob_world.set_fact(base=a1e1_road, pick=a1e1_road, open=20, nigh=23)
    assert (
        str(excinfo.value)
...
def test_WorldUnit_del_fact_CorrectlyModifiesAttr():
    ...
    # WHEN
    x_world.del_fact(base=weekday_road)
...
def test_WorldUnit_get_idea_list_FactHeirsCorrectlyInherited():
    ...
    fast_text = "fast"
    slow_text = "slow"
    fast_road = bob_world.make_road(swim_road, fast_text)
    slow_road = bob_world.make_road(swim_road, slow_text)
    bob_world.add_idea(ideaunit_shop(fast_text), parent_road=swim_road)
    bob_world.add_idea(ideaunit_shop(slow_text), parent_road=swim_road)

    earth_text = "earth"
    earth_road = bob_world.make_l1_road(earth_text)
    bob_world.add_l1_idea(ideaunit_shop(earth_text))

    swim_idea = bob_world.get_idea_obj(swim_road)
    fast_idea = bob_world.get_idea_obj(fast_road)
    slow_idea = bob_world.get_idea_obj(slow_road)
    ...
    # WHEN
    bob_world.set_fact(base=earth_road, pick=earth_road, open=1.0, nigh=5.0)
...
def test_WorldUnit_get_idea_list_FactUnitCorrectlyTransformsfactheir_shop():
    ...
    bob_world.set_fact(base=earth_road, pick=earth_road, open=3.0, nigh=5.0)
...
def test_WorldUnit_get_idea_list_FactHeirCorrectlyDeletesFactUnit():
    ...
    fast_text = "fast"
    slow_text = "slow"
    sue_world.add_idea(ideaunit_shop(fast_text), parent_road=swim_road)
    sue_world.add_idea(ideaunit_shop(slow_text), parent_road=swim_road)
    earth_text = "earth"
    earth_road = sue_world.make_l1_road(earth_text)
    sue_world.add_l1_idea(ideaunit_shop(earth_text))

    swim_idea = sue_world.get_idea_obj(swim_road)
    ...
    # WHEN
    sue_world.set_fact(base=earth_road, pick=earth_road, open=200.0, nigh=500.0)
...
def test_get_ranged_facts():
    ...
    # WHEN / THEN
    assert len(sue_world._get_rangeroot_factunits()) == 1

    # WHEN one ranged fact added
    place_text = "place_x"
    place_idea = ideaunit_shop(place_text, _begin=600, _close=800)
    sue_world.add_l1_idea(place_idea)
    place_road = sue_world.make_l1_road(place_text)
    sue_world.set_fact(base=place_road, pick=place_road, open=5, nigh=10)
    print(f"When one ranged fact added {sue_world._idearoot._factunits=}")
    assert len(sue_world._idearoot._factunits) == 2

    # THEN
    assert len(sue_world._get_rangeroot_factunits()) == 2

    # WHEN one non-ranged_fact added
    mood = "mood_x"
    sue_world.add_l1_idea(ideaunit_shop(mood))
    m_road = sue_world.make_l1_road(mood)
    sue_world.set_fact(base=m_road, pick=m_road)
    print(f"When one non-ranged_fact added {sue_world._idearoot._factunits=}")
    assert len(sue_world._idearoot._factunits) == 3
...
def test_get_roots_ranged_facts():
    ...
    time_road = sue_world.make_l1_road(time_text)
    mood_x = "mood_x"
    sue_world.add_l1_idea(ideaunit_shop(mood_x))
    m_x_road = sue_world.make_l1_road(mood_x)
    happy = "happy"
    sad = "Sad"
    sue_world.add_idea(ideaunit_shop(happy), parent_road=m_x_road)
    sue_world.add_idea(ideaunit_shop(sad), parent_road=m_x_road)
    sue_world.set_fact(base=time_road, pick=time_road, open=5, nigh=10)
    ...
    # WHEN / THEN
    assert len(sue_world._get_rangeroot_factunits()) == 1
    assert sue_world._get_rangeroot_factunits()[0].base == time_road

    # a fact who's idea range is defined by numeric_root is not "rangeroot"
    mirror_x = "mirror_x"
    sue_world.add_l1_idea(ideaunit_shop(mirror_x, _numeric_road=time_text))
    m_x_road = sue_world.make_l1_road(mirror_x)
    sue_world.set_fact(base=m_x_road, pick=time_road, open=5, nigh=10)
    assert len(sue_world._idearoot._factunits) == 3
...
def test_create_lemma_facts_CorrectlyCreates1stLevelLemmaFact_Scenario1():
    ...
    # # the pledge
    # clean = "clean"
    # sue_world.add_idea(ideaunit_shop(clean, pledge=True))
    ...
    sue_world.add_idea(age1st_idea, parent_road=time_road)
    sue_world.add_idea(age2nd_idea, parent_road=time_road)
    sue_world.add_idea(age3rd_idea, parent_road=time_road)
    sue_world.add_idea(age4th_idea, parent_road=time_road)
    sue_world.add_idea(age5th_idea, parent_road=time_road)
    sue_world.add_idea(age6th_idea, parent_road=time_road)
    sue_world.add_idea(age7th_idea, parent_road=time_road)

    # set for instant moment in 3rd age
    sue_world.set_fact(base=time_road, pick=time_road, open=45, nigh=45)
    lemma_dict = sue_world._get_lemma_factunits()
    print(f"{len(lemma_dict)=}")
    print(f"{lemma_dict=}")
    assert len(lemma_dict) == 7
    age1st_lemma = lemma_dict[sue_world.make_road(time_road, age1st_text)]
    age2nd_lemma = lemma_dict[sue_world.make_road(time_road, age2nd_text)]
    age3rd_lemma = lemma_dict[sue_world.make_road(time_road, age3rd_text)]
    age4th_lemma = lemma_dict[sue_world.make_road(time_road, age4th_text)]
    age5th_lemma = lemma_dict[sue_world.make_road(time_road, age5th_text)]
    age6th_lemma = lemma_dict[sue_world.make_road(time_road, age6th_text)]
    age7th_lemma = lemma_dict[sue_world.make_road(time_road, age7th_text)]
...
def test_create_lemma_facts_CorrectlyCreates1stLevelLemmaFact_Scenario2():
    ...
    # set for instant moment in 3rd age
    sue_world.set_fact(base=time_road, pick=time_road, open=35, nigh=65)
    lemma_dict = sue_world._get_lemma_factunits()
    assert len(lemma_dict) == 7
    age1st_lemma = lemma_dict[sue_world.make_road(time_road, age1st_text)]
    age2nd_lemma = lemma_dict[sue_world.make_road(time_road, age2nd_text)]
...
def test_create_lemma_facts_CorrectlyCreates1stLevelLemmaFact_Scenario3():
    ...
    a2_road = sue_world.make_road(time_road, age2nd_text)
    ...
    sue_world.add_idea(a2e1st_idea, parent_road=a2_road)
    sue_world.add_idea(a2e2nd_idea, parent_road=a2_road)
    sue_world.add_idea(a2e3rd_idea, parent_road=a2_road)
    sue_world.add_idea(a2e4th_idea, parent_road=a2_road)

    a3_road = sue_world.make_road(time_road, age3rd_text)
    ...
    sue_world.add_idea(a3e1st_idea, parent_road=a3_road)
    sue_world.add_idea(a3e2nd_idea, parent_road=a3_road)
    sue_world.add_idea(a3e3rd_idea, parent_road=a3_road)
    sue_world.add_idea(a3e4th_idea, parent_road=a3_road)

    # set for instant moment in 3rd age
    sue_world.set_fact(base=time_road, pick=time_road, open=35, nigh=55)
    lemma_dict = sue_world._get_lemma_factunits()
    assert len(lemma_dict) == 15
    a2e1st_lemma = lemma_dict[sue_world.make_road(a2_road, a2e1st_text)]
    a2e2nd_lemma = lemma_dict[sue_world.make_road(a2_road, a2e2nd_text)]
    a2e3rd_lemma = lemma_dict[sue_world.make_road(a2_road, a2e3rd_text)]
    a2e4th_lemma = lemma_dict[sue_world.make_road(a2_road, a2e4th_text)]
    a3e1st_lemma = lemma_dict[sue_world.make_road(a3_road, a3e1st_text)]
    a3e2nd_lemma = lemma_dict[sue_world.make_road(a3_road, a3e2nd_text)]
    a3e3rd_lemma = lemma_dict[sue_world.make_road(a3_road, a3e3rd_text)]
    a3e4th_lemma = lemma_dict[sue_world.make_road(a3_road, a3e4th_text)]
...
def test_create_lemma_facts_CorrectlyCreates1stLevelLemmaFact_Scenario4():
    ...
    as1_road = sue_world.make_l1_road(arsub1)
    sue_world.add_l1_idea(arsub1_idea)
    ...
    as2_road = sue_world.make_l1_road(arsub2)
    sue_world.add_l1_idea(arsub2_idea)
    ...
    a1_lemma = lemma_dict[sue_world.make_road(time_road, age1st)]
...
def test_create_lemma_facts_CorrectlyCreatesNthLevelLemmaFact_Scenario4_1():
    ...
    jajatime_road = sue_world.make_road(time_road, "jajatime")
    timetech_road = sue_world.make_road(time_road, "tech")
    sue_world.set_fact(jajatime_road, jajatime_road, open=1500, nigh=1500)
    lhu = sue_world._get_lemma_factunits()

    assert lhu[sue_world.make_road(jajatime_road, "400 year segment")].open == 1500
    assert lhu[sue_world.make_road(jajatime_road, "400 year segment")].nigh == 1500
    assert lhu[sue_world.make_road(jajatime_road, "400 year segments")].open > 0
    assert lhu[sue_world.make_road(jajatime_road, "400 year segments")].open < 1
    assert lhu[sue_world.make_road(jajatime_road, "400 year segments")].nigh > 0
    assert lhu[sue_world.make_road(jajatime_road, "400 year segments")].nigh < 1
    assert lhu[sue_world.make_road(jajatime_road, "days")].open >= 1
    assert lhu[sue_world.make_road(jajatime_road, "days")].open <= 2
    assert lhu[sue_world.make_road(jajatime_road, "days")].nigh >= 1
    assert lhu[sue_world.make_road(jajatime_road, "days")].nigh <= 2
    assert lhu[sue_world.make_road(jajatime_road, "day")].open == 60
    assert lhu[sue_world.make_road(jajatime_road, "day")].nigh == 60
    assert lhu[sue_world.make_road(jajatime_road, "week")].open == 1500
    assert int(lhu[sue_world.make_road(jajatime_road, "week")].nigh) == 1500
    assert lhu[sue_world.make_road(timetech_road, "week")].open == 1500
    assert int(lhu[sue_world.make_road(timetech_road, "week")].nigh) == 1500
...
def test_create_lemma_facts_CorrectlyCreatesNthLevelLemmaFact_Scenario5():
    ...
    timetech_road = sue_world.make_road(time_road, "tech")
    jajatime_road = sue_world.make_road(time_road, "jajatime")
    sue_world.set_fact(jajatime_road, jajatime_road, 1500, nigh=1063954002)
    lhu = sue_world._get_lemma_factunits()

    assert lhu[sue_world.make_road(jajatime_road, "400 year segment")].open == 0
    assert lhu[sue_world.make_road(jajatime_road, "400 year segment")].nigh == 210379680
    assert lhu[sue_world.make_road(jajatime_road, "400 year segments")].open > 0
    assert lhu[sue_world.make_road(jajatime_road, "400 year segments")].open < 1
    assert lhu[sue_world.make_road(jajatime_road, "400 year segments")].nigh > 5
    assert lhu[sue_world.make_road(jajatime_road, "400 year segments")].nigh < 6
    lemma_days = lhu[sue_world.make_road(jajatime_road, "days")]
    assert int(lemma_days.open) == 1  # 0 / 1440
    assert int(lemma_days.nigh) == 738856  # 1063953183 / 1440
    lemma_day = lhu[sue_world.make_road(jajatime_road, "day")]
    assert lemma_day.open == 0  # 0 / 1440
    assert lemma_day.nigh == 1440  # 1362  # 1063953183 / 1440
    lemma_jajatime_week = lhu[sue_world.make_road(jajatime_road, "week")]
    assert lemma_jajatime_week.open == 0  # 0 / 1440
    assert int(lemma_jajatime_week.nigh) == 10080  # 1063953183 / 1440
    lemma_timetech_week = lhu[sue_world.make_road(jajatime_road, "week")]
...
def test_create_lemma_facts_CorrectlyCreatesNthLevelLemmaFact_Scenario6():
    ...
    time_road = sue_world.make_l1_road("time")
    jajatime_road = sue_world.make_road(time_road, "jajatime")
    sue_world.set_fact(jajatime_road, jajatime_road, 1063954000, nigh=1063954002)
    lhu = sue_world._get_lemma_factunits()

    assert (
        lhu[sue_world.make_road(jajatime_road, "400 year segment")].open == 12055600.0
    )
    assert (
        lhu[sue_world.make_road(jajatime_road, "400 year segment")].nigh == 12055602.0
    )
    assert lhu[sue_world.make_road(jajatime_road, "400 year segments")].open > 5
    assert lhu[sue_world.make_road(jajatime_road, "400 year segments")].open < 6
    assert lhu[sue_world.make_road(jajatime_road, "400 year segments")].nigh > 5
    assert lhu[sue_world.make_road(jajatime_road, "400 year segments")].nigh < 6
    lemma_days = lhu[sue_world.make_road(jajatime_road, "days")]
...
def test_create_lemma_facts_CorrectlyCreatesNthLevelLemmaFact_Scenario7():
    ...
    time_road = sue_world.make_l1_road("time")
    timetech_road = sue_world.make_road(time_road, "tech")
    techweek_road = sue_world.make_road(timetech_road, "week")
    jajatime_road = sue_world.make_road(time_road, "jajatime")

    # WHEN minute range that should be Thursday to Monday midnight
    sue_world.set_fact(jajatime_road, jajatime_road, 1063951200, nigh=1063956960)
    lhu = sue_world._get_lemma_factunits()

    # THEN
    week_open = lhu[sue_world.make_road(jajatime_road, "week")].open
    week_nigh = lhu[sue_world.make_road(jajatime_road, "week")].nigh
    week_text = "week"
    print(
        f"for {sue_world.make_road(jajatime_road,week_text)}: {week_open=} {week_nigh=}"
    )
    assert lhu[sue_world.make_road(jajatime_road, "week")].open == 7200
    assert lhu[sue_world.make_road(jajatime_road, "week")].nigh == 2880

    week_open = lhu[sue_world.make_road(timetech_road, "week")].open
    week_nigh = lhu[sue_world.make_road(timetech_road, "week")].nigh
    print(
        f"for {sue_world.make_road(timetech_road,week_text)}: {week_open=} {week_nigh=}"
    )
    assert lhu[sue_world.make_road(timetech_road, "week")].open == 7200
    assert lhu[sue_world.make_road(timetech_road, "week")].nigh == 2880
    print(f"{techweek_road=}")
    print(lhu[techweek_road])
    print(lhu[sue_world.make_road(techweek_road, "Thursday")])
    print(lhu[sue_world.make_road(techweek_road, "Friday")])
    print(lhu[sue_world.make_road(techweek_road, "Saturday")])
    print(lhu[sue_world.make_road(techweek_road, "Sunday")])
    print(lhu[sue_world.make_road(techweek_road, "Monday")])
    print(lhu[sue_world.make_road(techweek_road, "Tuesday")])
    print(lhu[sue_world.make_road(techweek_road, "Wednesday")])
...
def test_create_lemma_facts_CorrectlyCreatesNthLevelLemmaFact_Scenario8():
    ...
    # WHEN minute range that should be Thursday to Monday midnight
    sue_world.set_fact(jajatime_road, jajatime_road, 1063951200, nigh=1063951200)
    lhu = sue_world._get_lemma_factunits()
    ...
    assert lhu[sue_world.make_road(timetech_road, "week")].open == 7200
    assert lhu[sue_world.make_road(timetech_road, "week")].nigh == 7200
    print(lhu[techweek_road])
    print(lhu[sue_world.make_road(techweek_road, "Thursday")])
    print(lhu[sue_world.make_road(techweek_road, "Friday")])
    print(lhu[sue_world.make_road(techweek_road, "Saturday")])
...
def test_WorldUnit_set_fact_create_missing_ideas_CreatesBaseAndFact():
    ...
    climate_road = sue_world.make_road(situations_road, climate_text)
    assert sue_world._idearoot.get_kid(situations_text) is None

    # WHEN
    sue_world.set_fact(situations_road, climate_road, create_missing_ideas=True)

    # THEN
    assert sue_world._idearoot.get_kid(situations_text) != None
    assert sue_world.get_idea_obj(situations_road) != None
    assert sue_world.get_idea_obj(climate_road) != None
...
def test_WorldUnit_get_fact_ReturnsFactUnit():
    ...
    # WHEN
    generated_situations_base = sue_world.get_fact(situations_road)
