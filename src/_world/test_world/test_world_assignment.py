...
def test_worldunit_get_assignment_ReturnsEmptyBecauseAssignorIsNotInOthers():
    # GIVEN
    noa_text = "Noa"
    noa_world = example_worlds_get_world_with_4_levels()
    noa_world.set_otherunit(otherunit_shop(other_id=noa_text))
    zia_text = "Zia"
    yao_text = "Yao"
    noa_world.set_otherunit(otherunit_shop(other_id=zia_text))
    noa_world.set_otherunit(otherunit_shop(other_id=yao_text))

    # WHEN
    bob_text = "Bob"
    y_world = worldunit_shop(_owner_id=noa_text)
    x_world = worldunit_shop()
    x_world.set_otherunit(otherunit=otherunit_shop(other_id=zia_text))
    x_world.set_otherunit(otherunit=otherunit_shop(other_id=noa_text))

    x_assignment_world = noa_world.get_assignment(y_world, x_world._others, bob_text)

    # THEN
    assert len(noa_world._others) == 3
    assert len(x_assignment_world._others) == 0


def test_worldunit_get_assignment_ReturnsCorrectOthers():
    # GIVEN
    jes_text = "Jessi"
    ...
    jes_world.set_otherunit(otherunit_shop(other_id=zia_text))
    jes_world.set_otherunit(otherunit_shop(other_id=noa_text))
    jes_world.set_otherunit(otherunit_shop(other_id=yao_text))
    ...
    x_assignment_world = jes_world.get_assignment(x_world, tx._others, bob_text)

    # THEN
    assert len(x_assignment_world._others) == 3
    assert x_assignment_world._others.get(bob_text) != None
    assert x_assignment_world._others.get(zia_text) != None
    assert x_assignment_world._others.get(noa_text) != None
    assert x_assignment_world._others.get(yao_text) is None
...
def test_worldunit_get_assignment_ReturnsCorrectBeliefs_Scenario1():
    ...
    x_assignment_world = jes_world.get_assignment(valueless_world, tx._others, bob_text)
...
def test_WorldUnit_get_assignment_getsCorrectIdeas_scenario1():
    ...
    dirty_road = x_world.make_road(status_road, dirty_text)
    bob_text = "Bob"
    x_world.add_otherunit(other_id=bob_text)
...
def test_WorldUnit_get_assignment_CorrectlyCreatesAssignmentWorldUnit_v1():
    ...
    cali_assignment = amer_world.get_assignment(
        world_x=cali_world,
        assignor_others={cali_text: -1, amer_world._owner_id: -1},
