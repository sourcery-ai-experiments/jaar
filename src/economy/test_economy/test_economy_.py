...
def test_economy_exists():
    economy_tag = "test1"
    sx = EconomyUnit(tag=economy_tag, economys_dir=get_test_economys_dir())
    assert sx.tag == economy_tag
...
def test_rename_example_economy_CorrectlyRenamesDirAndFiles(env_dir_setup_cleanup):
    ...
    sx = economyunit_shop(tag=old_economy_tag, economys_dir=get_test_economys_dir())
    # x_func_delete_dir(sx.get_object_root_dir())
...
def test_copy_evaluation_economy_CorrectlyCopiesDirAndFiles(env_dir_setup_cleanup):
    ...
    # WHEN
    copy_evaluation_economy(src_tag=sx.tag, dest_tag=new_economy_tag)
...
def test_copy_evaluation_economy_CorrectlyRaisesError(env_dir_setup_cleanup):
    ...
    with pytest_raises(Exception) as excinfo:
        copy_evaluation_economy(src_tag=sx.tag, dest_tag=old_economy_tag)
    assert (
        str(excinfo.value)
...
def test_economyunit_shop_CorrectlyReturnsObj(env_dir_setup_cleanup):
    ...
    # WHEN
    sx = economyunit_shop(tag=park_text, economys_dir=get_test_economys_dir())
