...
from src.economy.council import councilunit_shop, CouncilUnit
...
def test_CouncilUnit_exists(council_dir_setup_cleanup):
    ...
    assert x_council._economy_id is None
...
def test_councilunit_shop_exists(council_dir_setup_cleanup):
    ...
    assert x_council._economy_id != None
    assert x_council._economy_id == get_temp_economy_id()
    assert x_council._road_node_delimiter == get_node_delimiter()
    assert x_council._seed != None
    assert x_council._seed._economy_id == get_temp_economy_id()
...
def test_councilunit_auto_output_to_public_DoesNotSaveAgendaToPublicDirWhenFalse(
    ...
    print(f"{public_file_path=}")
    # public_file_path = f"src/economy/examples/ex_env/agendas/{public_file_name}"
    x_council = councilunit_shop(tim_text, env_dir, x_economy_id, False)
    x_council.create_core_dir_and_files()
    assert os_path.exists(public_file_path) is False
...
def test_councilunit_get_seed_getsMemoryAgendaIfExists(
    ...
    tim_council = councilunit_shop(
        tim_text, get_temp_councilunit_dir(), get_temp_economy_id()
    )
    tim_council.create_core_dir_and_files()
