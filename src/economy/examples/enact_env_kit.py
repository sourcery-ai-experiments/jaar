...
def get_temp_enactunit_dir() -> str:
...
@pytest_fixture()
def enact_dir_setup_cleanup():
    healer_dir = get_temp_enactunit_dir()
    delete_dir(dir=healer_dir)
    yield healer_dir
    delete_dir(dir=healer_dir)


def create_agenda_file(agenda_enactunit_dir: str, agenda_healer: str):
    agenda_x = agendaunit_shop(_healer=agenda_healer)
    # file_path = f"{agenda_enactunit_dir}/{agenda_x._healer}.json"
