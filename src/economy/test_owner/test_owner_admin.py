...
def test_OwnerAdmin_create_core_dir_and_files_CreatesDirsAndFiles(
    ...
    jul_text = "julian"
    env_dir = get_temp_owner_dir()
    pdx = OwnerAdmin(jul_text, env_dir, get_temp_economy_tag())
    pdx.set_dirs()
...
def test_OwnerAdmin_create_core_dir_and_files_DoesNotOverWriteIsolContract(
    ...
    jul_text = "julian"
    env_dir = get_temp_owner_dir()
    jul_pdx = OwnerAdmin(jul_text, env_dir, get_temp_economy_tag())
    jul_pdx.set_dirs()
    contract_x = example_owners_get_7nodeJRootWithH_contract()
...
def test_OwnerAdmin_set_owner_name_WorksCorrectly(owner_dir_setup_cleanup):
    ...
    old_owner_text = "bob"
    pdx = OwnerAdmin(old_owner_text, env_dir, get_temp_economy_tag())
    contract_x = example_owners_get_7nodeJRootWithH_contract()
    pdx.set_dirs()
    pdx.create_core_dir_and_files(contract_x)
...
def test_ownerunit_auto_output_to_public_SavesContractToPublicDir(
    ...
    # GIVEN
    bob_text = "bob"
    pdx = owneradmin_shop(bob_text, get_temp_owner_dir(), get_temp_economy_tag())
