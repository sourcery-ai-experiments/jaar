...
def test_AgendaUnit_ReasonUnits_create():
    ...
    # WHEN
    x_agenda.edit_fact_attr(road=casa_road, reason=casa_wk_reason)

    # THEN
    casa_fact = x_agenda.get_fact_obj(casa_road)
    assert casa_fact._reasonunits != None
    print(casa_fact._reasonunits)
    assert casa_fact._reasonunits[weekday_road] != None
    assert casa_fact._reasonunits[weekday_road] == casa_wk_reason


def test_AgendaUnit_edit_fact_attr_reasonunit_CorrectlySets_delimiter():
    # GIVEN
    x_agenda = example_agendas_get_agenda_with_4_levels()
    ...
    # WHEN
    x_agenda.edit_fact_attr(road=casa_road, reason=before_week_reason)

    # THEN
    casa_fact = x_agenda.get_fact_obj(casa_road)
    week_reasonunit = casa_fact._reasonunits.get(week_road)
    assert week_reasonunit.delimiter != slash_text
    assert week_reasonunit.delimiter == x_agenda._road_delimiter


def test_AgendaUnit_edit_fact_attr_reason_base_CorrectlySets_delimiter():
    # GIVEN
    slash_text = "/"
...
def test_intent_returned_WhenNoReasonsExist():
    ...
    # THEN
    casa_road = x_agenda.make_l1_road("casa")
    assert x_agenda.get_fact_obj(casa_road)._task == True
    cat_road = x_agenda.make_l1_road("feed cat")
    assert x_agenda.get_fact_obj(cat_road)._task == True
...
def test_AgendaUnit_reasonheirs_AreCorrectlyInherited_v1():
    ...
    print(f"{x_agenda._factroot._label=}")
    casa_text = "casa"
    casa_road = x_agenda.make_l1_road(casa_text)
    week_label = "weekdays"
    ...
    x_agenda.edit_fact_attr(road=casa_road, reason=casa_wk_build_reasonunit)
    casa_fact = x_agenda.get_fact_obj(casa_road)
    assert casa_fact._reasonunits != {}
    # print(casa_fact._reasonunits)
    assert casa_fact._reasonunits[week_road] != None
    assert casa_fact._reasonunits[week_road] == casa_wk_build_reasonunit
    try:
        casa_fact._reasonheirs[week_road]
    except KeyError as e:
        assert str(e) == f"'{x_agenda._real_id},weekdays'"

    x_agenda.calc_agenda_metrics()
    # fact_dict = x_agenda.get_fact_dict()
    # from_list_get_active(road=casa_road, fact_dict=fact_dict)

    casa_wk_cal_reasonheir = casa_fact._reasonheirs[week_road]
...
def test_AgendaUnit_reasonheirs_AreCorrectlyInheritedTo4LevelsFromRoot():
    ...
    a4_agenda.edit_fact_attr(road=casa_road, reason=casa_wk_build_reasonunit)

    # WHEN
    rla_text = "hp"
    rla_road = a4_agenda.make_road(casa_road, rla_text)
    a4_agenda.add_fact(factunit_shop(rla_text), parent_road=rla_road)
    cost_text = "cost_quantification"
    cost_road = a4_agenda.make_road(rla_road, cost_text)
    a4_agenda.add_fact(factunit_shop(cost_text), parent_road=cost_road)
    a4_agenda.calc_agenda_metrics()

    # THEN
    casa_fact = a4_agenda._factroot._kids[casa_text]
    rla_fact = casa_fact._kids[rla_text]
    cost_fact = rla_fact._kids[cost_text]

    # 1
    casa_wk_calc_reasonheir = casa_fact._reasonheirs[week_road]
    assert casa_wk_calc_reasonheir == casa_wk_built_reasonheir

    # 2
    rla_week_reasonheir = rla_fact._reasonheirs[week_road]
    ...
    assert (
        rla_week_reasonheir.suff_fact_active
        == casa_wk_built_reasonheir.suff_fact_active
    )
    ...
    assert rla_week_reasonheir._base_fact_active
    assert rla_week_reasonheir._base_fact_active != casa_wk_built_reasonheir

    # 3
    cost_week_reasonheir = cost_fact._reasonheirs[week_road]
    ...
    assert (
        cost_week_reasonheir.suff_fact_active
        == casa_wk_built_reasonheir.suff_fact_active
    )
    ...
    assert cost_week_reasonheir._base_fact_active
    assert cost_week_reasonheir._base_fact_active != casa_wk_built_reasonheir
...
def test_AgendaUnit_reasonheirs_AreCorrectlyInheritedTo4LevelsFromLevel2():
    ...
    casa_fact = a4_agenda._factroot.get_kid(casa_text)
    rla_fact = casa_fact.get_kid(rla_text)
    cost_fact = rla_fact.get_kid(cost_text)

    assert a4_agenda._factroot._reasonheirs == {}
    assert casa_fact._reasonheirs == {}
    assert rla_fact._reasonheirs == {}
    assert cost_fact._reasonheirs == {}

    # WHEN
    a4_agenda.calc_agenda_metrics()

    # THEN
    assert a4_agenda._factroot._reasonheirs == {}  # casa_wk_built_reasonheir

    # 1
    assert casa_fact._reasonheirs[week_road] == casa_wk_built_reasonheir
...
def test_AgendaUnit_ReasonUnits_set_UnCoupledMethod():
    ...
    # WHEN
    x_agenda.edit_fact_attr(
        road=casa_road, reason_base=week_road, reason_premise=wed_road
    )

    # THEN
    casa_fact1 = x_agenda.get_fact_obj(casa_road)
    assert casa_fact1._reasonunits != None
    print(casa_fact1._reasonunits)
    assert casa_fact1._reasonunits[week_road] != None
    assert casa_fact1._reasonunits[week_road].premises[wed_road].open is None
    assert casa_fact1._reasonunits[week_road].premises[wed_road].nigh is None
    ...
    assert casa_fact1._reasonunits[week_road] == casa_wk_reason1
    ...
    # THEN
    assert casa_fact1._reasonunits[week_road].premises[wed_road].open == 12
    assert casa_fact1._reasonunits[week_road].premises[wed_road].nigh == 12
    ...
    assert casa_fact1._reasonunits[week_road] == casa_wk_reason2
    ...
    # THEN
    assert len(casa_fact1._reasonunits[week_road].premises) == 2


def test_AgendaUnit_ReasonUnits_set_premiseFactWithDenomSetsPremiseDivision():
    # GIVEN
    x_agenda = example_agendas_get_agenda_with_4_levels()
    casa_text = "casa"
    casa_road = x_agenda.make_l1_road(casa_text)
    ...
    x_agenda.add_l1_fact(factunit_shop(time_text, _begin=100, _close=2000))
    x_agenda.add_fact(factunit_shop(week_text, _denom=7), parent_road=time_road)
    ...
    # THEN
    casa_fact1 = x_agenda.get_fact_obj(casa_road)
    assert casa_fact1._reasonunits[time_road] != None
    assert casa_fact1._reasonunits[time_road].premises[week_road].divisor == 7
    assert casa_fact1._reasonunits[time_road].premises[week_road].open == 2
    assert casa_fact1._reasonunits[time_road].premises[week_road].nigh == 5


def test_AgendaUnit_ReasonUnits_set_premiseFactWithBeginCloseSetsPremiseOpen_Nigh():
    # GIVEN
    x_agenda = example_agendas_get_agenda_with_4_levels()
    casa = "casa"
    casa_road = x_agenda.make_l1_road(casa)
    ...
    x_agenda.add_fact(factunit_shop(time, _begin=100, _close=2000), x_agenda._real_id)
    x_agenda.add_fact(factunit_shop(rus_war, _begin=22, _close=34), time_road)
    ...
    casa_fact1 = x_agenda.get_fact_obj(casa_road)
    assert casa_fact1._reasonunits[time_road] != None
    assert casa_fact1._reasonunits[time_road].premises[rus_war_road].divisor is None
    assert casa_fact1._reasonunits[time_road].premises[rus_war_road].open == 22
    assert casa_fact1._reasonunits[time_road].premises[rus_war_road].nigh == 34


def test_AgendaUnit_ReasonUnits_edit_fact_attr_CorrectlyDeletes_ReasonUnits_And_PremiseUnits():
    # GIVEN
    x_agenda = example_agendas_get_agenda_with_4_levels()
    casa_road = x_agenda.make_l1_road("casa")
    ...
    casa_fact1 = x_agenda.get_fact_obj(casa_road)
    assert len(casa_fact1._reasonunits[weekday_road].premises) == 2
    ...
    # THEN
    assert len(casa_fact1._reasonunits[weekday_road].premises) == 1
    ...
    with pytest_raises(KeyError) as excinfo:
        casa_fact1._reasonunits[weekday_road]
    assert str(excinfo.value) == f"'{weekday_road}'"
    assert casa_fact1._reasonunits == {}
...
def test_AgendaUnit_ReasonUnits_del_reason_premise_UncoupledMethod2():
    ...
    casa_fact1 = x_agenda.get_fact_obj(casa_road)
    assert len(casa_fact1._reasonunits) == 0

    # WHEN
    with pytest_raises(Exception) as excinfo:
        casa_fact1.del_reasonunit_base(weekdays_road)
    assert str(excinfo.value) == f"No ReasonUnit at '{weekdays_road}'"


def test_AgendaUnit_edit_fact_attr_agendaIsAbleToEdit_suff_fact_active_AnyFactIfInvaildThrowsError():
    # _suff_fact_active: str = None
    ...
    x_agenda.add_fact(factunit_shop(commute_text), x_agenda._real_id)
    x_agenda.calc_agenda_metrics()  # set tree metrics
    commute_fact = x_agenda.get_fact_obj(commute_road)
    assert len(commute_fact._reasonunits) == 0
    ...
    # THEN
    assert len(commute_fact._reasonunits) == 1
    reasonunit_casa = commute_fact._reasonunits.get(casa_road)
    assert reasonunit_casa.base == casa_road
    assert len(reasonunit_casa.premises) == 0
    assert reasonunit_casa.suff_fact_active == True
    ...
    assert reasonunit_casa.base == casa_road
    assert len(reasonunit_casa.premises) == 0
    assert reasonunit_casa.suff_fact_active is False

    # WHEN
    x_agenda.edit_fact_attr(
        road=commute_road,
        reason_base=casa_road,
        reason_suff_fact_active="Set to Ignore",
    )
    ...
    assert reasonunit_casa.base == casa_road
    assert len(reasonunit_casa.premises) == 0
    assert reasonunit_casa.suff_fact_active is None


def test_AgendaUnit_ReasonUnits_FactUnit_active_InfluencesReasonUnitStatus():
    # GIVEN an Agenda with 5 facts, 1 Belief:
    # 1. fact(...,weekdays) exists
    # 2. fact(...,weekdays,wednesday) exists
    # 3. fact(...,weekdays,thursday) exists
    x_agenda = example_agendas_get_agenda_with_4_levels()
    casa_text = "casa"
    casa_road = x_agenda.make_l1_road(casa_text)
    ...
    x_agenda.calc_agenda_metrics()  # set tree metrics
    casa_fact = x_agenda.get_fact_obj(casa_road)
    assert casa_fact._active is False

    # 5. fact(...,commute to casa) with
    # 5.1. ReasonUnit: fact(base=...,casa) has .suff_fact_active = True
    # 5.2. fact(...,casa).active = False
    commute_text = "commute to casa"
    commute_road = x_agenda.make_l1_road(commute_text)
    x_agenda.add_fact(factunit_shop(commute_text), x_agenda._real_id)
    x_agenda.edit_fact_attr(
    ...
    commute_fact = x_agenda.get_fact_obj(commute_road)
    x_agenda.calc_agenda_metrics()
    assert commute_fact._active is False
    ...
    x_agenda.calc_agenda_metrics()
    assert casa_fact._active == True

    # THEN
    assert commute_fact._active == True
...
def test_AgendaUnit_tree_traverses_StopWhenSameStatusIsDetected():
    ...
    # WHEN
    x_agenda.calc_agenda_metrics()
    # for fact_key in x_agenda._fact_dict.keys():
    #     print(f"{fact_key=}")
