...
def test_AgendaUnit_calc_agenda_metrics_RootOnlyCorrectlySetsDescendantAttributes():
    ...
    assert tim_agenda._idearoot._all_other_cred is None
    assert tim_agenda._idearoot._all_other_debt is None
    ...
    assert tim_agenda._idearoot._all_other_cred == True
    assert tim_agenda._idearoot._all_other_debt == True
...
def test_AgendaUnit_calc_agenda_metrics_NLevelCorrectlySetsDescendantAttributes_2():
    ...
    # THEN
    assert x_agenda._idearoot._all_other_cred is False
    assert x_agenda._idearoot._all_other_debt is False
...
def test_agenda4other_Exists():
    ...
    # WHEN
    sue_agenda4other = x_agenda.get_agenda4other(facts=None, other_id=sue_other_id)

    # THEN
    assert sue_agenda4other
    assert str(type(sue_agenda4other)).find(".agenda.AgendaUnit'>")
    assert sue_agenda4other._owner_id == sue_other_id


def test_agenda4other_hasCorrectLevel1StructureNoBelieflessAncestors():
    # GIVEN
    x_agenda = example_agendas_get_agenda_with_4_levels()
    email_text = "email"
    casa_text = "casa"
    ...
    # WHEN
    sue_agenda4other = x_agenda.get_agenda4other(sue_other_id, facts=None)

    # THEN
    assert len(sue_agenda4other._idearoot._kids) > 0
    print(f"{len(sue_agenda4other._idearoot._kids)=}")

    casa_idea = sue_agenda4other.get_idea_obj(casa_road)
    ...
    assert sue_agenda4other._idearoot._kids.get(feed_text) is None
    assert sue_agenda4other._idearoot._agenda_importance == 1
    assert casa_idea._agenda_importance == yrx._kids[casa_text]._agenda_importance
    agenda4other_road = sue_agenda4other.make_l1_road("__agenda4other__")
    assert sue_agenda4other.get_idea_obj(agenda4other_road) != None

    y4a_exteriors = sue_agenda4other.get_idea_obj(agenda4other_road)
