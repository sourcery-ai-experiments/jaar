...
def test_AgendaUnit_get_dict_ReturnsDictObject():
    ...
    day_hour_idea._originunit.set_originlink(guy_id="Bob", weight=2)
    x_agenda.set_fact(
        base=day_hour_road,
        pick=day_hour_road,
        open=0,
    ...
    x_guy_credor_pool = 22
    x_guy_debtor_pool = 22
    x_agenda.set_guy_credor_pool(x_guy_credor_pool)
    x_agenda.set_guy_debtor_pool(x_guy_debtor_pool)
    ...
    assert agenda_dict["_guy_credor_pool"] == x_agenda._guy_credor_pool
    assert agenda_dict["_guy_debtor_pool"] == x_agenda._guy_debtor_pool
    assert agenda_dict["_guy_debtor_pool"] == x_agenda._guy_debtor_pool
    assert agenda_dict["_meld_strategy"] == x_agenda._meld_strategy
    assert agenda_dict["_last_atom_id"] == x_agenda._last_atom_id
    assert len(agenda_dict["_guys"]) == len(x_agenda._guys)
    assert len(agenda_dict["_guys"]) != 12
    ...
    assert x_agenda_originlink["guy_id"] == yao_text
...
def test_AgendaUnit_get_dict_ReturnsDictWith_idearoot_healerhold():
    ...
    tom_agenda = agendaunit_shop("Tom")
    yao_text = "Yao"
    tom_agenda.add_guyunit(yao_text)
    run_text = ",runners"
    run_beliefunit = beliefunit_shop(run_text)
    run_beliefunit.set_guylink(guylink_shop(yao_text))
...
def test_AgendaUnit_get_dict_ReturnsDictWith_ideakid_AssignedUnit():
    ...
    tom_agenda = agendaunit_shop("Tom")
    run_text = ",run"
    tom_agenda.set_beliefunit(y_beliefunit=beliefunit_shop(run_text))
...
def test_AgendaUnit_get_json_ReturnsCorrectJSON_SimpleExample():
    # GIVEN
    zia_agenda = example_agendas_get_agenda_x1_3levels_1reason_1facts()
    tiger_real_id = "tiger"
    ...
    yao_text = "Yao"
    zia_agenda.add_guyunit(yao_text)
    run_text = ",runners"
    run_beliefunit = beliefunit_shop(run_text)
    run_beliefunit.set_guylink(guylink_shop(yao_text))
    ...
    assert str(excinfo.value) == "'_guy_credor_pool'"
    with pytest_raises(Exception) as excinfo:
        agenda_dict["_guy_debtor_pool"]
    assert str(excinfo.value) == "'_guy_debtor_pool'"
    with pytest_raises(Exception) as excinfo:
...
def test_AgendaUnit_get_json_ReturnsCorrectJSON_BigExample():
    ...
    yao_agenda.set_fact(base=day_hour_road, pick=day_hour_road, open=0, nigh=23)
    ...
    yao_agenda.edit_idea_attr(road=factunit_x.base, factunit=factunit_x)
...
def test_agendaunit_get_from_json_ReturnsCorrectObjSimpleExample():
    ...
    zia_guy_credor_pool = 2
    zia_guy_debtor_pool = 2
    zia_agenda.set_guy_credor_pool(zia_guy_credor_pool)
    zia_agenda.set_guy_debtor_pool(zia_guy_debtor_pool)
    zia_last_atom_id = 73
    ...
    shave_idea_y1._originunit.set_originlink(guy_id="Sue", weight=4.3)
    shave_idea_y1._problem_bool = True
    # print(f"{shave_road=}")
    # print(f"{json_shave_idea._label=} {json_shave_idea._parent_road=}")

    sue_text = "Sue"
    zia_agenda.add_guyunit(guy_id=sue_text)
    tim_text = "Tim"
    zia_agenda.add_guyunit(guy_id=tim_text)
    run_text = ",runners"
    run_belief = beliefunit_shop(belief_id=run_text)
    run_belief.set_guylink(guylink=guylink_shop(guy_id=sue_text))
    run_belief.set_guylink(guylink=guylink_shop(guy_id=tim_text))
    ...
    assert json_agenda._guy_credor_pool == zia_agenda._guy_credor_pool
    assert json_agenda._guy_debtor_pool == zia_agenda._guy_debtor_pool
    assert json_agenda._guy_credor_pool == zia_guy_credor_pool
    assert json_agenda._guy_debtor_pool == zia_guy_debtor_pool
    ...
    print(f"{json_agenda._beliefs.keys()=}")
    print(f"{zia_agenda._beliefs.keys()=}")
    assert json_agenda._beliefs == zia_agenda._beliefs
...
def test_agendaunit_get_from_json_ReturnsCorrectObj_road_delimiter_GuyExample():
    # GIVEN
    slash_delimiter = "/"
    before_bob_agenda = agendaunit_shop("Bob", _road_delimiter=slash_delimiter)
    bob_text = ",Bob"
    before_bob_agenda.add_guyunit(bob_text)
    assert before_bob_agenda.guy_exists(bob_text)
    ...
    # THEN
    after_bob_guyunit = after_bob_agenda.get_guy(bob_text)
    assert after_bob_guyunit._road_delimiter == slash_delimiter
...
def test_agendaunit_get_from_json_ReturnsCorrectObj_road_delimiter_BeliefExample():
    ...
    before_bob_agenda.set_beliefunit(
        beliefunit_shop(swimmers_text, _road_delimiter=slash_delimiter)
    )
    assert before_bob_agenda.get_beliefunit(swimmers_text) != None
    ...
    # THEN
    after_bob_beliefunit = after_bob_agenda.get_beliefunit(swimmers_text)
...
def test_get_dict_of_agenda_from_dict_ReturnsDictOfAgendaUnits():
    # GIVEN
    x1_agenda = example_agendas_agenda_v001()
    x2_agenda = example_agendas_get_agenda_x1_3levels_1reason_1facts()
