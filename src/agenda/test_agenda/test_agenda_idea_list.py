...
def test_agenda_get_idea_list_returns_correct_list():
    ...
    state_road = f"{x_agenda._culture_handle},{state_text}"
    france_text = "France"
    ...
    assert work_idea._pad == f"{x_agenda._culture_handle}"
...
def test_exammple_idea_list_HasCorrectData():
    ...
    # day_hour = f"{x_agenda._culture_handle},day_hour"
    # x_agenda.set_acptfact(base=day_hour, pick=day_hour, open=0, nigh=23)
    day_min_text = "day_minute"
    day_min_road = f"{x_agenda._culture_handle},{day_min_text}"
    ...
    yr_mon_road = f"{x_agenda._culture_handle},{yr_mon_text}"
    x_agenda.set_acptfact(base=yr_mon_road, pick=yr_mon_road)
    inter_text = "Internet"
    inter_road = f"{x_agenda._culture_handle},{inter_text}"
    ...
    ulty_road = f"{x_agenda._culture_handle},{ulty_text}"
    ...
    laundry_road = f"{x_agenda._culture_handle},casa,cleaning,{laundry_text}"
...
def test_exammple_idea_list_OptionWeekdaysCorrectlyWork():
    ...
    day_hr_road = f"{x_agenda._culture_handle},{day_hr_text}"
    ...
    mon_wk_road = f"{x_agenda._culture_handle},{mon_wk_text}"
    ...
    # x_agenda.set_acptfact(base=f"{x_agenda._culture_handle},weekdays", pick=f"{x_agenda._culture_handle},weekdays,Tuesday")
    # idea_list = x_agenda.get_idea_list()
    # assert YR.get_active_status(road=bird_idea, idea_list=idea_list) == True

    # x_agenda.set_acptfact(base=f"{x_agenda._culture_handle},weekdays", pick=f"{x_agenda._culture_handle},weekdays,Wednesday")
    # idea_list = x_agenda.get_idea_list()
...
def test_exammple_idea_list_Every6WeeksRequired():
    ...
    min_text = "day_minute"
    min_road = f"{x_agenda._culture_handle},{day_text}"
    ...
    # THEN
    ced_week_base = f"{x_agenda._culture_handle},ced_week"
    ...
    clean_sheet_road = (
        f"{x_agenda._culture_handle},casa,cleaning,clean sheets couch blankets"
    )
    ...
    clean_couch_road = f"{x_agenda._culture_handle},casa,cleaning,{clean_couch_text}"
...
def test_exammple_idea_list_EveryOtherMonthWorks():
    ...
    minute_road = f"{x_agenda._culture_handle},{minute_text}"
    x_agenda.set_acptfact(base=minute_road, pick=minute_road, open=0, nigh=1399)
    month_text = "month_week"
    month_road = f"{x_agenda._culture_handle},{month_text}"
