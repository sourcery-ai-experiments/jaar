...
def test_AgendaUnit_set_tree_traverse_starting_point_CorrectlySetsAttrs():
    ...
    assert sue_agenda._oath_dict == {
        sue_agenda._oathroot.get_road(): sue_agenda._oathroot
    }
...
def test_AgendaUnit_get_oath_obj_ReturnsOath():
    # GIVEN
    x_agenda = example_agendas_get_agenda_with_4_levels()
    ...
    # WHEN
    brazil_oath = x_agenda.get_oath_obj(road=brazil_road)

    # THEN
    assert brazil_oath != None
    assert brazil_oath._label == brazil_text

    # WHEN
    week_text = "weekdays"
    week_road = x_agenda.make_l1_road(week_text)
    week_oath = x_agenda.get_oath_obj(road=week_road)

    # THEN
    assert week_oath != None
    assert week_oath._label == week_text

    # WHEN
    root_oath = x_agenda.get_oath_obj(road=x_agenda._real_id)

    # THEN
    assert root_oath != None
    assert root_oath._label == x_agenda._real_id

    # WHEN / THEN
    bobdylan_text = "bobdylan"
    wrong_road = x_agenda.make_l1_road(bobdylan_text)
    with pytest_raises(Exception) as excinfo:
        x_agenda.get_oath_obj(road=wrong_road)
    assert str(excinfo.value) == f"get_oath_obj failed. no item at '{wrong_road}'"
...
def test_AgendaUnit_calc_agenda_metrics_RootOnlyCorrectlySetsDescendantAttributes():
    ...
    assert tim_agenda._oathroot._descendant_pledge_count is None
    assert tim_agenda._oathroot._all_party_cred is None
    assert tim_agenda._oathroot._all_party_debt is None

    # WHEN
    tim_agenda.calc_agenda_metrics()

    # THEN
    assert tim_agenda._oathroot._descendant_pledge_count == 0
    assert tim_agenda._oathroot._all_party_cred == True
    assert tim_agenda._oathroot._all_party_debt == True
...
def test_AgendaUnit_calc_agenda_metrics_NLevelCorrectlySetsDescendantAttributes_1():
    ...
    email_text = "email"
    email_oath = oathunit_shop(_label=email_text, pledge=True)
    x_agenda.add_oath(email_oath, parent_road=casa_road)

    # oath ",{week_text},Sunday"
    # oath ",{week_text},Monday"
    # oath ",{week_text},Tuesday"
    # oath ",{week_text},Wednesday"
    # oath ",{week_text},Thursday"
    # oath ",{week_text},Friday"
    # oath ",{week_text},Saturday"
    # oath ",{week_text}"
    # oath ",{nation_text},USA,Texas"
    # oath ",{nation_text},USA,Oregon"
    # oath ",{nation_text},USA"
    # oath ",{nation_text},France"
    # oath ",{nation_text},Brazil"
    # oath ",{nation_text}"
    # oath "casa"  # , pledge=True)
    # oath feed_text  # , pledge=True)
    # oath "

    # test root status:
    x_oathroot = x_agenda.get_oath_obj(x_agenda._real_id)
    assert x_oathroot._descendant_pledge_count is None
    assert x_oathroot._all_party_cred is None
    assert x_oathroot._all_party_debt is None
    assert x_oathroot._kids[casa_text]._descendant_pledge_count is None
    assert x_oathroot._kids[casa_text]._all_party_cred is None
    assert x_oathroot._kids[casa_text]._all_party_debt is None
    assert x_oathroot._kids[week_text]._kids[mon_text]._descendant_pledge_count is None
    assert x_oathroot._kids[week_text]._kids[mon_text]._all_party_cred is None
    assert x_oathroot._kids[week_text]._kids[mon_text]._all_party_debt is None

    # WHEN
    x_agenda.calc_agenda_metrics()

    # THEN
    assert x_oathroot._descendant_pledge_count == 3
    assert x_oathroot._kids[casa_text]._descendant_pledge_count == 1
    assert x_oathroot._kids[casa_text]._kids[email_text]._descendant_pledge_count == 0
    assert x_oathroot._kids[week_text]._kids[mon_text]._descendant_pledge_count == 0
    assert x_oathroot._all_party_cred == True
    assert x_oathroot._all_party_debt == True
    assert x_oathroot._kids[casa_text]._all_party_cred == True
    assert x_oathroot._kids[casa_text]._all_party_debt == True
    assert x_oathroot._kids[week_text]._kids[mon_text]._all_party_cred == True
    assert x_oathroot._kids[week_text]._kids[mon_text]._all_party_debt == True
...
def test_AgendaUnit_calc_agenda_metrics_NLevelCorrectlySetsDescendantAttributes_2():
    ...
    email_oath = oathunit_shop(_label=email_text, pledge=True)
    x_agenda.add_oath(email_oath, parent_road=casa_road)
    vacuum_oath = oathunit_shop(_label=vacuum_text, pledge=True)
    x_agenda.add_oath(vacuum_oath, parent_road=casa_road)

    x_agenda.add_partyunit(party_id=sue_text)
    x_balancelink = balancelink_shop(idea_id=sue_text)

    x_agenda._oathroot._kids[casa_text]._kids[email_text].set_balancelink(
    ...
    # THEN
    assert x_agenda._oathroot._all_party_cred is False
    assert x_agenda._oathroot._all_party_debt is False
    casa_oath = x_agenda._oathroot._kids[casa_text]
    assert casa_oath._all_party_cred is False
    assert casa_oath._all_party_debt is False
    assert casa_oath._kids[email_text]._all_party_cred is False
    assert casa_oath._kids[email_text]._all_party_debt is False
    assert casa_oath._kids[vacuum_text]._all_party_cred == True
    assert casa_oath._kids[vacuum_text]._all_party_debt == True
    week_oath = x_agenda._oathroot._kids[week_text]
    assert week_oath._all_party_cred == True
    assert week_oath._all_party_debt == True
    assert week_oath._kids[mon_text]._all_party_cred == True
    assert week_oath._kids[mon_text]._all_party_debt == True
    assert week_oath._kids[tue_text]._all_party_cred == True
    assert week_oath._kids[tue_text]._all_party_debt == True
...
def test_AgendaUnit_TreeTraverseSetsClearsBalanceLineestorsCorrectly():
    ...
    x_agenda = example_agendas_get_agenda_with_4_levels()
    x_agenda.calc_agenda_metrics()
    # oath tree has no balancelinks
    assert x_agenda._oathroot._balancelines == {}
    x_agenda._oathroot._balancelines = {1: "testtest"}
    assert x_agenda._oathroot._balancelines != {}
    ...
    # THEN
    assert not x_agenda._oathroot._balancelines

    # WHEN
    # test for level 1 and level n
    casa_text = "casa"
    casa_oath = x_agenda._oathroot._kids[casa_text]
    casa_oath._balancelines = {1: "testtest"}
    assert casa_oath._balancelines != {}
    x_agenda.calc_agenda_metrics()

    # THEN
    assert not x_agenda._oathroot._kids[casa_text]._balancelines
...
def test_AgendaUnit_calc_agenda_metrics_TreeTraverseSetsBalanceLineestorFromRootCorrectly():
    ...
    x_agenda.add_partyunit(party_id=sue_text)
    x_agenda._oathroot.set_balancelink(balancelink=sue_balancelink)
    # oath tree has balancelines
    assert x_agenda._oathroot._balanceheirs.get(sue_text) is None

    # WHEN
    x_agenda.calc_agenda_metrics()

    # THEN
    assert x_agenda._oathroot._balanceheirs.get(sue_text) != None
    assert x_agenda._oathroot._balanceheirs.get(sue_text).idea_id == sue_text
    assert x_agenda._oathroot._balancelines != {}
    root_oath = x_agenda.get_oath_obj(road=x_agenda._oathroot._label)
    sue_balanceline = x_agenda._oathroot._balancelines.get(sue_text)
    print(f"{sue_balanceline._agenda_cred=} {root_oath._agenda_importance=} ")
    print(f"  {sue_balanceline._agenda_debt=} {root_oath._agenda_importance=} ")
    sum_x = 0
    cat_road = x_agenda.make_l1_road("feed cat")
    cat_oath = x_agenda.get_oath_obj(cat_road)
    week_road = x_agenda.make_l1_road(week_text)
    week_oath = x_agenda.get_oath_obj(week_road)
    casa_text = "casa"
    casa_road = x_agenda.make_l1_road(casa_text)
    casa_oath = x_agenda.get_oath_obj(casa_road)
    nation_road = x_agenda.make_l1_road(nation_text)
    nation_oath = x_agenda.get_oath_obj(nation_road)
    sum_x = cat_oath._agenda_importance
    print(f"{cat_oath._agenda_importance=} {sum_x} ")
    sum_x += week_oath._agenda_importance
    print(f"{week_oath._agenda_importance=} {sum_x} ")
    sum_x += casa_oath._agenda_importance
    print(f"{casa_oath._agenda_importance=} {sum_x} ")
    sum_x += nation_oath._agenda_importance
    print(f"{nation_oath._agenda_importance=} {sum_x} ")
    assert sum_x >= 1.0
    assert sum_x < 1.00000000001

    # for kid_oath in root_oath._kids.values():
    #     sum_x += kid_oath._agenda_importance
    #     print(f"  {kid_oath._agenda_importance=} {sum_x=} {kid_oath.get_road()=}")
    ...
    assert x_agenda._oathroot._balancelines == {x_balanceline.idea_id: x_balanceline}
...
def test_AgendaUnit_calc_agenda_metrics_TreeTraverseSetsBalanceLineestorFromNonRootCorrectly():
    ...
    casa_text = "casa"
    email_text = "email"
    x_agenda._oathroot._kids[casa_text].set_balancelink(balancelink=x_balancelink)

    # WHEN
    # oath tree has balancelinks
    x_agenda.calc_agenda_metrics()
    ...
    assert x_agenda._oathroot._balancelines == {x_balanceline.idea_id: x_balanceline}
    assert x_agenda._oathroot._kids[casa_text]._balancelines != {}
    assert x_agenda._oathroot._kids[casa_text]._balancelines == {
...
def test_agenda4party_hasCorrectLevel1StructureNoIdealessAncestors():
    ...
    # THEN
    assert len(sue_agenda4party._oathroot._kids) > 0
    print(f"{len(sue_agenda4party._oathroot._kids)=}")

    casa_oath = sue_agenda4party.get_oath_obj(casa_road)
    type_check_OathUnit = str(type(casa_oath)).find(".oath.OathUnit'>")
    print(f"{type_check_OathUnit=}")
    type_check_OathUnit = str(type(casa_oath)).find(".oath.OathUnit'>")
    print(f"{type_check_OathUnit=}")
    assert str(type(casa_oath)).find(".oath.OathUnit'>") > 0

    assert sue_agenda4party._oathroot._kids.get(feed_text) is None
    assert sue_agenda4party._oathroot._agenda_importance == 1
    assert casa_oath._agenda_importance == yrx._kids[casa_text]._agenda_importance
    __other__road = sue_agenda4party.make_l1_road("__other__")
    assert sue_agenda4party.get_oath_obj(__other__road) != None

    y4a_others = sue_agenda4party.get_oath_obj(__other__road)
...
def test_AgendaUnit_get_oath_tree_ordered_road_list_ReturnsCorrectObj():
    # GIVEN
    x_agenda = example_agendas_get_agenda_with_4_levels()
    week_text = "weekdays"
    assert x_agenda.get_oath_tree_ordered_road_list()

    # WHEN
    ordered_node_list = x_agenda.get_oath_tree_ordered_road_list()
    ...
    x_1st_road_in_ordered_list = x_agenda.get_oath_tree_ordered_road_list()[0]
    assert x_1st_road_in_ordered_list == x_agenda._real_id
    x_8th_road_in_ordered_list = x_agenda.get_oath_tree_ordered_road_list()[9]
    ...
    # THEN
    y_1st_road_in_ordered_list = y_agenda.get_oath_tree_ordered_road_list()[0]
    assert y_1st_road_in_ordered_list == x_agenda._real_id


def test_AgendaUnit_get_oath_tree_ordered_road_list_CorrectlyFiltersRangedOathRoadUnits():
    # GIVEN
    tim_agenda = agendaunit_shop("Tim")

    # WHEN
    time = "timeline"
    tim_agenda.add_l1_oath(oathunit_shop(_label=time, _begin=0, _close=700))
    t_road = tim_agenda.make_l1_road(time)
    week = "weeks"
    tim_agenda.add_oath(oathunit_shop(_label=week, _denom=7), parent_road=t_road)

    # THEN
    assert len(tim_agenda.get_oath_tree_ordered_road_list()) == 3
    assert (
        len(tim_agenda.get_oath_tree_ordered_road_list(no_range_descendants=True)) == 2
    )
...
def test_AgendaUnit_oath_exists_ReturnsCorrectBool():
    # GIVEN
    ...
    # WHEN/THEN
    assert sue_agenda.oath_exists("") is False
    assert sue_agenda.oath_exists(None) is False
    assert sue_agenda.oath_exists(root_label())
    assert sue_agenda.oath_exists(cat_road)
    assert sue_agenda.oath_exists(week_road)
    assert sue_agenda.oath_exists(casa_road)
    assert sue_agenda.oath_exists(nation_road)
    assert sue_agenda.oath_exists(sun_road)
    assert sue_agenda.oath_exists(mon_road)
    assert sue_agenda.oath_exists(tue_road)
    assert sue_agenda.oath_exists(wed_road)
    assert sue_agenda.oath_exists(thu_road)
    assert sue_agenda.oath_exists(fri_road)
    assert sue_agenda.oath_exists(sat_road)
    assert sue_agenda.oath_exists(usa_road)
    assert sue_agenda.oath_exists(france_road)
    assert sue_agenda.oath_exists(brazil_road)
    assert sue_agenda.oath_exists(texas_road)
    assert sue_agenda.oath_exists(oregon_road)
    assert sue_agenda.oath_exists("B") is False
    assert sue_agenda.oath_exists(sports_road) is False
    assert sue_agenda.oath_exists(swim_road) is False
    assert sue_agenda.oath_exists(idaho_road) is False
    assert sue_agenda.oath_exists(japan_road) is False
...
def test_AgendaUnit_calc_agenda_metrics_CorrectlySets_econs_justified_WhenSingleOathUnit_healerhold_any_idea_id_exists_IsTrue():
    # GIVEN
    sue_agenda = agendaunit_shop("Sue")
    sue_agenda.add_l1_oath(oathunit_shop("Texas", _healerhold=healerhold_shop({"Yao"})))
    assert sue_agenda._econs_justified is False
...
def test_AgendaUnit_calc_agenda_metrics_CorrectlySets_econs_justified_WhenSingleProblemAndEcon():
    ...
    sue_agenda.add_l1_oath(
        oathunit_shop("Texas", _healerhold=yao_healerhold, _problem_bool=True)
    )
    assert sue_agenda._econs_justified is False
...
def test_AgendaUnit_calc_agenda_metrics_CorrectlySets_econs_justified_WhenEconIsLevelAboveProblem():
    ...
    sue_agenda.add_l1_oath(oathunit_shop(texas_text, _problem_bool=True))
    ep_text = "El Paso"
    sue_agenda.add_oath(oathunit_shop(ep_text, _healerhold=yao_healerhold), texas_road)
    assert sue_agenda._econs_justified is False
...
def test_AgendaUnit_calc_agenda_metrics_CorrectlySets_econs_justified_WhenEconIsLevelBelowProblem():
    ...
    sue_agenda.add_l1_oath(oathunit_shop(texas_text, _healerhold=yao_healerhold))
    sue_agenda.add_oath(oathunit_shop("El Paso", _problem_bool=True), texas_road)
    assert sue_agenda._econs_justified is False
...
def test_AgendaUnit_calc_agenda_metrics_CorrectlyRaisesErrorWhenEconIsLevelBelowProblem():
    ...
    texas_road = sue_agenda.make_l1_road(texas_text)
    yao_healerhold = healerhold_shop({"Yao"})
    texas_oath = oathunit_shop(texas_text, _healerhold=yao_healerhold)
    sue_agenda.add_l1_oath(texas_oath)
    elpaso_oath = oathunit_shop("El Paso", _problem_bool=True)
    sue_agenda.add_oath(elpaso_oath, texas_road)
    assert sue_agenda._econs_justified is False
    ...
    assert (
        str(excinfo.value)
        == f"OathUnit '{elpaso_oath.get_road()}' cannot sponsor ancestor econs."
    )
...
def test_AgendaUnit_get_oath_dict_RaisesErrorWhen_econs_justified_IsFalse():
    # GIVEN
    sue_agenda = agendaunit_shop("Sue")
    yao_healerhold = healerhold_shop({"Yao"})
    texas_text = "Texas"
    ...
    # WHEN / THEN
    with pytest_raises(Exception) as excinfo:
        sue_agenda.get_oath_dict(problem=True)
    assert (
        str(excinfo.value)
...
def test_AgendaUnit_get_oath_dict_ReturnsCorrectObjWhenSingle():
    # GIVEN
    sue_agenda = agendaunit_shop("Sue")
    texas_text = "Texas"
    sue_agenda.add_l1_oath(oathunit_shop(texas_text, _problem_bool=True))
    casa_text = "casa"
    sue_agenda.add_l1_oath(oathunit_shop(casa_text))

    # WHEN
    problems_dict = sue_agenda.get_oath_dict(problem=True)

    # THEN
    assert sue_agenda._econs_justified
    texas_road = sue_agenda.make_l1_road(texas_text)
    texas_oath = sue_agenda.get_oath_obj(texas_road)
    assert len(problems_dict) == 1
    assert problems_dict == {texas_road: texas_oath}
