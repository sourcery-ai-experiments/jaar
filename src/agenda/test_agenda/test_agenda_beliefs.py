...
from src.agenda.fact import factunit_shop, RoadUnit
...
def test_AgendaUnit_set_belief_CorrectlyModifiesAttr_1():
    ...
    x_factroot._beliefunits = {sunday_agenda_belief.base: sunday_agenda_belief}
    assert x_factroot._beliefunits != None
    x_factroot._beliefunits = {}
    assert not x_factroot._beliefunits

    # GIVEN
    x_agenda.set_belief(base=weekday_road, pick=sunday_road)

    # THEN
    assert x_factroot._beliefunits == {sunday_agenda_belief.base: sunday_agenda_belief}
    ...
    x_factroot._beliefunits = {usa_week_unit.base: usa_week_unit}
    ...
    # THEN
    assert x_factroot._beliefunits != None
    assert x_factroot._beliefunits == {usa_week_unit.base: usa_week_unit}
...
def test_AgendaUnit_set_belief_CorrectlyModifiesAttrWhen_open_IsNone():
    ...
    x_agenda.set_belief(base=weekday_road, open=5, nigh=7)
    x_factroot = x_agenda._factroot
    assert x_factroot._beliefunits.get(weekday_road) == beliefunit_shop(
...
def test_AgendaUnit_set_belief_FailsToCreateWhenBaseAndBeliefAreDifferenctAndBeliefFactIsNotRangeRoot():
    # GIVEN
    bob_agenda = agendaunit_shop("Bob")
    time_text = "time"
    time_fact = factunit_shop(time_text, _begin=0, _close=140)
    bob_agenda.add_l1_fact(time_fact)
    time_road = bob_agenda.make_l1_road(time_text)
    a1st = "age1st"
    a1st_road = bob_agenda.make_road(time_road, a1st)
    a1st_fact = factunit_shop(a1st, _begin=0, _close=20)
    bob_agenda.add_fact(a1st_fact, parent_road=time_road)
    a1e1st_text = "a1_era1st"
    a1e1st_fact = factunit_shop(a1e1st_text, _begin=20, _close=30)
    bob_agenda.add_fact(a1e1st_fact, parent_road=a1st_road)
    a1e1_road = bob_agenda.make_road(a1st_road, a1e1st_text)
    assert bob_agenda._factroot._beliefunits in (None, {})
...
def test_AgendaUnit_del_belief_CorrectlyModifiesAttr():
    ...
    # THEN
    assert x_factroot._beliefunits == {}


def test_AgendaUnit_get_fact_list_BeliefHeirsCorrectlyInherited():
    # GIVEN
    bob_agenda = agendaunit_shop("Bob")
    swim_text = "swim"
    swim_road = bob_agenda.make_l1_road(swim_text)
    bob_agenda.add_l1_fact(factunit_shop(swim_text))
    fast_text = "fast"
    slow_text = "slow"
    fast_road = bob_agenda.make_road(swim_road, fast_text)
    slow_road = bob_agenda.make_road(swim_road, slow_text)
    bob_agenda.add_fact(factunit_shop(fast_text), parent_road=swim_road)
    bob_agenda.add_fact(factunit_shop(slow_text), parent_road=swim_road)

    earth_text = "earth"
    earth_road = bob_agenda.make_l1_road(earth_text)
    bob_agenda.add_l1_fact(factunit_shop(earth_text))

    swim_fact = bob_agenda.get_fact_obj(swim_road)
    fast_fact = bob_agenda.get_fact_obj(fast_road)
    slow_fact = bob_agenda.get_fact_obj(slow_road)

    assert swim_fact._beliefheirs == {}
    assert fast_fact._beliefheirs == {}
    assert slow_fact._beliefheirs == {}
    ...
    # THEN
    assert swim_fact._beliefheirs != None
    assert fast_fact._beliefheirs != None
    assert slow_fact._beliefheirs != None
    assert swim_fact._beliefheirs == beliefheirs_set_range
    assert fast_fact._beliefheirs == beliefheirs_set_range
    assert slow_fact._beliefheirs == beliefheirs_set_range
    print(f"{swim_fact._beliefheirs=}")
    assert len(swim_fact._beliefheirs) == 1

    # WHEN
    swim_fact._beliefheirs.get(earth_road).set_range_null()

    # THEN
    assert swim_fact._beliefheirs == beliefs_none_range
    assert fast_fact._beliefheirs == beliefheirs_set_range
    assert slow_fact._beliefheirs == beliefheirs_set_range

    belief_x1 = swim_fact._beliefheirs.get(earth_road)
    belief_x1.set_range_null()
    print(type(belief_x1))
    assert str(type(belief_x1)).find(".reason.BeliefHeir'>")


def test_AgendaUnit_get_fact_list_BeliefUnitCorrectlyTransformsbeliefheir_shop():
    # GIVEN
    bob_agenda = agendaunit_shop("Bob")
    swim_text = "swim"
    swim_road = bob_agenda.make_l1_road(swim_text)
    ...
    assert swim_fact._beliefheirs == first_earthdict
    ...
    assert swim_fact._beliefheirs == after_earthdict


def test_AgendaUnit_get_fact_list_BeliefHeirCorrectlyDeletesBeliefUnit():
    # GIVEN
    ...
    fast_text = "fast"
    slow_text = "slow"
    sue_agenda.add_fact(factunit_shop(fast_text), parent_road=swim_road)
    sue_agenda.add_fact(factunit_shop(slow_text), parent_road=swim_road)
    earth_text = "earth"
    earth_road = sue_agenda.make_l1_road(earth_text)
    sue_agenda.add_l1_fact(factunit_shop(earth_text))

    swim_fact = sue_agenda.get_fact_obj(swim_road)
    ...
    earth_curb = beliefunit_shop(base=earth_road, pick=earth_road, open=3.0, nigh=4.0)
    swim_fact.set_beliefunit(beliefunit=earth_curb)
    sue_agenda.calc_agenda_metrics()
    assert swim_fact._beliefheirs == first_earthdict
    assert swim_fact._beliefunits == {}
...
def test_get_ranged_beliefs():
    ...
    # sue_agenda.edit_fact_attr(road=c_road, reason_base=time_road, reason_premise=time_road, reason_premise_open=5, reason_premise_nigh=10)

    sue_agenda.set_belief(base=time_road, pick=time_road, open=5, nigh=10)
    print(f"Given a single ranged belief {sue_agenda._factroot._beliefunits=}")
    assert len(sue_agenda._factroot._beliefunits) == 1
    ...
    place_text = "place_x"
    place_fact = factunit_shop(place_text, _begin=600, _close=800)
    sue_agenda.add_l1_fact(place_fact)
    place_road = sue_agenda.make_l1_road(place_text)
    sue_agenda.set_belief(base=place_road, pick=place_road, open=5, nigh=10)
    print(f"When one ranged belief added {sue_agenda._factroot._beliefunits=}")
    assert len(sue_agenda._factroot._beliefunits) == 2
    ...
    mood = "mood_x"
    sue_agenda.add_l1_fact(factunit_shop(mood))
    m_road = sue_agenda.make_l1_road(mood)
    sue_agenda.set_belief(base=m_road, pick=m_road)
    print(f"When one non-ranged_belief added {sue_agenda._factroot._beliefunits=}")
    assert len(sue_agenda._factroot._beliefunits) == 3
...
def test_get_roots_ranged_beliefs():
    ...
    sue_agenda = agendaunit_shop("Sue")
    time_text = "time"
    sue_agenda.add_l1_fact(factunit_shop(time_text, _begin=0, _close=140))
    time_road = sue_agenda.make_l1_road(time_text)
    mood_x = "mood_x"
    sue_agenda.add_l1_fact(factunit_shop(mood_x))
    m_x_road = sue_agenda.make_l1_road(mood_x)
    happy = "happy"
    sad = "Sad"
    sue_agenda.add_fact(factunit_shop(happy), parent_road=m_x_road)
    sue_agenda.add_fact(factunit_shop(sad), parent_road=m_x_road)
    sue_agenda.set_belief(base=time_road, pick=time_road, open=5, nigh=10)
    sue_agenda.set_belief(base=m_x_road, pick=sue_agenda.make_road(m_x_road, happy))
    print(
        f"Given a root ranged belief and non-range belief:\n{sue_agenda._factroot._beliefunits=}"
    )
    assert len(sue_agenda._factroot._beliefunits) == 2
    ...
    # a belief who's fact range is defined by numeric_root is not "rangeroot"
    mirror_x = "mirror_x"
    sue_agenda.add_l1_fact(factunit_shop(mirror_x, _numeric_road=time_text))
...
def test_create_lemma_beliefs_CorrectlyCreates1stLevelLemmaBelief_Scenario1():
    ...
    # # the action
    # clean = "clean"
    # sue_agenda.add_fact(factunit_shop(clean, pledge=True))
    ...
    age6th_text = "age6th"
    age7th_text = "age7th"
    age1st_fact = factunit_shop(age1st_text, _begin=0, _close=20)
    age2nd_fact = factunit_shop(age2nd_text, _begin=20, _close=40)
    age3rd_fact = factunit_shop(age3rd_text, _begin=40, _close=60)
    age4th_fact = factunit_shop(age4th_text, _begin=60, _close=80)
    age5th_fact = factunit_shop(age5th_text, _begin=80, _close=100)
    age6th_fact = factunit_shop(age6th_text, _begin=100, _close=120)
    age7th_fact = factunit_shop(age7th_text, _begin=120, _close=140)
    sue_agenda.add_fact(age1st_fact, parent_road=time_road)
    sue_agenda.add_fact(age2nd_fact, parent_road=time_road)
    sue_agenda.add_fact(age3rd_fact, parent_road=time_road)
    sue_agenda.add_fact(age4th_fact, parent_road=time_road)
    sue_agenda.add_fact(age5th_fact, parent_road=time_road)
    sue_agenda.add_fact(age6th_fact, parent_road=time_road)
    sue_agenda.add_fact(age7th_fact, parent_road=time_road)
...
def test_create_lemma_beliefs_CorrectlyCreates1stLevelLemmaBelief_Scenario3():
    ...
    a2e1st_fact = factunit_shop(a2e1st_text, _begin=20, _close=30)
    a2e2nd_fact = factunit_shop(a2e2nd_text, _begin=30, _close=34)
    a2e3rd_fact = factunit_shop(a2e3rd_text, _begin=34, _close=38)
    a2e4th_fact = factunit_shop(a2e4th_text, _begin=38, _close=40)
    sue_agenda.add_fact(a2e1st_fact, parent_road=a2_road)
    sue_agenda.add_fact(a2e2nd_fact, parent_road=a2_road)
    sue_agenda.add_fact(a2e3rd_fact, parent_road=a2_road)
    sue_agenda.add_fact(a2e4th_fact, parent_road=a2_road)
    ...
    a3e1st_fact = factunit_shop(a3e1st_text, _begin=40, _close=45)
    a3e2nd_fact = factunit_shop(a3e2nd_text, _begin=45, _close=50)
    a3e3rd_fact = factunit_shop(a3e3rd_text, _begin=55, _close=58)
    a3e4th_fact = factunit_shop(a3e4th_text, _begin=58, _close=60)
    sue_agenda.add_fact(a3e1st_fact, parent_road=a3_road)
    sue_agenda.add_fact(a3e2nd_fact, parent_road=a3_road)
    sue_agenda.add_fact(a3e3rd_fact, parent_road=a3_road)
    sue_agenda.add_fact(a3e4th_fact, parent_road=a3_road)
...
def test_create_lemma_beliefs_CorrectlyCreates1stLevelLemmaBelief_Scenario4():
    ...
    arsub1_fact = factunit_shop(arsub1, _begin=0, _close=140)
    as1_road = sue_agenda.make_l1_road(arsub1)
    sue_agenda.add_l1_fact(arsub1_fact)
    # range-root fact has range_source_road
    time_text = "time"
    time_fact = factunit_shop(
        time_text, _begin=0, _close=140, _range_source_road=as1_road
    )
    sue_agenda.add_l1_fact(time_fact)

    arsub2 = "descending_subsecction2"
    arsub2_fact = factunit_shop(arsub2, _begin=0, _close=20)
    as2_road = sue_agenda.make_l1_road(arsub2)
    sue_agenda.add_l1_fact(arsub2_fact)

    # non-range-root child fact has range_source_road
    time_road = sue_agenda.make_l1_road(time_text)
    age1st = "age1st"
    age1st_fact = factunit_shop(
...
def test_create_lemma_beliefs_CorrectlyCreatesNthLevelLemmaBelief_Scenario4_1():
    sue_agenda = agendaunit_shop("Sue")
    sue_agenda.set_time_hreg_facts(c400_count=7)
...
def test_AgendaUnit_set_belief_create_missing_facts_CreatesBaseAndBelief():
    # GIVEN
    sue_agenda = agendaunit_shop("Sue")
    ...
    assert sue_agenda._factroot.get_kid(situations_text) is None

    # WHEN
    sue_agenda.set_belief(situations_road, climate_road, create_missing_facts=True)

    # THEN
    assert sue_agenda._factroot.get_kid(situations_text) != None
    assert sue_agenda.get_fact_obj(situations_road) != None
    assert sue_agenda.get_fact_obj(climate_road) != None
...
def test_AgendaUnit_get_belief_ReturnsBeliefUnit():
    ...
    # THEN
    static_situations_base = sue_agenda._factroot._beliefunits.get(situations_road)
