...
def test_agenda_add_idea_CanAddKidToKidIdea():
    ...
    new_idea_parent_road = f"{x_agenda._culture_handle},work"
    ...
    assert (
        ...
        == f"{x_agenda._culture_handle},work"
    )
...
def test_agenda_add_idea_CanAddKidToGrandkidIdea():
    ...
    assert x_agenda.get_node_count() == 17
    assert x_agenda.get_level_count(level=3) == 2
    new_idea_parent_road = f"{x_agenda._culture_handle},weekdays,Wednesday"
    new_idea = IdeaKid(_weight=40, _label="new_idea")
...
def test_agenda_add_idea_CanCreateRoadToGrandkidIdea():
    ...
    assert x_agenda.get_node_count() == 17
    assert x_agenda.get_level_count(level=3) == 2
    new_idea_parent_road = f"{x_agenda._culture_handle},ww2,battles,coralsea"
...
def test_agenda_add_idea_creates_requireds_ideas():
    ...
    buildings_road = Road(f"{x_agenda._culture_handle},{buildings_text}")
    cookery_room_text = "cookery"
    cookery_room_road = Road(
        f"{x_agenda._culture_handle},{buildings_text},{cookery_room_text}"
    )
...
def test_agenda_del_idea_kid_Level0CannotBeDeleted():
    # GIVEN
    x_agenda = get_agenda_with_4_levels()
    root_road = f"{x_agenda._culture_handle}"
...
def test_agenda_del_idea_kid_Level1CanBeDeleted_ChildrenDeleted():
    ...
    new_sunday_road = f"{x_agenda._culture_handle},Sunday"
    with pytest_raises(Exception) as excinfo:
...
def test_agenda_del_idea_kid_Level2CanBeDeleted_ChildrenDeleted():
    # GIVEN
    x_agenda = get_agenda_with_4_levels()
    monday_road = f"{x_agenda._culture_handle},weekdays,Monday"
...
def test_agenda_edit_idea_attr_IsAbleToEditAnyAncestor_Idea():
    ...
    acptfact_road = f"{x_agenda._culture_handle},weekdays,Sunday"
...
def test_agenda_edit_idea_attr_agendaIsAbleToEditDenomAnyIdeaIfInvaildDenomThrowsError():
    ...
    work = "work"
    w_road = f"{x_agenda._culture_handle},{work}"
    work_idea = IdeaKid(_label=work)
    x_agenda.add_idea(work_idea, pad=x_agenda._culture_handle)
    clean = "clean"
    ...
    assert (
        str(excinfo.value)
        == f"Idea cannot edit numor=1/denom/reest of '{x_agenda._culture_handle},work,clean' if parent '{x_agenda._culture_handle},work' or ideacore._numeric_road does not have begin/close range"
    )
...
def test_agenda_edit_idea_attr_agendaIsAbleToEditDenomAnyIdeaInvaildDenomThrowsError():
    ...
    day_road = f"{x_agenda._culture_handle},{day}"
    x_agenda.add_idea(day_idea, pad=x_agenda._culture_handle)
...
def test_agenda_edit_idea_attr_agendaWhenParentAndNumeric_roadBothHaveRangeThrowError():
    ...
    work_text = "work"
    work_road = f"{x_agenda._culture_handle},{work_text}"
    x_agenda.add_idea(IdeaKid(_label=work_text), pad=x_agenda._culture_handle)
    day_text = "day_range"
    day_idea = IdeaKid(_label=day_text, _begin=44, _close=110)
    day_road = f"{x_agenda._culture_handle},{day_text}"
    x_agenda.add_idea(day_idea, pad=x_agenda._culture_handle)
    ...
    assert (
        str(excinfo.value)
        == f"Idea cannot edit numor=1/denom/reest of '{x_agenda._culture_handle},work' if parent '{x_agenda._culture_handle}' or ideacore._numeric_road does not have begin/close range"
    )
...
def test_agenda_add_idea_MustReorderKidsDictToBeAlphabetical():
    ...
    x_agenda.add_idea(IdeaKid(_label=work_text), pad=x_agenda._culture_handle)
    swim_text = "swim"
    x_agenda.add_idea(IdeaKid(_label=swim_text), pad=x_agenda._culture_handle)
...
def test_agenda_add_idea_adoptee_RaisesErrorIfAdopteeIdeaDoesNotHaveCorrectParent():
    ...
    x_agenda = agendaunit_shop(_healer=healer_text)
    sports_text = "sports"
    sports_road = f"{x_agenda._culture_handle},{sports_text}"
    x_agenda.add_idea(IdeaKid(_label=sports_text), pad=x_agenda._culture_handle)
    swim_text = "swim"
    x_agenda.add_idea(IdeaKid(_label=swim_text), pad=sports_road)
...
def test_agenda_add_idea_adoptee_CorrectlyAddsAdoptee():
    ...
    x_agenda.add_idea(IdeaKid(_label=swim_text), pad=sports_road)
    hike_text = "hike"
    x_agenda.add_idea(IdeaKid(_label=hike_text), pad=sports_road)
...
def test_agenda_add_idea_bundling_SetsNewParentWithWeightEqualToSumOfAdoptedIdeas():
    ...
    sports_road = f"{x_agenda._culture_handle},{sports_text}"
    x_agenda.add_idea(
        IdeaKid(_label=sports_text, _weight=2), pad=x_agenda._culture_handle
    )
    swim_text = "swim"
    swim_weight = 3
    x_agenda.add_idea(IdeaKid(_label=swim_text, _weight=swim_weight), pad=sports_road)
    hike_text = "hike"
    hike_weight = 5
    x_agenda.add_idea(IdeaKid(_label=hike_text, _weight=hike_weight), pad=sports_road)
    bball_text = "bball"
    bball_weight = 7
    x_agenda.add_idea(IdeaKid(_label=bball_text, _weight=bball_weight), pad=sports_road)
