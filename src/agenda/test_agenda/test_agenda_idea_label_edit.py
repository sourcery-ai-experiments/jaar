...
def test_agenda_edit_idea_label_Modifies_factunits():
    # GIVEN agenda with factunits that will be different
    tim_text = "Tim"
    tim_agenda = agendaunit_shop(tim_text)
...
def test_agenda_set_road_delimiter_CorrectlyModifiesFactUnit():
    # GIVEN
    luca_agenda = agendaunit_shop("Luca", "Texas")
    casa_text = "casa"
    luca_agenda.add_l1_idea(ideaunit_shop(casa_text))
    ...
    comma_casa_road = luca_agenda.make_l1_road(casa_text)
    luca_agenda.edit_idea_attr(comma_casa_road, factunit=comma_time_factunit)
    casa_idea = luca_agenda.get_idea_obj(comma_casa_road)
