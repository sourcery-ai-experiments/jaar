...
def test_agenda_set_road_delimiter_RaisesErrorIfNew_delimiter_IsAnIdea_label():
    ...
    work_text = "work"
    work_road = luca_agenda.make_l1_road(work_text)
    luca_agenda.add_l1_idea(ideaunit_shop(work_text))
    slash_text = "/"
...
def test_agenda_set_road_delimiter_CorrectlyChanges_parent_road():
    ...
    # THEN
    assert cook_idea.get_road() != comma_cook_road
    luca_economy_id = luca_agenda._economy_id
    slash_work_road = create_road(luca_economy_id, work_text, delimiter=slash_text)
