...
def test_agenda_acptfact_exists():
    ...
    weekday_road = Road(f"{sx._culture_handle},weekdays")
    sunday_road = Road(f"{sx._culture_handle},weekdays,Sunday")
    ...
    sx._idearoot._acptfactunits = None
    assert sx._idearoot._acptfactunits is None
    usa_week_road = Road(f"{sx._culture_handle},nation-state")
...
def test_create_lemma_acptfacts_CorrectlyCreatesNthLevelLemmaAcptFact_Scenario4_1():
    ...
    jajatime_road = f"{sx._culture_handle},time,jajatime"
    sx.set_acptfact(base=jajatime_road, pick=jajatime_road, open=1500, nigh=1500)
    lhu = sx._get_lemma_acptfactunits()

    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycle"].open == 1500
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycle"].nigh == 1500
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycles"].open > 0
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycles"].open < 1
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycles"].nigh > 0
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycles"].nigh < 1
    assert lhu[f"{sx._culture_handle},time,jajatime,days"].open >= 1
    assert lhu[f"{sx._culture_handle},time,jajatime,days"].open <= 2
    assert lhu[f"{sx._culture_handle},time,jajatime,days"].nigh >= 1
    assert lhu[f"{sx._culture_handle},time,jajatime,days"].nigh <= 2
    assert lhu[f"{sx._culture_handle},time,jajatime,day"].open == 60
    assert lhu[f"{sx._culture_handle},time,jajatime,day"].nigh == 60
    assert lhu[f"{sx._culture_handle},time,jajatime,week"].open == 1500
    assert int(lhu[f"{sx._culture_handle},time,jajatime,week"].nigh) == 1500
    assert lhu[f"{sx._culture_handle},time,tech,week"].open == 1500
    assert int(lhu[f"{sx._culture_handle},time,tech,week"].nigh) == 1500
...
def test_create_lemma_acptfacts_CorrectlyCreatesNthLevelLemmaAcptFact_Scenario5():
    ...
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycle"].open == 0
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycle"].nigh == 210379680
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycles"].open > 0
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycles"].open < 1
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycles"].nigh > 5
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycles"].nigh < 6
    assert int(lhu[f"{sx._culture_handle},time,jajatime,days"].open) == 1  # 0 / 1440
    assert (
        int(lhu[f"{sx._culture_handle},time,jajatime,days"].nigh) == 738856
    )  # 1063953183 / 1440
    assert lhu[f"{sx._culture_handle},time,jajatime,day"].open == 0  # 0 / 1440
    assert (
        lhu[f"{sx._culture_handle},time,jajatime,day"].nigh == 1440
    )  # 1362  # 1063953183 / 1440
    assert lhu[f"{sx._culture_handle},time,jajatime,week"].open == 0  # 0 / 1440
    assert (
        int(lhu[f"{sx._culture_handle},time,jajatime,week"].nigh) == 10080
    )  # 1063953183 / 1440
    assert lhu[f"{sx._culture_handle},time,tech,week"].open == 0  # 0 / 1440
    assert (
        int(lhu[f"{sx._culture_handle},time,tech,week"].nigh) == 10080
    )  # 1063953183 / 1440
...
def test_create_lemma_acptfacts_CorrectlyCreatesNthLevelLemmaAcptFact_Scenario6():
    ...
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycle"].open == 12055600.0
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycle"].nigh == 12055602.0
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycles"].open > 5
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycles"].open < 6
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycles"].nigh > 5
    assert lhu[f"{sx._culture_handle},time,jajatime,400 year cycles"].nigh < 6
    assert (
        int(lhu[f"{sx._culture_handle},time,jajatime,days"].open) == 738856
    )  # 1063954000 / 1440
    assert (
        int(lhu[f"{sx._culture_handle},time,jajatime,days"].nigh) == 738856
    )  # 1063954000 / 1440
    assert lhu[f"{sx._culture_handle},time,jajatime,day"].open == 1360  # 0 / 1440
    assert (
        int(lhu[f"{sx._culture_handle},time,jajatime,day"].nigh) == 1362
    )  # 1063953183 / 1440
...
def test_create_lemma_acptfacts_CorrectlyCreatesNthLevelLemmaAcptFact_Scenario7():
    ...
    # Then
    week_open = lhu[f"{sx._culture_handle},time,jajatime,week"].open
    week_nigh = lhu[f"{sx._culture_handle},time,jajatime,week"].nigh
    print(f"for {sx._culture_handle},time,jajatime,week: {week_open=} {week_nigh=}")
    assert lhu[f"{sx._culture_handle},time,jajatime,week"].open == 7200
    assert lhu[f"{sx._culture_handle},time,jajatime,week"].nigh == 2880

    week_open = lhu[f"{sx._culture_handle},time,tech,week"].open
    week_nigh = lhu[f"{sx._culture_handle},time,tech,week"].nigh
    print(f"for {sx._culture_handle},time,tech,week: {week_open=} {week_nigh=}")
    assert lhu[f"{sx._culture_handle},time,tech,week"].open == 7200
    assert lhu[f"{sx._culture_handle},time,tech,week"].nigh == 2880
    print(lhu[f"{sx._culture_handle},time,tech,week"])
    print(lhu[f"{sx._culture_handle},time,tech,week,Thursday"])
    print(lhu[f"{sx._culture_handle},time,tech,week,Friday"])
    print(lhu[f"{sx._culture_handle},time,tech,week,Saturday"])
    print(lhu[f"{sx._culture_handle},time,tech,week,Sunday"])
    print(lhu[f"{sx._culture_handle},time,tech,week,Monday"])
    print(lhu[f"{sx._culture_handle},time,tech,week,Tuesday"])
    print(lhu[f"{sx._culture_handle},time,tech,week,Wednesday"])

    # assert lhu[f"{sx._culture_handle},time,tech,week,Thursday"].active == True
    # assert lhu[f"{sx._culture_handle},time,tech,week,Friday"].active == True
    # assert lhu[f"{sx._culture_handle},time,tech,week,Saturday"].active == True
    # assert lhu[f"{sx._culture_handle},time,tech,week,Sunday"].active == True
    # assert lhu[f"{sx._culture_handle},time,tech,week,Monday"].active == False
    # assert lhu[f"{sx._culture_handle},time,tech,week,Tuesday"].active == False
    # assert lhu[f"{sx._culture_handle},time,tech,week,Wednesday"].active == False
...
def test_create_lemma_acptfacts_CorrectlyCreatesNthLevelLemmaAcptFact_Scenario8():
    ...
    print(f"for {sx._culture_handle},time,jajatime,week: {week_open=} {week_nigh=}")
    assert lhu[f"{sx._culture_handle},time,jajatime,week"].open == 7200
    assert lhu[f"{sx._culture_handle},time,jajatime,week"].nigh == 7200

    week_open = lhu[f"{sx._culture_handle},time,tech,week"].open
    week_nigh = lhu[f"{sx._culture_handle},time,tech,week"].nigh
    print(f"for {sx._culture_handle},time,tech,week: {week_open=} {week_nigh=}")
    assert lhu[f"{sx._culture_handle},time,tech,week"].open == 7200
    assert lhu[f"{sx._culture_handle},time,tech,week"].nigh == 7200
    print(lhu[f"{sx._culture_handle},time,tech,week"])
    print(lhu[f"{sx._culture_handle},time,tech,week,Thursday"])
    print(lhu[f"{sx._culture_handle},time,tech,week,Friday"])
    print(lhu[f"{sx._culture_handle},time,tech,week,Saturday"])
    ...
    # assert lhu[f"{sx._culture_handle},time,tech,week,Thursday"].active == True
    # assert lhu[f"{sx._culture_handle},time,tech,week,Friday"].active == False
    # assert lhu[f"{sx._culture_handle},time,tech,week,Saturday"].active == False
    # assert lhu[f"{sx._culture_handle},time,tech,week,Sunday"].active == False
    # assert lhu[f"{sx._culture_handle},time,tech,week,Monday"].active == False
    # assert lhu[f"{sx._culture_handle},time,tech,week,Tuesday"].active == False
    # assert lhu[f"{sx._culture_handle},time,tech,week,Wednesday"].active == False
...
def test_agenda_set_acptfact_create_missing_ideas_CreatesBaseAndAcptFact():
    ...
    issue_text = "issues"
    issue_road = Road(f"{sx._culture_handle},{issue_text}")
    climate_text = "climate"
    climate_road = Road(f"{sx._culture_handle},{issue_text},{climate_text}")
...
def test_agenda_get_acptfactunits_base_and_acptfact_list_CorrectlyReturnsListOfAcptFactUnits():
    ...
    weather_road = Road(f"{sx._culture_handle},{weather_text}")
    windy_text = "windy"
    windy_road = Road(f"{sx._culture_handle},{weather_text},{windy_text}")
    sx.set_acptfact(base=weather_road, pick=windy_road, create_missing_ideas=True)
    hot_text = "hot"
    hot_road = Road(f"{sx._culture_handle},{weather_text},{hot_text}")
    sx.set_acptfact(base=weather_road, pick=hot_road, create_missing_ideas=True)
    cold_text = "cold"
    cold_road = Road(f"{sx._culture_handle},{weather_text},{cold_text}")
    sx.set_acptfact(base=weather_road, pick=cold_road, create_missing_ideas=True)

    games_text = "games"
    games_road = Road(f"{sx._culture_handle},{games_text}")
    football_text = "football"
    football_road = Road(f"{sx._culture_handle},{games_text},{football_text}")
