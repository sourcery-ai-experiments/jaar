...
def test_agendaAddingIdeaWithAddinCorrectlyTransformsRangeScenario1():
    ...
    x_agenda.add_idea(pad=x_agenda._culture_handle, idea_kid=idea_kid_l1)
    l1_road = f"{x_agenda._culture_handle},{l1}"
...
def test_get_idea_ranged_kids_CorrectlyReturnsAllChildren():
    ...
    # WHEN
    weekunit_road = f"{x_agenda._culture_handle},time,tech,week"
    ranged_ideas = x_agenda.get_idea_ranged_kids(idea_road=weekunit_road)
...
def test_get_idea_ranged_kids_CorrectlyReturnsSomeChildrenScen1():
    ...
    begin_x = 1440
    close_x = 4 * 1440
    ranged_ideas = x_agenda.get_idea_ranged_kids(
...
def test_get_idea_ranged_kids_CorrectlyReturnsSomeChildrenScen2():
    ...
    # WHEN THEN
    week_road = f"{x_agenda._culture_handle},time,tech,week"
    assert (
        len(x_agenda.get_idea_ranged_kids(idea_road=week_road, begin=0, close=1440))
        == 1
    )
    assert (
        len(x_agenda.get_idea_ranged_kids(idea_road=week_road, begin=0, close=2000))
        == 2
    )
    assert (
        len(x_agenda.get_idea_ranged_kids(idea_road=week_road, begin=0, close=3000))
        == 3
    )
