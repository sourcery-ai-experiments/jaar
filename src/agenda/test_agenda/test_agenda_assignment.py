...
def test_agendaunit_get_assignment_ReturnsEmptyBecauseAssignorIsNotInOthers():
    # GIVEN
    noa_text = "Noa"
    noa_agenda = example_agendas_get_agenda_with_4_levels()
    noa_agenda.set_otherunit(otherunit_shop(other_id=noa_text))
    zia_text = "Zia"
    yao_text = "Yao"
    noa_agenda.set_otherunit(otherunit_shop(other_id=zia_text))
    noa_agenda.set_otherunit(otherunit_shop(other_id=yao_text))

    # WHEN
    bob_text = "Bob"
    y_agenda = agendaunit_shop(_owner_id=noa_text)
    x_agenda = agendaunit_shop()
    x_agenda.set_otherunit(otherunit=otherunit_shop(other_id=zia_text))
    x_agenda.set_otherunit(otherunit=otherunit_shop(other_id=noa_text))

    x_assignment_agenda = noa_agenda.get_assignment(
        y_agenda, x_agenda._others, bob_text
    )

    # THEN
    assert len(noa_agenda._others) == 3
    assert len(x_assignment_agenda._others) == 0


def test_agendaunit_get_assignment_ReturnsCorrectOthers():
    # GIVEN
    jes_text = "Jessi"
    ...
    jes_agenda.set_otherunit(otherunit_shop(other_id=zia_text))
    jes_agenda.set_otherunit(otherunit_shop(other_id=noa_text))
    jes_agenda.set_otherunit(otherunit_shop(other_id=yao_text))
    ...
    x_assignment_agenda = jes_agenda.get_assignment(x_agenda, tx._others, bob_text)

    # THEN
    assert len(x_assignment_agenda._others) == 3
    assert x_assignment_agenda._others.get(bob_text) != None
    assert x_assignment_agenda._others.get(zia_text) != None
    assert x_assignment_agenda._others.get(noa_text) != None
    assert x_assignment_agenda._others.get(yao_text) is None
...
def test_agendaunit_get_assignment_ReturnsCorrectBeliefs_Scenario1():
    ...
    x_assignment_agenda = jes_agenda.get_assignment(
        valueless_agenda, tx._others, bob_text
    )
...
def test_AgendaUnit_get_assignment_CorrectlyCreatesAssignmentAgendaUnit_v1():
    ...
    cali_assignment = amer_agenda.get_assignment(
        ...
        assignor_others={cali_text: -1, amer_agenda._owner_id: -1},
