...
def test_AgendaUnit_get_tree_metrics_TracksReasonsThatHaveNoFactBases():
...
def test_AgendaUnit_get_missing_fact_bases_ReturnsAllBasesNotCoveredByFacts():
    yao_agenda = agenda_v001()
    missing_bases = yao_agenda.get_missing_fact_bases()
...
def test_AgendaUnit_3AdvocatesNoideaunit_shop():
    ...
    all_beliefs = zia_agenda._beliefs
    beliefunit_rico = all_beliefs[rico_text]
    beliefunit_carm = all_beliefs[carm_text]
    beliefunit_patr = all_beliefs[patr_text]
    assert beliefunit_rico._party_mirror == True
    assert beliefunit_carm._party_mirror == True
    assert beliefunit_patr._party_mirror == True
...
def test_AgendaUnit_calc_agenda_metrics_CreatesFullyPopulated_idea_dict():
    # GIVEN
...
def test_AgendaUnit_calc_agenda_metrics_SetsSatiateStatusCorrectlyWhenFactSaysNo():
    # GIVEN
    sue_agenda = get_agenda_with_4_levels_and_2reasons()
    week_text = "weekdays"
    ...
    assert sue_agenda.get_idea_obj(casa_road)._active is None

    # WHEN
    sue_agenda.set_fact(base=week_road, pick=sun_road)
    sue_agenda.calc_agenda_metrics()
...
def test_AgendaUnit_calc_agenda_metrics_SetsSatiateStatusCorrectlyWhenFactModifies():
    # GIVEN
    sue_agenda = get_agenda_with_4_levels_and_2reasons()
    week_text = "weekdays"
    week_road = sue_agenda.make_l1_road(week_text)
    ...
    assert sue_agenda._idea_dict
    assert len(sue_agenda._idea_dict) == 17
    assert sue_agenda._idea_dict.get(casa_road)._active

    # WHEN
    france_text = "France"
    france_road = sue_agenda.make_road(states_road, france_text)
    sue_agenda.set_fact(base=states_road, pick=france_road)
...
def test_AgendaUnit_calc_agenda_metrics_CorrectlySets_idea_dict():
    ...
    sue_agenda.set_fact(base=state_road, pick=france_road)

    casa_text = "casa"
    casa_road = sue_agenda.make_l1_road(casa_text)
    casa_idea = sue_agenda.get_idea_obj(casa_road)
    print(f"{sue_agenda._owner_id=} {len(casa_idea._reasonunits)=}")
    # print(f"{casa_idea._reasonunits=}")
    print(f"{sue_agenda._owner_id=} {len(sue_agenda._idearoot._factunits)=}")
    # print(f"{sue_agenda._idearoot._factunits=}")
    ...
    # WHEN
    sue_agenda.set_fact(base=state_road, pick=oregon_road)

    # THEN
    casa_idea = sue_agenda._idea_dict.get(casa_road)
    print(f"\nlook at {casa_idea.get_road()=}")
    assert casa_idea._parent_road == sue_agenda._real_id
...
def test_AgendaUnit_calc_agenda_metrics_CorrectlyClears_agenda_fund():
    ...
    x_agenda._idearoot._agenda_fund_onset = 13
    x_agenda._idearoot._agenda_fund_cease = 13
    x_agenda.get_idea_obj(casa_road)._agenda_fund_onset = 13
    x_agenda.get_idea_obj(casa_road)._agenda_fund_cease = 13
    x_agenda.get_idea_obj(catt_road)._agenda_fund_onset = 13
    x_agenda.get_idea_obj(catt_road)._agenda_fund_cease = 13
    x_agenda.get_idea_obj(week_road)._agenda_fund_onset = 13
    x_agenda.get_idea_obj(week_road)._agenda_fund_cease = 13

    assert x_agenda._idearoot._agenda_fund_onset == 13
    assert x_agenda._idearoot._agenda_fund_cease == 13
    assert x_agenda.get_idea_obj(casa_road)._agenda_fund_onset == 13
    assert x_agenda.get_idea_obj(casa_road)._agenda_fund_cease == 13
    assert x_agenda.get_idea_obj(catt_road)._agenda_fund_onset == 13
    assert x_agenda.get_idea_obj(catt_road)._agenda_fund_cease == 13
    assert x_agenda.get_idea_obj(week_road)._agenda_fund_onset == 13
    assert x_agenda.get_idea_obj(week_road)._agenda_fund_cease == 13
...
def test_AgendaUnit_calc_agenda_metrics_CorrectlyCalculatesIdeaAttr_agenda_fund():
    # GIVEN
    ...
    assert yao_agenda._idearoot._agenda_fund_onset is None
    assert yao_agenda._idearoot._agenda_fund_cease is None
    assert yao_agenda.get_idea_obj(auto_road)._agenda_fund_onset is None
    assert yao_agenda.get_idea_obj(auto_road)._agenda_fund_cease is None
    assert yao_agenda.get_idea_obj(barn_road)._agenda_fund_onset is None
    assert yao_agenda.get_idea_obj(barn_road)._agenda_fund_cease is None
    assert yao_agenda.get_idea_obj(coal_road)._agenda_fund_onset is None
    assert yao_agenda.get_idea_obj(coal_road)._agenda_fund_cease is None
    ...
    # THEN
    assert yao_agenda._idearoot._agenda_fund_onset == 0.0
    assert yao_agenda._idearoot._agenda_fund_cease == 1.0
    assert yao_agenda.get_idea_obj(auto_road)._agenda_fund_onset == 0.0
    assert yao_agenda.get_idea_obj(auto_road)._agenda_fund_cease == 0.1
    assert yao_agenda.get_idea_obj(barn_road)._agenda_fund_onset == 0.1
    assert yao_agenda.get_idea_obj(barn_road)._agenda_fund_cease == 0.7
    assert yao_agenda.get_idea_obj(coal_road)._agenda_fund_onset == 0.7
    assert yao_agenda.get_idea_obj(coal_road)._agenda_fund_cease == 1.0
...
def test_AgendaUnit_calc_agenda_metrics_CorrectlyCalculatesRangeAttributes():
    ...
    day24hr_open = 8.0
    day24hr_nigh = 10.0
    print(sue_agenda._idearoot._factunits[day24hr_road])
    sue_agenda.set_fact(
        base=day24hr_base, pick=day24hr_pick, open=day24hr_open, nigh=day24hr_nigh
    )
    print(sue_agenda._idearoot._factunits[day24hr_road])
...
def test_AgendaUnit_calc_agenda_metrics_CorrectlySetsData_agenda_v001():
    ...
    # yao_agenda.set_fact(base=day_hour, pick=day_hour, open=0, nigh=23)
    day_min_text = "day_minute"
    day_min_road = yao_agenda.make_l1_road(day_min_text)
    yao_agenda.set_fact(base=day_min_road, pick=day_min_road, open=0, nigh=1439)
    ...
    yao_agenda.set_fact(base=yr_mon_road, pick=yr_mon_road)
    inter_text = "Internet"
    inter_road = yao_agenda.make_l1_road(inter_text)
    yao_agenda.set_fact(base=inter_road, pick=inter_road)
    ...
    # for fact in yao_agenda._idearoot._factunits.values():
    #     print(f"{fact=}")
    ...
    yao_agenda.set_fact(base=week_road, pick=mon_road)
    yao_agenda.calc_agenda_metrics()
...
def test_AgendaUnit_calc_agenda_metrics_OptionWeekdaysReturnsCorrectObj_agenda_v001():
    ...
    yao_agenda.set_fact(base=day_hr_road, pick=day_hr_road, open=0, nigh=23)
    day_min_text = "day_minute"
    day_min_road = yao_agenda.make_l1_road(day_min_text)
    yao_agenda.set_fact(base=day_min_road, pick=day_min_road, open=0, nigh=59)
    mon_wk_text = "month_week"
    mon_wk_road = yao_agenda.make_l1_road(mon_wk_text)
    yao_agenda.set_fact(base=mon_wk_road, pick=mon_wk_road)
    nation_text = "Nation-States"
    nation_road = yao_agenda.make_l1_road(nation_text)
    yao_agenda.set_fact(base=nation_road, pick=nation_road)
    mood_text = "Moods"
    mood_road = yao_agenda.make_l1_road(mood_text)
    yao_agenda.set_fact(base=mood_road, pick=mood_road)
    ...
    yr_mon_text = "year_month"
    yr_mon_road = yao_agenda.make_l1_road(yr_mon_text)
    yao_agenda.set_fact(base=yr_mon_road, pick=yr_mon_road, open=0, nigh=1000)

    yao_agenda.calc_agenda_metrics()
    missing_facts = yao_agenda.get_missing_fact_bases()
    ...
    # yao_agenda.set_fact(base=f"{yao_agenda._real_id},weekdays", pick=f"{yao_agenda._real_id},weekdays,Tuesday")
    # idea_dict = yao_agenda.get_idea_dict()
    # assert YR.get_active(road=bird_idea, idea_dict=idea_dict) == True

    # yao_agenda.set_fact(base=f"{yao_agenda._real_id},weekdays", pick=f"{yao_agenda._real_id},weekdays,Wednesday")
    # idea_dict = yao_agenda.get_idea_dict()
...
def test_AgendaUnit_calc_agenda_metrics_CorrectlySetsIdeaUnitsActiveWithEvery6WeeksReason_agenda_v001():
    ...
    # WHEN
    yao_agenda.set_fact(base=day_road, pick=day_road, open=0, nigh=23)
    yao_agenda.set_fact(base=min_road, pick=min_road, open=0, nigh=59)
    yao_agenda.calc_agenda_metrics()
    ...
    print(f"{yao_agenda._idearoot._factunits=}")
    yao_agenda.calc_agenda_metrics()
...
def test_AgendaUnit_calc_agenda_metrics_EveryIdeaHasActiveStatus_agenda_v001():
    # GIVEN
    yao_agenda = agenda_v001()
...
def test_AgendaUnit_calc_agenda_metrics_EveryOtherMonthReturnsCorrectObj_agenda_v001():
    ...
    yao_agenda.set_fact(base=minute_road, pick=minute_road, open=0, nigh=1399)
    month_text = "month_week"
    month_road = yao_agenda.make_l1_road(month_text)
    yao_agenda.set_fact(base=month_road, pick=month_road)
    ...
    yao_agenda.set_fact(base=internet_road, pick=internet_road)
    ...
    # WHEN
    yao_agenda.set_fact(base=year_month_base, pick=year_month_base, open=0, nigh=8)
    ced_week = yao_agenda.make_l1_road("ced_week")
    yao_agenda.set_fact(base=ced_week, pick=ced_week, open=0, nigh=4)
    yao_agenda.calc_agenda_metrics()

    # THEN
    print(f"{len(idea_dict)=}")
    print(f"{len(yao_agenda._idearoot._factunits)=}")
