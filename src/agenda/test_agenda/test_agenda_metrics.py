...
def test_AgendaUnit_set_agenda_metrics_CorrectlySetsEmpty_sum_leaderunit_importance():
    # GIVEN
...
def test_AgendaUnit_set_agenda_metrics_CorrectlySets_sum_leaderunit_importance():
    # GIVEN
    sue_agenda = get_agenda_with_4_levels_and_2reasons()
    ...
    sue_leaderunit = leaderunit_shop({"Sue"})
    sue_agenda.edit_idea_attr(oregon_road, problem_bool=True, leaderunit=sue_leaderunit)
    oregon_idea = sue_agenda.get_idea_obj(oregon_road)
    print(f"{oregon_idea._agenda_importance=}")
    assert sue_agenda._sum_leaderunit_importance == 0
    assert oregon_idea._leaderunit_importance == 0

    # WHEN
    sue_agenda.set_agenda_metrics()
    # THEN
    assert sue_agenda._sum_leaderunit_importance == 0.038461538461538464
    assert oregon_idea._leaderunit_importance == 1
    ...
    sue_agenda.edit_idea_attr(mon_road, leaderunit=sue_leaderunit)
    mon_idea = sue_agenda.get_idea_obj(mon_road)
    # print(f"{mon_idea._problem_bool=} {mon_idea._agenda_importance=}")
    sue_agenda.set_agenda_metrics()
    # THEN
    assert sue_agenda._sum_leaderunit_importance != 0.038461538461538464
    assert sue_agenda._sum_leaderunit_importance == 0.06923076923076923
    assert oregon_idea._leaderunit_importance == 0.5555555555555556
    assert mon_idea._leaderunit_importance == 0.4444444444444444

    # WHEN
    tue_road = sue_agenda.make_road(week_road, "Tuesday")
    sue_agenda.edit_idea_attr(tue_road, leaderunit=sue_leaderunit)
    ...
    # THEN
    assert sue_agenda._sum_leaderunit_importance != 0.06923076923076923
    assert sue_agenda._sum_leaderunit_importance == 0.1
    assert oregon_idea._leaderunit_importance == 0.38461538461538464
    assert mon_idea._leaderunit_importance == 0.3076923076923077
    assert tue_idea._leaderunit_importance == 0.3076923076923077

    # WHEN
    sue_agenda.edit_idea_attr(week_road, leaderunit=sue_leaderunit)
    ...
    assert sue_agenda._sum_leaderunit_importance == 0
    assert oregon_idea._leaderunit_importance == 0
    assert mon_idea._leaderunit_importance == 0
    assert tue_idea._leaderunit_importance == 0
...
def test_AgendaUnit_set_agenda_metrics_CorrectlySets_leaders_dict():
    # GIVEN
    sue_text = "Sue"
    bob_text = "Bob"
    sue_agenda = get_agenda_with_4_levels_and_2reasons()
    ...
    usa_road = sue_agenda.make_road(nation_road, "USA")
    oregon_road = sue_agenda.make_road(usa_road, "Oregon")
    sue_leaderunit = leaderunit_shop({sue_text})
    sue_agenda.edit_idea_attr(oregon_road, problem_bool=True, leaderunit=sue_leaderunit)

    week_road = sue_agenda.make_l1_road("weekdays")
    bob_leaderunit = leaderunit_shop({bob_text})
    sue_agenda.edit_idea_attr(week_road, problem_bool=True, leaderunit=bob_leaderunit)
    assert sue_agenda._leaders_dict == {}
    ...
    # THEN
    assert len(sue_agenda._leaders_dict) == 2
    week_idea = sue_agenda.get_idea_obj(week_road)
    assert sue_agenda._leaders_dict.get(bob_text) == {week_road: week_idea}
    oregon_idea = sue_agenda.get_idea_obj(oregon_road)
    assert sue_agenda._leaders_dict.get(sue_text) == {oregon_road: oregon_idea}
...
def test_AgendaUnit_set_agenda_metrics_CorrectlySets_econs_buildable_False():
    ...
    sue_leaderunit = leaderunit_shop({sue_text})
    sue_agenda.edit_idea_attr(bend_road, problem_bool=True, leaderunit=sue_leaderunit)
    assert sue_agenda._econs_buildable
