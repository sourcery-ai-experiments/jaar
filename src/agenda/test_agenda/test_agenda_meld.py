...
def test_agenda_idearoot_meld_IdeaRootAttrCorrectlyMelded():
    ...
    assert (
        str(excinfo.value)
        == f"Meld fail idea={x_agenda1._culture_id} _uid:1 with {x_agenda2._culture_id} _uid:4"
    )
...
def test_agenda_idearoot_meld_Add4IdeasScenario():
    ...
    tech_text = "tech"
    tech_road = x_agenda1.make_road(x_agenda1._culture_id, tech_text)
    bowl_text = "bowl"
    ...
    x_agenda2.add_idea(ideacore_shop(tech_text), pad=x_agenda2._culture_id)
    x_agenda2.add_idea(ideacore_shop(bowl_text), pad=tech_road)
    x_agenda2.add_idea(ideacore_shop(free_text), pad=swim_road)
...
def test_agenda_idearoot_meld_2SameIdeasScenario():
    ...
    x_agenda1.add_idea(ideacore_shop(tech_text), pad=x_agenda1._culture_id)
    x_agenda1.add_idea(ideacore_shop(bowl_text), pad=tech_road)
...
def test_agenda_acptfactunits_meld_2AcptFactUnitsWorks():
    ...
    x_agenda1.add_idea(ideacore_shop(tech_text), pad=x_agenda1._culture_id)
    x_agenda1.add_idea(ideacore_shop(bowl_text), pad=tech_road)
    x_agenda1.add_idea(ideacore_shop(free_text), pad=swim_road)
    x_agenda1.set_acptfact(base=tech_road, pick=bowl_road)
...
def test_agenda_acptfactunits_meld_AcptFactsAttributeCorrectlySet():
    ...
    swim_road = x_agenda1.make_road(x_agenda1._culture_id, swim_text)
    free_text = "freestyle"
    free_road = x_agenda1.make_road(x_agenda1._culture_id, free_text)
    x_agenda1.add_idea(ideacore_shop(free_text), pad=swim_road)
...
def test_agenda_meld_OriginUnitsCorrectlySet():
    ...
    swim_text = "swim"
    swim_road = bob_x_agenda.make_road(bob_x_agenda._culture_id, swim_text)
    free_text = "freestyle"
    ...
    bob_x_agenda.add_idea(ideacore_shop(free_text), pad=swim_road)

    sue_text = "Sue"
    sue_weight = 4
    sue_x_agenda = agendaunit_shop(_healer=sue_text)
    sue_x_agenda.add_idea(ideacore_shop(free_text), pad=swim_road)
    sue_x_agenda.set_acptfact(base=swim_road, pick=free_road, open=23, nigh=27)
    sue_x_agenda.add_idea(ideacore_shop(back_text), pad=swim_road)
    assert len(bob_x_agenda._originunit._links) == 0
