...
def test_forkunit_shop_CorrectlyReturnsObj():
    ...
    assert cook_fork.delimiter == default_road_delimiter_if_none()
...
def test_ForkUnit_is_moral_ReturnsCorrectBool():
    ...
    # WHEN / THEN
    cheap_prong = prongunit_shop(create_road(cook_road, "cheap food"), -2)
    cook_fork.set_prong(cheap_prong)
    assert len(cook_fork.prongs) == 1
    assert cook_fork.is_moral() == False
    ...
    farm_prong = prongunit_shop(create_road(cook_road, farm_text), 3)
    cook_fork.set_prong(farm_prong)
    assert len(cook_fork.prongs) == 2
    assert cook_fork.is_moral()

    # WHEN / THEN
    cook_fork.del_prong(create_road(cook_road, farm_text))
    assert len(cook_fork.prongs) == 1
    assert cook_fork.is_moral() == False

    # WHEN / THEN
    plastic_prong = prongunit_shop(create_road(cook_road, "plastic pots"), -5)
    cook_fork.set_prong(plastic_prong)
    assert len(cook_fork.prongs) == 2
    assert cook_fork.is_moral() == False

    # WHEN / THEN
    metal_prong = prongunit_shop(create_road(cook_road, "metal pots"), 7)
    cook_fork.set_prong(metal_prong)
...
def test_ForkUnit_get_all_roads_ReturnsCorrectObj():
    ...
    cook_fork.set_prong(prongunit_shop(create_road(cook_road, cheap_text), -2))
    cook_fork.set_prong(prongunit_shop(create_road(cook_road, farm_text), 3))
    cook_fork.set_prong(prongunit_shop(create_road(cook_road, plastic_text), -5))
    cook_fork.set_prong(prongunit_shop(create_road(cook_road, metal_text), 7))
    assert len(cook_fork.prongs) == 4
