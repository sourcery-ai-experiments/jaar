...
def test_lemma_attributes_exist():
    x_lemma = Lemma(src_belief=1, calc_belief=2, x_fact=3, eval_status=4, eval_count=5)
    assert x_lemma.src_belief == 1
    assert x_lemma.calc_belief == 2
    assert x_lemma.x_fact == 3
...
def test_lemmas_create_new_belief_createsCorrectBelief_scenario1():
    ...
    fact_kid = factunit_shop("timerange1", _parent_road=hist_road, _begin=0, _close=12)
    src_fact = factunit_shop(
        "sub_timerange", _parent_road=hist_road, _begin=-13, _close=500
    )
    tr1 = create_road(fact_kid._parent_road, fact_kid._label)
    src_belief = beliefunit_shop(base=tr1, pick=tr1, open=0, nigh=30)
    new_belief = x_lemmas_x._create_new_belief(
        x_fact=fact_kid, src_belief=src_belief, src_fact=src_fact
    )
...
def test_lemmas_create_new_belief_createsCorrectBelief_scenario2():
    ...
    # WHEN
    hist_road = create_road(root_label(), "histoory")
    fact_kid = factunit_shop("timerange1", _parent_road=hist_road, _begin=7, _close=12)
    tr1 = create_road(fact_kid._parent_road, fact_kid._label)
    src_belief = beliefunit_shop(base=tr1, pick=tr1, open=0, nigh=30)
    src_fact = factunit_shop(
        "sub_timerange", _parent_road=hist_road, _begin=-13, _close=500
...
def test_lemmas_create_new_belief_createsCorrectBelief_scenario3_2_denom():
    ...
    # WHEN
    hist_road = create_road(root_label(), "histoory")
    ex_fact = factunit_shop("range_x", _parent_road=hist_road, _begin=0, _close=10080)
    fact_kid = factunit_shop(
        _label="timerange1",
    ...
    ex_road = create_road(ex_fact._parent_road, ex_fact._label)
    ex_belief = beliefunit_shop(base=ex_road, pick=ex_road, open=7200, nigh=7200)
    new_belief = x_lemmas._create_new_belief(
        x_fact=fact_kid, src_belief=ex_belief, src_fact=ex_fact
    )
...
def test_lemmas_create_new_belief_createsCorrectBelief_scenario6_denomReest():
    ...
    tr3_30_20_belief = x_lemmas_x._create_new_belief(
        x_fact=tr3_kid, src_belief=src_belief, src_fact=fact_src
    )
    assert tr3_30_20_belief.open == 40
    assert tr3_30_20_belief.nigh == 50

    # WHEN / THEN Check
    trb_kid = factunit_shop(
        _label="subera",
        _parent_road=hist_road,
        _begin=40,
        _close=60,
    ...
    trb_30_20_belief = x_lemmas_x._create_new_belief(
        x_fact=trb_kid, src_belief=src_belief, src_fact=fact_src
    )
    assert trb_30_20_belief.open == 40
    assert trb_30_20_belief.nigh == 60

    # WHEN / THEN Check
    tr4_kid = factunit_shop(
        _label="subera",
        _parent_road=hist_road,
        _begin=55,
        _close=10,
    ...
    tr4_30_20_belief = x_lemmas_x._create_new_belief(
        x_fact=tr4_kid, src_belief=src_belief, src_fact=fact_src
    )
    assert tr4_30_20_belief.open == 55
    assert tr4_30_20_belief.nigh == 10

    # WHEN / THEN Check
    tr5_kid = factunit_shop(
        _label="subera",
        _parent_road=hist_road,
        _begin=0,
        _close=60,
    ...
    tr5_0_60_belief = x_lemmas_x._create_new_belief(
        x_fact=tr5_kid, src_belief=src_belief, src_fact=fact_src
    )
...
def test_lemmas_get_unevaluated_lemma_ReturnsCorrectLemmaWhenPopulated():
    ...
    tr1_fact = factunit_shop("timerange1", _parent_road=hist_road, _begin=7, _close=12)
    tr1 = create_road(tr1_fact._parent_road, tr1_fact._label)
    src_belief = beliefunit_shop(base=tr1, pick=tr1, open=0, nigh=30)
    x_lemmas_x.eval(x_fact=tr1_fact, src_belief=src_belief, src_fact=src_fact)

    tr2_fact = factunit_shop("timerange2", _parent_road=hist_road, _begin=40, _close=60)
    tr2 = create_road(tr2_fact._parent_road, tr2_fact._label)
    src_belief = beliefunit_shop(base=tr2, pick=tr2, open=55, nigh=60)
    x_lemmas_x.eval(x_fact=tr2_fact, src_belief=src_belief, src_fact=src_fact)
    ...
    # THEN
    assert lem1.x_fact in (tr1_fact, tr2_fact)
    assert lem2.x_fact in (tr1_fact, tr2_fact)
    assert lem3 is None
...
def test_lemmas_is_lemmas_incomplete_ReturnsCorrectBoolWhenPopulated():
    ...
    tr1_fact = factunit_shop("timerange1", _parent_road=hist_road, _begin=7, _close=12)
    tr1_road = create_road(tr1_fact._parent_road, tr1_fact._label)
    src_belief = beliefunit_shop(base=tr1_road, pick=tr1_road, open=0, nigh=30)
    z_lemmas.eval(x_fact=tr1_fact, src_belief=src_belief, src_fact=src_fact)

    tr2_fact = factunit_shop("timerange2", _parent_road=hist_road, _begin=40, _close=60)
    tr2_road = create_road(tr2_fact._parent_road, tr2_fact._label)
    src_belief = beliefunit_shop(base=tr2_road, pick=tr2_road, open=55, nigh=60)
    z_lemmas.eval(x_fact=tr2_fact, src_belief=src_belief, src_fact=src_fact)
