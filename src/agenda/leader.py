...
class LeaderUnit:
    ...
    def get_dict(self):
        return {"leaderunit_group_ids": list(self._group_ids)}


def leaderunit_shop(_group_ids: set[GroupID] = None) -> LeaderUnit:
    return LeaderUnit(_group_ids=get_empty_set_if_none(_group_ids))


def leaderunit_get_from_dict(x_dict: dict[str:set]) -> LeaderUnit:
    x_leaderunit = leaderunit_shop()
    if x_dict.get("leaderunit_group_ids") != None:
        for x_group_id in x_dict.get("leaderunit_group_ids"):
            x_leaderunit.set_group_id(x_group_id=x_group_id)
    return x_leaderunit
