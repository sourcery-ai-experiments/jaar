...
def test_IdeaUnit_exists():
    ...
    assert x_ideaunit._all_guy_cred is None
    assert x_ideaunit._all_guy_debt is None
...
def test_IdeaUnit_clear_all_guy_cred_debt_ClearsCorrectly():
    # GIVEN
    ball_text = "ball"
    ball_idea = ideaunit_shop(_label=ball_text, _all_guy_cred=55, _all_guy_debt=33)
    assert ball_idea._all_guy_cred == 55
    assert ball_idea._all_guy_debt == 33

    # WHEN
    ball_idea.clear_all_guy_cred_debt()

    # THEN
    assert ball_idea._all_guy_cred is None
    assert ball_idea._all_guy_debt is None
...
def test_IdeaUnit_get_dict_ReturnsCorrectCompleteDict():
    ...
    casa_idea._originunit.set_originlink(guy_id="Ray", weight=None)
    casa_idea._originunit.set_originlink(guy_id="Lei", weight=4)
    x_begin = 11
    x_close = 12
    x_addin = 13
    x_denom = 14
