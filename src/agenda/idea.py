...
class IdeaUnit:
    ...
    _leaderunit_importance: float = None
    ...
    def _set_idea_attr(self, idea_attr: IdeaAttrFilter):
        ...
        if idea_attr.leaderunit != None:
            self._leaderunit = idea_attr.leaderunit
    ...
    def get_dict(self) -> dict[str:str]:
        ...
        if self._leaderunit not in [None, leaderunit_shop()]:
            x_dict["_leaderunit"] = self._leaderunit.get_dict()
...
def ideaunit_shop(
    ...
    if _leaderunit is None:
        _leaderunit = leaderunit_shop()
    ...
    x_ideakid = IdeaUnit(
        ...
        _leaderunit_importance=get_0_if_None(_leaderunit_importance),
    )
    if x_ideakid._root:
...
def get_obj_from_idea_dict(x_dict: dict[str:], dict_key: str) -> any:
    ...
    elif dict_key == "_leaderunit":
        return (
            leaderunit_get_from_dict(x_dict[dict_key])
            if x_dict.get(dict_key) != None
            else leaderunit_shop()
        )
