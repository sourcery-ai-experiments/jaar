...
class EditAcptFactTime(qtw.QTableWidget, Ui_Form):
    ...
    def update_acptfact(self):
        ...
        self.oath_x.set_time_acptfacts(open=open_dt_x, nigh=nigh_dt_x)
        self.root_changes_submitted.emit(True)
        self.close()

    def display_acptfact_time(self):
        # minutes_idea = self.oath_x.get_idea_kid(
        #     road=f"{root_label},time,jajatime"
        # )
        minutes_acptfact = self.oath_x._idearoot._acptfactunits[
            f"{self.oath_x._cure_handle},time,jajatime"
        ]

        dt_open = self.oath_x.get_time_dt_from_min(min=minutes_acptfact.open)
        dt_nigh = self.oath_x.get_time_dt_from_min(min=minutes_acptfact.nigh)
