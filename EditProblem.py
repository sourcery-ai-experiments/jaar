...
from pyqt_func import oath_importance_diplay, get_pyqttree, num2str
from src.oath.group import groupunit_shop, balancelink_shop
from src.oath.idea import IdeaKid
from src.oath.road import Road, get_pad_from_road, get_terminus_node_from_road
...
class EditProblem(qtw.QWidget, Ui_Form):
    ...
    def set_problem_dominate_action_idea(self, road):
        if road != "":
            ...
            self.oath_x.set_dominate_promise_idea(idea_kid=prob_idea)

    def add_group(self):
        if self.add_group_text not in (None, ""):
            self.oath_x.set_groupunit(groupunit_shop(brand=self.add_group_text.text()))
        self.refresh_all()
    ...
    def refresh_all(self):
        ...
        if self.oath_x != None:
            ...
            self.group1_title_combo.addItems(self.oath_x.get_groupunits_brand_list())
            self.group2_title_combo.addItems(self.oath_x.get_groupunits_brand_list())
            self.group3_title_combo.addItems(self.oath_x.get_groupunits_brand_list())
